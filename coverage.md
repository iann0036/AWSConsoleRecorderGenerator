## CloudFormation Resource Coverage

**370/435 (85%)** Resources Covered

| Type | Coverage |
| --- | --- |
| *AWS::AmazonMQ::Broker* | 1 |
| *AWS::AmazonMQ::Configuration* | 1 |
| *AWS::AmazonMQ::ConfigurationAssociation* | 1 |
| *AWS::Amplify::App* | 0 |
| *AWS::Amplify::Branch* | 0 |
| *AWS::Amplify::Domain* | 0 |
| *AWS::ApiGateway::Account* | 1 |
| *AWS::ApiGateway::ApiKey* | 1 |
| *AWS::ApiGateway::Authorizer* | 1 |
| *AWS::ApiGateway::BasePathMapping* | 1 |
| *AWS::ApiGateway::ClientCertificate* | 1 |
| *AWS::ApiGateway::Deployment* | 1 |
| *AWS::ApiGateway::DocumentationPart* | 1 |
| *AWS::ApiGateway::DocumentationVersion* | 1 |
| *AWS::ApiGateway::DomainName* | 1 |
| *AWS::ApiGateway::GatewayResponse* | 1 |
| *AWS::ApiGateway::Method* | 1 |
| *AWS::ApiGateway::Model* | 1 |
| *AWS::ApiGateway::RequestValidator* | 1 |
| *AWS::ApiGateway::Resource* | 1 |
| *AWS::ApiGateway::RestApi* | 1 |
| *AWS::ApiGateway::Stage* | 1 |
| *AWS::ApiGateway::UsagePlan* | 1 |
| *AWS::ApiGateway::UsagePlanKey* | 1 |
| *AWS::ApiGateway::VpcLink* | 1 |
| *AWS::ApiGatewayV2::Api* | 1 |
| *AWS::ApiGatewayV2::ApiMapping* | 0 |
| *AWS::ApiGatewayV2::Authorizer* | 1 |
| *AWS::ApiGatewayV2::Deployment* | 1 |
| *AWS::ApiGatewayV2::DomainName* | 0 |
| *AWS::ApiGatewayV2::Integration* | 1 |
| *AWS::ApiGatewayV2::IntegrationResponse* | 1 |
| *AWS::ApiGatewayV2::Model* | 1 |
| *AWS::ApiGatewayV2::Route* | 1 |
| *AWS::ApiGatewayV2::RouteResponse* | 1 |
| *AWS::ApiGatewayV2::Stage* | 1 |
| *AWS::AppMesh::Mesh* | 0 |
| *AWS::AppMesh::Route* | 0 |
| *AWS::AppMesh::VirtualNode* | 0 |
| *AWS::AppMesh::VirtualRouter* | 0 |
| *AWS::AppMesh::VirtualService* | 0 |
| *AWS::AppStream::DirectoryConfig* | 1 |
| *AWS::AppStream::Fleet* | 1 |
| *AWS::AppStream::ImageBuilder* | 1 |
| *AWS::AppStream::Stack* | 1 |
| *AWS::AppStream::StackFleetAssociation* | 1 |
| *AWS::AppStream::StackUserAssociation* | 1 |
| *AWS::AppStream::User* | 1 |
| *AWS::AppSync::ApiKey* | 1 |
| *AWS::AppSync::DataSource* | 1 |
| *AWS::AppSync::FunctionConfiguration* | 1 |
| *AWS::AppSync::GraphQLApi* | 1 |
| *AWS::AppSync::GraphQLSchema* | 1 |
| *AWS::AppSync::Resolver* | 1 |
| *AWS::ApplicationAutoScaling::ScalableTarget* | 1 |
| *AWS::ApplicationAutoScaling::ScalingPolicy* | 1 |
| *AWS::Athena::NamedQuery* | 1 |
| *AWS::AutoScaling::AutoScalingGroup* | 1 |
| *AWS::AutoScaling::LaunchConfiguration* | 1 |
| *AWS::AutoScaling::LifecycleHook* | 1 |
| *AWS::AutoScaling::ScalingPolicy* | 1 |
| *AWS::AutoScaling::ScheduledAction* | 1 |
| *AWS::AutoScalingPlans::ScalingPlan* | 1 |
| *AWS::Backup::BackupPlan* | 1 |
| *AWS::Backup::BackupSelection* | 0 |
| *AWS::Backup::BackupVault* | 1 |
| *AWS::Batch::ComputeEnvironment* | 1 |
| *AWS::Batch::JobDefinition* | 1 |
| *AWS::Batch::JobQueue* | 1 |
| *AWS::Budgets::Budget* | 1 |
| *AWS::CDK::Metadata* | 1 |
| *AWS::CertificateManager::Certificate* | 1 |
| *AWS::Cloud9::EnvironmentEC2* | 1 |
| *AWS::CloudFormation::CustomResource* | 1 |
| *AWS::CloudFormation::Macro* | 1 |
| *AWS::CloudFormation::Stack* | 1 |
| *AWS::CloudFormation::Transform* | 1 |
| *AWS::CloudFormation::WaitCondition* | 1 |
| *AWS::CloudFormation::WaitConditionHandle* | 1 |
| *AWS::CloudFront::CloudFrontOriginAccessIdentity* | 1 |
| *AWS::CloudFront::Distribution* | 1 |
| *AWS::CloudFront::StreamingDistribution* | 1 |
| *AWS::CloudTrail::Trail* | 1 |
| *AWS::CloudWatch::Alarm* | 1 |
| *AWS::CloudWatch::Dashboard* | 1 |
| *AWS::CodeBuild::Project* | 1 |
| *AWS::CodeCommit::Repository* | 1 |
| *AWS::CodeDeploy::Application* | 1 |
| *AWS::CodeDeploy::DeploymentConfig* | 1 |
| *AWS::CodeDeploy::DeploymentGroup* | 1 |
| *AWS::CodePipeline::CustomActionType* | 1 |
| *AWS::CodePipeline::Pipeline* | 1 |
| *AWS::CodePipeline::Webhook* | 1 |
| *AWS::Cognito::IdentityPool* | 1 |
| *AWS::Cognito::IdentityPoolRoleAttachment* | 1 |
| *AWS::Cognito::UserPool* | 1 |
| *AWS::Cognito::UserPoolClient* | 1 |
| *AWS::Cognito::UserPoolGroup* | 1 |
| *AWS::Cognito::UserPoolUser* | 1 |
| *AWS::Cognito::UserPoolUserToGroupAttachment* | 1 |
| *AWS::Config::AggregationAuthorization* | 1 |
| *AWS::Config::ConfigRule* | 1 |
| *AWS::Config::ConfigurationAggregator* | 1 |
| *AWS::Config::ConfigurationRecorder* | 1 |
| *AWS::Config::DeliveryChannel* | 1 |
| *AWS::Config::RemediationConfiguration* | 0 |
| *AWS::DAX::Cluster* | 1 |
| *AWS::DAX::ParameterGroup* | 1 |
| *AWS::DAX::SubnetGroup* | 1 |
| *AWS::DLM::LifecyclePolicy* | 1 |
| *AWS::DMS::Certificate* | 1 |
| *AWS::DMS::Endpoint* | 1 |
| *AWS::DMS::EventSubscription* | 1 |
| *AWS::DMS::ReplicationInstance* | 1 |
| *AWS::DMS::ReplicationSubnetGroup* | 1 |
| *AWS::DMS::ReplicationTask* | 1 |
| *AWS::DataPipeline::Pipeline* | 1 |
| *AWS::DirectoryService::MicrosoftAD* | 1 |
| *AWS::DirectoryService::SimpleAD* | 1 |
| *AWS::DocDB::DBCluster* | 1 |
| *AWS::DocDB::DBClusterParameterGroup* | 1 |
| *AWS::DocDB::DBInstance* | 1 |
| *AWS::DocDB::DBSubnetGroup* | 1 |
| *AWS::DynamoDB::Table* | 2 |
| *AWS::EC2::CapacityReservation* | 0 |
| *AWS::EC2::ClientVpnAuthorizationRule* | 0 |
| *AWS::EC2::ClientVpnEndpoint* | 1 |
| *AWS::EC2::ClientVpnRoute* | 0 |
| *AWS::EC2::ClientVpnTargetNetworkAssociation* | 0 |
| *AWS::EC2::CustomerGateway* | 1 |
| *AWS::EC2::DHCPOptions* | 1 |
| *AWS::EC2::EC2Fleet* | 1 |
| *AWS::EC2::EIP* | 2 |
| *AWS::EC2::EIPAssociation* | 1 |
| *AWS::EC2::EgressOnlyInternetGateway* | 1 |
| *AWS::EC2::FlowLog* | 1 |
| *AWS::EC2::Host* | 1 |
| *AWS::EC2::Instance* | 1 |
| *AWS::EC2::InternetGateway* | 2 |
| *AWS::EC2::LaunchTemplate* | 1 |
| *AWS::EC2::NatGateway* | 1 |
| *AWS::EC2::NetworkAcl* | 2 |
| *AWS::EC2::NetworkAclEntry* | 1 |
| *AWS::EC2::NetworkInterface* | 1 |
| *AWS::EC2::NetworkInterfaceAttachment* | 1 |
| *AWS::EC2::NetworkInterfacePermission* | 1 |
| *AWS::EC2::PlacementGroup* | 1 |
| *AWS::EC2::Route* | 2 |
| *AWS::EC2::RouteTable* | 3 |
| *AWS::EC2::SecurityGroup* | 2 |
| *AWS::EC2::SecurityGroupEgress* | 1 |
| *AWS::EC2::SecurityGroupIngress* | 1 |
| *AWS::EC2::SpotFleet* | 1 |
| *AWS::EC2::Subnet* | 2 |
| *AWS::EC2::SubnetCidrBlock* | 1 |
| *AWS::EC2::SubnetNetworkAclAssociation* | 1 |
| *AWS::EC2::SubnetRouteTableAssociation* | 2 |
| *AWS::EC2::TransitGateway* | 1 |
| *AWS::EC2::TransitGatewayAttachment* | 1 |
| *AWS::EC2::TransitGatewayRoute* | 1 |
| *AWS::EC2::TransitGatewayRouteTable* | 1 |
| *AWS::EC2::TransitGatewayRouteTableAssociation* | 1 |
| *AWS::EC2::TransitGatewayRouteTablePropagation* | 1 |
| *AWS::EC2::VPC* | 4 |
| *AWS::EC2::VPCCidrBlock* | 2 |
| *AWS::EC2::VPCDHCPOptionsAssociation* | 1 |
| *AWS::EC2::VPCEndpoint* | 1 |
| *AWS::EC2::VPCEndpointConnectionNotification* | 1 |
| *AWS::EC2::VPCEndpointService* | 1 |
| *AWS::EC2::VPCEndpointServicePermissions* | 1 |
| *AWS::EC2::VPCGatewayAttachment* | 3 |
| *AWS::EC2::VPCPeeringConnection* | 1 |
| *AWS::EC2::VPNConnection* | 1 |
| *AWS::EC2::VPNConnectionRoute* | 1 |
| *AWS::EC2::VPNGateway* | 1 |
| *AWS::EC2::VPNGatewayRoutePropagation* | 1 |
| *AWS::EC2::Volume* | 1 |
| *AWS::EC2::VolumeAttachment* | 1 |
| *AWS::ECR::Repository* | 2 |
| *AWS::ECS::Cluster* | 1 |
| *AWS::ECS::Service* | 1 |
| *AWS::ECS::TaskDefinition* | 1 |
| *AWS::EFS::FileSystem* | 1 |
| *AWS::EFS::MountTarget* | 1 |
| *AWS::EKS::Cluster* | 1 |
| *AWS::EMR::Cluster* | 1 |
| *AWS::EMR::InstanceFleetConfig* | 1 |
| *AWS::EMR::InstanceGroupConfig* | 1 |
| *AWS::EMR::SecurityConfiguration* | 1 |
| *AWS::EMR::Step* | 1 |
| *AWS::ElastiCache::CacheCluster* | 1 |
| *AWS::ElastiCache::ParameterGroup* | 1 |
| *AWS::ElastiCache::ReplicationGroup* | 1 |
| *AWS::ElastiCache::SecurityGroup* | 1 |
| *AWS::ElastiCache::SecurityGroupIngress* | 1 |
| *AWS::ElastiCache::SubnetGroup* | 1 |
| *AWS::ElasticBeanstalk::Application* | 1 |
| *AWS::ElasticBeanstalk::ApplicationVersion* | 1 |
| *AWS::ElasticBeanstalk::ConfigurationTemplate* | 1 |
| *AWS::ElasticBeanstalk::Environment* | 1 |
| *AWS::ElasticLoadBalancing::LoadBalancer* | 1 |
| *AWS::ElasticLoadBalancingV2::Listener* | 1 |
| *AWS::ElasticLoadBalancingV2::ListenerCertificate* | 1 |
| *AWS::ElasticLoadBalancingV2::ListenerRule* | 1 |
| *AWS::ElasticLoadBalancingV2::LoadBalancer* | 1 |
| *AWS::ElasticLoadBalancingV2::TargetGroup* | 1 |
| *AWS::Elasticsearch::Domain* | 1 |
| *AWS::Events::EventBusPolicy* | 1 |
| *AWS::Events::Rule* | 1 |
| *AWS::FSx::FileSystem* | 1 |
| *AWS::GameLift::Alias* | 1 |
| *AWS::GameLift::Build* | 1 |
| *AWS::GameLift::Fleet* | 1 |
| *AWS::Glue::Classifier* | 1 |
| *AWS::Glue::Connection* | 1 |
| *AWS::Glue::Crawler* | 1 |
| *AWS::Glue::DataCatalogEncryptionSettings* | 0 |
| *AWS::Glue::Database* | 1 |
| *AWS::Glue::DevEndpoint* | 1 |
| *AWS::Glue::Job* | 1 |
| *AWS::Glue::Partition* | 1 |
| *AWS::Glue::SecurityConfiguration* | 0 |
| *AWS::Glue::Table* | 1 |
| *AWS::Glue::Trigger* | 1 |
| *AWS::Greengrass::ConnectorDefinition* | 0 |
| *AWS::Greengrass::ConnectorDefinitionVersion* | 0 |
| *AWS::Greengrass::CoreDefinition* | 0 |
| *AWS::Greengrass::CoreDefinitionVersion* | 0 |
| *AWS::Greengrass::DeviceDefinition* | 0 |
| *AWS::Greengrass::DeviceDefinitionVersion* | 0 |
| *AWS::Greengrass::FunctionDefinition* | 0 |
| *AWS::Greengrass::FunctionDefinitionVersion* | 0 |
| *AWS::Greengrass::Group* | 0 |
| *AWS::Greengrass::GroupVersion* | 0 |
| *AWS::Greengrass::LoggerDefinition* | 0 |
| *AWS::Greengrass::LoggerDefinitionVersion* | 0 |
| *AWS::Greengrass::ResourceDefinition* | 0 |
| *AWS::Greengrass::ResourceDefinitionVersion* | 0 |
| *AWS::Greengrass::SubscriptionDefinition* | 0 |
| *AWS::Greengrass::SubscriptionDefinitionVersion* | 0 |
| *AWS::GuardDuty::Detector* | 1 |
| *AWS::GuardDuty::Filter* | 1 |
| *AWS::GuardDuty::IPSet* | 1 |
| *AWS::GuardDuty::Master* | 1 |
| *AWS::GuardDuty::Member* | 1 |
| *AWS::GuardDuty::ThreatIntelSet* | 1 |
| *AWS::IAM::AccessKey* | 1 |
| *AWS::IAM::Group* | 1 |
| *AWS::IAM::InstanceProfile* | 1 |
| *AWS::IAM::ManagedPolicy* | 1 |
| *AWS::IAM::Policy* | 3 |
| *AWS::IAM::Role* | 1 |
| *AWS::IAM::ServiceLinkedRole* | 1 |
| *AWS::IAM::User* | 1 |
| *AWS::IAM::UserToGroupAddition* | 1 |
| *AWS::Inspector::AssessmentTarget* | 1 |
| *AWS::Inspector::AssessmentTemplate* | 1 |
| *AWS::Inspector::ResourceGroup* | 1 |
| *AWS::IoT1Click::Device* | 1 |
| *AWS::IoT1Click::Placement* | 1 |
| *AWS::IoT1Click::Project* | 1 |
| *AWS::IoT::Certificate* | 1 |
| *AWS::IoT::Policy* | 1 |
| *AWS::IoT::PolicyPrincipalAttachment* | 1 |
| *AWS::IoT::Thing* | 1 |
| *AWS::IoT::ThingPrincipalAttachment* | 1 |
| *AWS::IoT::TopicRule* | 1 |
| *AWS::IoTAnalytics::Channel* | 1 |
| *AWS::IoTAnalytics::Dataset* | 1 |
| *AWS::IoTAnalytics::Datastore* | 1 |
| *AWS::IoTAnalytics::Pipeline* | 1 |
| *AWS::IoTThingsGraph::FlowTemplate* | 0 |
| *AWS::KMS::Alias* | 1 |
| *AWS::KMS::Key* | 2 |
| *AWS::Kinesis::Stream* | 2 |
| *AWS::Kinesis::StreamConsumer* | 1 |
| *AWS::KinesisAnalytics::Application* | 1 |
| *AWS::KinesisAnalytics::ApplicationOutput* | 1 |
| *AWS::KinesisAnalytics::ApplicationReferenceDataSource* | 1 |
| *AWS::KinesisAnalyticsV2::Application* | 1 |
| *AWS::KinesisAnalyticsV2::ApplicationCloudWatchLoggingOption* | 1 |
| *AWS::KinesisAnalyticsV2::ApplicationOutput* | 1 |
| *AWS::KinesisAnalyticsV2::ApplicationReferenceDataSource* | 1 |
| *AWS::KinesisFirehose::DeliveryStream* | 1 |
| *AWS::Lambda::Alias* | 1 |
| *AWS::Lambda::EventSourceMapping* | 1 |
| *AWS::Lambda::Function* | 1 |
| *AWS::Lambda::LayerVersion* | 1 |
| *AWS::Lambda::LayerVersionPermission* | 1 |
| *AWS::Lambda::Permission* | 1 |
| *AWS::Lambda::Version* | 1 |
| *AWS::Logs::Destination* | 1 |
| *AWS::Logs::LogGroup* | 1 |
| *AWS::Logs::LogStream* | 1 |
| *AWS::Logs::MetricFilter* | 1 |
| *AWS::Logs::SubscriptionFilter* | 1 |
| *AWS::MSK::Cluster* | 0 |
| *AWS::MediaLive::Channel* | 0 |
| *AWS::MediaLive::Input* | 0 |
| *AWS::MediaLive::InputSecurityGroup* | 0 |
| *AWS::MediaStore::Container* | 0 |
| *AWS::Neptune::DBCluster* | 1 |
| *AWS::Neptune::DBClusterParameterGroup* | 1 |
| *AWS::Neptune::DBInstance* | 1 |
| *AWS::Neptune::DBParameterGroup* | 1 |
| *AWS::Neptune::DBSubnetGroup* | 1 |
| *AWS::OpsWorks::App* | 1 |
| *AWS::OpsWorks::ElasticLoadBalancerAttachment* | 1 |
| *AWS::OpsWorks::Instance* | 1 |
| *AWS::OpsWorks::Layer* | 1 |
| *AWS::OpsWorks::Stack* | 1 |
| *AWS::OpsWorks::UserProfile* | 1 |
| *AWS::OpsWorks::Volume* | 1 |
| *AWS::OpsWorksCM::Server* | 1 |
| *AWS::Pinpoint::ADMChannel* | 0 |
| *AWS::Pinpoint::APNSChannel* | 0 |
| *AWS::Pinpoint::APNSSandboxChannel* | 0 |
| *AWS::Pinpoint::APNSVoipChannel* | 0 |
| *AWS::Pinpoint::APNSVoipSandboxChannel* | 0 |
| *AWS::Pinpoint::App* | 0 |
| *AWS::Pinpoint::ApplicationSettings* | 0 |
| *AWS::Pinpoint::BaiduChannel* | 0 |
| *AWS::Pinpoint::Campaign* | 0 |
| *AWS::Pinpoint::EmailChannel* | 0 |
| *AWS::Pinpoint::EventStream* | 0 |
| *AWS::Pinpoint::GCMChannel* | 0 |
| *AWS::Pinpoint::SMSChannel* | 0 |
| *AWS::Pinpoint::Segment* | 0 |
| *AWS::Pinpoint::VoiceChannel* | 0 |
| *AWS::PinpointEmail::ConfigurationSet* | 0 |
| *AWS::PinpointEmail::ConfigurationSetEventDestination* | 0 |
| *AWS::PinpointEmail::DedicatedIpPool* | 0 |
| *AWS::PinpointEmail::Identity* | 0 |
| *AWS::RAM::ResourceShare* | 1 |
| *AWS::RDS::DBCluster* | 1 |
| *AWS::RDS::DBClusterParameterGroup* | 1 |
| *AWS::RDS::DBInstance* | 1 |
| *AWS::RDS::DBParameterGroup* | 1 |
| *AWS::RDS::DBSecurityGroup* | 1 |
| *AWS::RDS::DBSecurityGroupIngress* | 1 |
| *AWS::RDS::DBSubnetGroup* | 1 |
| *AWS::RDS::EventSubscription* | 1 |
| *AWS::RDS::OptionGroup* | 1 |
| *AWS::Redshift::Cluster* | 1 |
| *AWS::Redshift::ClusterParameterGroup* | 1 |
| *AWS::Redshift::ClusterSecurityGroup* | 1 |
| *AWS::Redshift::ClusterSecurityGroupIngress* | 1 |
| *AWS::Redshift::ClusterSubnetGroup* | 1 |
| *AWS::RoboMaker::Fleet* | 1 |
| *AWS::RoboMaker::Robot* | 1 |
| *AWS::RoboMaker::RobotApplication* | 1 |
| *AWS::RoboMaker::RobotApplicationVersion* | 1 |
| *AWS::RoboMaker::SimulationApplication* | 1 |
| *AWS::RoboMaker::SimulationApplicationVersion* | 1 |
| *AWS::Route53::HealthCheck* | 1 |
| *AWS::Route53::HostedZone* | 2 |
| *AWS::Route53::RecordSet* | 1 |
| *AWS::Route53::RecordSetGroup* | 1 |
| *AWS::Route53Resolver::ResolverEndpoint* | 1 |
| *AWS::Route53Resolver::ResolverRule* | 1 |
| *AWS::Route53Resolver::ResolverRuleAssociation* | 1 |
| *AWS::S3::Bucket* | 3 |
| *AWS::S3::BucketPolicy* | 1 |
| *AWS::SDB::Domain* | 1 |
| *AWS::SES::ConfigurationSet* | 1 |
| *AWS::SES::ConfigurationSetEventDestination* | 1 |
| *AWS::SES::ReceiptFilter* | 1 |
| *AWS::SES::ReceiptRule* | 1 |
| *AWS::SES::ReceiptRuleSet* | 1 |
| *AWS::SES::Template* | 1 |
| *AWS::SNS::Subscription* | 1 |
| *AWS::SNS::Topic* | 1 |
| *AWS::SNS::TopicPolicy* | 1 |
| *AWS::SQS::Queue* | 1 |
| *AWS::SQS::QueuePolicy* | 1 |
| *AWS::SSM::Association* | 1 |
| *AWS::SSM::Document* | 1 |
| *AWS::SSM::MaintenanceWindow* | 1 |
| *AWS::SSM::MaintenanceWindowTask* | 1 |
| *AWS::SSM::Parameter* | 1 |
| *AWS::SSM::PatchBaseline* | 1 |
| *AWS::SSM::ResourceDataSync* | 1 |
| *AWS::SageMaker::Endpoint* | 1 |
| *AWS::SageMaker::EndpointConfig* | 1 |
| *AWS::SageMaker::Model* | 1 |
| *AWS::SageMaker::NotebookInstance* | 1 |
| *AWS::SageMaker::NotebookInstanceLifecycleConfig* | 1 |
| *AWS::SecretsManager::ResourcePolicy* | 1 |
| *AWS::SecretsManager::RotationSchedule* | 1 |
| *AWS::SecretsManager::Secret* | 1 |
| *AWS::SecretsManager::SecretTargetAttachment* | 1 |
| *AWS::SecurityHub::Hub* | 0 |
| *AWS::ServiceCatalog::AcceptedPortfolioShare* | 1 |
| *AWS::ServiceCatalog::CloudFormationProduct* | 1 |
| *AWS::ServiceCatalog::CloudFormationProvisionedProduct* | 1 |
| *AWS::ServiceCatalog::LaunchNotificationConstraint* | 1 |
| *AWS::ServiceCatalog::LaunchRoleConstraint* | 1 |
| *AWS::ServiceCatalog::LaunchTemplateConstraint* | 1 |
| *AWS::ServiceCatalog::Portfolio* | 1 |
| *AWS::ServiceCatalog::PortfolioPrincipalAssociation* | 1 |
| *AWS::ServiceCatalog::PortfolioProductAssociation* | 1 |
| *AWS::ServiceCatalog::PortfolioShare* | 1 |
| *AWS::ServiceCatalog::ResourceUpdateConstraint* | 0 |
| *AWS::ServiceCatalog::StackSetConstraint* | 0 |
| *AWS::ServiceCatalog::TagOption* | 1 |
| *AWS::ServiceCatalog::TagOptionAssociation* | 1 |
| *AWS::ServiceDiscovery::HttpNamespace* | 1 |
| *AWS::ServiceDiscovery::Instance* | 1 |
| *AWS::ServiceDiscovery::PrivateDnsNamespace* | 2 |
| *AWS::ServiceDiscovery::PublicDnsNamespace* | 2 |
| *AWS::ServiceDiscovery::Service* | 1 |
| *AWS::StepFunctions::Activity* | 1 |
| *AWS::StepFunctions::StateMachine* | 1 |
| *AWS::Transfer::Server* | 0 |
| *AWS::Transfer::User* | 0 |
| *AWS::WAF::ByteMatchSet* | 1 |
| *AWS::WAF::IPSet* | 1 |
| *AWS::WAF::Rule* | 1 |
| *AWS::WAF::SizeConstraintSet* | 1 |
| *AWS::WAF::SqlInjectionMatchSet* | 1 |
| *AWS::WAF::WebACL* | 1 |
| *AWS::WAF::XssMatchSet* | 1 |
| *AWS::WAFRegional::ByteMatchSet* | 1 |
| *AWS::WAFRegional::GeoMatchSet* | 0 |
| *AWS::WAFRegional::IPSet* | 1 |
| *AWS::WAFRegional::RateBasedRule* | 0 |
| *AWS::WAFRegional::RegexPatternSet* | 0 |
| *AWS::WAFRegional::Rule* | 1 |
| *AWS::WAFRegional::SizeConstraintSet* | 1 |
| *AWS::WAFRegional::SqlInjectionMatchSet* | 1 |
| *AWS::WAFRegional::WebACL* | 1 |
| *AWS::WAFRegional::WebACLAssociation* | 1 |
| *AWS::WAFRegional::XssMatchSet* | 1 |
| *AWS::WorkSpaces::Workspace* | 1 |
| *Alexa::ASK::Skill* | 1 |

## Terraform Coverage

**279/500 (55%)** Resources Covered

| Type | Coverage |
| --- | --- |
| *aws_acm_certificate* | 1 |
| *aws_acm_certificate_validation* | 1 |
| *aws_acmpca_certificate_authority* | 1 |
| *aws_ami* | 1 |
| *aws_ami_copy* | 1 |
| *aws_ami_from_instance* | 1 |
| *aws_ami_launch_permission* | 2 |
| *aws_api_gateway_account* | 0 |
| *aws_api_gateway_api_key* | 1 |
| *aws_api_gateway_authorizer* | 0 |
| *aws_api_gateway_base_path_mapping* | 1 |
| *aws_api_gateway_client_certificate* | 1 |
| *aws_api_gateway_deployment* | 1 |
| *aws_api_gateway_documentation_part* | 0 |
| *aws_api_gateway_documentation_version* | 1 |
| *aws_api_gateway_domain_name* | 0 |
| *aws_api_gateway_gateway_response* | 0 |
| *aws_api_gateway_integration* | 1 |
| *aws_api_gateway_integration_response* | 1 |
| *aws_api_gateway_method* | 1 |
| *aws_api_gateway_method_response* | 1 |
| *aws_api_gateway_method_settings* | 0 |
| *aws_api_gateway_model* | 1 |
| *aws_api_gateway_request_validator* | 0 |
| *aws_api_gateway_resource* | 1 |
| *aws_api_gateway_rest_api* | 0 |
| *aws_api_gateway_stage* | 1 |
| *aws_api_gateway_usage_plan* | 1 |
| *aws_api_gateway_usage_plan_key* | 1 |
| *aws_api_gateway_vpc_link* | 1 |
| *aws_app_cookie_stickiness_policy* | 0 |
| *aws_appautoscaling_policy* | 1 |
| *aws_appautoscaling_scheduled_action* | 0 |
| *aws_appautoscaling_target* | 1 |
| *aws_appmesh_mesh* | 1 |
| *aws_appmesh_route* | 1 |
| *aws_appmesh_virtual_node* | 1 |
| *aws_appmesh_virtual_router* | 1 |
| *aws_appmesh_virtual_service* | 0 |
| *aws_appsync_api_key* | 1 |
| *aws_appsync_datasource* | 1 |
| *aws_appsync_graphql_api* | 1 |
| *aws_appsync_resolver* | 0 |
| *aws_athena_database* | 0 |
| *aws_athena_named_query* | 1 |
| *aws_autoscaling_attachment* | 0 |
| *aws_autoscaling_group* | 1 |
| *aws_autoscaling_lifecycle_hook* | 1 |
| *aws_autoscaling_notification* | 0 |
| *aws_autoscaling_policy* | 0 |
| *aws_autoscaling_schedule* | 0 |
| *aws_backup_plan* | 0 |
| *aws_backup_selection* | 0 |
| *aws_backup_vault* | 0 |
| *aws_batch_compute_environment* | 0 |
| *aws_batch_job_definition* | 1 |
| *aws_batch_job_queue* | 1 |
| *aws_budgets_budget* | 0 |
| *aws_cloud9_environment_ec2* | 1 |
| *aws_cloudformation_stack* | 1 |
| *aws_cloudformation_stack_set* | 0 |
| *aws_cloudformation_stack_set_instance* | 0 |
| *aws_cloudfront_distribution* | 0 |
| *aws_cloudfront_origin_access_identity* | 1 |
| *aws_cloudfront_public_key* | 0 |
| *aws_cloudhsm_v2_cluster* | 0 |
| *aws_cloudhsm_v2_hsm* | 0 |
| *aws_cloudtrail* | 1 |
| *aws_cloudwatch_dashboard* | 1 |
| *aws_cloudwatch_event_permission* | 0 |
| *aws_cloudwatch_event_rule* | 1 |
| *aws_cloudwatch_event_target* | 0 |
| *aws_cloudwatch_log_destination* | 0 |
| *aws_cloudwatch_log_destination_policy* | 0 |
| *aws_cloudwatch_log_group* | 1 |
| *aws_cloudwatch_log_metric_filter* | 0 |
| *aws_cloudwatch_log_resource_policy* | 0 |
| *aws_cloudwatch_log_stream* | 1 |
| *aws_cloudwatch_log_subscription_filter* | 1 |
| *aws_cloudwatch_metric_alarm* | 1 |
| *aws_codebuild_project* | 0 |
| *aws_codebuild_webhook* | 0 |
| *aws_codecommit_repository* | 1 |
| *aws_codecommit_trigger* | 0 |
| *aws_codedeploy_app* | 1 |
| *aws_codedeploy_deployment_config* | 1 |
| *aws_codedeploy_deployment_group* | 0 |
| *aws_codepipeline* | 1 |
| *aws_codepipeline_webhook* | 0 |
| *aws_cognito_identity_pool* | 1 |
| *aws_cognito_identity_pool_roles_attachment* | 1 |
| *aws_cognito_identity_provider* | 0 |
| *aws_cognito_resource_server* | 0 |
| *aws_cognito_user_group* | 1 |
| *aws_cognito_user_pool* | 0 |
| *aws_cognito_user_pool_client* | 1 |
| *aws_cognito_user_pool_domain* | 1 |
| *aws_config_aggregate_authorization* | 1 |
| *aws_config_config_rule* | 1 |
| *aws_config_configuration_aggregator* | 0 |
| *aws_config_configuration_recorder* | 1 |
| *aws_config_configuration_recorder_status* | 0 |
| *aws_config_delivery_channel* | 1 |
| *aws_cur_report_definition* | 0 |
| *aws_customer_gateway* | 1 |
| *aws_datasync_agent* | 1 |
| *aws_datasync_location_efs* | 1 |
| *aws_datasync_location_nfs* | 1 |
| *aws_datasync_location_s3* | 1 |
| *aws_datasync_task* | 1 |
| *aws_dax_cluster* | 1 |
| *aws_dax_parameter_group* | 1 |
| *aws_dax_subnet_group* | 1 |
| *aws_db_cluster_snapshot* | 0 |
| *aws_db_event_subscription* | 1 |
| *aws_db_instance* | 1 |
| *aws_db_instance_role_association* | 0 |
| *aws_db_option_group* | 1 |
| *aws_db_parameter_group* | 1 |
| *aws_db_security_group* | 0 |
| *aws_db_snapshot* | 0 |
| *aws_db_subnet_group* | 1 |
| *aws_default_network_acl* | 0 |
| *aws_default_route_table* | 0 |
| *aws_default_security_group* | 0 |
| *aws_default_subnet* | 0 |
| *aws_default_vpc* | 1 |
| *aws_default_vpc_dhcp_options* | 0 |
| *aws_devicefarm_project* | 0 |
| *aws_directory_service_conditional_forwarder* | 0 |
| *aws_directory_service_directory* | 0 |
| *aws_dlm_lifecycle_policy* | 0 |
| *aws_dms_certificate* | 1 |
| *aws_dms_endpoint* | 1 |
| *aws_dms_replication_instance* | 0 |
| *aws_dms_replication_subnet_group* | 1 |
| *aws_dms_replication_task* | 0 |
| *aws_docdb_cluster* | 0 |
| *aws_docdb_cluster_instance* | 0 |
| *aws_docdb_cluster_parameter_group* | 1 |
| *aws_docdb_cluster_snapshot* | 0 |
| *aws_docdb_subnet_group* | 1 |
| *aws_dx_bgp_peer* | 0 |
| *aws_dx_connection* | 0 |
| *aws_dx_connection_association* | 0 |
| *aws_dx_gateway* | 0 |
| *aws_dx_gateway_association* | 0 |
| *aws_dx_gateway_association_proposal* | 0 |
| *aws_dx_hosted_private_virtual_interface* | 0 |
| *aws_dx_hosted_private_virtual_interface_accepter* | 0 |
| *aws_dx_hosted_public_virtual_interface* | 0 |
| *aws_dx_hosted_public_virtual_interface_accepter* | 0 |
| *aws_dx_lag* | 0 |
| *aws_dx_private_virtual_interface* | 0 |
| *aws_dx_public_virtual_interface* | 0 |
| *aws_dynamodb_global_table* | 1 |
| *aws_dynamodb_table* | 1 |
| *aws_dynamodb_table_item* | 1 |
| *aws_ebs_snapshot* | 0 |
| *aws_ebs_snapshot_copy* | 0 |
| *aws_ebs_volume* | 1 |
| *aws_ec2_capacity_reservation* | 0 |
| *aws_ec2_client_vpn_endpoint* | 0 |
| *aws_ec2_client_vpn_network_association* | 0 |
| *aws_ec2_fleet* | 0 |
| *aws_ec2_transit_gateway* | 1 |
| *aws_ec2_transit_gateway_route* | 1 |
| *aws_ec2_transit_gateway_route_table* | 1 |
| *aws_ec2_transit_gateway_route_table_association* | 1 |
| *aws_ec2_transit_gateway_route_table_propagation* | 1 |
| *aws_ec2_transit_gateway_vpc_attachment* | 1 |
| *aws_ec2_transit_gateway_vpc_attachment_accepter* | 0 |
| *aws_ecr_lifecycle_policy* | 1 |
| *aws_ecr_repository* | 2 |
| *aws_ecr_repository_policy* | 1 |
| *aws_ecs_cluster* | 1 |
| *aws_ecs_service* | 0 |
| *aws_ecs_task_definition* | 0 |
| *aws_efs_file_system* | 1 |
| *aws_efs_mount_target* | 1 |
| *aws_egress_only_internet_gateway* | 1 |
| *aws_eip* | 2 |
| *aws_eip_association* | 1 |
| *aws_eks_cluster* | 1 |
| *aws_elastic_beanstalk_application* | 1 |
| *aws_elastic_beanstalk_application_version* | 0 |
| *aws_elastic_beanstalk_configuration_template* | 1 |
| *aws_elastic_beanstalk_environment* | 0 |
| *aws_elasticache_cluster* | 1 |
| *aws_elasticache_parameter_group* | 1 |
| *aws_elasticache_replication_group* | 1 |
| *aws_elasticache_security_group* | 0 |
| *aws_elasticache_subnet_group* | 1 |
| *aws_elasticsearch_domain* | 1 |
| *aws_elasticsearch_domain_policy* | 0 |
| *aws_elastictranscoder_pipeline* | 0 |
| *aws_elastictranscoder_preset* | 0 |
| *aws_elb* | 0 |
| *aws_elb_attachment* | 0 |
| *aws_emr_cluster* | 0 |
| *aws_emr_instance_group* | 0 |
| *aws_emr_security_configuration* | 1 |
| *aws_flow_log* | 1 |
| *aws_gamelift_alias* | 1 |
| *aws_gamelift_build* | 1 |
| *aws_gamelift_fleet* | 0 |
| *aws_gamelift_game_session_queue* | 0 |
| *aws_glacier_vault* | 1 |
| *aws_glacier_vault_lock* | 1 |
| *aws_globalaccelerator_accelerator* | 1 |
| *aws_globalaccelerator_listener* | 0 |
| *aws_glue_catalog_database* | 0 |
| *aws_glue_catalog_table* | 0 |
| *aws_glue_classifier* | 0 |
| *aws_glue_connection* | 0 |
| *aws_glue_crawler* | 0 |
| *aws_glue_job* | 0 |
| *aws_glue_security_configuration* | 0 |
| *aws_glue_trigger* | 0 |
| *aws_guardduty_detector* | 1 |
| *aws_guardduty_invite_accepter* | 0 |
| *aws_guardduty_ipset* | 1 |
| *aws_guardduty_member* | 1 |
| *aws_guardduty_threatintelset* | 1 |
| *aws_iam_access_key* | 1 |
| *aws_iam_account_alias* | 1 |
| *aws_iam_account_password_policy* | 1 |
| *aws_iam_group* | 1 |
| *aws_iam_group_membership* | 0 |
| *aws_iam_group_policy* | 1 |
| *aws_iam_group_policy_attachment* | 1 |
| *aws_iam_instance_profile* | 1 |
| *aws_iam_openid_connect_provider* | 0 |
| *aws_iam_policy* | 1 |
| *aws_iam_policy_attachment* | 0 |
| *aws_iam_role* | 1 |
| *aws_iam_role_policy* | 1 |
| *aws_iam_role_policy_attachment* | 0 |
| *aws_iam_saml_provider* | 0 |
| *aws_iam_server_certificate* | 1 |
| *aws_iam_service_linked_role* | 1 |
| *aws_iam_user* | 1 |
| *aws_iam_user_group_membership* | 1 |
| *aws_iam_user_login_profile* | 1 |
| *aws_iam_user_policy* | 1 |
| *aws_iam_user_policy_attachment* | 0 |
| *aws_iam_user_ssh_key* | 0 |
| *aws_inspector_assessment_target* | 0 |
| *aws_inspector_assessment_template* | 1 |
| *aws_inspector_resource_group* | 0 |
| *aws_instance* | 1 |
| *aws_internet_gateway* | 2 |
| *aws_iot_certificate* | 1 |
| *aws_iot_policy* | 1 |
| *aws_iot_policy_attachment* | 1 |
| *aws_iot_role_alias* | 0 |
| *aws_iot_thing* | 0 |
| *aws_iot_thing_principal_attachment* | 1 |
| *aws_iot_thing_type* | 0 |
| *aws_iot_topic_rule* | 0 |
| *aws_key_pair* | 1 |
| *aws_kinesis_analytics_application* | 1 |
| *aws_kinesis_firehose_delivery_stream* | 1 |
| *aws_kinesis_stream* | 2 |
| *aws_kms_alias* | 1 |
| *aws_kms_ciphertext* | 0 |
| *aws_kms_external_key* | 0 |
| *aws_kms_grant* | 0 |
| *aws_kms_key* | 2 |
| *aws_lambda_alias* | 1 |
| *aws_lambda_event_source_mapping* | 1 |
| *aws_lambda_function* | 1 |
| *aws_lambda_layer_version* | 0 |
| *aws_lambda_permission* | 0 |
| *aws_launch_configuration* | 1 |
| *aws_launch_template* | 1 |
| *aws_lb* | 1 |
| *aws_lb_cookie_stickiness_policy* | 0 |
| *aws_lb_listener* | 1 |
| *aws_lb_listener_certificate* | 1 |
| *aws_lb_listener_rule* | 0 |
| *aws_lb_ssl_negotiation_policy* | 0 |
| *aws_lb_target_group* | 1 |
| *aws_lb_target_group_attachment* | 3 |
| *aws_licensemanager_association* | 1 |
| *aws_licensemanager_license_configuration* | 1 |
| *aws_lightsail_domain* | 1 |
| *aws_lightsail_instance* | 1 |
| *aws_lightsail_key_pair* | 1 |
| *aws_lightsail_static_ip* | 1 |
| *aws_lightsail_static_ip_attachment* | 1 |
| *aws_load_balancer_backend_server_policy* | 0 |
| *aws_load_balancer_listener_policy* | 0 |
| *aws_load_balancer_policy* | 0 |
| *aws_macie_member_account_association* | 0 |
| *aws_macie_s3_bucket_association* | 0 |
| *aws_main_route_table_association* | 0 |
| *aws_media_package_channel* | 0 |
| *aws_media_store_container* | 1 |
| *aws_media_store_container_policy* | 1 |
| *aws_mq_broker* | 1 |
| *aws_mq_configuration* | 1 |
| *aws_msk_cluster* | 0 |
| *aws_msk_configuration* | 0 |
| *aws_nat_gateway* | 1 |
| *aws_neptune_cluster* | 1 |
| *aws_neptune_cluster_instance* | 1 |
| *aws_neptune_cluster_parameter_group* | 1 |
| *aws_neptune_cluster_snapshot* | 0 |
| *aws_neptune_event_subscription* | 0 |
| *aws_neptune_parameter_group* | 1 |
| *aws_neptune_subnet_group* | 1 |
| *aws_network_acl* | 2 |
| *aws_network_acl_rule* | 1 |
| *aws_network_interface* | 2 |
| *aws_network_interface_attachment* | 1 |
| *aws_network_interface_sg_attachment* | 0 |
| *aws_opsworks_application* | 1 |
| *aws_opsworks_custom_layer* | 1 |
| *aws_opsworks_ganglia_layer* | 1 |
| *aws_opsworks_haproxy_layer* | 1 |
| *aws_opsworks_instance* | 1 |
| *aws_opsworks_java_app_layer* | 1 |
| *aws_opsworks_memcached_layer* | 1 |
| *aws_opsworks_mysql_layer* | 1 |
| *aws_opsworks_nodejs_app_layer* | 1 |
| *aws_opsworks_permission* | 0 |
| *aws_opsworks_php_app_layer* | 1 |
| *aws_opsworks_rails_app_layer* | 1 |
| *aws_opsworks_rds_db_instance* | 1 |
| *aws_opsworks_stack* | 1 |
| *aws_opsworks_static_web_layer* | 1 |
| *aws_opsworks_user_profile* | 1 |
| *aws_organizations_account* | 1 |
| *aws_organizations_organization* | 0 |
| *aws_organizations_organizational_unit* | 0 |
| *aws_organizations_policy* | 1 |
| *aws_organizations_policy_attachment* | 0 |
| *aws_pinpoint_adm_channel* | 1 |
| *aws_pinpoint_apns_channel* | 1 |
| *aws_pinpoint_apns_sandbox_channel* | 0 |
| *aws_pinpoint_apns_voip_channel* | 0 |
| *aws_pinpoint_apns_voip_sandbox_channel* | 0 |
| *aws_pinpoint_app* | 0 |
| *aws_pinpoint_baidu_channel* | 1 |
| *aws_pinpoint_email_channel* | 0 |
| *aws_pinpoint_event_stream* | 0 |
| *aws_pinpoint_gcm_channel* | 1 |
| *aws_pinpoint_sms_channel* | 1 |
| *aws_placement_group* | 1 |
| *aws_proxy_protocol_policy* | 0 |
| *aws_ram_principal_association* | 0 |
| *aws_ram_resource_association* | 0 |
| *aws_ram_resource_share* | 1 |
| *aws_rds_cluster* | 1 |
| *aws_rds_cluster_endpoint* | 0 |
| *aws_rds_cluster_instance* | 0 |
| *aws_rds_cluster_parameter_group* | 1 |
| *aws_rds_global_cluster* | 0 |
| *aws_redshift_cluster* | 1 |
| *aws_redshift_event_subscription* | 0 |
| *aws_redshift_parameter_group* | 1 |
| *aws_redshift_security_group* | 0 |
| *aws_redshift_snapshot_copy_grant* | 0 |
| *aws_redshift_subnet_group* | 1 |
| *aws_resourcegroups_group* | 0 |
| *aws_route* | 2 |
| *aws_route53_delegation_set* | 1 |
| *aws_route53_health_check* | 1 |
| *aws_route53_query_log* | 0 |
| *aws_route53_record* | 1 |
| *aws_route53_resolver_endpoint* | 0 |
| *aws_route53_resolver_rule* | 0 |
| *aws_route53_resolver_rule_association* | 0 |
| *aws_route53_zone* | 2 |
| *aws_route53_zone_association* | 0 |
| *aws_route_table* | 3 |
| *aws_route_table_association* | 0 |
| *aws_s3_account_public_access_block* | 0 |
| *aws_s3_bucket* | 3 |
| *aws_s3_bucket_inventory* | 0 |
| *aws_s3_bucket_metric* | 0 |
| *aws_s3_bucket_notification* | 0 |
| *aws_s3_bucket_object* | 0 |
| *aws_s3_bucket_policy* | 1 |
| *aws_s3_bucket_public_access_block* | 0 |
| *aws_sagemaker_endpoint* | 0 |
| *aws_sagemaker_endpoint_configuration* | 0 |
| *aws_sagemaker_model* | 0 |
| *aws_sagemaker_notebook_instance* | 0 |
| *aws_sagemaker_notebook_instance_lifecycle_configuration* | 0 |
| *aws_secretsmanager_secret* | 1 |
| *aws_secretsmanager_secret_version* | 0 |
| *aws_security_group* | 2 |
| *aws_security_group_rule* | 0 |
| *aws_securityhub_account* | 1 |
| *aws_securityhub_product_subscription* | 1 |
| *aws_securityhub_standards_subscription* | 1 |
| *aws_service_discovery_http_namespace* | 0 |
| *aws_service_discovery_private_dns_namespace* | 1 |
| *aws_service_discovery_public_dns_namespace* | 1 |
| *aws_service_discovery_service* | 0 |
| *aws_servicecatalog_portfolio* | 0 |
| *aws_ses_active_receipt_rule_set* | 0 |
| *aws_ses_configuration_set* | 1 |
| *aws_ses_domain_dkim* | 0 |
| *aws_ses_domain_identity* | 0 |
| *aws_ses_domain_identity_verification* | 0 |
| *aws_ses_domain_mail_from* | 0 |
| *aws_ses_email_identity* | 0 |
| *aws_ses_event_destination* | 0 |
| *aws_ses_identity_notification_topic* | 0 |
| *aws_ses_receipt_filter* | 1 |
| *aws_ses_receipt_rule* | 0 |
| *aws_ses_receipt_rule_set* | 1 |
| *aws_ses_template* | 0 |
| *aws_sfn_activity* | 1 |
| *aws_sfn_state_machine* | 1 |
| *aws_shield_protection* | 0 |
| *aws_simpledb_domain* | 1 |
| *aws_snapshot_create_volume_permission* | 0 |
| *aws_sns_platform_application* | 0 |
| *aws_sns_sms_preferences* | 0 |
| *aws_sns_topic* | 1 |
| *aws_sns_topic_policy* | 1 |
| *aws_sns_topic_subscription* | 1 |
| *aws_spot_datafeed_subscription* | 0 |
| *aws_spot_fleet_request* | 0 |
| *aws_spot_instance_request* | 0 |
| *aws_sqs_queue* | 1 |
| *aws_sqs_queue_policy* | 1 |
| *aws_ssm_activation* | 0 |
| *aws_ssm_association* | 0 |
| *aws_ssm_document* | 1 |
| *aws_ssm_maintenance_window* | 1 |
| *aws_ssm_maintenance_window_target* | 1 |
| *aws_ssm_maintenance_window_task* | 0 |
| *aws_ssm_parameter* | 1 |
| *aws_ssm_patch_baseline* | 0 |
| *aws_ssm_patch_group* | 0 |
| *aws_ssm_resource_data_sync* | 1 |
| *aws_storagegateway_cache* | 1 |
| *aws_storagegateway_cached_iscsi_volume* | 1 |
| *aws_storagegateway_gateway* | 1 |
| *aws_storagegateway_nfs_file_share* | 1 |
| *aws_storagegateway_smb_file_share* | 1 |
| *aws_storagegateway_upload_buffer* | 1 |
| *aws_storagegateway_working_storage* | 0 |
| *aws_subnet* | 2 |
| *aws_swf_domain* | 1 |
| *aws_transfer_server* | 2 |
| *aws_transfer_ssh_key* | 1 |
| *aws_transfer_user* | 0 |
| *aws_volume_attachment* | 1 |
| *aws_vpc* | 4 |
| *aws_vpc_dhcp_options* | 1 |
| *aws_vpc_dhcp_options_association* | 1 |
| *aws_vpc_endpoint* | 1 |
| *aws_vpc_endpoint_connection_notification* | 0 |
| *aws_vpc_endpoint_route_table_association* | 0 |
| *aws_vpc_endpoint_service* | 1 |
| *aws_vpc_endpoint_service_allowed_principal* | 1 |
| *aws_vpc_endpoint_subnet_association* | 0 |
| *aws_vpc_ipv4_cidr_block_association* | 1 |
| *aws_vpc_peering_connection* | 1 |
| *aws_vpc_peering_connection_accepter* | 0 |
| *aws_vpc_peering_connection_options* | 0 |
| *aws_vpn_connection* | 1 |
| *aws_vpn_connection_route* | 1 |
| *aws_vpn_gateway* | 1 |
| *aws_vpn_gateway_attachment* | 1 |
| *aws_vpn_gateway_route_propagation* | 1 |
| *aws_waf_byte_match_set* | 1 |
| *aws_waf_geo_match_set* | 0 |
| *aws_waf_ipset* | 0 |
| *aws_waf_rate_based_rule* | 0 |
| *aws_waf_regex_match_set* | 0 |
| *aws_waf_regex_pattern_set* | 0 |
| *aws_waf_rule* | 1 |
| *aws_waf_rule_group* | 0 |
| *aws_waf_size_constraint_set* | 1 |
| *aws_waf_sql_injection_match_set* | 1 |
| *aws_waf_web_acl* | 0 |
| *aws_waf_xss_match_set* | 0 |
| *aws_wafregional_byte_match_set* | 1 |
| *aws_wafregional_geo_match_set* | 0 |
| *aws_wafregional_ipset* | 2 |
| *aws_wafregional_rate_based_rule* | 0 |
| *aws_wafregional_regex_match_set* | 0 |
| *aws_wafregional_regex_pattern_set* | 0 |
| *aws_wafregional_rule* | 1 |
| *aws_wafregional_rule_group* | 0 |
| *aws_wafregional_size_constraint_set* | 0 |
| *aws_wafregional_sql_injection_match_set* | 1 |
| *aws_wafregional_web_acl* | 0 |
| *aws_wafregional_web_acl_association* | 1 |
| *aws_wafregional_xss_match_set* | 1 |
| *aws_worklink_fleet* | 0 |
| *aws_worklink_website_certificate_authority_association* | 0 |
| *aws_xray_sampling_rule* | 0 |

## Service Coverage

| Service | Coverage |
| --- | --- |
| *acm* | 3/13 (23%) |
| *acm-pca* | 2/20 (10%) |
| *amplify* | 11/33 (33%) |
| *apigateway* | 42/120 (35%) |
| *apigatewayv2* | 5/59 (8%) |
| *application-autoscaling* | 2/10 (20%) |
| *appmesh* | 6/28 (21%) |
| *appstream* | 15/47 (31%) |
| *appsync* | 17/36 (47%) |
| *athena* | 1/19 (5%) |
| *autoscaling* | 19/54 (35%) |
| *autoscaling-plans* | 3/6 (50%) |
| *backup* | 17/43 (39%) |
| *batch* | 11/16 (68%) |
| *budgets* | 1/14 (7%) |
| *cloud9* | 6/10 (60%) |
| *cloudformation* | 3/45 (6%) |
| *cloudtrail* | 8/16 (50%) |
| *cloudwatch* | 4/23 (17%) |
| *codebuild* | 4/19 (21%) |
| *codecommit* | 5/54 (9%) |
| *codedeploy* | 15/46 (32%) |
| *codepipeline* | 11/36 (30%) |
| *cognito-identity* | 2/21 (9%) |
| *cognito-idp* | 12/100 (12%) |
| *config* | 9/64 (14%) |
| *datapipeline* | 1/19 (5%) |
| *datasync* | 14/27 (51%) |
| *dax* | 10/21 (47%) |
| *devicefarm* | 6/67 (8%) |
| *dlm* | 4/5 (80%) |
| *dms* | 27/47 (57%) |
| *docdb* | 12/42 (28%) |
| *ds* | 4/50 (8%) |
| *dynamodb* | 5/36 (13%) |
| *ec2* | 142/359 (39%) |
| *ecr* | 10/29 (34%) |
| *ecs* | 5/45 (11%) |
| *efs* | 11/14 (78%) |
| *eks* | 2/11 (18%) |
| *elasticache* | 11/48 (22%) |
| *elasticbeanstalk* | 8/44 (18%) |
| *elb* | 4/29 (13%) |
| *elbv2* | 18/34 (52%) |
| *emr* | 9/29 (31%) |
| *es* | 11/23 (47%) |
| *events* | 11/31 (35%) |
| *firehose* | 3/12 (25%) |
| *fsx* | 3/11 (27%) |
| *gamelift* | 8/73 (10%) |
| *glacier* | 3/33 (9%) |
| *globalaccelerator* | 2/17 (11%) |
| *glue* | 35/123 (28%) |
| *greengrass* | 5/90 (5%) |
| *guardduty* | 25/45 (55%) |
| *iam* | 38/140 (27%) |
| *inspector* | 11/37 (29%) |
| *iot* | 15/178 (8%) |
| *iot1click-devices* | 3/13 (23%) |
| *iot1click-projects* | 3/16 (18%) |
| *iotanalytics* | 11/34 (32%) |
| *kinesis* | 9/28 (32%) |
| *kinesisanalytics* | 9/20 (45%) |
| *kinesisanalyticsv2* | 1/24 (4%) |
| *kms* | 7/41 (17%) |
| *lambda* | 15/39 (38%) |
| *license-manager* | 6/15 (40%) |
| *lightsail* | 21/105 (20%) |
| *logs* | 5/39 (12%) |
| *medialive* | 4/31 (12%) |
| *mediastore* | 7/18 (38%) |
| *mq* | 7/22 (31%) |
| *neptune* | 12/57 (21%) |
| *opsworks* | 18/74 (24%) |
| *opsworkscm* | 4/16 (25%) |
| *organizations* | 15/46 (32%) |
| *pinpoint* | 7/101 (6%) |
| *ram* | 4/17 (23%) |
| *rds* | 25/117 (21%) |
| *redshift* | 5/82 (6%) |
| *rekognition* | 3/34 (8%) |
| *resource-groups* | 1/12 (8%) |
| *robomaker* | 18/36 (50%) |
| *route53* | 9/56 (16%) |
| *route53resolver* | 10/22 (45%) |
| *s3* | 27/88 (30%) |
| *s3control* | 1/8 (12%) |
| *sagemaker* | 16/75 (21%) |
| *secretsmanager* | 6/18 (33%) |
| *securityhub* | 10/38 (26%) |
| *serverlessrepo* | 1/13 (7%) |
| *servicecatalog* | 14/83 (16%) |
| *servicediscovery* | 6/20 (30%) |
| *ses* | 10/71 (14%) |
| *sns* | 8/33 (24%) |
| *sqs* | 6/20 (30%) |
| *ssm* | 21/120 (17%) |
| *stepfunctions* | 7/22 (31%) |
| *storagegateway* | 29/73 (39%) |
| *sts* | 1/8 (12%) |
| *support* | 7/14 (50%) |
| *swf* | 1/37 (2%) |
| *transfer* | 9/18 (50%) |
| *waf* | 32/76 (42%) |
| *waf-regional* | 34/80 (42%) |
| *worklink* | 9/30 (30%) |
| *workspaces* | 4/35 (11%) |
| *xray* | 7/20 (35%) |

## Operation Coverage

| Service | Operation | Occurances |
| --- | --- | --- |
| *acm* | `RenewCertificate` | 0 |
| *acm* | `ResendValidationEmail` | 0 |
| *acm* | `AddTagsToCertificate` | 0 |
| *acm* | `ListCertificates` | 4 |
| *acm* | `UpdateCertificateOptions` | 0 |
| *acm* | `GetCertificate` | 0 |
| *acm* | `ListTagsForCertificate` | 0 |
| *acm* | `RemoveTagsFromCertificate` | 0 |
| *acm* | `RequestCertificate` | 1 |
| *acm* | `ImportCertificate` | 0 |
| *acm* | `DeleteCertificate` | 0 |
| *acm* | `ExportCertificate` | 0 |
| *acm* | `DescribeCertificate` | 1 |
| *acm-pca* | `RestoreCertificateAuthority` | 0 |
| *acm-pca* | `ListCertificateAuthorities` | 2 |
| *acm-pca* | `GetCertificateAuthorityCsr` | 0 |
| *acm-pca* | `DeletePermission` | 0 |
| *acm-pca* | `UntagCertificateAuthority` | 0 |
| *acm-pca* | `UpdateCertificateAuthority` | 0 |
| *acm-pca* | `GetCertificateAuthorityCertificate` | 0 |
| *acm-pca* | `ImportCertificateAuthorityCertificate` | 0 |
| *acm-pca* | `IssueCertificate` | 0 |
| *acm-pca* | `GetCertificate` | 0 |
| *acm-pca* | `TagCertificateAuthority` | 0 |
| *acm-pca* | `ListTags` | 0 |
| *acm-pca* | `ListPermissions` | 0 |
| *acm-pca* | `DescribeCertificateAuthorityAuditReport` | 0 |
| *acm-pca* | `DescribeCertificateAuthority` | 0 |
| *acm-pca* | `CreatePermission` | 0 |
| *acm-pca* | `CreateCertificateAuthority` | 1 |
| *acm-pca* | `DeleteCertificateAuthority` | 0 |
| *acm-pca* | `RevokeCertificate` | 0 |
| *acm-pca* | `CreateCertificateAuthorityAuditReport` | 0 |
| *alexaforbusiness* | `ListSkillsStoreCategories` | 0 |
| *alexaforbusiness* | `DeleteNetworkProfile` | 0 |
| *alexaforbusiness* | `DeleteGatewayGroup` | 0 |
| *alexaforbusiness* | `ListSkillsStoreSkillsByCategory` | 0 |
| *alexaforbusiness* | `CreateProfile` | 0 |
| *alexaforbusiness* | `GetConferencePreference` | 0 |
| *alexaforbusiness* | `SearchNetworkProfiles` | 0 |
| *alexaforbusiness* | `DeleteSkillGroup` | 0 |
| *alexaforbusiness* | `DeleteBusinessReportSchedule` | 0 |
| *alexaforbusiness* | `SendAnnouncement` | 0 |
| *alexaforbusiness* | `DisassociateDeviceFromRoom` | 0 |
| *alexaforbusiness* | `ListBusinessReportSchedules` | 0 |
| *alexaforbusiness* | `GetNetworkProfile` | 0 |
| *alexaforbusiness* | `PutConferencePreference` | 0 |
| *alexaforbusiness* | `ListSkills` | 0 |
| *alexaforbusiness* | `DisassociateSkillGroupFromRoom` | 0 |
| *alexaforbusiness* | `SearchAddressBooks` | 0 |
| *alexaforbusiness* | `CreateUser` | 0 |
| *alexaforbusiness* | `UpdateGateway` | 0 |
| *alexaforbusiness* | `RevokeInvitation` | 0 |
| *alexaforbusiness* | `UpdateContact` | 0 |
| *alexaforbusiness* | `GetContact` | 0 |
| *alexaforbusiness* | `DeleteConferenceProvider` | 0 |
| *alexaforbusiness* | `SearchSkillGroups` | 0 |
| *alexaforbusiness* | `SearchContacts` | 0 |
| *alexaforbusiness* | `SendInvitation` | 0 |
| *alexaforbusiness* | `DeleteRoomSkillParameter` | 0 |
| *alexaforbusiness* | `TagResource` | 0 |
| *alexaforbusiness* | `ListDeviceEvents` | 0 |
| *alexaforbusiness* | `UpdateNetworkProfile` | 0 |
| *alexaforbusiness* | `SearchRooms` | 0 |
| *alexaforbusiness* | `DisassociateSkillFromUsers` | 0 |
| *alexaforbusiness* | `UpdateBusinessReportSchedule` | 0 |
| *alexaforbusiness* | `CreateConferenceProvider` | 0 |
| *alexaforbusiness* | `UntagResource` | 0 |
| *alexaforbusiness* | `SearchDevices` | 0 |
| *alexaforbusiness* | `GetRoomSkillParameter` | 0 |
| *alexaforbusiness* | `ListGatewayGroups` | 0 |
| *alexaforbusiness* | `AssociateContactWithAddressBook` | 0 |
| *alexaforbusiness* | `SearchUsers` | 0 |
| *alexaforbusiness* | `GetProfile` | 0 |
| *alexaforbusiness* | `RegisterAVSDevice` | 0 |
| *alexaforbusiness* | `ApproveSkill` | 0 |
| *alexaforbusiness* | `AssociateDeviceWithNetworkProfile` | 0 |
| *alexaforbusiness* | `PutInvitationConfiguration` | 0 |
| *alexaforbusiness* | `AssociateSkillGroupWithRoom` | 0 |
| *alexaforbusiness* | `StartSmartHomeApplianceDiscovery` | 0 |
| *alexaforbusiness* | `DeleteProfile` | 0 |
| *alexaforbusiness* | `UpdateDevice` | 0 |
| *alexaforbusiness* | `ListTags` | 0 |
| *alexaforbusiness* | `DeleteContact` | 0 |
| *alexaforbusiness* | `DeleteUser` | 0 |
| *alexaforbusiness* | `CreateContact` | 0 |
| *alexaforbusiness* | `CreateGatewayGroup` | 0 |
| *alexaforbusiness* | `ListSmartHomeAppliances` | 0 |
| *alexaforbusiness* | `GetSkillGroup` | 0 |
| *alexaforbusiness* | `ResolveRoom` | 0 |
| *alexaforbusiness* | `GetGatewayGroup` | 0 |
| *alexaforbusiness* | `SearchProfiles` | 0 |
| *alexaforbusiness* | `CreateRoom` | 0 |
| *alexaforbusiness* | `DeleteSkillAuthorization` | 0 |
| *alexaforbusiness* | `ListConferenceProviders` | 0 |
| *alexaforbusiness* | `UpdateAddressBook` | 0 |
| *alexaforbusiness* | `GetConferenceProvider` | 0 |
| *alexaforbusiness* | `AssociateDeviceWithRoom` | 0 |
| *alexaforbusiness* | `GetInvitationConfiguration` | 0 |
| *alexaforbusiness* | `PutSkillAuthorization` | 0 |
| *alexaforbusiness* | `UpdateSkillGroup` | 0 |
| *alexaforbusiness* | `GetRoom` | 0 |
| *alexaforbusiness* | `UpdateGatewayGroup` | 0 |
| *alexaforbusiness* | `CreateSkillGroup` | 0 |
| *alexaforbusiness* | `CreateAddressBook` | 0 |
| *alexaforbusiness* | `AssociateSkillWithUsers` | 0 |
| *alexaforbusiness* | `AssociateSkillWithSkillGroup` | 0 |
| *alexaforbusiness* | `DisassociateSkillFromSkillGroup` | 0 |
| *alexaforbusiness* | `DeleteDevice` | 0 |
| *alexaforbusiness* | `StartDeviceSync` | 0 |
| *alexaforbusiness* | `ListGateways` | 0 |
| *alexaforbusiness* | `RejectSkill` | 0 |
| *alexaforbusiness* | `CreateBusinessReportSchedule` | 0 |
| *alexaforbusiness* | `UpdateRoom` | 0 |
| *alexaforbusiness* | `GetAddressBook` | 0 |
| *alexaforbusiness* | `PutRoomSkillParameter` | 0 |
| *alexaforbusiness* | `GetGateway` | 0 |
| *alexaforbusiness* | `ForgetSmartHomeAppliances` | 0 |
| *alexaforbusiness* | `GetDevice` | 0 |
| *alexaforbusiness* | `DeleteDeviceUsageData` | 0 |
| *alexaforbusiness* | `UpdateConferenceProvider` | 0 |
| *alexaforbusiness* | `UpdateProfile` | 0 |
| *alexaforbusiness* | `DeleteRoom` | 0 |
| *alexaforbusiness* | `DisassociateContactFromAddressBook` | 0 |
| *alexaforbusiness* | `DeleteAddressBook` | 0 |
| *alexaforbusiness* | `CreateNetworkProfile` | 0 |
| *amplify* | `GetArtifactUrl` | 0 |
| *amplify* | `StartJob` | 0 |
| *amplify* | `StartDeployment` | 1 |
| *amplify* | `GetWebhook` | 0 |
| *amplify* | `GetApp` | 0 |
| *amplify* | `ListWebhooks` | 1 |
| *amplify* | `CreateDeployment` | 0 |
| *amplify* | `CreateBranch` | 1 |
| *amplify* | `GetDomainAssociation` | 0 |
| *amplify* | `UpdateDomainAssociation` | 0 |
| *amplify* | `CreateWebhook` | 0 |
| *amplify* | `DeleteApp` | 1 |
| *amplify* | `TagResource` | 0 |
| *amplify* | `ListBranches` | 1 |
| *amplify* | `DeleteWebhook` | 0 |
| *amplify* | `ListDomainAssociations` | 1 |
| *amplify* | `ListJobs` | 1 |
| *amplify* | `StopJob` | 0 |
| *amplify* | `UpdateBranch` | 0 |
| *amplify* | `GetBranch` | 0 |
| *amplify* | `DeleteDomainAssociation` | 0 |
| *amplify* | `GenerateAccessLogs` | 0 |
| *amplify* | `ListArtifacts` | 0 |
| *amplify* | `UpdateWebhook` | 0 |
| *amplify* | `CreateApp` | 1 |
| *amplify* | `UntagResource` | 0 |
| *amplify* | `DeleteBranch` | 1 |
| *amplify* | `DeleteJob` | 0 |
| *amplify* | `CreateDomainAssociation` | 0 |
| *amplify* | `GetJob` | 1 |
| *amplify* | `UpdateApp` | 0 |
| *amplify* | `ListApps` | 1 |
| *amplify* | `ListTagsForResource` | 0 |
| *apigateway* | `GetGatewayResponses` | 0 |
| *apigateway* | `ImportDocumentationParts` | 0 |
| *apigateway* | `DeleteAuthorizer` | 0 |
| *apigateway* | `UpdateModel` | 0 |
| *apigateway* | `CreateDeployment` | 1 |
| *apigateway* | `GetRestApi` | 1 |
| *apigateway* | `UpdateRequestValidator` | 0 |
| *apigateway* | `GetGatewayResponse` | 0 |
| *apigateway* | `DeleteClientCertificate` | 1 |
| *apigateway* | `GetAuthorizer` | 0 |
| *apigateway* | `GetModels` | 0 |
| *apigateway* | `GenerateClientCertificate` | 1 |
| *apigateway* | `PutGatewayResponse` | 1 |
| *apigateway* | `DeleteRestApi` | 0 |
| *apigateway* | `GetMethod` | 0 |
| *apigateway* | `GetSdkTypes` | 0 |
| *apigateway* | `UpdateDocumentationVersion` | 0 |
| *apigateway* | `CreateModel` | 1 |
| *apigateway* | `FlushStageCache` | 0 |
| *apigateway* | `CreateUsagePlanKey` | 1 |
| *apigateway* | `GetUsagePlanKey` | 0 |
| *apigateway* | `DeleteIntegration` | 0 |
| *apigateway* | `GetMethodResponse` | 0 |
| *apigateway* | `PutMethodResponse` | 1 |
| *apigateway* | `DeleteModel` | 1 |
| *apigateway* | `GetStage` | 1 |
| *apigateway* | `GetStages` | 1 |
| *apigateway* | `DeleteDocumentationVersion` | 0 |
| *apigateway* | `UpdateIntegration` | 0 |
| *apigateway* | `DeleteUsagePlan` | 1 |
| *apigateway* | `GetDomainNames` | 1 |
| *apigateway* | `UpdateDocumentationPart` | 0 |
| *apigateway* | `TestInvokeMethod` | 0 |
| *apigateway* | `CreateVpcLink` | 1 |
| *apigateway* | `DeleteVpcLink` | 0 |
| *apigateway* | `UpdateStage` | 0 |
| *apigateway* | `DeleteBasePathMapping` | 0 |
| *apigateway* | `CreateRequestValidator` | 1 |
| *apigateway* | `TestInvokeAuthorizer` | 0 |
| *apigateway* | `GetDocumentationVersion` | 0 |
| *apigateway* | `UpdateMethod` | 0 |
| *apigateway* | `GetApiKey` | 1 |
| *apigateway* | `GetSdkType` | 0 |
| *apigateway* | `GetIntegration` | 0 |
| *apigateway* | `TagResource` | 0 |
| *apigateway* | `UpdateGatewayResponse` | 0 |
| *apigateway* | `DeleteDeployment` | 0 |
| *apigateway* | `GetUsagePlan` | 0 |
| *apigateway* | `GetVpcLink` | 0 |
| *apigateway* | `UpdateUsagePlan` | 0 |
| *apigateway* | `CreateAuthorizer` | 1 |
| *apigateway* | `GetClientCertificate` | 0 |
| *apigateway* | `UpdateDeployment` | 0 |
| *apigateway* | `GetModelTemplate` | 0 |
| *apigateway* | `GetBasePathMappings` | 0 |
| *apigateway* | `GetSdk` | 0 |
| *apigateway* | `GetRequestValidator` | 0 |
| *apigateway* | `FlushStageAuthorizersCache` | 0 |
| *apigateway* | `GetIntegrationResponse` | 0 |
| *apigateway* | `GetDeployments` | 0 |
| *apigateway* | `CreateResource` | 1 |
| *apigateway* | `GetApiKeys` | 0 |
| *apigateway* | `DeleteApiKey` | 0 |
| *apigateway* | `CreateApiKey` | 1 |
| *apigateway* | `UpdateAuthorizer` | 0 |
| *apigateway* | `GetModel` | 0 |
| *apigateway* | `GetUsagePlanKeys` | 0 |
| *apigateway* | `DeleteMethodResponse` | 0 |
| *apigateway* | `GetResources` | 1 |
| *apigateway* | `GetRestApis` | 1 |
| *apigateway* | `GetResource` | 0 |
| *apigateway* | `DeleteIntegrationResponse` | 0 |
| *apigateway* | `GetUsagePlans` | 1 |
| *apigateway* | `ImportApiKeys` | 0 |
| *apigateway* | `GetBasePathMapping` | 0 |
| *apigateway* | `GetExport` | 0 |
| *apigateway* | `UpdateResource` | 0 |
| *apigateway* | `GetDomainName` | 0 |
| *apigateway* | `UpdateDomainName` | 0 |
| *apigateway* | `UpdateClientCertificate` | 0 |
| *apigateway* | `CreateDomainName` | 1 |
| *apigateway* | `DeleteDomainName` | 0 |
| *apigateway* | `PutMethod` | 1 |
| *apigateway* | `DeleteStage` | 1 |
| *apigateway* | `GetDocumentationPart` | 0 |
| *apigateway* | `UpdateAccount` | 1 |
| *apigateway* | `PutRestApi` | 0 |
| *apigateway* | `GetUsage` | 0 |
| *apigateway* | `GetDeployment` | 1 |
| *apigateway* | `CreateRestApi` | 1 |
| *apigateway* | `GetTags` | 0 |
| *apigateway* | `UpdateVpcLink` | 0 |
| *apigateway* | `GetAuthorizers` | 1 |
| *apigateway* | `GetClientCertificates` | 0 |
| *apigateway* | `DeleteDocumentationPart` | 1 |
| *apigateway* | `DeleteResource` | 0 |
| *apigateway* | `CreateStage` | 1 |
| *apigateway* | `UpdateBasePathMapping` | 0 |
| *apigateway* | `UpdateMethodResponse` | 0 |
| *apigateway* | `UpdateUsage` | 0 |
| *apigateway* | `GetDocumentationParts` | 1 |
| *apigateway* | `CreateDocumentationPart` | 1 |
| *apigateway* | `CreateBasePathMapping` | 1 |
| *apigateway* | `CreateUsagePlan` | 1 |
| *apigateway* | `UpdateApiKey` | 1 |
| *apigateway* | `DeleteGatewayResponse` | 0 |
| *apigateway* | `ImportRestApi` | 0 |
| *apigateway* | `UpdateRestApi` | 0 |
| *apigateway* | `CreateDocumentationVersion` | 1 |
| *apigateway* | `PutIntegrationResponse` | 1 |
| *apigateway* | `GetDocumentationVersions` | 0 |
| *apigateway* | `DeleteMethod` | 1 |
| *apigateway* | `GetRequestValidators` | 1 |
| *apigateway* | `GetAccount` | 1 |
| *apigateway* | `DeleteUsagePlanKey` | 0 |
| *apigateway* | `DeleteRequestValidator` | 0 |
| *apigateway* | `UntagResource` | 0 |
| *apigateway* | `PutIntegration` | 1 |
| *apigateway* | `UpdateIntegrationResponse` | 0 |
| *apigateway* | `GetVpcLinks` | 0 |
| *apigatewaymanagementapi* | `DeleteConnection` | 0 |
| *apigatewaymanagementapi* | `PostToConnection` | 0 |
| *apigatewaymanagementapi* | `GetConnection` | 0 |
| *apigatewayv2* | `GetStages` | 0 |
| *apigatewayv2* | `GetTags` | 0 |
| *apigatewayv2* | `UpdateIntegration` | 0 |
| *apigatewayv2* | `GetDomainNames` | 0 |
| *apigatewayv2* | `GetApi` | 0 |
| *apigatewayv2* | `GetApiMapping` | 0 |
| *apigatewayv2* | `GetRouteResponse` | 0 |
| *apigatewayv2* | `DeleteRoute` | 0 |
| *apigatewayv2* | `DeleteAuthorizer` | 0 |
| *apigatewayv2* | `CreateRouteResponse` | 1 |
| *apigatewayv2* | `UpdateModel` | 0 |
| *apigatewayv2* | `UpdateStage` | 0 |
| *apigatewayv2* | `CreateDeployment` | 0 |
| *apigatewayv2* | `CreateIntegrationResponse` | 1 |
| *apigatewayv2* | `UpdateIntegrationResponse` | 0 |
| *apigatewayv2* | `CreateStage` | 0 |
| *apigatewayv2* | `UpdateApi` | 0 |
| *apigatewayv2* | `GetModelTemplate` | 0 |
| *apigatewayv2* | `CreateRoute` | 1 |
| *apigatewayv2* | `DeleteApi` | 0 |
| *apigatewayv2* | `CreateIntegration` | 1 |
| *apigatewayv2* | `UpdateRoute` | 0 |
| *apigatewayv2* | `GetAuthorizer` | 0 |
| *apigatewayv2* | `DeleteIntegrationResponse` | 0 |
| *apigatewayv2* | `GetModels` | 0 |
| *apigatewayv2* | `GetIntegration` | 0 |
| *apigatewayv2* | `UpdateApiMapping` | 0 |
| *apigatewayv2* | `TagResource` | 0 |
| *apigatewayv2* | `GetRouteResponses` | 0 |
| *apigatewayv2* | `DeleteDeployment` | 0 |
| *apigatewayv2* | `GetAuthorizers` | 0 |
| *apigatewayv2* | `DeleteRouteResponse` | 0 |
| *apigatewayv2* | `GetModel` | 0 |
| *apigatewayv2* | `GetRoute` | 0 |
| *apigatewayv2* | `GetApis` | 0 |
| *apigatewayv2* | `GetIntegrations` | 0 |
| *apigatewayv2* | `UpdateRouteResponse` | 0 |
| *apigatewayv2* | `CreateAuthorizer` | 0 |
| *apigatewayv2* | `UpdateAuthorizer` | 0 |
| *apigatewayv2* | `GetDomainName` | 0 |
| *apigatewayv2* | `UpdateDomainName` | 0 |
| *apigatewayv2* | `UpdateDeployment` | 0 |
| *apigatewayv2* | `CreateModel` | 0 |
| *apigatewayv2* | `CreateDomainName` | 0 |
| *apigatewayv2* | `DeleteDomainName` | 0 |
| *apigatewayv2* | `UntagResource` | 0 |
| *apigatewayv2* | `DeleteStage` | 0 |
| *apigatewayv2* | `DeleteIntegration` | 0 |
| *apigatewayv2* | `GetRoutes` | 0 |
| *apigatewayv2* | `GetIntegrationResponse` | 0 |
| *apigatewayv2* | `GetDeployments` | 0 |
| *apigatewayv2* | `GetApiMappings` | 0 |
| *apigatewayv2* | `DeleteModel` | 0 |
| *apigatewayv2* | `CreateApiMapping` | 0 |
| *apigatewayv2* | `GetStage` | 0 |
| *apigatewayv2* | `DeleteApiMapping` | 0 |
| *apigatewayv2* | `GetDeployment` | 0 |
| *apigatewayv2* | `CreateApi` | 1 |
| *apigatewayv2* | `GetIntegrationResponses` | 0 |
| *application-autoscaling* | `DescribeScalingActivities` | 0 |
| *application-autoscaling* | `DescribeScalingPolicies` | 0 |
| *application-autoscaling* | `DeleteScheduledAction` | 0 |
| *application-autoscaling* | `PutScalingPolicy` | 1 |
| *application-autoscaling* | `RegisterScalableTarget` | 1 |
| *application-autoscaling* | `DeregisterScalableTarget` | 0 |
| *application-autoscaling* | `PutScheduledAction` | 0 |
| *application-autoscaling* | `DeleteScalingPolicy` | 0 |
| *application-autoscaling* | `DescribeScalableTargets` | 0 |
| *application-autoscaling* | `DescribeScheduledActions` | 0 |
| *application-insights* | `ListProblems` | 0 |
| *application-insights* | `UpdateApplication` | 0 |
| *application-insights* | `UpdateComponent` | 0 |
| *application-insights* | `DescribeComponentConfiguration` | 0 |
| *application-insights* | `ListApplications` | 0 |
| *application-insights* | `DescribeProblem` | 0 |
| *application-insights* | `ListComponents` | 0 |
| *application-insights* | `DescribeComponent` | 0 |
| *application-insights* | `CreateComponent` | 0 |
| *application-insights* | `DescribeObservation` | 0 |
| *application-insights* | `DescribeApplication` | 0 |
| *application-insights* | `UpdateComponentConfiguration` | 0 |
| *application-insights* | `DescribeComponentConfigurationRecommendation` | 0 |
| *application-insights* | `DeleteApplication` | 0 |
| *application-insights* | `DescribeProblemObservations` | 0 |
| *application-insights* | `DeleteComponent` | 0 |
| *application-insights* | `CreateApplication` | 0 |
| *appmesh* | `DescribeRoute` | 0 |
| *appmesh* | `CreateVirtualNode` | 1 |
| *appmesh* | `ListVirtualNodes` | 0 |
| *appmesh* | `ListMeshes` | 1 |
| *appmesh* | `DeleteVirtualService` | 0 |
| *appmesh* | `UpdateVirtualNode` | 0 |
| *appmesh* | `ListRoutes` | 0 |
| *appmesh* | `UpdateRoute` | 0 |
| *appmesh* | `DescribeVirtualService` | 0 |
| *appmesh* | `DeleteVirtualRouter` | 0 |
| *appmesh* | `CreateRoute` | 0 |
| *appmesh* | `DescribeMesh` | 1 |
| *appmesh* | `UpdateVirtualRouter` | 0 |
| *appmesh* | `DeleteVirtualNode` | 0 |
| *appmesh* | `UpdateMesh` | 0 |
| *appmesh* | `TagResource` | 0 |
| *appmesh* | `DescribeVirtualNode` | 0 |
| *appmesh* | `ListVirtualServices` | 0 |
| *appmesh* | `CreateVirtualRouter` | 0 |
| *appmesh* | `ListVirtualRouters` | 0 |
| *appmesh* | `CreateMesh` | 1 |
| *appmesh* | `DeleteMesh` | 1 |
| *appmesh* | `DescribeVirtualRouter` | 1 |
| *appmesh* | `UntagResource` | 0 |
| *appmesh* | `CreateVirtualService` | 0 |
| *appmesh* | `DeleteRoute` | 0 |
| *appmesh* | `UpdateVirtualService` | 0 |
| *appmesh* | `ListTagsForResource` | 0 |
| *appstream* | `StopImageBuilder` | 0 |
| *appstream* | `DeleteFleet` | 1 |
| *appstream* | `ListAssociatedStacks` | 0 |
| *appstream* | `ExpireSession` | 0 |
| *appstream* | `AssociateFleet` | 1 |
| *appstream* | `CreateImageBuilder` | 1 |
| *appstream* | `DeleteImageBuilder` | 1 |
| *appstream* | `DescribeImages` | 1 |
| *appstream* | `EnableUser` | 0 |
| *appstream* | `DescribeFleets` | 1 |
| *appstream* | `DescribeUsers` | 0 |
| *appstream* | `CreateDirectoryConfig` | 1 |
| *appstream* | `BatchDisassociateUserStack` | 0 |
| *appstream* | `UpdateImagePermissions` | 0 |
| *appstream* | `CreateStack` | 1 |
| *appstream* | `DescribeDirectoryConfigs` | 0 |
| *appstream* | `DisableUser` | 0 |
| *appstream* | `TagResource` | 0 |
| *appstream* | `CreateImageBuilderStreamingURL` | 0 |
| *appstream* | `UpdateDirectoryConfig` | 0 |
| *appstream* | `CopyImage` | 0 |
| *appstream* | `DescribeUsageReportSubscriptions` | 0 |
| *appstream* | `CreateFleet` | 1 |
| *appstream* | `DescribeSessions` | 0 |
| *appstream* | `DescribeImagePermissions` | 0 |
| *appstream* | `UpdateStack` | 0 |
| *appstream* | `DeleteUser` | 0 |
| *appstream* | `StopFleet` | 0 |
| *appstream* | `CreateUsageReportSubscription` | 0 |
| *appstream* | `UpdateFleet` | 0 |
| *appstream* | `DeleteUsageReportSubscription` | 0 |
| *appstream* | `DescribeImageBuilders` | 1 |
| *appstream* | `ListAssociatedFleets` | 0 |
| *appstream* | `StartFleet` | 0 |
| *appstream* | `DescribeUserStackAssociations` | 0 |
| *appstream* | `DescribeStacks` | 1 |
| *appstream* | `BatchAssociateUserStack` | 1 |
| *appstream* | `ListTagsForResource` | 0 |
| *appstream* | `CreateStreamingURL` | 0 |
| *appstream* | `UntagResource` | 0 |
| *appstream* | `DisassociateFleet` | 0 |
| *appstream* | `DeleteImagePermissions` | 0 |
| *appstream* | `DeleteStack` | 1 |
| *appstream* | `CreateUser` | 1 |
| *appstream* | `DeleteDirectoryConfig` | 1 |
| *appstream* | `DeleteImage` | 0 |
| *appstream* | `StartImageBuilder` | 0 |
| *appsync* | `UpdateResolver` | 0 |
| *appsync* | `DeleteFunction` | 1 |
| *appsync* | `GetGraphqlApi` | 1 |
| *appsync* | `DeleteResolver` | 0 |
| *appsync* | `ListTypes` | 1 |
| *appsync* | `ListFunctions` | 1 |
| *appsync* | `ListApiKeys` | 1 |
| *appsync* | `ListTagsForResource` | 0 |
| *appsync* | `ListGraphqlApis` | 1 |
| *appsync* | `DeleteGraphqlApi` | 1 |
| *appsync* | `GetIntrospectionSchema` | 0 |
| *appsync* | `GetFunction` | 0 |
| *appsync* | `CreateResolver` | 1 |
| *appsync* | `GetResolver` | 0 |
| *appsync* | `UpdateDataSource` | 0 |
| *appsync* | `ListResolvers` | 1 |
| *appsync* | `UpdateApiKey` | 0 |
| *appsync* | `GetSchemaCreationStatus` | 1 |
| *appsync* | `UpdateType` | 0 |
| *appsync* | `TagResource` | 0 |
| *appsync* | `CreateGraphqlApi` | 1 |
| *appsync* | `CreateType` | 0 |
| *appsync* | `ListResolversByFunction` | 0 |
| *appsync* | `DeleteDataSource` | 0 |
| *appsync* | `CreateApiKey` | 1 |
| *appsync* | `ListDataSources` | 1 |
| *appsync* | `StartSchemaCreation` | 1 |
| *appsync* | `CreateDataSource` | 1 |
| *appsync* | `UntagResource` | 0 |
| *appsync* | `UpdateFunction` | 0 |
| *appsync* | `GetType` | 0 |
| *appsync* | `GetDataSource` | 0 |
| *appsync* | `CreateFunction` | 1 |
| *appsync* | `DeleteType` | 0 |
| *appsync* | `DeleteApiKey` | 0 |
| *appsync* | `UpdateGraphqlApi` | 1 |
| *athena* | `GetQueryResults` | 0 |
| *athena* | `ListWorkGroups` | 0 |
| *athena* | `ListTagsForResource` | 0 |
| *athena* | `BatchGetQueryExecution` | 0 |
| *athena* | `UntagResource` | 0 |
| *athena* | `CreateWorkGroup` | 0 |
| *athena* | `DeleteNamedQuery` | 0 |
| *athena* | `ListNamedQueries` | 0 |
| *athena* | `UpdateWorkGroup` | 0 |
| *athena* | `TagResource` | 0 |
| *athena* | `GetWorkGroup` | 0 |
| *athena* | `BatchGetNamedQuery` | 0 |
| *athena* | `StopQueryExecution` | 0 |
| *athena* | `StartQueryExecution` | 0 |
| *athena* | `GetNamedQuery` | 0 |
| *athena* | `ListQueryExecutions` | 0 |
| *athena* | `GetQueryExecution` | 0 |
| *athena* | `DeleteWorkGroup` | 0 |
| *athena* | `CreateNamedQuery` | 1 |
| *autoscaling* | `DeleteTags` | 0 |
| *autoscaling* | `DescribeScalingActivities` | 1 |
| *autoscaling* | `CreateAutoScalingGroup` | 1 |
| *autoscaling* | `ExecutePolicy` | 0 |
| *autoscaling* | `SetInstanceProtection` | 0 |
| *autoscaling* | `DetachLoadBalancerTargetGroups` | 0 |
| *autoscaling* | `DescribeNotificationConfigurations` | 1 |
| *autoscaling* | `DeleteAutoScalingGroup` | 1 |
| *autoscaling* | `DeletePolicy` | 0 |
| *autoscaling* | `ExitStandby` | 0 |
| *autoscaling* | `SetInstanceHealth` | 0 |
| *autoscaling* | `CreateOrUpdateTags` | 0 |
| *autoscaling* | `DescribeScalingProcessTypes` | 0 |
| *autoscaling* | `PutLifecycleHook` | 1 |
| *autoscaling* | `DetachLoadBalancers` | 0 |
| *autoscaling* | `AttachInstances` | 0 |
| *autoscaling* | `PutScheduledUpdateGroupAction` | 1 |
| *autoscaling* | `PutScalingPolicy` | 1 |
| *autoscaling* | `DeleteScheduledAction` | 1 |
| *autoscaling* | `AttachLoadBalancers` | 0 |
| *autoscaling* | `DetachInstances` | 0 |
| *autoscaling* | `DescribeAdjustmentTypes` | 0 |
| *autoscaling* | `TerminateInstanceInAutoScalingGroup` | 0 |
| *autoscaling* | `AttachLoadBalancerTargetGroups` | 0 |
| *autoscaling* | `DescribeLoadBalancers` | 1 |
| *autoscaling* | `CompleteLifecycleAction` | 0 |
| *autoscaling* | `DescribeMetricCollectionTypes` | 0 |
| *autoscaling* | `UpdateAutoScalingGroup` | 0 |
| *autoscaling* | `RecordLifecycleActionHeartbeat` | 0 |
| *autoscaling* | `PutNotificationConfiguration` | 1 |
| *autoscaling* | `ResumeProcesses` | 0 |
| *autoscaling* | `CreateLaunchConfiguration` | 1 |
| *autoscaling* | `DescribeLifecycleHookTypes` | 0 |
| *autoscaling* | `DescribeAutoScalingInstances` | 0 |
| *autoscaling* | `DescribeTags` | 1 |
| *autoscaling* | `BatchDeleteScheduledAction` | 0 |
| *autoscaling* | `DisableMetricsCollection` | 0 |
| *autoscaling* | `BatchPutScheduledUpdateGroupAction` | 0 |
| *autoscaling* | `DescribeLaunchConfigurations` | 1 |
| *autoscaling* | `EnterStandby` | 0 |
| *autoscaling* | `SetDesiredCapacity` | 0 |
| *autoscaling* | `DescribeLoadBalancerTargetGroups` | 0 |
| *autoscaling* | `DeleteLaunchConfiguration` | 1 |
| *autoscaling* | `DeleteLifecycleHook` | 1 |
| *autoscaling* | `DescribePolicies` | 1 |
| *autoscaling* | `DescribeLifecycleHooks` | 1 |
| *autoscaling* | `DescribeTerminationPolicyTypes` | 0 |
| *autoscaling* | `DescribeScheduledActions` | 1 |
| *autoscaling* | `DescribeAutoScalingGroups` | 1 |
| *autoscaling* | `EnableMetricsCollection` | 0 |
| *autoscaling* | `DeleteNotificationConfiguration` | 0 |
| *autoscaling* | `DescribeAutoScalingNotificationTypes` | 0 |
| *autoscaling* | `SuspendProcesses` | 0 |
| *autoscaling* | `DescribeAccountLimits` | 0 |
| *autoscaling-plans* | `CreateScalingPlan` | 1 |
| *autoscaling-plans* | `UpdateScalingPlan` | 0 |
| *autoscaling-plans* | `DescribeScalingPlans` | 1 |
| *autoscaling-plans* | `DescribeScalingPlanResources` | 0 |
| *autoscaling-plans* | `GetScalingPlanResourceForecastData` | 0 |
| *autoscaling-plans* | `DeleteScalingPlan` | 1 |
| *backup* | `ListBackupPlans` | 1 |
| *backup* | `ExportBackupPlanTemplate` | 0 |
| *backup* | `GetBackupPlanFromTemplate` | 1 |
| *backup* | `DeleteBackupVaultNotifications` | 0 |
| *backup* | `ListBackupPlanVersions` | 0 |
| *backup* | `ListRecoveryPointsByResource` | 0 |
| *backup* | `ListRecoveryPointsByBackupVault` | 1 |
| *backup* | `DescribeRecoveryPoint` | 0 |
| *backup* | `ListTags` | 1 |
| *backup* | `PutBackupVaultNotifications` | 0 |
| *backup* | `ListBackupPlanTemplates` | 1 |
| *backup* | `UpdateBackupPlan` | 0 |
| *backup* | `DescribeProtectedResource` | 0 |
| *backup* | `GetBackupVaultNotifications` | 0 |
| *backup* | `GetSupportedResourceTypes` | 0 |
| *backup* | `DeleteRecoveryPoint` | 0 |
| *backup* | `PutBackupVaultAccessPolicy` | 0 |
| *backup* | `UpdateRecoveryPointLifecycle` | 0 |
| *backup* | `DescribeBackupJob` | 0 |
| *backup* | `DeleteBackupVault` | 1 |
| *backup* | `GetBackupSelection` | 0 |
| *backup* | `ListRestoreJobs` | 1 |
| *backup* | `DeleteBackupVaultAccessPolicy` | 0 |
| *backup* | `TagResource` | 0 |
| *backup* | `StopBackupJob` | 0 |
| *backup* | `DeleteBackupPlan` | 1 |
| *backup* | `CreateBackupVault` | 1 |
| *backup* | `ListBackupJobs` | 1 |
| *backup* | `DeleteBackupSelection` | 0 |
| *backup* | `ListBackupVaults` | 1 |
| *backup* | `GetBackupVaultAccessPolicy` | 1 |
| *backup* | `UntagResource` | 0 |
| *backup* | `ListBackupSelections` | 1 |
| *backup* | `StartBackupJob` | 0 |
| *backup* | `ListProtectedResources` | 1 |
| *backup* | `DescribeBackupVault` | 1 |
| *backup* | `DescribeRestoreJob` | 0 |
| *backup* | `GetBackupPlanFromJSON` | 0 |
| *backup* | `CreateBackupSelection` | 0 |
| *backup* | `CreateBackupPlan` | 1 |
| *backup* | `StartRestoreJob` | 0 |
| *backup* | `GetRecoveryPointRestoreMetadata` | 0 |
| *backup* | `GetBackupPlan` | 1 |
| *batch* | `ListJobs` | 1 |
| *batch* | `SubmitJob` | 1 |
| *batch* | `UpdateJobQueue` | 0 |
| *batch* | `UpdateComputeEnvironment` | 1 |
| *batch* | `DeregisterJobDefinition` | 1 |
| *batch* | `DescribeJobQueues` | 0 |
| *batch* | `CreateComputeEnvironment` | 1 |
| *batch* | `DeleteComputeEnvironment` | 1 |
| *batch* | `DescribeComputeEnvironments` | 2 |
| *batch* | `CreateJobQueue` | 1 |
| *batch* | `RegisterJobDefinition` | 1 |
| *batch* | `CancelJob` | 1 |
| *batch* | `DescribeJobDefinitions` | 1 |
| *batch* | `DeleteJobQueue` | 0 |
| *batch* | `DescribeJobs` | 0 |
| *batch* | `TerminateJob` | 0 |
| *budgets* | `DescribeBudget` | 0 |
| *budgets* | `DescribeNotificationsForBudget` | 0 |
| *budgets* | `DescribeBudgetPerformanceHistory` | 0 |
| *budgets* | `CreateSubscriber` | 0 |
| *budgets* | `CreateNotification` | 0 |
| *budgets* | `UpdateNotification` | 0 |
| *budgets* | `UpdateBudget` | 0 |
| *budgets* | `CreateBudget` | 1 |
| *budgets* | `DeleteNotification` | 0 |
| *budgets* | `DescribeSubscribersForNotification` | 0 |
| *budgets* | `DeleteSubscriber` | 0 |
| *budgets* | `DescribeBudgets` | 0 |
| *budgets* | `DeleteBudget` | 0 |
| *budgets* | `UpdateSubscriber` | 0 |
| *ce* | `GetTags` | 0 |
| *ce* | `GetRightsizingRecommendation` | 0 |
| *ce* | `GetReservationPurchaseRecommendation` | 0 |
| *ce* | `GetReservationUtilization` | 0 |
| *ce* | `GetUsageForecast` | 0 |
| *ce* | `GetDimensionValues` | 0 |
| *ce* | `GetCostAndUsage` | 0 |
| *ce* | `GetCostForecast` | 0 |
| *ce* | `GetReservationCoverage` | 0 |
| *chime* | `GetVoiceConnectorStreamingConfiguration` | 0 |
| *chime* | `AssociatePhoneNumbersWithVoiceConnectorGroup` | 0 |
| *chime* | `PutVoiceConnectorOrigination` | 0 |
| *chime* | `AssociatePhoneNumberWithUser` | 0 |
| *chime* | `UpdatePhoneNumberSettings` | 0 |
| *chime* | `GetPhoneNumberSettings` | 0 |
| *chime* | `RestorePhoneNumber` | 0 |
| *chime* | `GetBot` | 0 |
| *chime* | `CreateVoiceConnector` | 0 |
| *chime* | `ListUsers` | 0 |
| *chime* | `ResetPersonalPIN` | 0 |
| *chime* | `RegenerateSecurityToken` | 0 |
| *chime* | `ListPhoneNumbers` | 0 |
| *chime* | `PutEventsConfiguration` | 0 |
| *chime* | `LogoutUser` | 0 |
| *chime* | `GetPhoneNumberOrder` | 0 |
| *chime* | `UpdateGlobalSettings` | 0 |
| *chime* | `DeletePhoneNumber` | 0 |
| *chime* | `PutVoiceConnectorTermination` | 0 |
| *chime* | `UpdateUser` | 0 |
| *chime* | `ListPhoneNumberOrders` | 0 |
| *chime* | `DeleteVoiceConnector` | 0 |
| *chime* | `DisassociatePhoneNumbersFromVoiceConnectorGroup` | 0 |
| *chime* | `DisassociatePhoneNumbersFromVoiceConnector` | 0 |
| *chime* | `DeleteAccount` | 0 |
| *chime* | `PutVoiceConnectorStreamingConfiguration` | 0 |
| *chime* | `CreateBot` | 0 |
| *chime* | `GetGlobalSettings` | 0 |
| *chime* | `UpdateBot` | 0 |
| *chime* | `PutVoiceConnectorLoggingConfiguration` | 0 |
| *chime* | `BatchUpdateUser` | 0 |
| *chime* | `GetVoiceConnectorOrigination` | 0 |
| *chime* | `GetUser` | 0 |
| *chime* | `GetVoiceConnectorTermination` | 0 |
| *chime* | `DeleteVoiceConnectorTerminationCredentials` | 0 |
| *chime* | `ListVoiceConnectors` | 0 |
| *chime* | `DeleteVoiceConnectorStreamingConfiguration` | 0 |
| *chime* | `GetVoiceConnectorTerminationHealth` | 0 |
| *chime* | `UpdateUserSettings` | 0 |
| *chime* | `CreateAccount` | 0 |
| *chime* | `CreatePhoneNumberOrder` | 0 |
| *chime* | `ListAccounts` | 0 |
| *chime* | `DeleteVoiceConnectorTermination` | 0 |
| *chime* | `GetPhoneNumber` | 0 |
| *chime* | `UpdatePhoneNumber` | 0 |
| *chime* | `BatchUpdatePhoneNumber` | 0 |
| *chime* | `UpdateVoiceConnector` | 0 |
| *chime* | `PutVoiceConnectorTerminationCredentials` | 0 |
| *chime* | `UpdateVoiceConnectorGroup` | 0 |
| *chime* | `GetAccount` | 0 |
| *chime* | `GetVoiceConnectorLoggingConfiguration` | 0 |
| *chime* | `InviteUsers` | 0 |
| *chime* | `GetEventsConfiguration` | 0 |
| *chime* | `SearchAvailablePhoneNumbers` | 0 |
| *chime* | `GetVoiceConnector` | 0 |
| *chime* | `DisassociatePhoneNumberFromUser` | 0 |
| *chime* | `DeleteEventsConfiguration` | 0 |
| *chime* | `UpdateAccount` | 0 |
| *chime* | `GetUserSettings` | 0 |
| *chime* | `BatchSuspendUser` | 0 |
| *chime* | `ListVoiceConnectorTerminationCredentials` | 0 |
| *chime* | `BatchUnsuspendUser` | 0 |
| *chime* | `CreateVoiceConnectorGroup` | 0 |
| *chime* | `BatchDeletePhoneNumber` | 0 |
| *chime* | `DeleteVoiceConnectorOrigination` | 0 |
| *chime* | `ListBots` | 0 |
| *chime* | `GetVoiceConnectorGroup` | 0 |
| *chime* | `GetAccountSettings` | 0 |
| *chime* | `UpdateAccountSettings` | 0 |
| *chime* | `ListVoiceConnectorGroups` | 0 |
| *chime* | `AssociatePhoneNumbersWithVoiceConnector` | 0 |
| *chime* | `DeleteVoiceConnectorGroup` | 0 |
| *cloud9* | `UpdateEnvironment` | 1 |
| *cloud9* | `CreateEnvironmentMembership` | 0 |
| *cloud9* | `DescribeEnvironments` | 1 |
| *cloud9* | `DeleteEnvironmentMembership` | 0 |
| *cloud9* | `UpdateEnvironmentMembership` | 0 |
| *cloud9* | `DescribeEnvironmentStatus` | 0 |
| *cloud9* | `ListEnvironments` | 1 |
| *cloud9* | `DescribeEnvironmentMemberships` | 1 |
| *cloud9* | `DeleteEnvironment` | 1 |
| *cloud9* | `CreateEnvironmentEC2` | 1 |
| *clouddirectory* | `ListTypedLinkFacetNames` | 0 |
| *clouddirectory* | `ListIncomingTypedLinks` | 0 |
| *clouddirectory* | `ListDirectories` | 0 |
| *clouddirectory* | `CreateSchema` | 0 |
| *clouddirectory* | `UpdateSchema` | 0 |
| *clouddirectory* | `CreateObject` | 0 |
| *clouddirectory* | `EnableDirectory` | 0 |
| *clouddirectory* | `GetObjectInformation` | 0 |
| *clouddirectory* | `ListTagsForResource` | 0 |
| *clouddirectory* | `GetSchemaAsJson` | 0 |
| *clouddirectory* | `BatchWrite` | 0 |
| *clouddirectory* | `ListFacetNames` | 0 |
| *clouddirectory* | `CreateDirectory` | 0 |
| *clouddirectory* | `CreateIndex` | 0 |
| *clouddirectory* | `DeleteFacet` | 0 |
| *clouddirectory* | `DetachPolicy` | 0 |
| *clouddirectory* | `DetachObject` | 0 |
| *clouddirectory* | `ListObjectChildren` | 0 |
| *clouddirectory* | `ListObjectAttributes` | 0 |
| *clouddirectory* | `AddFacetToObject` | 0 |
| *clouddirectory* | `PutSchemaFromJson` | 0 |
| *clouddirectory* | `UpdateLinkAttributes` | 0 |
| *clouddirectory* | `ListPolicyAttachments` | 0 |
| *clouddirectory* | `CreateFacet` | 0 |
| *clouddirectory* | `ListObjectParents` | 0 |
| *clouddirectory* | `BatchRead` | 0 |
| *clouddirectory* | `DetachTypedLink` | 0 |
| *clouddirectory* | `RemoveFacetFromObject` | 0 |
| *clouddirectory* | `UpgradeAppliedSchema` | 0 |
| *clouddirectory* | `LookupPolicy` | 0 |
| *clouddirectory* | `ListObjectParentPaths` | 0 |
| *clouddirectory* | `TagResource` | 0 |
| *clouddirectory* | `ListAttachedIndices` | 0 |
| *clouddirectory* | `AttachTypedLink` | 0 |
| *clouddirectory* | `GetObjectAttributes` | 0 |
| *clouddirectory* | `GetLinkAttributes` | 0 |
| *clouddirectory* | `ListPublishedSchemaArns` | 0 |
| *clouddirectory* | `DeleteDirectory` | 0 |
| *clouddirectory* | `PublishSchema` | 0 |
| *clouddirectory* | `GetFacet` | 0 |
| *clouddirectory* | `DeleteSchema` | 0 |
| *clouddirectory* | `AttachPolicy` | 0 |
| *clouddirectory* | `DisableDirectory` | 0 |
| *clouddirectory* | `ListFacetAttributes` | 0 |
| *clouddirectory* | `DeleteTypedLinkFacet` | 0 |
| *clouddirectory* | `ListObjectPolicies` | 0 |
| *clouddirectory* | `DetachFromIndex` | 0 |
| *clouddirectory* | `UpgradePublishedSchema` | 0 |
| *clouddirectory* | `ListAppliedSchemaArns` | 0 |
| *clouddirectory* | `GetAppliedSchemaVersion` | 0 |
| *clouddirectory* | `ApplySchema` | 0 |
| *clouddirectory* | `ListDevelopmentSchemaArns` | 0 |
| *clouddirectory* | `GetDirectory` | 0 |
| *clouddirectory* | `AttachObject` | 0 |
| *clouddirectory* | `ListOutgoingTypedLinks` | 0 |
| *clouddirectory* | `UpdateObjectAttributes` | 0 |
| *clouddirectory* | `UntagResource` | 0 |
| *clouddirectory* | `CreateTypedLinkFacet` | 0 |
| *clouddirectory* | `AttachToIndex` | 0 |
| *clouddirectory* | `ListIndex` | 0 |
| *clouddirectory* | `ListTypedLinkFacetAttributes` | 0 |
| *clouddirectory* | `UpdateFacet` | 0 |
| *clouddirectory* | `UpdateTypedLinkFacet` | 0 |
| *clouddirectory* | `ListManagedSchemaArns` | 0 |
| *clouddirectory* | `DeleteObject` | 0 |
| *clouddirectory* | `GetTypedLinkFacetInformation` | 0 |
| *cloudformation* | `DescribeStackResource` | 0 |
| *cloudformation* | `DescribeStackInstance` | 0 |
| *cloudformation* | `UpdateStackSet` | 0 |
| *cloudformation* | `CreateStackInstances` | 0 |
| *cloudformation* | `UpdateTerminationProtection` | 0 |
| *cloudformation* | `ValidateTemplate` | 0 |
| *cloudformation* | `DescribeStackResources` | 0 |
| *cloudformation* | `ListStackSetOperations` | 0 |
| *cloudformation* | `ListChangeSets` | 0 |
| *cloudformation* | `CreateStack` | 1 |
| *cloudformation* | `GetTemplateSummary` | 0 |
| *cloudformation* | `CancelUpdateStack` | 0 |
| *cloudformation* | `UpdateStackInstances` | 0 |
| *cloudformation* | `ContinueUpdateRollback` | 0 |
| *cloudformation* | `ListImports` | 0 |
| *cloudformation* | `DescribeStackDriftDetectionStatus` | 0 |
| *cloudformation* | `GetStackPolicy` | 0 |
| *cloudformation* | `ListStackSets` | 0 |
| *cloudformation* | `ListExports` | 0 |
| *cloudformation* | `DeleteStackSet` | 0 |
| *cloudformation* | `DescribeStackSet` | 0 |
| *cloudformation* | `DescribeStackSetOperation` | 0 |
| *cloudformation* | `ListStackResources` | 0 |
| *cloudformation* | `DescribeStackEvents` | 0 |
| *cloudformation* | `CreateStackSet` | 0 |
| *cloudformation* | `SignalResource` | 0 |
| *cloudformation* | `GetTemplate` | 0 |
| *cloudformation* | `DescribeChangeSet` | 0 |
| *cloudformation* | `DeleteChangeSet` | 0 |
| *cloudformation* | `DescribeStackResourceDrifts` | 0 |
| *cloudformation* | `CreateChangeSet` | 0 |
| *cloudformation* | `DescribeStacks` | 1 |
| *cloudformation* | `UpdateStack` | 0 |
| *cloudformation* | `StopStackSetOperation` | 0 |
| *cloudformation* | `DetectStackResourceDrift` | 0 |
| *cloudformation* | `ListStackInstances` | 0 |
| *cloudformation* | `DeleteStackInstances` | 0 |
| *cloudformation* | `DetectStackDrift` | 0 |
| *cloudformation* | `ExecuteChangeSet` | 0 |
| *cloudformation* | `DeleteStack` | 1 |
| *cloudformation* | `ListStackSetOperationResults` | 0 |
| *cloudformation* | `SetStackPolicy` | 0 |
| *cloudformation* | `ListStacks` | 0 |
| *cloudformation* | `EstimateTemplateCost` | 0 |
| *cloudformation* | `DescribeAccountLimits` | 0 |
| *cloudfront* | `UpdateStreamingDistribution2019_03_26` | 0 |
| *cloudfront* | `DeleteFieldLevelEncryptionConfig2019_03_26` | 0 |
| *cloudfront* | `ListDistributions2019_03_26` | 0 |
| *cloudfront* | `GetStreamingDistribution2019_03_26` | 0 |
| *cloudfront* | `GetPublicKey2019_03_26` | 0 |
| *cloudfront* | `CreateStreamingDistributionWithTags2019_03_26` | 0 |
| *cloudfront* | `UpdatePublicKey2019_03_26` | 0 |
| *cloudfront* | `DeleteFieldLevelEncryptionProfile2019_03_26` | 0 |
| *cloudfront* | `CreateDistribution2019_03_26` | 0 |
| *cloudfront* | `CreateFieldLevelEncryptionProfile2019_03_26` | 0 |
| *cloudfront* | `ListInvalidations2019_03_26` | 0 |
| *cloudfront* | `ListStreamingDistributions2019_03_26` | 0 |
| *cloudfront* | `UpdateDistribution2019_03_26` | 0 |
| *cloudfront* | `CreateStreamingDistribution2019_03_26` | 0 |
| *cloudfront* | `GetPublicKeyConfig2019_03_26` | 0 |
| *cloudfront* | `DeleteDistribution2019_03_26` | 0 |
| *cloudfront* | `TagResource2019_03_26` | 0 |
| *cloudfront* | `CreatePublicKey2019_03_26` | 0 |
| *cloudfront* | `DeletePublicKey2019_03_26` | 0 |
| *cloudfront* | `GetInvalidation2019_03_26` | 0 |
| *cloudfront* | `DeleteCloudFrontOriginAccessIdentity2019_03_26` | 0 |
| *cloudfront* | `UpdateFieldLevelEncryptionProfile2019_03_26` | 0 |
| *cloudfront* | `ListCloudFrontOriginAccessIdentities2019_03_26` | 0 |
| *cloudfront* | `CreateFieldLevelEncryptionConfig2019_03_26` | 0 |
| *cloudfront* | `ListPublicKeys2019_03_26` | 0 |
| *cloudfront* | `GetStreamingDistributionConfig2019_03_26` | 0 |
| *cloudfront* | `GetFieldLevelEncryptionProfileConfig2019_03_26` | 0 |
| *cloudfront* | `GetCloudFrontOriginAccessIdentity2019_03_26` | 0 |
| *cloudfront* | `GetDistribution2019_03_26` | 0 |
| *cloudfront* | `GetCloudFrontOriginAccessIdentityConfig2019_03_26` | 0 |
| *cloudfront* | `ListFieldLevelEncryptionProfiles2019_03_26` | 0 |
| *cloudfront* | `GetDistributionConfig2019_03_26` | 0 |
| *cloudfront* | `CreateCloudFrontOriginAccessIdentity2019_03_26` | 0 |
| *cloudfront* | `UpdateFieldLevelEncryptionConfig2019_03_26` | 0 |
| *cloudfront* | `DeleteStreamingDistribution2019_03_26` | 0 |
| *cloudfront* | `CreateDistributionWithTags2019_03_26` | 0 |
| *cloudfront* | `UpdateCloudFrontOriginAccessIdentity2019_03_26` | 0 |
| *cloudfront* | `UntagResource2019_03_26` | 0 |
| *cloudfront* | `ListFieldLevelEncryptionConfigs2019_03_26` | 0 |
| *cloudfront* | `GetFieldLevelEncryptionProfile2019_03_26` | 0 |
| *cloudfront* | `GetFieldLevelEncryption2019_03_26` | 0 |
| *cloudfront* | `CreateInvalidation2019_03_26` | 0 |
| *cloudfront* | `ListDistributionsByWebACLId2019_03_26` | 0 |
| *cloudfront* | `ListTagsForResource2019_03_26` | 0 |
| *cloudfront* | `GetFieldLevelEncryptionConfig2019_03_26` | 0 |
| *cloudhsm* | `CreateHsm` | 0 |
| *cloudhsm* | `ListHapgs` | 0 |
| *cloudhsm* | `ModifyHapg` | 0 |
| *cloudhsm* | `DescribeHapg` | 0 |
| *cloudhsm* | `DeleteHsm` | 0 |
| *cloudhsm* | `ListHsms` | 0 |
| *cloudhsm* | `AddTagsToResource` | 0 |
| *cloudhsm* | `DeleteLunaClient` | 0 |
| *cloudhsm* | `CreateLunaClient` | 0 |
| *cloudhsm* | `CreateHapg` | 0 |
| *cloudhsm* | `RemoveTagsFromResource` | 0 |
| *cloudhsm* | `DeleteHapg` | 0 |
| *cloudhsm* | `DescribeHsm` | 0 |
| *cloudhsm* | `GetConfig` | 0 |
| *cloudhsm* | `ListAvailableZones` | 0 |
| *cloudhsm* | `ModifyHsm` | 0 |
| *cloudhsm* | `ListTagsForResource` | 0 |
| *cloudhsm* | `ModifyLunaClient` | 0 |
| *cloudhsm* | `DescribeLunaClient` | 0 |
| *cloudhsm* | `ListLunaClients` | 0 |
| *cloudhsmv2* | `CreateHsm` | 0 |
| *cloudhsmv2* | `ListTags` | 0 |
| *cloudhsmv2* | `UntagResource` | 0 |
| *cloudhsmv2* | `CreateCluster` | 0 |
| *cloudhsmv2* | `DeleteHsm` | 0 |
| *cloudhsmv2* | `DescribeBackups` | 0 |
| *cloudhsmv2* | `DeleteBackup` | 0 |
| *cloudhsmv2* | `RestoreBackup` | 0 |
| *cloudhsmv2* | `InitializeCluster` | 0 |
| *cloudhsmv2* | `TagResource` | 0 |
| *cloudhsmv2* | `CopyBackupToRegion` | 0 |
| *cloudhsmv2* | `DescribeClusters` | 0 |
| *cloudhsmv2* | `DeleteCluster` | 0 |
| *cloudsearch* | `DescribeServiceAccessPolicies` | 0 |
| *cloudsearch* | `UpdateScalingParameters` | 0 |
| *cloudsearch* | `DescribeDomains` | 0 |
| *cloudsearch* | `DescribeScalingParameters` | 0 |
| *cloudsearch* | `DescribeExpressions` | 0 |
| *cloudsearch* | `DescribeIndexFields` | 0 |
| *cloudsearch* | `ListDomainNames` | 0 |
| *cloudsearch* | `DescribeAvailabilityOptions` | 0 |
| *cloudsearch* | `DefineIndexField` | 0 |
| *cloudsearch* | `DeleteAnalysisScheme` | 0 |
| *cloudsearch* | `UpdateAvailabilityOptions` | 0 |
| *cloudsearch* | `DeleteExpression` | 0 |
| *cloudsearch* | `DescribeAnalysisSchemes` | 0 |
| *cloudsearch* | `DefineSuggester` | 0 |
| *cloudsearch* | `CreateDomain` | 0 |
| *cloudsearch* | `IndexDocuments` | 0 |
| *cloudsearch* | `DescribeSuggesters` | 0 |
| *cloudsearch* | `UpdateServiceAccessPolicies` | 0 |
| *cloudsearch* | `DeleteIndexField` | 0 |
| *cloudsearch* | `DefineAnalysisScheme` | 0 |
| *cloudsearch* | `DefineExpression` | 0 |
| *cloudsearch* | `BuildSuggesters` | 0 |
| *cloudsearch* | `DeleteDomain` | 0 |
| *cloudsearch* | `DeleteSuggester` | 0 |
| *cloudsearchdomain* | `Suggest` | 0 |
| *cloudsearchdomain* | `Search` | 0 |
| *cloudsearchdomain* | `UploadDocuments` | 0 |
| *cloudtrail* | `DeleteTrail` | 0 |
| *cloudtrail* | `RemoveTags` | 0 |
| *cloudtrail* | `GetEventSelectors` | 3 |
| *cloudtrail* | `AddTags` | 0 |
| *cloudtrail* | `StartLogging` | 1 |
| *cloudtrail* | `GetTrail` | 0 |
| *cloudtrail* | `ListPublicKeys` | 0 |
| *cloudtrail* | `LookupEvents` | 2 |
| *cloudtrail* | `StopLogging` | 0 |
| *cloudtrail* | `GetTrailStatus` | 1 |
| *cloudtrail* | `DescribeTrails` | 4 |
| *cloudtrail* | `CreateTrail` | 1 |
| *cloudtrail* | `UpdateTrail` | 0 |
| *cloudtrail* | `PutEventSelectors` | 1 |
| *cloudtrail* | `ListTags` | 1 |
| *cloudtrail* | `ListTrails` | 0 |
| *cloudwatch* | `ListDashboards` | 0 |
| *cloudwatch* | `ListMetrics` | 0 |
| *cloudwatch* | `PutMetricAlarm` | 1 |
| *cloudwatch* | `DisableAlarmActions` | 0 |
| *cloudwatch* | `DescribeAnomalyDetectors` | 0 |
| *cloudwatch* | `DeleteDashboards` | 0 |
| *cloudwatch* | `EnableAlarmActions` | 0 |
| *cloudwatch* | `TagResource` | 0 |
| *cloudwatch* | `DescribeAlarms` | 0 |
| *cloudwatch* | `GetDashboard` | 0 |
| *cloudwatch* | `DescribeAlarmsForMetric` | 0 |
| *cloudwatch* | `GetMetricWidgetImage` | 0 |
| *cloudwatch* | `DeleteAnomalyDetector` | 0 |
| *cloudwatch* | `DeleteAlarms` | 1 |
| *cloudwatch* | `SetAlarmState` | 0 |
| *cloudwatch* | `GetMetricData` | 0 |
| *cloudwatch* | `DescribeAlarmHistory` | 0 |
| *cloudwatch* | `PutDashboard` | 1 |
| *cloudwatch* | `UntagResource` | 0 |
| *cloudwatch* | `PutAnomalyDetector` | 0 |
| *cloudwatch* | `GetMetricStatistics` | 1 |
| *cloudwatch* | `PutMetricData` | 0 |
| *cloudwatch* | `ListTagsForResource` | 0 |
| *codebuild* | `ListBuilds` | 0 |
| *codebuild* | `UpdateWebhook` | 0 |
| *codebuild* | `ImportSourceCredentials` | 0 |
| *codebuild* | `ListSourceCredentials` | 0 |
| *codebuild* | `ListBuildsForProject` | 0 |
| *codebuild* | `BatchGetBuilds` | 0 |
| *codebuild* | `ListProjects` | 1 |
| *codebuild* | `CreateProject` | 1 |
| *codebuild* | `ListCuratedEnvironmentImages` | 1 |
| *codebuild* | `StopBuild` | 0 |
| *codebuild* | `CreateWebhook` | 0 |
| *codebuild* | `InvalidateProjectCache` | 0 |
| *codebuild* | `StartBuild` | 0 |
| *codebuild* | `DeleteProject` | 0 |
| *codebuild* | `UpdateProject` | 0 |
| *codebuild* | `DeleteWebhook` | 0 |
| *codebuild* | `BatchGetProjects` | 1 |
| *codebuild* | `BatchDeleteBuilds` | 0 |
| *codebuild* | `DeleteSourceCredentials` | 0 |
| *codecommit* | `TestRepositoryTriggers` | 0 |
| *codecommit* | `DeleteRepository` | 1 |
| *codecommit* | `MergeBranchesByFastForward` | 0 |
| *codecommit* | `BatchDescribeMergeConflicts` | 0 |
| *codecommit* | `ListPullRequests` | 1 |
| *codecommit* | `UpdateRepositoryDescription` | 0 |
| *codecommit* | `UpdatePullRequestDescription` | 0 |
| *codecommit* | `DeleteBranch` | 0 |
| *codecommit* | `BatchGetRepositories` | 0 |
| *codecommit* | `GetFolder` | 0 |
| *codecommit* | `PutRepositoryTriggers` | 0 |
| *codecommit* | `DeleteCommentContent` | 0 |
| *codecommit* | `GetFile` | 0 |
| *codecommit* | `GetDifferences` | 0 |
| *codecommit* | `BatchGetCommits` | 0 |
| *codecommit* | `MergePullRequestByThreeWay` | 0 |
| *codecommit* | `PostCommentForPullRequest` | 0 |
| *codecommit* | `MergeBranchesBySquash` | 0 |
| *codecommit* | `CreateBranch` | 0 |
| *codecommit* | `GetRepository` | 1 |
| *codecommit* | `GetMergeConflicts` | 0 |
| *codecommit* | `GetBlob` | 0 |
| *codecommit* | `TagResource` | 0 |
| *codecommit* | `ListBranches` | 0 |
| *codecommit* | `GetRepositoryTriggers` | 0 |
| *codecommit* | `UpdateComment` | 0 |
| *codecommit* | `DeleteFile` | 0 |
| *codecommit* | `UpdateRepositoryName` | 0 |
| *codecommit* | `GetMergeOptions` | 0 |
| *codecommit* | `UpdateDefaultBranch` | 0 |
| *codecommit* | `MergePullRequestBySquash` | 0 |
| *codecommit* | `GetCommentsForComparedCommit` | 0 |
| *codecommit* | `GetBranch` | 0 |
| *codecommit* | `CreateCommit` | 0 |
| *codecommit* | `CreateRepository` | 1 |
| *codecommit* | `GetCommit` | 0 |
| *codecommit* | `PostCommentReply` | 0 |
| *codecommit* | `PutFile` | 0 |
| *codecommit* | `DescribePullRequestEvents` | 0 |
| *codecommit* | `GetMergeCommit` | 0 |
| *codecommit* | `UpdatePullRequestTitle` | 0 |
| *codecommit* | `UntagResource` | 0 |
| *codecommit* | `MergeBranchesByThreeWay` | 0 |
| *codecommit* | `GetPullRequest` | 0 |
| *codecommit* | `CreatePullRequest` | 0 |
| *codecommit* | `GetCommentsForPullRequest` | 0 |
| *codecommit* | `UpdatePullRequestStatus` | 0 |
| *codecommit* | `PostCommentForComparedCommit` | 0 |
| *codecommit* | `ListRepositories` | 2 |
| *codecommit* | `GetComment` | 0 |
| *codecommit* | `ListTagsForResource` | 0 |
| *codecommit* | `DescribeMergeConflicts` | 0 |
| *codecommit* | `CreateUnreferencedMergeCommit` | 0 |
| *codecommit* | `MergePullRequestByFastForward` | 0 |
| *codedeploy* | `DeleteGitHubAccountToken` | 0 |
| *codedeploy* | `GetDeploymentConfig` | 0 |
| *codedeploy* | `UpdateDeploymentGroup` | 0 |
| *codedeploy* | `BatchGetDeployments` | 1 |
| *codedeploy* | `ContinueDeployment` | 0 |
| *codedeploy* | `GetDeploymentGroup` | 1 |
| *codedeploy* | `ListDeploymentGroups` | 1 |
| *codedeploy* | `DeleteApplication` | 1 |
| *codedeploy* | `CreateDeployment` | 1 |
| *codedeploy* | `StopDeployment` | 1 |
| *codedeploy* | `DeleteDeploymentConfig` | 1 |
| *codedeploy* | `ListDeploymentTargets` | 0 |
| *codedeploy* | `RemoveTagsFromOnPremisesInstances` | 0 |
| *codedeploy* | `RegisterOnPremisesInstance` | 0 |
| *codedeploy* | `ListOnPremisesInstances` | 0 |
| *codedeploy* | `ListDeploymentConfigs` | 2 |
| *codedeploy* | `GetApplication` | 1 |
| *codedeploy* | `GetDeploymentInstance` | 0 |
| *codedeploy* | `SkipWaitTimeForInstanceTermination` | 0 |
| *codedeploy* | `BatchGetDeploymentGroups` | 1 |
| *codedeploy* | `BatchGetOnPremisesInstances` | 0 |
| *codedeploy* | `GetApplicationRevision` | 0 |
| *codedeploy* | `BatchGetDeploymentInstances` | 0 |
| *codedeploy* | `GetDeploymentTarget` | 0 |
| *codedeploy* | `UpdateApplication` | 0 |
| *codedeploy* | `CreateDeploymentGroup` | 1 |
| *codedeploy* | `BatchGetDeploymentTargets` | 0 |
| *codedeploy* | `ListDeploymentInstances` | 0 |
| *codedeploy* | `BatchGetApplicationRevisions` | 0 |
| *codedeploy* | `TagResource` | 0 |
| *codedeploy* | `PutLifecycleEventHookExecutionStatus` | 0 |
| *codedeploy* | `DeregisterOnPremisesInstance` | 0 |
| *codedeploy* | `CreateDeploymentConfig` | 1 |
| *codedeploy* | `ListApplicationRevisions` | 1 |
| *codedeploy* | `GetOnPremisesInstance` | 0 |
| *codedeploy* | `AddTagsToOnPremisesInstances` | 0 |
| *codedeploy* | `CreateApplication` | 1 |
| *codedeploy* | `ListDeployments` | 1 |
| *codedeploy* | `ListApplications` | 0 |
| *codedeploy* | `UntagResource` | 0 |
| *codedeploy* | `DeleteDeploymentGroup` | 0 |
| *codedeploy* | `ListGitHubAccountTokenNames` | 0 |
| *codedeploy* | `RegisterApplicationRevision` | 0 |
| *codedeploy* | `ListTagsForResource` | 0 |
| *codedeploy* | `GetDeployment` | 0 |
| *codedeploy* | `BatchGetApplications` | 0 |
| *codepipeline* | `EnableStageTransition` | 1 |
| *codepipeline* | `RetryStageExecution` | 0 |
| *codepipeline* | `DisableStageTransition` | 1 |
| *codepipeline* | `ListWebhooks` | 0 |
| *codepipeline* | `PollForThirdPartyJobs` | 0 |
| *codepipeline* | `GetThirdPartyJobDetails` | 0 |
| *codepipeline* | `ListActionExecutions` | 0 |
| *codepipeline* | `DeletePipeline` | 1 |
| *codepipeline* | `UpdatePipeline` | 1 |
| *codepipeline* | `PutJobFailureResult` | 0 |
| *codepipeline* | `ListActionTypes` | 1 |
| *codepipeline* | `AcknowledgeThirdPartyJob` | 0 |
| *codepipeline* | `PutWebhook` | 1 |
| *codepipeline* | `TagResource` | 0 |
| *codepipeline* | `DeleteWebhook` | 0 |
| *codepipeline* | `DeregisterWebhookWithThirdParty` | 0 |
| *codepipeline* | `GetPipelineExecution` | 1 |
| *codepipeline* | `PutThirdPartyJobSuccessResult` | 0 |
| *codepipeline* | `AcknowledgeJob` | 0 |
| *codepipeline* | `PutActionRevision` | 0 |
| *codepipeline* | `ListPipelines` | 1 |
| *codepipeline* | `StartPipelineExecution` | 0 |
| *codepipeline* | `PutApprovalResult` | 0 |
| *codepipeline* | `PutThirdPartyJobFailureResult` | 0 |
| *codepipeline* | `PollForJobs` | 0 |
| *codepipeline* | `GetPipelineState` | 1 |
| *codepipeline* | `CreateCustomActionType` | 0 |
| *codepipeline* | `CreatePipeline` | 1 |
| *codepipeline* | `RegisterWebhookWithThirdParty` | 0 |
| *codepipeline* | `GetJobDetails` | 0 |
| *codepipeline* | `GetPipeline` | 1 |
| *codepipeline* | `UntagResource` | 0 |
| *codepipeline* | `DeleteCustomActionType` | 0 |
| *codepipeline* | `ListPipelineExecutions` | 0 |
| *codepipeline* | `ListTagsForResource` | 0 |
| *codepipeline* | `PutJobSuccessResult` | 0 |
| *codestar* | `AssociateTeamMember` | 0 |
| *codestar* | `DisassociateTeamMember` | 0 |
| *codestar* | `UntagProject` | 0 |
| *codestar* | `UpdateTeamMember` | 0 |
| *codestar* | `ListTagsForProject` | 0 |
| *codestar* | `CreateUserProfile` | 0 |
| *codestar* | `UpdateUserProfile` | 0 |
| *codestar* | `ListUserProfiles` | 0 |
| *codestar* | `ListResources` | 0 |
| *codestar* | `CreateProject` | 0 |
| *codestar* | `TagProject` | 0 |
| *codestar* | `ListProjects` | 0 |
| *codestar* | `ListTeamMembers` | 0 |
| *codestar* | `DeleteProject` | 0 |
| *codestar* | `DescribeProject` | 0 |
| *codestar* | `UpdateProject` | 0 |
| *codestar* | `DescribeUserProfile` | 0 |
| *codestar* | `DeleteUserProfile` | 0 |
| *codestar-notifications* | `DescribeNotificationRule` | 0 |
| *codestar-notifications* | `UntagResource` | 0 |
| *codestar-notifications* | `Subscribe` | 0 |
| *codestar-notifications* | `DeleteNotificationRule` | 0 |
| *codestar-notifications* | `ListNotificationRules` | 0 |
| *codestar-notifications* | `CreateNotificationRule` | 0 |
| *codestar-notifications* | `UpdateNotificationRule` | 0 |
| *codestar-notifications* | `Unsubscribe` | 0 |
| *codestar-notifications* | `ListEventTypes` | 0 |
| *codestar-notifications* | `DeleteTarget` | 0 |
| *codestar-notifications* | `TagResource` | 0 |
| *codestar-notifications* | `ListTargets` | 0 |
| *codestar-notifications* | `ListTagsForResource` | 0 |
| *cognito-identity* | `MergeDeveloperIdentities` | 0 |
| *cognito-identity* | `GetIdentityPoolRoles` | 0 |
| *cognito-identity* | `DeleteIdentities` | 0 |
| *cognito-identity* | `GetOpenIdTokenForDeveloperIdentity` | 0 |
| *cognito-identity* | `UntagResource` | 0 |
| *cognito-identity* | `LookupDeveloperIdentity` | 0 |
| *cognito-identity* | `ListIdentityPools` | 0 |
| *cognito-identity* | `TagResource` | 0 |
| *cognito-identity* | `DescribeIdentity` | 0 |
| *cognito-identity* | `GetId` | 0 |
| *cognito-identity* | `UnlinkIdentity` | 0 |
| *cognito-identity* | `ListIdentities` | 0 |
| *cognito-identity* | `ListTagsForResource` | 0 |
| *cognito-identity* | `DeleteIdentityPool` | 0 |
| *cognito-identity* | `GetCredentialsForIdentity` | 0 |
| *cognito-identity* | `UpdateIdentityPool` | 0 |
| *cognito-identity* | `SetIdentityPoolRoles` | 1 |
| *cognito-identity* | `UnlinkDeveloperIdentity` | 0 |
| *cognito-identity* | `DescribeIdentityPool` | 0 |
| *cognito-identity* | `GetOpenIdToken` | 0 |
| *cognito-identity* | `CreateIdentityPool` | 1 |
| *cognito-idp* | `VerifyUserAttribute` | 0 |
| *cognito-idp* | `AdminListDevices` | 0 |
| *cognito-idp* | `SignUp` | 0 |
| *cognito-idp* | `ListIdentityProviders` | 0 |
| *cognito-idp* | `DescribeResourceServer` | 0 |
| *cognito-idp* | `ListUserPoolClients` | 1 |
| *cognito-idp* | `DescribeUserPool` | 0 |
| *cognito-idp* | `AdminUserGlobalSignOut` | 0 |
| *cognito-idp* | `DeleteUserPoolDomain` | 1 |
| *cognito-idp* | `GetUser` | 0 |
| *cognito-idp* | `GetUICustomization` | 0 |
| *cognito-idp* | `DescribeIdentityProvider` | 0 |
| *cognito-idp* | `UpdateIdentityProvider` | 0 |
| *cognito-idp* | `UpdateAuthEventFeedback` | 0 |
| *cognito-idp* | `AdminForgetDevice` | 0 |
| *cognito-idp* | `UpdateUserPoolDomain` | 0 |
| *cognito-idp* | `DescribeRiskConfiguration` | 0 |
| *cognito-idp* | `SetUICustomization` | 0 |
| *cognito-idp* | `DeleteUser` | 0 |
| *cognito-idp* | `AdminDeleteUser` | 0 |
| *cognito-idp* | `AdminInitiateAuth` | 0 |
| *cognito-idp* | `AdminUpdateAuthEventFeedback` | 0 |
| *cognito-idp* | `SetRiskConfiguration` | 0 |
| *cognito-idp* | `AdminSetUserSettings` | 0 |
| *cognito-idp* | `AdminSetUserMFAPreference` | 0 |
| *cognito-idp* | `ListTagsForResource` | 0 |
| *cognito-idp* | `ListResourceServers` | 0 |
| *cognito-idp* | `UpdateDeviceStatus` | 0 |
| *cognito-idp* | `AdminDisableUser` | 0 |
| *cognito-idp* | `AdminUpdateUserAttributes` | 0 |
| *cognito-idp* | `ForgetDevice` | 0 |
| *cognito-idp* | `ConfirmForgotPassword` | 0 |
| *cognito-idp* | `AdminDisableProviderForUser` | 0 |
| *cognito-idp* | `ListUserPools` | 2 |
| *cognito-idp* | `AdminGetDevice` | 0 |
| *cognito-idp* | `UpdateGroup` | 0 |
| *cognito-idp* | `TagResource` | 0 |
| *cognito-idp* | `GetGroup` | 0 |
| *cognito-idp* | `AdminAddUserToGroup` | 1 |
| *cognito-idp* | `AssociateSoftwareToken` | 0 |
| *cognito-idp* | `AdminListGroupsForUser` | 0 |
| *cognito-idp* | `DescribeUserPoolClient` | 0 |
| *cognito-idp* | `ListDevices` | 0 |
| *cognito-idp* | `InitiateAuth` | 0 |
| *cognito-idp* | `UntagResource` | 0 |
| *cognito-idp* | `RespondToAuthChallenge` | 0 |
| *cognito-idp* | `CreateUserPoolClient` | 1 |
| *cognito-idp* | `DeleteUserAttributes` | 0 |
| *cognito-idp* | `GlobalSignOut` | 0 |
| *cognito-idp* | `ConfirmSignUp` | 0 |
| *cognito-idp* | `ForgotPassword` | 0 |
| *cognito-idp* | `AdminCreateUser` | 1 |
| *cognito-idp* | `DeleteIdentityProvider` | 0 |
| *cognito-idp* | `CreateGroup` | 1 |
| *cognito-idp* | `DeleteUserPool` | 0 |
| *cognito-idp* | `AdminListUserAuthEvents` | 0 |
| *cognito-idp* | `GetUserPoolMfaConfig` | 0 |
| *cognito-idp* | `AdminLinkProviderForUser` | 0 |
| *cognito-idp* | `DeleteGroup` | 0 |
| *cognito-idp* | `ResendConfirmationCode` | 0 |
| *cognito-idp* | `GetUserAttributeVerificationCode` | 0 |
| *cognito-idp* | `DeleteUserPoolClient` | 0 |
| *cognito-idp* | `CreateResourceServer` | 0 |
| *cognito-idp* | `GetCSVHeader` | 0 |
| *cognito-idp* | `AdminUpdateDeviceStatus` | 0 |
| *cognito-idp* | `UpdateUserPoolClient` | 0 |
| *cognito-idp* | `DeleteResourceServer` | 0 |
| *cognito-idp* | `AdminRemoveUserFromGroup` | 0 |
| *cognito-idp* | `UpdateUserPool` | 0 |
| *cognito-idp* | `UpdateResourceServer` | 0 |
| *cognito-idp* | `ListGroups` | 1 |
| *cognito-idp* | `ListUserImportJobs` | 0 |
| *cognito-idp* | `ListUsers` | 1 |
| *cognito-idp* | `DescribeUserImportJob` | 0 |
| *cognito-idp* | `AdminDeleteUserAttributes` | 0 |
| *cognito-idp* | `AdminRespondToAuthChallenge` | 0 |
| *cognito-idp* | `CreateIdentityProvider` | 0 |
| *cognito-idp* | `GetIdentityProviderByIdentifier` | 0 |
| *cognito-idp* | `CreateUserPool` | 1 |
| *cognito-idp* | `VerifySoftwareToken` | 0 |
| *cognito-idp* | `UpdateUserAttributes` | 0 |
| *cognito-idp* | `DescribeUserPoolDomain` | 0 |
| *cognito-idp* | `AdminSetUserPassword` | 0 |
| *cognito-idp* | `AdminResetUserPassword` | 0 |
| *cognito-idp* | `AdminConfirmSignUp` | 0 |
| *cognito-idp* | `StopUserImportJob` | 0 |
| *cognito-idp* | `ConfirmDevice` | 0 |
| *cognito-idp* | `ChangePassword` | 0 |
| *cognito-idp* | `ListUsersInGroup` | 0 |
| *cognito-idp* | `AdminEnableUser` | 0 |
| *cognito-idp* | `AddCustomAttributes` | 0 |
| *cognito-idp* | `StartUserImportJob` | 0 |
| *cognito-idp* | `AdminGetUser` | 0 |
| *cognito-idp* | `SetUserPoolMfaConfig` | 1 |
| *cognito-idp* | `GetDevice` | 0 |
| *cognito-idp* | `CreateUserImportJob` | 0 |
| *cognito-idp* | `SetUserMFAPreference` | 0 |
| *cognito-idp* | `SetUserSettings` | 0 |
| *cognito-idp* | `CreateUserPoolDomain` | 1 |
| *cognito-idp* | `GetSigningCertificate` | 0 |
| *cognito-sync* | `SetCognitoEvents` | 0 |
| *cognito-sync* | `DescribeIdentityUsage` | 0 |
| *cognito-sync* | `ListRecords` | 0 |
| *cognito-sync* | `DescribeDataset` | 0 |
| *cognito-sync* | `BulkPublish` | 0 |
| *cognito-sync* | `UnsubscribeFromDataset` | 0 |
| *cognito-sync* | `ListIdentityPoolUsage` | 0 |
| *cognito-sync* | `RegisterDevice` | 0 |
| *cognito-sync* | `GetBulkPublishDetails` | 0 |
| *cognito-sync* | `GetCognitoEvents` | 0 |
| *cognito-sync* | `DeleteDataset` | 0 |
| *cognito-sync* | `DescribeIdentityPoolUsage` | 0 |
| *cognito-sync* | `UpdateRecords` | 0 |
| *cognito-sync* | `SubscribeToDataset` | 0 |
| *cognito-sync* | `SetIdentityPoolConfiguration` | 0 |
| *cognito-sync* | `GetIdentityPoolConfiguration` | 0 |
| *cognito-sync* | `ListDatasets` | 0 |
| *comprehend* | `DeleteEntityRecognizer` | 0 |
| *comprehend* | `StartDominantLanguageDetectionJob` | 0 |
| *comprehend* | `StopEntitiesDetectionJob` | 0 |
| *comprehend* | `DescribeDocumentClassifier` | 0 |
| *comprehend* | `DetectSyntax` | 0 |
| *comprehend* | `DeleteDocumentClassifier` | 0 |
| *comprehend* | `ListKeyPhrasesDetectionJobs` | 0 |
| *comprehend* | `BatchDetectDominantLanguage` | 0 |
| *comprehend* | `StartSentimentDetectionJob` | 0 |
| *comprehend* | `DescribeEntitiesDetectionJob` | 0 |
| *comprehend* | `DescribeEntityRecognizer` | 0 |
| *comprehend* | `ListDominantLanguageDetectionJobs` | 0 |
| *comprehend* | `BatchDetectSentiment` | 0 |
| *comprehend* | `StopKeyPhrasesDetectionJob` | 0 |
| *comprehend* | `TagResource` | 0 |
| *comprehend* | `StopDominantLanguageDetectionJob` | 0 |
| *comprehend* | `DetectEntities` | 0 |
| *comprehend* | `DescribeTopicsDetectionJob` | 0 |
| *comprehend* | `ListEntitiesDetectionJobs` | 0 |
| *comprehend* | `StartEntitiesDetectionJob` | 0 |
| *comprehend* | `DetectSentiment` | 0 |
| *comprehend* | `DescribeSentimentDetectionJob` | 0 |
| *comprehend* | `StartTopicsDetectionJob` | 0 |
| *comprehend* | `DescribeDocumentClassificationJob` | 0 |
| *comprehend* | `ListTopicsDetectionJobs` | 0 |
| *comprehend* | `StopTrainingDocumentClassifier` | 0 |
| *comprehend* | `DetectKeyPhrases` | 0 |
| *comprehend* | `ListDocumentClassificationJobs` | 0 |
| *comprehend* | `ListSentimentDetectionJobs` | 0 |
| *comprehend* | `ListEntityRecognizers` | 0 |
| *comprehend* | `CreateEntityRecognizer` | 0 |
| *comprehend* | `StopTrainingEntityRecognizer` | 0 |
| *comprehend* | `DetectDominantLanguage` | 0 |
| *comprehend* | `StartKeyPhrasesDetectionJob` | 0 |
| *comprehend* | `CreateDocumentClassifier` | 0 |
| *comprehend* | `BatchDetectSyntax` | 0 |
| *comprehend* | `UntagResource` | 0 |
| *comprehend* | `DescribeDominantLanguageDetectionJob` | 0 |
| *comprehend* | `StopSentimentDetectionJob` | 0 |
| *comprehend* | `BatchDetectKeyPhrases` | 0 |
| *comprehend* | `StartDocumentClassificationJob` | 0 |
| *comprehend* | `DescribeKeyPhrasesDetectionJob` | 0 |
| *comprehend* | `ListTagsForResource` | 0 |
| *comprehend* | `ListDocumentClassifiers` | 0 |
| *comprehend* | `BatchDetectEntities` | 0 |
| *comprehendmedical* | `StartEntitiesDetectionV2Job` | 0 |
| *comprehendmedical* | `ListEntitiesDetectionV2Jobs` | 0 |
| *comprehendmedical* | `StartPHIDetectionJob` | 0 |
| *comprehendmedical* | `DescribeEntitiesDetectionV2Job` | 0 |
| *comprehendmedical* | `StopPHIDetectionJob` | 0 |
| *comprehendmedical* | `DescribePHIDetectionJob` | 0 |
| *comprehendmedical* | `StopEntitiesDetectionV2Job` | 0 |
| *comprehendmedical* | `DetectEntities` | 0 |
| *comprehendmedical* | `DetectEntitiesV2` | 0 |
| *comprehendmedical* | `ListPHIDetectionJobs` | 0 |
| *comprehendmedical* | `DetectPHI` | 0 |
| *config* | `UntagResource` | 0 |
| *config* | `StartRemediationExecution` | 0 |
| *config* | `GetOrganizationConfigRuleDetailedStatus` | 0 |
| *config* | `StopConfigurationRecorder` | 0 |
| *config* | `GetComplianceDetailsByResource` | 0 |
| *config* | `DescribeRemediationExecutionStatus` | 0 |
| *config* | `ListDiscoveredResources` | 0 |
| *config* | `DeleteDeliveryChannel` | 0 |
| *config* | `DeleteAggregationAuthorization` | 0 |
| *config* | `GetAggregateConfigRuleComplianceSummary` | 0 |
| *config* | `DeleteConfigurationAggregator` | 0 |
| *config* | `DeleteRetentionConfiguration` | 0 |
| *config* | `DescribeRemediationConfigurations` | 0 |
| *config* | `PutRemediationConfigurations` | 0 |
| *config* | `ListAggregateDiscoveredResources` | 0 |
| *config* | `PutEvaluations` | 0 |
| *config* | `PutRemediationExceptions` | 0 |
| *config* | `DeleteConfigRule` | 0 |
| *config* | `DescribeConfigurationRecorders` | 3 |
| *config* | `DescribeAggregationAuthorizations` | 1 |
| *config* | `PutConfigurationRecorder` | 1 |
| *config* | `DescribeConfigurationRecorderStatus` | 0 |
| *config* | `GetComplianceDetailsByConfigRule` | 0 |
| *config* | `GetResourceConfigHistory` | 0 |
| *config* | `DescribeDeliveryChannelStatus` | 0 |
| *config* | `DeliverConfigSnapshot` | 0 |
| *config* | `StartConfigRulesEvaluation` | 0 |
| *config* | `GetComplianceSummaryByConfigRule` | 0 |
| *config* | `StartConfigurationRecorder` | 0 |
| *config* | `DeletePendingAggregationRequest` | 0 |
| *config* | `GetAggregateComplianceDetailsByConfigRule` | 0 |
| *config* | `GetDiscoveredResourceCounts` | 0 |
| *config* | `DescribeOrganizationConfigRuleStatuses` | 0 |
| *config* | `BatchGetResourceConfig` | 0 |
| *config* | `DescribeComplianceByResource` | 0 |
| *config* | `DescribeOrganizationConfigRules` | 0 |
| *config* | `TagResource` | 0 |
| *config* | `DescribeConfigurationAggregatorSourcesStatus` | 0 |
| *config* | `DeleteRemediationExceptions` | 0 |
| *config* | `PutAggregationAuthorization` | 1 |
| *config* | `DescribeConfigRuleEvaluationStatus` | 0 |
| *config* | `PutRetentionConfiguration` | 0 |
| *config* | `DescribeDeliveryChannels` | 1 |
| *config* | `PutOrganizationConfigRule` | 0 |
| *config* | `GetComplianceSummaryByResourceType` | 0 |
| *config* | `DeleteRemediationConfiguration` | 0 |
| *config* | `SelectResourceConfig` | 0 |
| *config* | `DescribePendingAggregationRequests` | 1 |
| *config* | `DescribeConfigRules` | 0 |
| *config* | `DescribeAggregateComplianceByConfigRules` | 0 |
| *config* | `PutConfigRule` | 1 |
| *config* | `DeleteEvaluationResults` | 0 |
| *config* | `GetAggregateDiscoveredResourceCounts` | 0 |
| *config* | `GetAggregateResourceConfig` | 0 |
| *config* | `DescribeComplianceByConfigRule` | 0 |
| *config* | `BatchGetAggregateResourceConfig` | 0 |
| *config* | `DescribeConfigurationAggregators` | 1 |
| *config* | `PutDeliveryChannel` | 0 |
| *config* | `DescribeRetentionConfigurations` | 0 |
| *config* | `PutConfigurationAggregator` | 1 |
| *config* | `DeleteOrganizationConfigRule` | 0 |
| *config* | `ListTagsForResource` | 0 |
| *config* | `DeleteConfigurationRecorder` | 0 |
| *config* | `DescribeRemediationExceptions` | 0 |
| *connect* | `StopContact` | 0 |
| *connect* | `UpdateUserSecurityProfiles` | 0 |
| *connect* | `ListUsers` | 0 |
| *connect* | `ListPhoneNumbers` | 0 |
| *connect* | `UpdateUserRoutingProfile` | 0 |
| *connect* | `ListQueues` | 0 |
| *connect* | `DescribeUserHierarchyStructure` | 0 |
| *connect* | `DescribeUserHierarchyGroup` | 0 |
| *connect* | `GetCurrentMetricData` | 0 |
| *connect* | `DescribeUser` | 0 |
| *connect* | `ListContactFlows` | 0 |
| *connect* | `ListUserHierarchyGroups` | 0 |
| *connect* | `UpdateContactAttributes` | 0 |
| *connect* | `UpdateUserPhoneConfig` | 0 |
| *connect* | `GetFederationToken` | 0 |
| *connect* | `UpdateUserIdentityInfo` | 0 |
| *connect* | `UpdateUserHierarchy` | 0 |
| *connect* | `GetContactAttributes` | 0 |
| *connect* | `GetMetricData` | 0 |
| *connect* | `DeleteUser` | 0 |
| *connect* | `ListRoutingProfiles` | 0 |
| *connect* | `ListSecurityProfiles` | 0 |
| *connect* | `StartOutboundVoiceContact` | 0 |
| *connect* | `ListHoursOfOperations` | 0 |
| *connect* | `CreateUser` | 0 |
| *cur* | `DescribeReportDefinitions` | 0 |
| *cur* | `ModifyReportDefinition` | 0 |
| *cur* | `PutReportDefinition` | 0 |
| *cur* | `DeleteReportDefinition` | 0 |
| *datapipeline* | `RemoveTags` | 0 |
| *datapipeline* | `DeactivatePipeline` | 0 |
| *datapipeline* | `AddTags` | 0 |
| *datapipeline* | `ValidatePipelineDefinition` | 0 |
| *datapipeline* | `ListPipelines` | 0 |
| *datapipeline* | `DeletePipeline` | 0 |
| *datapipeline* | `DescribeObjects` | 0 |
| *datapipeline* | `QueryObjects` | 0 |
| *datapipeline* | `SetStatus` | 0 |
| *datapipeline* | `SetTaskStatus` | 0 |
| *datapipeline* | `PutPipelineDefinition` | 0 |
| *datapipeline* | `PollForTask` | 0 |
| *datapipeline* | `ActivatePipeline` | 0 |
| *datapipeline* | `CreatePipeline` | 1 |
| *datapipeline* | `EvaluateExpression` | 0 |
| *datapipeline* | `GetPipelineDefinition` | 0 |
| *datapipeline* | `ReportTaskProgress` | 0 |
| *datapipeline* | `ReportTaskRunnerHeartbeat` | 0 |
| *datapipeline* | `DescribePipelines` | 0 |
| *datasync* | `CreateTask` | 1 |
| *datasync* | `DescribeAgent` | 1 |
| *datasync* | `ListLocations` | 1 |
| *datasync* | `ListTaskExecutions` | 0 |
| *datasync* | `DescribeLocationS3` | 0 |
| *datasync* | `CancelTaskExecution` | 0 |
| *datasync* | `DeleteLocation` | 1 |
| *datasync* | `StartTaskExecution` | 0 |
| *datasync* | `DescribeTaskExecution` | 0 |
| *datasync* | `CreateLocationS3` | 1 |
| *datasync* | `CreateAgent` | 1 |
| *datasync* | `TagResource` | 0 |
| *datasync* | `DeleteTask` | 1 |
| *datasync* | `ListAgents` | 1 |
| *datasync* | `CreateLocationNfs` | 1 |
| *datasync* | `CreateLocationSmb` | 0 |
| *datasync* | `UpdateTask` | 0 |
| *datasync* | `CreateLocationEfs` | 1 |
| *datasync* | `UpdateAgent` | 0 |
| *datasync* | `DescribeTask` | 1 |
| *datasync* | `UntagResource` | 0 |
| *datasync* | `DescribeLocationNfs` | 2 |
| *datasync* | `ListTasks` | 1 |
| *datasync* | `DescribeLocationSmb` | 0 |
| *datasync* | `DeleteAgent` | 1 |
| *datasync* | `DescribeLocationEfs` | 0 |
| *datasync* | `ListTagsForResource` | 0 |
| *dax* | `CreateParameterGroup` | 1 |
| *dax* | `DeleteParameterGroup` | 1 |
| *dax* | `UpdateCluster` | 0 |
| *dax* | `UntagResource` | 0 |
| *dax* | `CreateCluster` | 1 |
| *dax* | `DescribeSubnetGroups` | 1 |
| *dax* | `IncreaseReplicationFactor` | 0 |
| *dax* | `TagResource` | 0 |
| *dax* | `UpdateSubnetGroup` | 0 |
| *dax* | `DescribeParameters` | 0 |
| *dax* | `DecreaseReplicationFactor` | 0 |
| *dax* | `RebootNode` | 0 |
| *dax* | `CreateSubnetGroup` | 1 |
| *dax* | `UpdateParameterGroup` | 1 |
| *dax* | `ListTags` | 0 |
| *dax* | `DescribeEvents` | 0 |
| *dax* | `DeleteSubnetGroup` | 1 |
| *dax* | `DescribeDefaultParameters` | 0 |
| *dax* | `DescribeClusters` | 1 |
| *dax* | `DescribeParameterGroups` | 1 |
| *dax* | `DeleteCluster` | 1 |
| *devicefarm* | `ListSamples` | 0 |
| *devicefarm* | `PurchaseOffering` | 0 |
| *devicefarm* | `GetUpload` | 0 |
| *devicefarm* | `ListOfferingPromotions` | 0 |
| *devicefarm* | `UpdateDevicePool` | 0 |
| *devicefarm* | `StopRun` | 0 |
| *devicefarm* | `CreateRemoteAccessSession` | 0 |
| *devicefarm* | `DeleteInstanceProfile` | 0 |
| *devicefarm* | `ListOfferings` | 0 |
| *devicefarm* | `ListDeviceInstances` | 0 |
| *devicefarm* | `GetTest` | 0 |
| *devicefarm* | `DeleteNetworkProfile` | 0 |
| *devicefarm* | `ListProjects` | 1 |
| *devicefarm* | `ListNetworkProfiles` | 0 |
| *devicefarm* | `UpdateInstanceProfile` | 0 |
| *devicefarm* | `UpdateVPCEConfiguration` | 0 |
| *devicefarm* | `DeleteUpload` | 0 |
| *devicefarm* | `GetSuite` | 0 |
| *devicefarm* | `DeleteRemoteAccessSession` | 0 |
| *devicefarm* | `GetInstanceProfile` | 0 |
| *devicefarm* | `DeleteRun` | 0 |
| *devicefarm* | `ListOfferingTransactions` | 0 |
| *devicefarm* | `GetProject` | 1 |
| *devicefarm* | `InstallToRemoteAccessSession` | 0 |
| *devicefarm* | `ListInstanceProfiles` | 0 |
| *devicefarm* | `UpdateUpload` | 0 |
| *devicefarm* | `ListUniqueProblems` | 0 |
| *devicefarm* | `ListDevicePools` | 0 |
| *devicefarm* | `CreateUpload` | 0 |
| *devicefarm* | `GetVPCEConfiguration` | 0 |
| *devicefarm* | `GetDevicePoolCompatibility` | 0 |
| *devicefarm* | `DeleteDevicePool` | 0 |
| *devicefarm* | `UpdateDeviceInstance` | 0 |
| *devicefarm* | `ListTests` | 0 |
| *devicefarm* | `UpdateProject` | 0 |
| *devicefarm* | `ListSuites` | 0 |
| *devicefarm* | `GetDevicePool` | 0 |
| *devicefarm* | `ListDevices` | 0 |
| *devicefarm* | `ListVPCEConfigurations` | 0 |
| *devicefarm* | `StopRemoteAccessSession` | 0 |
| *devicefarm* | `RenewOffering` | 0 |
| *devicefarm* | `GetRemoteAccessSession` | 0 |
| *devicefarm* | `DeleteProject` | 0 |
| *devicefarm* | `ListJobs` | 0 |
| *devicefarm* | `GetRun` | 0 |
| *devicefarm* | `CreateVPCEConfiguration` | 0 |
| *devicefarm* | `TagResource` | 0 |
| *devicefarm* | `UpdateNetworkProfile` | 0 |
| *devicefarm* | `StopJob` | 0 |
| *devicefarm* | `ListRemoteAccessSessions` | 1 |
| *devicefarm* | `GetNetworkProfile` | 0 |
| *devicefarm* | `GetOfferingStatus` | 0 |
| *devicefarm* | `GetDeviceInstance` | 0 |
| *devicefarm* | `GetDevice` | 0 |
| *devicefarm* | `CreateDevicePool` | 0 |
| *devicefarm* | `ScheduleRun` | 0 |
| *devicefarm* | `ListArtifacts` | 0 |
| *devicefarm* | `UntagResource` | 0 |
| *devicefarm* | `CreateProject` | 1 |
| *devicefarm* | `GetJob` | 0 |
| *devicefarm* | `DeleteVPCEConfiguration` | 0 |
| *devicefarm* | `CreateInstanceProfile` | 0 |
| *devicefarm* | `ListRuns` | 1 |
| *devicefarm* | `ListUploads` | 0 |
| *devicefarm* | `GetAccountSettings` | 1 |
| *devicefarm* | `CreateNetworkProfile` | 0 |
| *devicefarm* | `ListTagsForResource` | 0 |
| *directconnect* | `DescribeVirtualGateways` | 0 |
| *directconnect* | `DescribeDirectConnectGatewayAssociations` | 0 |
| *directconnect* | `CreateBGPPeer` | 0 |
| *directconnect* | `AllocateHostedConnection` | 0 |
| *directconnect* | `DescribeHostedConnections` | 0 |
| *directconnect* | `AssociateHostedConnection` | 0 |
| *directconnect* | `AllocatePublicVirtualInterface` | 0 |
| *directconnect* | `DeleteVirtualInterface` | 0 |
| *directconnect* | `CreateInterconnect` | 0 |
| *directconnect* | `DescribeLags` | 0 |
| *directconnect* | `DeleteInterconnect` | 0 |
| *directconnect* | `AssociateVirtualInterface` | 0 |
| *directconnect* | `DeleteDirectConnectGatewayAssociation` | 0 |
| *directconnect* | `DescribeDirectConnectGatewayAttachments` | 0 |
| *directconnect* | `AllocatePrivateVirtualInterface` | 0 |
| *directconnect* | `AssociateConnectionWithLag` | 0 |
| *directconnect* | `CreateLag` | 0 |
| *directconnect* | `UpdateDirectConnectGatewayAssociation` | 0 |
| *directconnect* | `ConfirmPublicVirtualInterface` | 0 |
| *directconnect* | `DescribeLoa` | 0 |
| *directconnect* | `DeleteDirectConnectGatewayAssociationProposal` | 0 |
| *directconnect* | `DeleteBGPPeer` | 0 |
| *directconnect* | `DescribeVirtualInterfaces` | 0 |
| *directconnect* | `CreateDirectConnectGateway` | 0 |
| *directconnect* | `ConfirmPrivateVirtualInterface` | 0 |
| *directconnect* | `DescribeConnectionLoa` | 0 |
| *directconnect* | `CreateTransitVirtualInterface` | 0 |
| *directconnect* | `DescribeConnectionsOnInterconnect` | 0 |
| *directconnect* | `DisassociateConnectionFromLag` | 0 |
| *directconnect* | `AllocateConnectionOnInterconnect` | 0 |
| *directconnect* | `DeleteDirectConnectGateway` | 0 |
| *directconnect* | `ConfirmConnection` | 0 |
| *directconnect* | `DescribeInterconnects` | 0 |
| *directconnect* | `AcceptDirectConnectGatewayAssociationProposal` | 0 |
| *directconnect* | `DescribeTags` | 0 |
| *directconnect* | `DescribeConnections` | 0 |
| *directconnect* | `CreatePublicVirtualInterface` | 0 |
| *directconnect* | `DescribeInterconnectLoa` | 0 |
| *directconnect* | `CreateConnection` | 0 |
| *directconnect* | `TagResource` | 0 |
| *directconnect* | `DescribeLocations` | 0 |
| *directconnect* | `DescribeDirectConnectGatewayAssociationProposals` | 0 |
| *directconnect* | `UntagResource` | 0 |
| *directconnect* | `CreatePrivateVirtualInterface` | 0 |
| *directconnect* | `CreateDirectConnectGatewayAssociation` | 0 |
| *directconnect* | `CreateDirectConnectGatewayAssociationProposal` | 0 |
| *directconnect* | `DescribeDirectConnectGateways` | 0 |
| *directconnect* | `DeleteLag` | 0 |
| *directconnect* | `DeleteConnection` | 0 |
| *directconnect* | `UpdateVirtualInterfaceAttributes` | 0 |
| *directconnect* | `UpdateLag` | 0 |
| *directconnect* | `ConfirmTransitVirtualInterface` | 0 |
| *directconnect* | `AllocateTransitVirtualInterface` | 0 |
| *discovery* | `DeleteTags` | 0 |
| *discovery* | `StopDataCollectionByAgentIds` | 0 |
| *discovery* | `ListConfigurations` | 0 |
| *discovery* | `StartDataCollectionByAgentIds` | 0 |
| *discovery* | `ExportConfigurations` | 0 |
| *discovery* | `StartImportTask` | 0 |
| *discovery* | `DescribeExportConfigurations` | 0 |
| *discovery* | `StartContinuousExport` | 0 |
| *discovery* | `BatchDeleteImportData` | 0 |
| *discovery* | `DescribeContinuousExports` | 0 |
| *discovery* | `StartExportTask` | 0 |
| *discovery* | `DeleteApplications` | 0 |
| *discovery* | `CreateApplication` | 0 |
| *discovery* | `UpdateApplication` | 0 |
| *discovery* | `DescribeExportTasks` | 0 |
| *discovery* | `AssociateConfigurationItemsToApplication` | 0 |
| *discovery* | `DescribeConfigurations` | 0 |
| *discovery* | `StopContinuousExport` | 0 |
| *discovery* | `DescribeAgents` | 0 |
| *discovery* | `CreateTags` | 0 |
| *discovery* | `DescribeTags` | 0 |
| *discovery* | `GetDiscoverySummary` | 0 |
| *discovery* | `ListServerNeighbors` | 0 |
| *discovery* | `DescribeImportTasks` | 0 |
| *discovery* | `DisassociateConfigurationItemsFromApplication` | 0 |
| *dlm* | `DeleteLifecyclePolicy` | 1 |
| *dlm* | `GetLifecyclePolicies` | 1 |
| *dlm* | `CreateLifecyclePolicy` | 1 |
| *dlm* | `GetLifecyclePolicy` | 1 |
| *dlm* | `UpdateLifecyclePolicy` | 0 |
| *dms* | `RefreshSchemas` | 1 |
| *dms* | `DescribeCertificates` | 1 |
| *dms* | `DescribeReplicationInstances` | 1 |
| *dms* | `CreateReplicationSubnetGroup` | 1 |
| *dms* | `DescribeReplicationInstanceTaskLogs` | 1 |
| *dms* | `DescribeEvents` | 0 |
| *dms* | `DescribeEventSubscriptions` | 1 |
| *dms* | `CreateReplicationTask` | 1 |
| *dms* | `DeleteReplicationInstance` | 1 |
| *dms* | `ModifyReplicationInstance` | 0 |
| *dms* | `DescribeTableStatistics` | 1 |
| *dms* | `RebootReplicationInstance` | 0 |
| *dms* | `DescribeEventCategories` | 1 |
| *dms* | `StartReplicationTask` | 0 |
| *dms* | `DeleteReplicationSubnetGroup` | 1 |
| *dms* | `AddTagsToResource` | 0 |
| *dms* | `DescribeOrderableReplicationInstances` | 1 |
| *dms* | `DeleteCertificate` | 0 |
| *dms* | `TestConnection` | 0 |
| *dms* | `DescribeEndpoints` | 1 |
| *dms* | `RemoveTagsFromResource` | 0 |
| *dms* | `CreateEndpoint` | 1 |
| *dms* | `DeleteReplicationTask` | 1 |
| *dms* | `DescribeReplicationTaskAssessmentResults` | 1 |
| *dms* | `DescribeRefreshSchemasStatus` | 1 |
| *dms* | `ModifyEndpoint` | 0 |
| *dms* | `ReloadTables` | 0 |
| *dms* | `ModifyReplicationSubnetGroup` | 0 |
| *dms* | `DescribeSchemas` | 1 |
| *dms* | `DeleteEventSubscription` | 1 |
| *dms* | `StartReplicationTaskAssessment` | 0 |
| *dms* | `ModifyEventSubscription` | 0 |
| *dms* | `DescribeConnections` | 1 |
| *dms* | `CreateReplicationInstance` | 1 |
| *dms* | `ApplyPendingMaintenanceAction` | 0 |
| *dms* | `ImportCertificate` | 1 |
| *dms* | `DescribeReplicationSubnetGroups` | 1 |
| *dms* | `StopReplicationTask` | 0 |
| *dms* | `DescribeEndpointTypes` | 1 |
| *dms* | `DescribeReplicationTasks` | 0 |
| *dms* | `CreateEventSubscription` | 1 |
| *dms* | `DeleteEndpoint` | 1 |
| *dms* | `DeleteConnection` | 0 |
| *dms* | `DescribePendingMaintenanceActions` | 0 |
| *dms* | `DescribeAccountAttributes` | 1 |
| *dms* | `ModifyReplicationTask` | 0 |
| *dms* | `ListTagsForResource` | 0 |
| *docdb* | `DeleteDBSubnetGroup` | 1 |
| *docdb* | `DescribeCertificates` | 0 |
| *docdb* | `DescribeDBClusterParameters` | 0 |
| *docdb* | `DescribeDBSubnetGroups` | 1 |
| *docdb* | `RestoreDBClusterToPointInTime` | 0 |
| *docdb* | `DeleteDBInstance` | 1 |
| *docdb* | `CreateDBClusterParameterGroup` | 1 |
| *docdb* | `DescribeEngineDefaultClusterParameters` | 0 |
| *docdb* | `DeleteDBClusterSnapshot` | 0 |
| *docdb* | `RestoreDBClusterFromSnapshot` | 0 |
| *docdb* | `DescribePendingMaintenanceActions` | 1 |
| *docdb* | `RebootDBInstance` | 0 |
| *docdb* | `CreateDBSubnetGroup` | 1 |
| *docdb* | `DescribeEventCategories` | 0 |
| *docdb* | `ModifyDBCluster` | 0 |
| *docdb* | `AddTagsToResource` | 0 |
| *docdb* | `ModifyDBClusterParameterGroup` | 0 |
| *docdb* | `DescribeDBEngineVersions` | 0 |
| *docdb* | `FailoverDBCluster` | 0 |
| *docdb* | `RemoveTagsFromResource` | 0 |
| *docdb* | `ApplyPendingMaintenanceAction` | 0 |
| *docdb* | `DescribeDBInstances` | 0 |
| *docdb* | `StopDBCluster` | 0 |
| *docdb* | `DescribeOrderableDBInstanceOptions` | 0 |
| *docdb* | `CreateDBInstance` | 1 |
| *docdb* | `DescribeDBClusters` | 1 |
| *docdb* | `ModifyDBSubnetGroup` | 0 |
| *docdb* | `DescribeEvents` | 0 |
| *docdb* | `DescribeDBClusterSnapshots` | 0 |
| *docdb* | `CopyDBClusterParameterGroup` | 0 |
| *docdb* | `StartDBCluster` | 0 |
| *docdb* | `CreateDBClusterSnapshot` | 0 |
| *docdb* | `DeleteDBClusterParameterGroup` | 1 |
| *docdb* | `DescribeDBClusterSnapshotAttributes` | 0 |
| *docdb* | `CreateDBCluster` | 1 |
| *docdb* | `DescribeDBClusterParameterGroups` | 1 |
| *docdb* | `CopyDBClusterSnapshot` | 0 |
| *docdb* | `ModifyDBInstance` | 0 |
| *docdb* | `DeleteDBCluster` | 1 |
| *docdb* | `ResetDBClusterParameterGroup` | 0 |
| *docdb* | `ModifyDBClusterSnapshotAttribute` | 0 |
| *docdb* | `ListTagsForResource` | 0 |
| *ds* | `RemoveTagsFromResource` | 0 |
| *ds* | `DisableRadius` | 0 |
| *ds* | `RejectSharedDirectory` | 0 |
| *ds* | `AcceptSharedDirectory` | 0 |
| *ds* | `CreateComputer` | 0 |
| *ds* | `EnableRadius` | 0 |
| *ds* | `ListLogSubscriptions` | 0 |
| *ds* | `DeleteConditionalForwarder` | 0 |
| *ds* | `CreateDirectory` | 1 |
| *ds* | `ResetUserPassword` | 0 |
| *ds* | `UpdateTrust` | 0 |
| *ds* | `DescribeSnapshots` | 0 |
| *ds* | `VerifyTrust` | 0 |
| *ds* | `DeleteSnapshot` | 0 |
| *ds* | `UpdateRadius` | 0 |
| *ds* | `DescribeDomainControllers` | 0 |
| *ds* | `AddTagsToResource` | 0 |
| *ds* | `DescribeConditionalForwarders` | 0 |
| *ds* | `DescribeEventTopics` | 0 |
| *ds* | `DeleteLogSubscription` | 0 |
| *ds* | `EnableSso` | 0 |
| *ds* | `ListIpRoutes` | 0 |
| *ds* | `RemoveIpRoutes` | 0 |
| *ds* | `CreateAlias` | 0 |
| *ds* | `ConnectDirectory` | 0 |
| *ds* | `DeleteDirectory` | 1 |
| *ds* | `AddIpRoutes` | 0 |
| *ds* | `CreateLogSubscription` | 0 |
| *ds* | `CancelSchemaExtension` | 0 |
| *ds* | `CreateConditionalForwarder` | 0 |
| *ds* | `RestoreFromSnapshot` | 0 |
| *ds* | `DescribeSharedDirectories` | 0 |
| *ds* | `ListSchemaExtensions` | 0 |
| *ds* | `DescribeTrusts` | 0 |
| *ds* | `UnshareDirectory` | 0 |
| *ds* | `DeregisterEventTopic` | 0 |
| *ds* | `UpdateConditionalForwarder` | 0 |
| *ds* | `CreateTrust` | 0 |
| *ds* | `GetDirectoryLimits` | 0 |
| *ds* | `CreateMicrosoftAD` | 1 |
| *ds* | `RegisterEventTopic` | 0 |
| *ds* | `GetSnapshotLimits` | 0 |
| *ds* | `DeleteTrust` | 0 |
| *ds* | `ShareDirectory` | 0 |
| *ds* | `UpdateNumberOfDomainControllers` | 0 |
| *ds* | `DescribeDirectories` | 5 |
| *ds* | `StartSchemaExtension` | 0 |
| *ds* | `ListTagsForResource` | 0 |
| *ds* | `CreateSnapshot` | 0 |
| *ds* | `DisableSso` | 0 |
| *dynamodb* | `DescribeContinuousBackups` | 0 |
| *dynamodb* | `UpdateContinuousBackups` | 0 |
| *dynamodb* | `DescribeBackup` | 0 |
| *dynamodb* | `DescribeTimeToLive` | 0 |
| *dynamodb* | `DeleteBackup` | 0 |
| *dynamodb* | `DescribeGlobalTable` | 0 |
| *dynamodb* | `CreateTable` | 2 |
| *dynamodb* | `RestoreTableFromBackup` | 0 |
| *dynamodb* | `TransactWriteItems` | 0 |
| *dynamodb* | `UpdateTimeToLive` | 0 |
| *dynamodb* | `UpdateTable` | 0 |
| *dynamodb* | `DescribeLimits` | 0 |
| *dynamodb* | `CreateBackup` | 0 |
| *dynamodb* | `BatchWriteItem` | 0 |
| *dynamodb* | `DescribeEndpoints` | 0 |
| *dynamodb* | `TagResource` | 0 |
| *dynamodb* | `RestoreTableToPointInTime` | 0 |
| *dynamodb* | `ListTagsOfResource` | 0 |
| *dynamodb* | `DeleteItem` | 0 |
| *dynamodb* | `ListGlobalTables` | 0 |
| *dynamodb* | `UpdateItem` | 0 |
| *dynamodb* | `BatchGetItem` | 0 |
| *dynamodb* | `DescribeGlobalTableSettings` | 0 |
| *dynamodb* | `DescribeTable` | 2 |
| *dynamodb* | `ListTables` | 1 |
| *dynamodb* | `Query` | 0 |
| *dynamodb* | `DeleteTable` | 0 |
| *dynamodb* | `UntagResource` | 0 |
| *dynamodb* | `Scan` | 0 |
| *dynamodb* | `UpdateGlobalTable` | 0 |
| *dynamodb* | `ListBackups` | 0 |
| *dynamodb* | `UpdateGlobalTableSettings` | 0 |
| *dynamodb* | `CreateGlobalTable` | 1 |
| *dynamodb* | `GetItem` | 0 |
| *dynamodb* | `PutItem` | 1 |
| *dynamodb* | `TransactGetItems` | 0 |
| *dynamodbstreams* | `GetRecords` | 0 |
| *dynamodbstreams* | `ListStreams` | 0 |
| *dynamodbstreams* | `GetShardIterator` | 0 |
| *dynamodbstreams* | `DescribeStream` | 0 |
| *ec2* | `CreateVpnConnectionRoute` | 1 |
| *ec2* | `DescribeRouteTables` | 5 |
| *ec2* | `EnableTransitGatewayRouteTablePropagation` | 1 |
| *ec2* | `DescribeVpnGateways` | 2 |
| *ec2* | `ImportKeyPair` | 1 |
| *ec2* | `AttachVpnGateway` | 1 |
| *ec2* | `CreateTransitGatewayVpcAttachment` | 1 |
| *ec2* | `AssociateTransitGatewayRouteTable` | 1 |
| *ec2* | `CreateNetworkAcl` | 2 |
| *ec2* | `CreateFleet` | 0 |
| *ec2* | `AssociateAddress` | 1 |
| *ec2* | `ModifyIdentityIdFormat` | 0 |
| *ec2* | `ReplaceIamInstanceProfileAssociation` | 0 |
| *ec2* | `CreateInternetGateway` | 2 |
| *ec2* | `BundleInstance` | 0 |
| *ec2* | `CreateTrafficMirrorFilterRule` | 0 |
| *ec2* | `AuthorizeClientVpnIngress` | 0 |
| *ec2* | `ModifyEbsDefaultKmsKeyId` | 0 |
| *ec2* | `GetHostReservationPurchasePreview` | 0 |
| *ec2* | `CreateSubnet` | 2 |
| *ec2* | `DescribeVpcEndpointServicePermissions` | 1 |
| *ec2* | `DisableVgwRoutePropagation` | 0 |
| *ec2* | `UnmonitorInstances` | 0 |
| *ec2* | `CreatePlacementGroup` | 1 |
| *ec2* | `SendDiagnosticInterrupt` | 0 |
| *ec2* | `CopyFpgaImage` | 0 |
| *ec2* | `CreateLaunchTemplate` | 1 |
| *ec2* | `ModifySnapshotAttribute` | 0 |
| *ec2* | `DescribeTrafficMirrorTargets` | 0 |
| *ec2* | `DetachNetworkInterface` | 1 |
| *ec2* | `AcceptVpcPeeringConnection` | 0 |
| *ec2* | `CreateFlowLogs` | 1 |
| *ec2* | `DisableTransitGatewayRouteTablePropagation` | 0 |
| *ec2* | `DeleteNetworkInterface` | 1 |
| *ec2* | `DeleteVpcEndpoints` | 1 |
| *ec2* | `DescribeVpcEndpointConnections` | 0 |
| *ec2* | `ModifyInstancePlacement` | 0 |
| *ec2* | `CreateVolume` | 1 |
| *ec2* | `RunInstances` | 1 |
| *ec2* | `ResetNetworkInterfaceAttribute` | 0 |
| *ec2* | `DeleteRoute` | 0 |
| *ec2* | `AssociateSubnetCidrBlock` | 1 |
| *ec2* | `CreateKeyPair` | 1 |
| *ec2* | `ModifyLaunchTemplate` | 0 |
| *ec2* | `RevokeSecurityGroupEgress` | 0 |
| *ec2* | `DescribeSnapshotAttribute` | 1 |
| *ec2* | `DescribeAddresses` | 4 |
| *ec2* | `ConfirmProductInstance` | 0 |
| *ec2* | `DetachInternetGateway` | 1 |
| *ec2* | `CreateSecurityGroup` | 2 |
| *ec2* | `ReplaceNetworkAclEntry` | 0 |
| *ec2* | `AttachVolume` | 1 |
| *ec2* | `DescribeFleetHistory` | 0 |
| *ec2* | `ModifyHosts` | 0 |
| *ec2* | `DescribeFleetInstances` | 0 |
| *ec2* | `RebootInstances` | 1 |
| *ec2* | `DescribeImages` | 2 |
| *ec2* | `AllocateAddress` | 2 |
| *ec2* | `DescribeNetworkInterfacePermissions` | 0 |
| *ec2* | `AuthorizeSecurityGroupIngress` | 1 |
| *ec2* | `DescribeFpgaImageAttribute` | 0 |
| *ec2* | `DescribeFpgaImages` | 0 |
| *ec2* | `DeleteVpc` | 2 |
| *ec2* | `DescribeSubnets` | 21 |
| *ec2* | `DeleteVpcEndpointServiceConfigurations` | 0 |
| *ec2* | `DescribeReservedInstancesListings` | 0 |
| *ec2* | `CopyImage` | 1 |
| *ec2* | `DescribeSpotDatafeedSubscription` | 0 |
| *ec2* | `DescribeVpcClassicLink` | 0 |
| *ec2* | `ModifyVpcPeeringConnectionOptions` | 0 |
| *ec2* | `RestoreAddressToClassic` | 0 |
| *ec2* | `DescribeVpcEndpointServiceConfigurations` | 1 |
| *ec2* | `RejectVpcEndpointConnections` | 0 |
| *ec2* | `ModifyFleet` | 0 |
| *ec2* | `CreateVpnConnection` | 1 |
| *ec2* | `CreateReservedInstancesListing` | 0 |
| *ec2* | `DeleteNetworkInterfacePermission` | 0 |
| *ec2* | `GetCapacityReservationUsage` | 0 |
| *ec2* | `DisassociateClientVpnTargetNetwork` | 0 |
| *ec2* | `ModifyVolume` | 0 |
| *ec2* | `DescribeCustomerGateways` | 2 |
| *ec2* | `DescribeNetworkAcls` | 3 |
| *ec2* | `AdvertiseByoipCidr` | 0 |
| *ec2* | `CancelConversionTask` | 0 |
| *ec2* | `CreateVpcEndpointConnectionNotification` | 1 |
| *ec2* | `ResetImageAttribute` | 0 |
| *ec2* | `EnableVgwRoutePropagation` | 1 |
| *ec2* | `CreateVpcEndpoint` | 2 |
| *ec2* | `DescribeTransitGatewayVpcAttachments` | 0 |
| *ec2* | `GetTransitGatewayRouteTableAssociations` | 1 |
| *ec2* | `DescribeReservedInstancesModifications` | 0 |
| *ec2* | `UnassignIpv6Addresses` | 0 |
| *ec2* | `AssignPrivateIpAddresses` | 0 |
| *ec2* | `ResetSnapshotAttribute` | 0 |
| *ec2* | `CancelExportTask` | 0 |
| *ec2* | `ImportClientVpnClientCertificateRevocationList` | 0 |
| *ec2* | `CreateImage` | 1 |
| *ec2* | `CreateTransitGateway` | 1 |
| *ec2* | `ModifyInstanceCreditSpecification` | 0 |
| *ec2* | `DescribeIamInstanceProfileAssociations` | 0 |
| *ec2* | `ModifyIdFormat` | 0 |
| *ec2* | `DescribeSpotFleetRequests` | 1 |
| *ec2* | `DeleteClientVpnEndpoint` | 0 |
| *ec2* | `DescribeClientVpnTargetNetworks` | 0 |
| *ec2* | `ExportTransitGatewayRoutes` | 0 |
| *ec2* | `CopySnapshot` | 0 |
| *ec2* | `DescribeTransitGatewayAttachments` | 0 |
| *ec2* | `DescribeHostReservationOfferings` | 0 |
| *ec2* | `DeleteSubnet` | 1 |
| *ec2* | `AttachClassicLinkVpc` | 0 |
| *ec2* | `DescribeDhcpOptions` | 4 |
| *ec2* | `DescribeVpcEndpointServices` | 2 |
| *ec2* | `DescribeByoipCidrs` | 0 |
| *ec2* | `ReleaseHosts` | 0 |
| *ec2* | `ModifyVpnTunnelCertificate` | 0 |
| *ec2* | `DescribeRegions` | 3 |
| *ec2* | `AttachInternetGateway` | 2 |
| *ec2* | `DescribeVolumeAttribute` | 0 |
| *ec2* | `ModifyReservedInstances` | 0 |
| *ec2* | `DescribeSpotFleetInstances` | 0 |
| *ec2* | `EnableVpcClassicLink` | 0 |
| *ec2* | `DescribeTransitGatewayRouteTables` | 1 |
| *ec2* | `CreateDefaultSubnet` | 0 |
| *ec2* | `DeleteTrafficMirrorFilterRule` | 0 |
| *ec2* | `DescribeVpcPeeringConnections` | 1 |
| *ec2* | `AssociateClientVpnTargetNetwork` | 0 |
| *ec2* | `UnassignPrivateIpAddresses` | 0 |
| *ec2* | `CreateEgressOnlyInternetGateway` | 1 |
| *ec2* | `CreateDefaultVpc` | 1 |
| *ec2* | `AttachNetworkInterface` | 1 |
| *ec2* | `ApplySecurityGroupsToClientVpnTargetNetwork` | 0 |
| *ec2* | `DescribeIdFormat` | 1 |
| *ec2* | `ModifyTrafficMirrorSession` | 0 |
| *ec2* | `DescribeInstanceAttribute` | 1 |
| *ec2* | `DeregisterImage` | 1 |
| *ec2* | `PurchaseReservedInstancesOffering` | 0 |
| *ec2* | `ModifyNetworkInterfaceAttribute` | 1 |
| *ec2* | `ModifyTransitGatewayVpcAttachment` | 0 |
| *ec2* | `DescribeLaunchTemplates` | 3 |
| *ec2* | `ProvisionByoipCidr` | 0 |
| *ec2* | `DescribeExportTasks` | 0 |
| *ec2* | `CreateCustomerGateway` | 1 |
| *ec2* | `ModifyVpcTenancy` | 0 |
| *ec2* | `DeleteSpotDatafeedSubscription` | 0 |
| *ec2* | `DeletePlacementGroup` | 1 |
| *ec2* | `DeleteRouteTable` | 3 |
| *ec2* | `ExportClientVpnClientCertificateRevocationList` | 0 |
| *ec2* | `DescribeReservedInstancesOfferings` | 0 |
| *ec2* | `DisableVpcClassicLinkDnsSupport` | 0 |
| *ec2* | `AssignIpv6Addresses` | 0 |
| *ec2* | `RevokeSecurityGroupIngress` | 1 |
| *ec2* | `EnableEbsEncryptionByDefault` | 0 |
| *ec2* | `ModifyVpcEndpointServiceConfiguration` | 0 |
| *ec2* | `RejectVpcPeeringConnection` | 0 |
| *ec2* | `MonitorInstances` | 0 |
| *ec2* | `DeleteTransitGatewayRouteTable` | 0 |
| *ec2* | `DescribeTrafficMirrorSessions` | 0 |
| *ec2* | `ModifyVpcEndpointConnectionNotification` | 0 |
| *ec2* | `DescribeInstances` | 6 |
| *ec2* | `DescribeLaunchTemplateVersions` | 2 |
| *ec2* | `DeleteQueuedReservedInstances` | 0 |
| *ec2* | `ModifyInstanceEventStartTime` | 0 |
| *ec2* | `DeleteVolume` | 1 |
| *ec2* | `DeleteSecurityGroup` | 0 |
| *ec2* | `CreateCapacityReservation` | 0 |
| *ec2* | `ModifyVolumeAttribute` | 0 |
| *ec2* | `CreateClientVpnEndpoint` | 1 |
| *ec2* | `GetConsoleOutput` | 0 |
| *ec2* | `DeleteClientVpnRoute` | 0 |
| *ec2* | `DeprovisionByoipCidr` | 0 |
| *ec2* | `DescribeClientVpnConnections` | 0 |
| *ec2* | `AcceptReservedInstancesExchangeQuote` | 0 |
| *ec2* | `DescribeSecurityGroupReferences` | 0 |
| *ec2* | `ReplaceNetworkAclAssociation` | 1 |
| *ec2* | `DetachClassicLinkVpc` | 0 |
| *ec2* | `DescribeInternetGateways` | 3 |
| *ec2* | `EnableVpcClassicLinkDnsSupport` | 0 |
| *ec2* | `CancelImportTask` | 0 |
| *ec2* | `DeleteTransitGatewayVpcAttachment` | 0 |
| *ec2* | `EnableVolumeIO` | 0 |
| *ec2* | `ReplaceRouteTableAssociation` | 0 |
| *ec2* | `DescribeCapacityReservations` | 1 |
| *ec2* | `RegisterImage` | 1 |
| *ec2* | `ModifyVpcEndpointServicePermissions` | 1 |
| *ec2* | `DescribeAvailabilityZones` | 13 |
| *ec2* | `CreateTrafficMirrorSession` | 0 |
| *ec2* | `RunScheduledInstances` | 0 |
| *ec2* | `DeleteVpnConnection` | 0 |
| *ec2* | `DescribeImageAttribute` | 1 |
| *ec2* | `DeleteTransitGatewayRoute` | 0 |
| *ec2* | `DescribeReservedInstances` | 0 |
| *ec2* | `DescribeVpcEndpoints` | 3 |
| *ec2* | `DetachVolume` | 0 |
| *ec2* | `ImportImage` | 0 |
| *ec2* | `AcceptVpcEndpointConnections` | 0 |
| *ec2* | `CancelBundleTask` | 0 |
| *ec2* | `DisassociateSubnetCidrBlock` | 0 |
| *ec2* | `CreateVpcPeeringConnection` | 1 |
| *ec2* | `ReplaceTransitGatewayRoute` | 0 |
| *ec2* | `ExportImage` | 0 |
| *ec2* | `DescribeVpnConnections` | 1 |
| *ec2* | `DisassociateAddress` | 1 |
| *ec2* | `DescribeAccountAttributes` | 2 |
| *ec2* | `DisassociateTransitGatewayRouteTable` | 0 |
| *ec2* | `DeleteCustomerGateway` | 1 |
| *ec2* | `CreateNetworkAclEntry` | 1 |
| *ec2* | `DeleteLaunchTemplateVersions` | 0 |
| *ec2* | `CreateNetworkInterface` | 1 |
| *ec2* | `CreateInstanceExportTask` | 0 |
| *ec2* | `ResetInstanceAttribute` | 0 |
| *ec2* | `DeleteVpcEndpointConnectionNotifications` | 0 |
| *ec2* | `CreateClientVpnRoute` | 0 |
| *ec2* | `ModifyTrafficMirrorFilterNetworkServices` | 0 |
| *ec2* | `CreateTransitGatewayRoute` | 1 |
| *ec2* | `DeleteInternetGateway` | 1 |
| *ec2* | `CreateTrafficMirrorTarget` | 0 |
| *ec2* | `ModifyInstanceAttribute` | 0 |
| *ec2* | `CreateNatGateway` | 1 |
| *ec2* | `DeleteVpnConnectionRoute` | 0 |
| *ec2* | `DescribeNetworkInterfaceAttribute` | 0 |
| *ec2* | `ExportClientVpnClientConfiguration` | 0 |
| *ec2* | `DisableVpcClassicLink` | 0 |
| *ec2* | `ModifyVpnTunnelOptions` | 0 |
| *ec2* | `ReportInstanceStatus` | 0 |
| *ec2* | `CancelReservedInstancesListing` | 0 |
| *ec2* | `ModifyCapacityReservation` | 0 |
| *ec2* | `ModifySubnetAttribute` | 0 |
| *ec2* | `DescribeNetworkInterfaces` | 3 |
| *ec2* | `ModifyImageAttribute` | 1 |
| *ec2* | `DescribeVpcClassicLinkDnsSupport` | 1 |
| *ec2* | `DescribeEgressOnlyInternetGateways` | 2 |
| *ec2* | `DescribeSnapshots` | 3 |
| *ec2* | `TerminateInstances` | 1 |
| *ec2* | `DescribeClientVpnRoutes` | 0 |
| *ec2* | `DescribeKeyPairs` | 3 |
| *ec2* | `ModifyInstanceCapacityReservationAttributes` | 0 |
| *ec2* | `ReleaseAddress` | 1 |
| *ec2* | `WithdrawByoipCidr` | 0 |
| *ec2* | `DescribeFleets` | 0 |
| *ec2* | `DeleteFleets` | 0 |
| *ec2* | `DescribeVolumeStatus` | 0 |
| *ec2* | `DescribePrincipalIdFormat` | 0 |
| *ec2* | `DescribePublicIpv4Pools` | 0 |
| *ec2* | `GetPasswordData` | 0 |
| *ec2* | `ImportVolume` | 0 |
| *ec2* | `RevokeClientVpnIngress` | 0 |
| *ec2* | `CreateDhcpOptions` | 1 |
| *ec2* | `ModifySpotFleetRequest` | 0 |
| *ec2* | `DescribeConversionTasks` | 0 |
| *ec2* | `ModifyFpgaImageAttribute` | 0 |
| *ec2* | `GetLaunchTemplateData` | 0 |
| *ec2* | `CreateLaunchTemplateVersion` | 0 |
| *ec2* | `DescribeTags` | 2 |
| *ec2* | `DescribeInstanceStatus` | 0 |
| *ec2* | `DeleteFlowLogs` | 1 |
| *ec2* | `UpdateSecurityGroupRuleDescriptionsIngress` | 0 |
| *ec2* | `DescribeClientVpnEndpoints` | 1 |
| *ec2* | `DeleteTags` | 1 |
| *ec2* | `ModifyVpnConnection` | 0 |
| *ec2* | `CreateRouteTable` | 3 |
| *ec2* | `DeleteNetworkAcl` | 2 |
| *ec2* | `TerminateClientVpnConnections` | 0 |
| *ec2* | `DeleteTransitGateway` | 0 |
| *ec2* | `AssociateDhcpOptions` | 1 |
| *ec2* | `DescribeImportSnapshotTasks` | 0 |
| *ec2* | `CreateNetworkInterfacePermission` | 0 |
| *ec2* | `UpdateSecurityGroupRuleDescriptionsEgress` | 0 |
| *ec2* | `DeleteLaunchTemplate` | 0 |
| *ec2* | `AssociateIamInstanceProfile` | 0 |
| *ec2* | `GetReservedInstancesExchangeQuote` | 0 |
| *ec2* | `DescribePlacementGroups` | 1 |
| *ec2* | `DeleteTrafficMirrorSession` | 0 |
| *ec2* | `DetachVpnGateway` | 1 |
| *ec2* | `PurchaseScheduledInstances` | 0 |
| *ec2* | `MoveAddressToVpc` | 0 |
| *ec2* | `DescribeNatGateways` | 3 |
| *ec2* | `AllocateHosts` | 1 |
| *ec2* | `DescribeTrafficMirrorFilters` | 0 |
| *ec2* | `DescribeVpcs` | 29 |
| *ec2* | `CreateSnapshot` | 0 |
| *ec2* | `RequestSpotFleet` | 1 |
| *ec2* | `StartInstances` | 1 |
| *ec2* | `DescribeSpotInstanceRequests` | 0 |
| *ec2* | `RejectTransitGatewayVpcAttachment` | 0 |
| *ec2* | `CreateVpc` | 4 |
| *ec2* | `CancelSpotFleetRequests` | 1 |
| *ec2* | `ModifyClientVpnEndpoint` | 0 |
| *ec2* | `DeleteTrafficMirrorFilter` | 0 |
| *ec2* | `AssociateVpcCidrBlock` | 1 |
| *ec2* | `GetTransitGatewayRouteTablePropagations` | 1 |
| *ec2* | `ResetFpgaImageAttribute` | 0 |
| *ec2* | `DescribeExportImageTasks` | 0 |
| *ec2* | `DescribeSpotPriceHistory` | 1 |
| *ec2* | `DeleteVpcPeeringConnection` | 1 |
| *ec2* | `DescribeFlowLogs` | 2 |
| *ec2* | `StopInstances` | 1 |
| *ec2* | `DeleteDhcpOptions` | 1 |
| *ec2* | `AcceptTransitGatewayVpcAttachment` | 0 |
| *ec2* | `DescribeImportImageTasks` | 0 |
| *ec2* | `DisassociateVpcCidrBlock` | 1 |
| *ec2* | `CreateTransitGatewayRouteTable` | 1 |
| *ec2* | `DescribeHosts` | 2 |
| *ec2* | `DescribeVolumes` | 3 |
| *ec2* | `DeleteKeyPair` | 1 |
| *ec2* | `DeleteNatGateway` | 2 |
| *ec2* | `DescribeTransitGateways` | 1 |
| *ec2* | `DisassociateIamInstanceProfile` | 0 |
| *ec2* | `DescribeInstanceCreditSpecifications` | 1 |
| *ec2* | `DescribeScheduledInstances` | 0 |
| *ec2* | `ModifyTrafficMirrorFilterRule` | 0 |
| *ec2* | `CancelSpotInstanceRequests` | 0 |
| *ec2* | `DeleteSnapshot` | 1 |
| *ec2* | `DescribeMovingAddresses` | 0 |
| *ec2* | `CreateSnapshots` | 0 |
| *ec2* | `DeleteVpnGateway` | 1 |
| *ec2* | `CreateTrafficMirrorFilter` | 0 |
| *ec2* | `DescribeElasticGpus` | 0 |
| *ec2* | `DeleteNetworkAclEntry` | 0 |
| *ec2* | `CancelCapacityReservation` | 0 |
| *ec2* | `CreateFpgaImage` | 0 |
| *ec2* | `DescribeIdentityIdFormat` | 0 |
| *ec2* | `DescribeScheduledInstanceAvailability` | 0 |
| *ec2* | `CreateTags` | 4 |
| *ec2* | `DescribeSpotFleetRequestHistory` | 0 |
| *ec2* | `GetConsoleScreenshot` | 0 |
| *ec2* | `DescribeVolumesModifications` | 0 |
| *ec2* | `AuthorizeSecurityGroupEgress` | 0 |
| *ec2* | `CreateSpotDatafeedSubscription` | 0 |
| *ec2* | `ImportInstance` | 0 |
| *ec2* | `DeleteFpgaImage` | 0 |
| *ec2* | `DescribePrefixLists` | 1 |
| *ec2* | `DisassociateRouteTable` | 0 |
| *ec2* | `GetTransitGatewayAttachmentPropagations` | 0 |
| *ec2* | `ReplaceRoute` | 0 |
| *ec2* | `AssociateRouteTable` | 2 |
| *ec2* | `RequestSpotInstances` | 0 |
| *ec2* | `CreateRoute` | 2 |
| *ec2* | `SearchTransitGatewayRoutes` | 1 |
| *ec2* | `DeleteEgressOnlyInternetGateway` | 1 |
| *ec2* | `DescribeSecurityGroups` | 22 |
| *ec2* | `CreateVpnGateway` | 1 |
| *ec2* | `DescribeBundleTasks` | 0 |
| *ec2* | `GetEbsDefaultKmsKeyId` | 0 |
| *ec2* | `PurchaseHostReservation` | 0 |
| *ec2* | `DescribeClassicLinkInstances` | 0 |
| *ec2* | `DeleteTrafficMirrorTarget` | 0 |
| *ec2* | `ImportSnapshot` | 0 |
| *ec2* | `DescribeVpcAttribute` | 3 |
| *ec2* | `DescribeClientVpnAuthorizationRules` | 0 |
| *ec2* | `DescribeVpcEndpointConnectionNotifications` | 1 |
| *ec2* | `ModifyVpcEndpoint` | 0 |
| *ec2* | `GetEbsEncryptionByDefault` | 0 |
| *ec2* | `CreateVpcEndpointServiceConfiguration` | 1 |
| *ec2* | `DescribeAggregateIdFormat` | 0 |
| *ec2* | `DescribeStaleSecurityGroups` | 2 |
| *ec2* | `ResetEbsDefaultKmsKeyId` | 0 |
| *ec2* | `DisableEbsEncryptionByDefault` | 0 |
| *ec2* | `DescribeHostReservations` | 0 |
| *ec2* | `ModifyVpcAttribute` | 1 |
| *ec2-instance-connect* | `SendSSHPublicKey` | 0 |
| *ecr* | `DeleteRepository` | 1 |
| *ecr* | `GetAuthorizationToken` | 0 |
| *ecr* | `StartLifecyclePolicyPreview` | 0 |
| *ecr* | `DescribeImages` | 1 |
| *ecr* | `DeleteLifecyclePolicy` | 0 |
| *ecr* | `DescribeRepositories` | 2 |
| *ecr* | `GetLifecyclePolicy` | 1 |
| *ecr* | `SetRepositoryPolicy` | 1 |
| *ecr* | `PutImage` | 0 |
| *ecr* | `StartImageScan` | 0 |
| *ecr* | `UploadLayerPart` | 0 |
| *ecr* | `TagResource` | 0 |
| *ecr* | `DeleteRepositoryPolicy` | 1 |
| *ecr* | `ListImages` | 0 |
| *ecr* | `PutImageTagMutability` | 0 |
| *ecr* | `CompleteLayerUpload` | 0 |
| *ecr* | `InitiateLayerUpload` | 0 |
| *ecr* | `CreateRepository` | 2 |
| *ecr* | `BatchCheckLayerAvailability` | 0 |
| *ecr* | `DescribeImageScanFindings` | 0 |
| *ecr* | `GetLifecyclePolicyPreview` | 1 |
| *ecr* | `UntagResource` | 0 |
| *ecr* | `BatchDeleteImage` | 0 |
| *ecr* | `GetDownloadUrlForLayer` | 0 |
| *ecr* | `GetRepositoryPolicy` | 1 |
| *ecr* | `PutLifecyclePolicy` | 1 |
| *ecr* | `PutImageScanningConfiguration` | 0 |
| *ecr* | `BatchGetImage` | 0 |
| *ecr* | `ListTagsForResource` | 0 |
| *ecs* | `CreateService` | 1 |
| *ecs* | `SubmitContainerStateChange` | 0 |
| *ecs* | `ListAttributes` | 0 |
| *ecs* | `CreateCluster` | 1 |
| *ecs* | `SubmitTaskStateChange` | 0 |
| *ecs* | `ListTaskDefinitionFamilies` | 1 |
| *ecs* | `DeleteService` | 0 |
| *ecs* | `ListContainerInstances` | 0 |
| *ecs* | `DescribeServices` | 0 |
| *ecs* | `DeleteCluster` | 0 |
| *ecs* | `UpdateService` | 0 |
| *ecs* | `UpdateServicePrimaryTaskSet` | 0 |
| *ecs* | `DeleteTaskSet` | 0 |
| *ecs* | `ListClusters` | 1 |
| *ecs* | `SubmitAttachmentStateChanges` | 0 |
| *ecs* | `UpdateTaskSet` | 0 |
| *ecs* | `DeregisterTaskDefinition` | 0 |
| *ecs* | `PutAccountSetting` | 0 |
| *ecs* | `StopTask` | 0 |
| *ecs* | `ListTaskDefinitions` | 0 |
| *ecs* | `DescribeClusters` | 0 |
| *ecs* | `UpdateContainerAgent` | 0 |
| *ecs* | `RegisterTaskDefinition` | 1 |
| *ecs* | `DeleteAttributes` | 0 |
| *ecs* | `ListServices` | 0 |
| *ecs* | `PutAttributes` | 0 |
| *ecs* | `StartTask` | 0 |
| *ecs* | `TagResource` | 0 |
| *ecs* | `PutAccountSettingDefault` | 0 |
| *ecs* | `RegisterContainerInstance` | 0 |
| *ecs* | `DescribeTaskDefinition` | 0 |
| *ecs* | `DescribeTasks` | 0 |
| *ecs* | `DeleteAccountSetting` | 0 |
| *ecs* | `UpdateContainerInstancesState` | 0 |
| *ecs* | `CreateTaskSet` | 0 |
| *ecs* | `UpdateClusterSettings` | 0 |
| *ecs* | `ListTagsForResource` | 0 |
| *ecs* | `DeregisterContainerInstance` | 0 |
| *ecs* | `UntagResource` | 0 |
| *ecs* | `ListTasks` | 0 |
| *ecs* | `DescribeContainerInstances` | 0 |
| *ecs* | `ListAccountSettings` | 0 |
| *ecs* | `RunTask` | 0 |
| *ecs* | `DescribeTaskSets` | 0 |
| *ecs* | `DiscoverPollEndpoint` | 0 |
| *efs* | `DeleteTags` | 1 |
| *efs* | `DescribeMountTargetSecurityGroups` | 0 |
| *efs* | `DeleteFileSystem` | 1 |
| *efs* | `CreateTags` | 1 |
| *efs* | `CreateMountTarget` | 1 |
| *efs* | `DescribeFileSystems` | 2 |
| *efs* | `DescribeTags` | 1 |
| *efs* | `PutLifecycleConfiguration` | 0 |
| *efs* | `UpdateFileSystem` | 1 |
| *efs* | `ModifyMountTargetSecurityGroups` | 1 |
| *efs* | `DescribeLifecycleConfiguration` | 0 |
| *efs* | `DescribeMountTargets` | 2 |
| *efs* | `CreateFileSystem` | 1 |
| *efs* | `DeleteMountTarget` | 1 |
| *eks* | `ListUpdates` | 0 |
| *eks* | `UpdateClusterConfig` | 0 |
| *eks* | `ListClusters` | 1 |
| *eks* | `UntagResource` | 0 |
| *eks* | `CreateCluster` | 1 |
| *eks* | `DeleteCluster` | 0 |
| *eks* | `DescribeUpdate` | 0 |
| *eks* | `TagResource` | 0 |
| *eks* | `ListTagsForResource` | 0 |
| *eks* | `DescribeCluster` | 0 |
| *eks* | `UpdateClusterVersion` | 0 |
| *elasticache* | `DescribeCacheEngineVersions` | 1 |
| *elasticache* | `DeleteCacheSubnetGroup` | 0 |
| *elasticache* | `ResetCacheParameterGroup` | 0 |
| *elasticache* | `DeleteCacheSecurityGroup` | 0 |
| *elasticache* | `DescribeEvents` | 0 |
| *elasticache* | `DescribeEngineDefaultParameters` | 0 |
| *elasticache* | `DescribeUpdateActions` | 0 |
| *elasticache* | `DescribeCacheSubnetGroups` | 1 |
| *elasticache* | `DescribeCacheSecurityGroups` | 0 |
| *elasticache* | `RebootCacheCluster` | 0 |
| *elasticache* | `DecreaseReplicaCount` | 0 |
| *elasticache* | `ModifyCacheSubnetGroup` | 0 |
| *elasticache* | `DescribeCacheParameterGroups` | 1 |
| *elasticache* | `DeleteCacheParameterGroup` | 0 |
| *elasticache* | `AuthorizeCacheSecurityGroupIngress` | 0 |
| *elasticache* | `CopySnapshot` | 0 |
| *elasticache* | `AddTagsToResource` | 0 |
| *elasticache* | `IncreaseReplicaCount` | 0 |
| *elasticache* | `DescribeCacheClusters` | 1 |
| *elasticache* | `ModifyCacheCluster` | 0 |
| *elasticache* | `CreateReplicationGroup` | 1 |
| *elasticache* | `RemoveTagsFromResource` | 0 |
| *elasticache* | `DescribeCacheParameters` | 0 |
| *elasticache* | `CreateCacheParameterGroup` | 1 |
| *elasticache* | `TestFailover` | 0 |
| *elasticache* | `DescribeReplicationGroups` | 1 |
| *elasticache* | `DeleteReplicationGroup` | 0 |
| *elasticache* | `DescribeReservedCacheNodesOfferings` | 0 |
| *elasticache* | `BatchStopUpdateAction` | 0 |
| *elasticache* | `CreateCacheCluster` | 1 |
| *elasticache* | `CreateCacheSubnetGroup` | 1 |
| *elasticache* | `StartMigration` | 0 |
| *elasticache* | `ModifyReplicationGroup` | 0 |
| *elasticache* | `CompleteMigration` | 0 |
| *elasticache* | `ModifyCacheParameterGroup` | 0 |
| *elasticache* | `DescribeSnapshots` | 1 |
| *elasticache* | `DescribeServiceUpdates` | 0 |
| *elasticache* | `ListTagsForResource` | 0 |
| *elasticache* | `DeleteCacheCluster` | 0 |
| *elasticache* | `PurchaseReservedCacheNodesOffering` | 0 |
| *elasticache* | `DeleteSnapshot` | 0 |
| *elasticache* | `RevokeCacheSecurityGroupIngress` | 0 |
| *elasticache* | `ListAllowedNodeTypeModifications` | 0 |
| *elasticache* | `DescribeReservedCacheNodes` | 1 |
| *elasticache* | `CreateCacheSecurityGroup` | 0 |
| *elasticache* | `BatchApplyUpdateAction` | 0 |
| *elasticache* | `CreateSnapshot` | 0 |
| *elasticache* | `ModifyReplicationGroupShardConfiguration` | 0 |
| *elasticbeanstalk* | `UpdateTagsForResource` | 0 |
| *elasticbeanstalk* | `DescribeApplicationVersions` | 1 |
| *elasticbeanstalk* | `RequestEnvironmentInfo` | 0 |
| *elasticbeanstalk* | `DescribeEnvironmentResources` | 0 |
| *elasticbeanstalk* | `DescribeEvents` | 0 |
| *elasticbeanstalk* | `DescribeEnvironmentHealth` | 0 |
| *elasticbeanstalk* | `UpdateApplicationResourceLifecycle` | 0 |
| *elasticbeanstalk* | `TerminateEnvironment` | 0 |
| *elasticbeanstalk* | `UpdateApplicationVersion` | 0 |
| *elasticbeanstalk* | `DeleteApplication` | 0 |
| *elasticbeanstalk* | `ApplyEnvironmentManagedAction` | 0 |
| *elasticbeanstalk* | `DescribeConfigurationOptions` | 0 |
| *elasticbeanstalk* | `DescribeEnvironmentManagedActionHistory` | 0 |
| *elasticbeanstalk* | `DeleteConfigurationTemplate` | 0 |
| *elasticbeanstalk* | `ValidateConfigurationSettings` | 0 |
| *elasticbeanstalk* | `ListAvailableSolutionStacks` | 0 |
| *elasticbeanstalk* | `ComposeEnvironments` | 0 |
| *elasticbeanstalk* | `DescribeConfigurationSettings` | 0 |
| *elasticbeanstalk* | `CreateApplication` | 1 |
| *elasticbeanstalk* | `UpdateConfigurationTemplate` | 0 |
| *elasticbeanstalk* | `DeleteApplicationVersion` | 1 |
| *elasticbeanstalk* | `RetrieveEnvironmentInfo` | 0 |
| *elasticbeanstalk* | `UpdateApplication` | 0 |
| *elasticbeanstalk* | `UpdateEnvironment` | 0 |
| *elasticbeanstalk* | `DescribeEnvironments` | 0 |
| *elasticbeanstalk* | `ListPlatformVersions` | 1 |
| *elasticbeanstalk* | `DescribeEnvironmentManagedActions` | 0 |
| *elasticbeanstalk* | `AbortEnvironmentUpdate` | 0 |
| *elasticbeanstalk* | `RestartAppServer` | 0 |
| *elasticbeanstalk* | `DeleteEnvironmentConfiguration` | 1 |
| *elasticbeanstalk* | `CreateEnvironment` | 1 |
| *elasticbeanstalk* | `SwapEnvironmentCNAMEs` | 0 |
| *elasticbeanstalk* | `DeletePlatformVersion` | 0 |
| *elasticbeanstalk* | `CreateStorageLocation` | 0 |
| *elasticbeanstalk* | `RebuildEnvironment` | 0 |
| *elasticbeanstalk* | `ListTagsForResource` | 0 |
| *elasticbeanstalk* | `DescribeApplications` | 0 |
| *elasticbeanstalk* | `CreatePlatformVersion` | 0 |
| *elasticbeanstalk* | `DescribePlatformVersion` | 0 |
| *elasticbeanstalk* | `DescribeAccountAttributes` | 0 |
| *elasticbeanstalk* | `DescribeInstancesHealth` | 0 |
| *elasticbeanstalk* | `CreateApplicationVersion` | 1 |
| *elasticbeanstalk* | `CreateConfigurationTemplate` | 1 |
| *elasticbeanstalk* | `CheckDNSAvailability` | 0 |
| *elastictranscoder* | `ReadPreset` | 0 |
| *elastictranscoder* | `CreateJob` | 0 |
| *elastictranscoder* | `CreatePreset` | 0 |
| *elastictranscoder* | `UpdatePipelineNotifications` | 0 |
| *elastictranscoder* | `ListPipelines` | 0 |
| *elastictranscoder* | `DeletePipeline` | 0 |
| *elastictranscoder* | `ReadJob` | 0 |
| *elastictranscoder* | `UpdatePipeline` | 0 |
| *elastictranscoder* | `ListPresets` | 0 |
| *elastictranscoder* | `DeletePreset` | 0 |
| *elastictranscoder* | `ListJobsByPipeline` | 0 |
| *elastictranscoder* | `CancelJob` | 0 |
| *elastictranscoder* | `UpdatePipelineStatus` | 0 |
| *elastictranscoder* | `CreatePipeline` | 0 |
| *elastictranscoder* | `ReadPipeline` | 0 |
| *elastictranscoder* | `TestRole` | 0 |
| *elastictranscoder* | `ListJobsByStatus` | 0 |
| *elb* | `CreateAppCookieStickinessPolicy` | 0 |
| *elb* | `EnableAvailabilityZonesForLoadBalancer` | 0 |
| *elb* | `SetLoadBalancerPoliciesOfListener` | 0 |
| *elb* | `DescribeLoadBalancerAttributes` | 0 |
| *elb* | `DeleteLoadBalancerListeners` | 0 |
| *elb* | `DeleteLoadBalancer` | 1 |
| *elb* | `CreateLoadBalancerPolicy` | 0 |
| *elb* | `DeleteLoadBalancerPolicy` | 0 |
| *elb* | `CreateLBCookieStickinessPolicy` | 0 |
| *elb* | `DescribeInstanceHealth` | 0 |
| *elb* | `DescribeLoadBalancers` | 0 |
| *elb* | `RemoveTags` | 0 |
| *elb* | `SetLoadBalancerPoliciesForBackendServer` | 0 |
| *elb* | `DescribeLoadBalancerPolicies` | 0 |
| *elb* | `DescribeLoadBalancerPolicyTypes` | 0 |
| *elb* | `ApplySecurityGroupsToLoadBalancer` | 0 |
| *elb* | `DescribeTags` | 0 |
| *elb* | `ConfigureHealthCheck` | 1 |
| *elb* | `AttachLoadBalancerToSubnets` | 0 |
| *elb* | `SetLoadBalancerListenerSSLCertificate` | 0 |
| *elb* | `DetachLoadBalancerFromSubnets` | 0 |
| *elb* | `DeregisterInstancesFromLoadBalancer` | 0 |
| *elb* | `RegisterInstancesWithLoadBalancer` | 0 |
| *elb* | `AddTags` | 0 |
| *elb* | `ModifyLoadBalancerAttributes` | 1 |
| *elb* | `CreateLoadBalancerListeners` | 0 |
| *elb* | `CreateLoadBalancer` | 1 |
| *elb* | `DescribeAccountLimits` | 0 |
| *elb* | `DisableAvailabilityZonesForLoadBalancer` | 0 |
| *elbv2* | `RegisterTargets` | 1 |
| *elbv2* | `DescribeSSLPolicies` | 1 |
| *elbv2* | `SetSecurityGroups` | 0 |
| *elbv2* | `AddListenerCertificates` | 1 |
| *elbv2* | `DeleteListener` | 1 |
| *elbv2* | `ModifyTargetGroup` | 0 |
| *elbv2* | `CreateRule` | 1 |
| *elbv2* | `SetRulePriorities` | 0 |
| *elbv2* | `ModifyTargetGroupAttributes` | 0 |
| *elbv2* | `DescribeLoadBalancerAttributes` | 1 |
| *elbv2* | `SetIpAddressType` | 0 |
| *elbv2* | `RemoveListenerCertificates` | 0 |
| *elbv2* | `ModifyListener` | 0 |
| *elbv2* | `DeleteLoadBalancer` | 1 |
| *elbv2* | `DeleteRule` | 1 |
| *elbv2* | `DeleteTargetGroup` | 1 |
| *elbv2* | `DescribeTargetGroupAttributes` | 0 |
| *elbv2* | `DescribeLoadBalancers` | 4 |
| *elbv2* | `DescribeListeners` | 1 |
| *elbv2* | `RemoveTags` | 0 |
| *elbv2* | `CreateListener` | 1 |
| *elbv2* | `SetSubnets` | 0 |
| *elbv2* | `DescribeTags` | 0 |
| *elbv2* | `ModifyRule` | 0 |
| *elbv2* | `CreateTargetGroup` | 1 |
| *elbv2* | `AddTags` | 0 |
| *elbv2* | `DescribeTargetGroups` | 1 |
| *elbv2* | `DeregisterTargets` | 0 |
| *elbv2* | `ModifyLoadBalancerAttributes` | 0 |
| *elbv2* | `DescribeRules` | 1 |
| *elbv2* | `DescribeListenerCertificates` | 0 |
| *elbv2* | `DescribeTargetHealth` | 1 |
| *elbv2* | `CreateLoadBalancer` | 1 |
| *elbv2* | `DescribeAccountLimits` | 1 |
| *emr* | `CancelSteps` | 0 |
| *emr* | `RemoveAutoScalingPolicy` | 0 |
| *emr* | `CreateSecurityConfiguration` | 1 |
| *emr* | `RunJobFlow` | 1 |
| *emr* | `PutBlockPublicAccessConfiguration` | 0 |
| *emr* | `DescribeSecurityConfiguration` | 0 |
| *emr* | `ListSecurityConfigurations` | 0 |
| *emr* | `SetTerminationProtection` | 0 |
| *emr* | `DeleteSecurityConfiguration` | 0 |
| *emr* | `AddInstanceGroups` | 1 |
| *emr* | `ListBootstrapActions` | 1 |
| *emr* | `ListClusters` | 0 |
| *emr* | `ListInstances` | 0 |
| *emr* | `DescribeCluster` | 1 |
| *emr* | `ListSteps` | 1 |
| *emr* | `RemoveTags` | 0 |
| *emr* | `ModifyInstanceGroups` | 0 |
| *emr* | `PutAutoScalingPolicy` | 0 |
| *emr* | `AddInstanceFleet` | 0 |
| *emr* | `SetVisibleToAllUsers` | 0 |
| *emr* | `ListInstanceFleets` | 0 |
| *emr* | `TerminateJobFlows` | 1 |
| *emr* | `AddTags` | 0 |
| *emr* | `DescribeJobFlows` | 0 |
| *emr* | `GetBlockPublicAccessConfiguration` | 0 |
| *emr* | `ModifyInstanceFleet` | 0 |
| *emr* | `AddJobFlowSteps` | 1 |
| *emr* | `ListInstanceGroups` | 1 |
| *emr* | `DescribeStep` | 0 |
| *es* | `DescribeElasticsearchInstanceTypeLimits` | 0 |
| *es* | `PurchaseReservedElasticsearchInstanceOffering` | 0 |
| *es* | `DescribeElasticsearchDomainConfig` | 0 |
| *es* | `CreateElasticsearchDomain` | 1 |
| *es* | `ListElasticsearchInstanceTypes` | 1 |
| *es* | `GetUpgradeStatus` | 1 |
| *es* | `ListDomainNames` | 1 |
| *es* | `StartElasticsearchServiceSoftwareUpdate` | 0 |
| *es* | `DeleteElasticsearchServiceRole` | 0 |
| *es* | `DescribeElasticsearchDomains` | 1 |
| *es* | `UpdateElasticsearchDomainConfig` | 0 |
| *es* | `RemoveTags` | 0 |
| *es* | `ListTags` | 0 |
| *es* | `CancelElasticsearchServiceSoftwareUpdate` | 0 |
| *es* | `ListElasticsearchVersions` | 1 |
| *es* | `UpgradeElasticsearchDomain` | 0 |
| *es* | `DeleteElasticsearchDomain` | 1 |
| *es* | `AddTags` | 0 |
| *es* | `GetCompatibleElasticsearchVersions` | 1 |
| *es* | `DescribeElasticsearchDomain` | 1 |
| *es* | `DescribeReservedElasticsearchInstanceOfferings` | 0 |
| *es* | `GetUpgradeHistory` | 1 |
| *es* | `DescribeReservedElasticsearchInstances` | 1 |
| *events* | `DeleteEventBus` | 0 |
| *events* | `ListTargetsByRule` | 1 |
| *events* | `CreateEventBus` | 0 |
| *events* | `EnableRule` | 1 |
| *events* | `PutPartnerEvents` | 0 |
| *events* | `RemoveTargets` | 1 |
| *events* | `PutEvents` | 0 |
| *events* | `ListPartnerEventSources` | 0 |
| *events* | `ListPartnerEventSourceAccounts` | 0 |
| *events* | `DeleteRule` | 2 |
| *events* | `RemovePermission` | 0 |
| *events* | `TagResource` | 0 |
| *events* | `PutRule` | 3 |
| *events* | `DescribeRule` | 1 |
| *events* | `DescribePartnerEventSource` | 0 |
| *events* | `ListRuleNamesByTarget` | 1 |
| *events* | `DescribeEventSource` | 0 |
| *events* | `CreatePartnerEventSource` | 0 |
| *events* | `ListRules` | 2 |
| *events* | `PutTargets` | 2 |
| *events* | `UntagResource` | 0 |
| *events* | `ListEventSources` | 0 |
| *events* | `DescribeEventBus` | 0 |
| *events* | `DeletePartnerEventSource` | 0 |
| *events* | `ActivateEventSource` | 0 |
| *events* | `TestEventPattern` | 0 |
| *events* | `PutPermission` | 1 |
| *events* | `ListEventBuses` | 0 |
| *events* | `DeactivateEventSource` | 0 |
| *events* | `ListTagsForResource` | 0 |
| *events* | `DisableRule` | 1 |
| *firehose* | `CreateDeliveryStream` | 1 |
| *firehose* | `DescribeDeliveryStream` | 0 |
| *firehose* | `UpdateDestination` | 0 |
| *firehose* | `TagDeliveryStream` | 0 |
| *firehose* | `DeleteDeliveryStream` | 1 |
| *firehose* | `StopDeliveryStreamEncryption` | 0 |
| *firehose* | `ListDeliveryStreams` | 3 |
| *firehose* | `PutRecordBatch` | 0 |
| *firehose* | `UntagDeliveryStream` | 0 |
| *firehose* | `PutRecord` | 0 |
| *firehose* | `StartDeliveryStreamEncryption` | 0 |
| *firehose* | `ListTagsForDeliveryStream` | 0 |
| *fms* | `PutPolicy` | 0 |
| *fms* | `GetComplianceDetail` | 0 |
| *fms* | `ListComplianceStatus` | 0 |
| *fms* | `DeleteNotificationChannel` | 0 |
| *fms* | `GetNotificationChannel` | 0 |
| *fms* | `PutNotificationChannel` | 0 |
| *fms* | `ListPolicies` | 0 |
| *fms* | `GetAdminAccount` | 0 |
| *fms* | `GetProtectionStatus` | 0 |
| *fms* | `DeletePolicy` | 0 |
| *fms* | `DisassociateAdminAccount` | 0 |
| *fms* | `ListMemberAccounts` | 0 |
| *fms* | `GetPolicy` | 0 |
| *fms* | `AssociateAdminAccount` | 0 |
| *forecast* | `ListDatasetGroups` | 0 |
| *forecast* | `CreateForecastExportJob` | 0 |
| *forecast* | `DescribeForecast` | 0 |
| *forecast* | `DescribeForecastExportJob` | 0 |
| *forecast* | `DeleteDatasetImportJob` | 0 |
| *forecast* | `DeleteDataset` | 0 |
| *forecast* | `CreatePredictor` | 0 |
| *forecast* | `ListDatasets` | 0 |
| *forecast* | `DescribeDatasetImportJob` | 0 |
| *forecast* | `ListForecastExportJobs` | 0 |
| *forecast* | `DescribeDataset` | 0 |
| *forecast* | `CreateDataset` | 0 |
| *forecast* | `DescribeDatasetGroup` | 0 |
| *forecast* | `CreateDatasetGroup` | 0 |
| *forecast* | `GetAccuracyMetrics` | 0 |
| *forecast* | `DescribePredictor` | 0 |
| *forecast* | `ListDatasetImportJobs` | 0 |
| *forecast* | `ListForecasts` | 0 |
| *forecast* | `CreateDatasetImportJob` | 0 |
| *forecast* | `UpdateDatasetGroup` | 0 |
| *forecast* | `DeletePredictor` | 0 |
| *forecast* | `DeleteForecastExportJob` | 0 |
| *forecast* | `DeleteForecast` | 0 |
| *forecast* | `ListPredictors` | 0 |
| *forecast* | `DeleteDatasetGroup` | 0 |
| *forecast* | `CreateForecast` | 0 |
| *forecastquery* | `QueryForecast` | 0 |
| *fsx* | `DeleteFileSystem` | 1 |
| *fsx* | `CreateFileSystemFromBackup` | 0 |
| *fsx* | `UntagResource` | 0 |
| *fsx* | `DescribeBackups` | 0 |
| *fsx* | `UpdateFileSystem` | 0 |
| *fsx* | `DescribeFileSystems` | 1 |
| *fsx* | `DeleteBackup` | 0 |
| *fsx* | `CreateBackup` | 0 |
| *fsx* | `TagResource` | 0 |
| *fsx* | `CreateFileSystem` | 1 |
| *fsx* | `ListTagsForResource` | 0 |
| *gamelift* | `RequestUploadCredentials` | 0 |
| *gamelift* | `UpdateRuntimeConfiguration` | 0 |
| *gamelift* | `DeleteFleet` | 0 |
| *gamelift* | `UpdateFleetCapacity` | 0 |
| *gamelift* | `StartFleetActions` | 0 |
| *gamelift* | `DescribeRuntimeConfiguration` | 0 |
| *gamelift* | `DescribeGameSessions` | 0 |
| *gamelift* | `DeleteMatchmakingRuleSet` | 0 |
| *gamelift* | `CreatePlayerSessions` | 0 |
| *gamelift* | `CreateMatchmakingConfiguration` | 0 |
| *gamelift* | `DescribeMatchmakingRuleSets` | 0 |
| *gamelift* | `SearchGameSessions` | 0 |
| *gamelift* | `ListFleets` | 1 |
| *gamelift* | `UpdateGameSession` | 0 |
| *gamelift* | `CreateGameSession` | 0 |
| *gamelift* | `DescribeFleetCapacity` | 0 |
| *gamelift* | `DescribeGameSessionPlacement` | 0 |
| *gamelift* | `ListAliases` | 0 |
| *gamelift* | `UpdateBuild` | 0 |
| *gamelift* | `UpdateMatchmakingConfiguration` | 0 |
| *gamelift* | `DeleteBuild` | 1 |
| *gamelift* | `PutScalingPolicy` | 1 |
| *gamelift* | `DescribeVpcPeeringAuthorizations` | 0 |
| *gamelift* | `DescribeScalingPolicies` | 0 |
| *gamelift* | `DescribeInstances` | 0 |
| *gamelift* | `CreateBuild` | 1 |
| *gamelift* | `DeleteGameSessionQueue` | 0 |
| *gamelift* | `CreatePlayerSession` | 0 |
| *gamelift* | `StartMatchBackfill` | 0 |
| *gamelift* | `DeleteAlias` | 1 |
| *gamelift* | `UpdateGameSessionQueue` | 0 |
| *gamelift* | `StartMatchmaking` | 0 |
| *gamelift* | `DescribeFleetPortSettings` | 0 |
| *gamelift* | `ListBuilds` | 0 |
| *gamelift* | `DescribePlayerSessions` | 0 |
| *gamelift* | `UpdateScript` | 0 |
| *gamelift* | `DeleteScalingPolicy` | 0 |
| *gamelift* | `DescribeGameSessionQueues` | 0 |
| *gamelift* | `CreateAlias` | 1 |
| *gamelift* | `UpdateAlias` | 0 |
| *gamelift* | `DeleteMatchmakingConfiguration` | 0 |
| *gamelift* | `CreateFleet` | 1 |
| *gamelift* | `DeleteVpcPeeringConnection` | 0 |
| *gamelift* | `GetInstanceAccess` | 0 |
| *gamelift* | `CreateVpcPeeringAuthorization` | 0 |
| *gamelift* | `DeleteVpcPeeringAuthorization` | 0 |
| *gamelift* | `DescribeMatchmakingConfigurations` | 0 |
| *gamelift* | `UpdateFleetAttributes` | 0 |
| *gamelift* | `DescribeFleetAttributes` | 1 |
| *gamelift* | `StopMatchmaking` | 0 |
| *gamelift* | `ResolveAlias` | 0 |
| *gamelift* | `DescribeGameSessionDetails` | 0 |
| *gamelift* | `CreateScript` | 0 |
| *gamelift* | `DeleteScript` | 0 |
| *gamelift* | `ValidateMatchmakingRuleSet` | 0 |
| *gamelift* | `StopFleetActions` | 0 |
| *gamelift* | `DescribeScript` | 0 |
| *gamelift* | `StopGameSessionPlacement` | 0 |
| *gamelift* | `CreateGameSessionQueue` | 0 |
| *gamelift* | `DescribeMatchmaking` | 0 |
| *gamelift* | `DescribeAlias` | 0 |
| *gamelift* | `StartGameSessionPlacement` | 0 |
| *gamelift* | `CreateVpcPeeringConnection` | 0 |
| *gamelift* | `DescribeEC2InstanceLimits` | 0 |
| *gamelift* | `DescribeVpcPeeringConnections` | 0 |
| *gamelift* | `GetGameSessionLogUrl` | 0 |
| *gamelift* | `DescribeBuild` | 0 |
| *gamelift* | `DescribeFleetEvents` | 0 |
| *gamelift* | `ListScripts` | 0 |
| *gamelift* | `DescribeFleetUtilization` | 0 |
| *gamelift* | `AcceptMatch` | 0 |
| *gamelift* | `UpdateFleetPortSettings` | 0 |
| *gamelift* | `CreateMatchmakingRuleSet` | 0 |
| *glacier* | `ListTagsForVault` | 0 |
| *glacier* | `ListJobs` | 0 |
| *glacier* | `DeleteVault` | 0 |
| *glacier* | `DeleteVaultNotifications` | 0 |
| *glacier* | `ListParts` | 0 |
| *glacier* | `ListProvisionedCapacity` | 0 |
| *glacier* | `DeleteArchive` | 0 |
| *glacier* | `PurchaseProvisionedCapacity` | 0 |
| *glacier* | `GetJobOutput` | 0 |
| *glacier* | `AddTagsToVault` | 0 |
| *glacier* | `InitiateVaultLock` | 1 |
| *glacier* | `ListVaults` | 1 |
| *glacier* | `GetVaultLock` | 0 |
| *glacier* | `DescribeJob` | 0 |
| *glacier* | `GetVaultAccessPolicy` | 0 |
| *glacier* | `CompleteMultipartUpload` | 0 |
| *glacier* | `SetVaultAccessPolicy` | 0 |
| *glacier* | `UploadMultipartPart` | 0 |
| *glacier* | `GetVaultNotifications` | 0 |
| *glacier* | `DeleteVaultAccessPolicy` | 0 |
| *glacier* | `DescribeVault` | 0 |
| *glacier* | `ListMultipartUploads` | 0 |
| *glacier* | `CompleteVaultLock` | 0 |
| *glacier* | `SetDataRetrievalPolicy` | 0 |
| *glacier* | `AbortVaultLock` | 0 |
| *glacier* | `AbortMultipartUpload` | 0 |
| *glacier* | `RemoveTagsFromVault` | 0 |
| *glacier* | `CreateVault` | 1 |
| *glacier* | `InitiateJob` | 0 |
| *glacier* | `InitiateMultipartUpload` | 0 |
| *glacier* | `GetDataRetrievalPolicy` | 0 |
| *glacier* | `SetVaultNotifications` | 0 |
| *glacier* | `UploadArchive` | 0 |
| *globalaccelerator* | `ListEndpointGroups` | 0 |
| *globalaccelerator* | `CreateListener` | 0 |
| *globalaccelerator* | `DescribeAccelerator` | 0 |
| *globalaccelerator* | `DescribeEndpointGroup` | 0 |
| *globalaccelerator* | `DescribeAcceleratorAttributes` | 0 |
| *globalaccelerator* | `DeleteAccelerator` | 1 |
| *globalaccelerator* | `CreateAccelerator` | 1 |
| *globalaccelerator* | `UpdateEndpointGroup` | 0 |
| *globalaccelerator* | `CreateEndpointGroup` | 0 |
| *globalaccelerator* | `ListAccelerators` | 0 |
| *globalaccelerator* | `ListListeners` | 0 |
| *globalaccelerator* | `DeleteListener` | 0 |
| *globalaccelerator* | `DeleteEndpointGroup` | 0 |
| *globalaccelerator* | `UpdateAccelerator` | 0 |
| *globalaccelerator* | `UpdateAcceleratorAttributes` | 0 |
| *globalaccelerator* | `UpdateListener` | 0 |
| *globalaccelerator* | `DescribeListener` | 0 |
| *glue* | `GetUserDefinedFunctions` | 0 |
| *glue* | `BatchGetJobs` | 0 |
| *glue* | `BatchDeleteTable` | 1 |
| *glue* | `GetCrawlers` | 0 |
| *glue* | `StartWorkflowRun` | 0 |
| *glue* | `UpdateCrawlerSchedule` | 0 |
| *glue* | `StartCrawler` | 0 |
| *glue* | `BatchGetWorkflows` | 0 |
| *glue* | `GetPartitions` | 0 |
| *glue* | `GetUserDefinedFunction` | 0 |
| *glue* | `UpdateJob` | 0 |
| *glue* | `GetCrawlerMetrics` | 0 |
| *glue* | `StopCrawler` | 0 |
| *glue* | `DeleteDevEndpoint` | 0 |
| *glue* | `StartMLEvaluationTaskRun` | 0 |
| *glue* | `CreatePartition` | 0 |
| *glue* | `GetWorkflowRun` | 0 |
| *glue* | `DeleteDatabase` | 1 |
| *glue* | `BatchDeleteTableVersion` | 0 |
| *glue* | `ResetJobBookmark` | 0 |
| *glue* | `GetDatabases` | 1 |
| *glue* | `ListCrawlers` | 0 |
| *glue* | `BatchDeleteConnection` | 1 |
| *glue* | `BatchGetPartition` | 0 |
| *glue* | `DeleteConnection` | 0 |
| *glue* | `DeletePartition` | 0 |
| *glue* | `CreateClassifier` | 1 |
| *glue* | `GetTableVersions` | 0 |
| *glue* | `GetJob` | 1 |
| *glue* | `GetDatabase` | 0 |
| *glue* | `BatchStopJobRun` | 0 |
| *glue* | `DeleteTableVersion` | 0 |
| *glue* | `SearchTables` | 0 |
| *glue* | `PutWorkflowRunProperties` | 0 |
| *glue* | `DeleteClassifier` | 1 |
| *glue* | `DeleteJob` | 1 |
| *glue* | `UpdateDevEndpoint` | 0 |
| *glue* | `GetTriggers` | 1 |
| *glue* | `GetDevEndpoint` | 0 |
| *glue* | `GetJobRuns` | 1 |
| *glue* | `StartMLLabelingSetGenerationTaskRun` | 0 |
| *glue* | `TagResource` | 0 |
| *glue* | `DeleteWorkflow` | 0 |
| *glue* | `CreateDatabase` | 1 |
| *glue* | `StartJobRun` | 0 |
| *glue* | `GetJobs` | 1 |
| *glue* | `GetDevEndpoints` | 1 |
| *glue* | `GetClassifier` | 0 |
| *glue* | `UpdateTrigger` | 0 |
| *glue* | `GetPartition` | 0 |
| *glue* | `UntagResource` | 0 |
| *glue* | `CreateMLTransform` | 0 |
| *glue* | `BatchGetDevEndpoints` | 0 |
| *glue* | `DeleteResourcePolicy` | 0 |
| *glue* | `GetResourcePolicy` | 0 |
| *glue* | `UpdateConnection` | 0 |
| *glue* | `GetTrigger` | 1 |
| *glue* | `GetMLTransform` | 0 |
| *glue* | `UpdateMLTransform` | 0 |
| *glue* | `UpdateWorkflow` | 0 |
| *glue* | `StartCrawlerSchedule` | 0 |
| *glue* | `PutResourcePolicy` | 0 |
| *glue* | `GetTableVersion` | 0 |
| *glue* | `BatchGetCrawlers` | 0 |
| *glue* | `StartImportLabelsTaskRun` | 0 |
| *glue* | `DeleteTrigger` | 1 |
| *glue* | `DeleteSecurityConfiguration` | 1 |
| *glue* | `ListDevEndpoints` | 0 |
| *glue* | `DeleteMLTransform` | 0 |
| *glue* | `StartExportLabelsTaskRun` | 0 |
| *glue* | `GetWorkflowRuns` | 0 |
| *glue* | `ListJobs` | 0 |
| *glue* | `GetDataCatalogEncryptionSettings` | 1 |
| *glue* | `GetSecurityConfigurations` | 1 |
| *glue* | `DeleteCrawler` | 0 |
| *glue* | `UpdateClassifier` | 0 |
| *glue* | `CreateCrawler` | 1 |
| *glue* | `CreateDevEndpoint` | 1 |
| *glue* | `CreateConnection` | 1 |
| *glue* | `DeleteTable` | 0 |
| *glue* | `CreateWorkflow` | 0 |
| *glue* | `GetJobRun` | 0 |
| *glue* | `GetWorkflowRunProperties` | 0 |
| *glue* | `BatchCreatePartition` | 0 |
| *glue* | `StartTrigger` | 0 |
| *glue* | `GetMLTransforms` | 0 |
| *glue* | `PutDataCatalogEncryptionSettings` | 2 |
| *glue* | `GetClassifiers` | 1 |
| *glue* | `CancelMLTaskRun` | 0 |
| *glue* | `GetTags` | 0 |
| *glue* | `CreateSecurityConfiguration` | 1 |
| *glue* | `GetWorkflow` | 0 |
| *glue* | `ListTriggers` | 0 |
| *glue* | `UpdateUserDefinedFunction` | 0 |
| *glue* | `GetConnections` | 1 |
| *glue* | `GetPlan` | 1 |
| *glue* | `CreateTable` | 1 |
| *glue* | `CreateTrigger` | 1 |
| *glue* | `StopTrigger` | 0 |
| *glue* | `DeleteUserDefinedFunction` | 0 |
| *glue* | `GetSecurityConfiguration` | 0 |
| *glue* | `ListWorkflows` | 0 |
| *glue* | `GetConnection` | 2 |
| *glue* | `GetTables` | 1 |
| *glue* | `CreateScript` | 0 |
| *glue* | `GetCatalogImportStatus` | 1 |
| *glue* | `StopCrawlerSchedule` | 0 |
| *glue* | `GetDataflowGraph` | 1 |
| *glue* | `UpdatePartition` | 0 |
| *glue* | `CreateJob` | 1 |
| *glue* | `GetCrawler` | 0 |
| *glue* | `UpdateTable` | 0 |
| *glue* | `BatchDeletePartition` | 0 |
| *glue* | `BatchGetTriggers` | 0 |
| *glue* | `UpdateCrawler` | 0 |
| *glue* | `CreateUserDefinedFunction` | 0 |
| *glue* | `ImportCatalogToGlue` | 0 |
| *glue* | `GetMLTaskRun` | 0 |
| *glue* | `GetMapping` | 1 |
| *glue* | `GetTable` | 1 |
| *glue* | `UpdateDatabase` | 0 |
| *glue* | `GetMLTaskRuns` | 0 |
| *glue* | `GetJobBookmark` | 0 |
| *greengrass* | `ListDeviceDefinitionVersions` | 0 |
| *greengrass* | `StartBulkDeployment` | 0 |
| *greengrass* | `ListBulkDeploymentDetailedReports` | 0 |
| *greengrass* | `UpdateCoreDefinition` | 0 |
| *greengrass* | `CreateDeployment` | 0 |
| *greengrass* | `ListBulkDeployments` | 0 |
| *greengrass* | `ListResourceDefinitions` | 0 |
| *greengrass* | `GetSubscriptionDefinitionVersion` | 0 |
| *greengrass* | `DeleteResourceDefinition` | 0 |
| *greengrass* | `CreateLoggerDefinitionVersion` | 0 |
| *greengrass* | `ListGroups` | 0 |
| *greengrass* | `ListCoreDefinitionVersions` | 0 |
| *greengrass* | `GetConnectorDefinition` | 0 |
| *greengrass* | `UpdateConnectivityInfo` | 0 |
| *greengrass* | `ListConnectorDefinitionVersions` | 0 |
| *greengrass* | `DeleteSubscriptionDefinition` | 0 |
| *greengrass* | `DisassociateServiceRoleFromAccount` | 0 |
| *greengrass* | `DeleteFunctionDefinition` | 0 |
| *greengrass* | `ListTagsForResource` | 0 |
| *greengrass* | `CreateSubscriptionDefinitionVersion` | 0 |
| *greengrass* | `GetDeviceDefinitionVersion` | 0 |
| *greengrass* | `AssociateRoleToGroup` | 1 |
| *greengrass* | `ListFunctionDefinitions` | 0 |
| *greengrass* | `AssociateServiceRoleToAccount` | 0 |
| *greengrass* | `ListFunctionDefinitionVersions` | 0 |
| *greengrass* | `TagResource` | 0 |
| *greengrass* | `GetFunctionDefinition` | 0 |
| *greengrass* | `GetGroup` | 0 |
| *greengrass* | `GetResourceDefinition` | 0 |
| *greengrass* | `DeleteCoreDefinition` | 0 |
| *greengrass* | `UpdateGroupCertificateConfiguration` | 0 |
| *greengrass* | `CreateGroupCertificateAuthority` | 0 |
| *greengrass* | `UpdateLoggerDefinition` | 0 |
| *greengrass* | `ListDeployments` | 0 |
| *greengrass* | `UntagResource` | 0 |
| *greengrass* | `UpdateFunctionDefinition` | 0 |
| *greengrass* | `CreateFunctionDefinition` | 0 |
| *greengrass* | `GetConnectivityInfo` | 0 |
| *greengrass* | `UpdateSubscriptionDefinition` | 0 |
| *greengrass* | `StopBulkDeployment` | 0 |
| *greengrass* | `ListSubscriptionDefinitionVersions` | 0 |
| *greengrass* | `CreateCoreDefinition` | 1 |
| *greengrass* | `CreateGroup` | 1 |
| *greengrass* | `GetGroupCertificateAuthority` | 0 |
| *greengrass* | `CreateResourceDefinition` | 0 |
| *greengrass* | `GetConnectorDefinitionVersion` | 0 |
| *greengrass* | `ListLoggerDefinitionVersions` | 0 |
| *greengrass* | `CreateDeviceDefinition` | 0 |
| *greengrass* | `DeleteGroup` | 0 |
| *greengrass* | `CreateDeviceDefinitionVersion` | 0 |
| *greengrass* | `GetBulkDeploymentStatus` | 0 |
| *greengrass* | `UpdateConnectorDefinition` | 0 |
| *greengrass* | `ResetDeployments` | 0 |
| *greengrass* | `CreateGroupVersion` | 1 |
| *greengrass* | `CreateConnectorDefinition` | 0 |
| *greengrass* | `CreateConnectorDefinitionVersion` | 0 |
| *greengrass* | `GetServiceRoleForAccount` | 0 |
| *greengrass* | `CreateSoftwareUpdateJob` | 0 |
| *greengrass* | `CreateLoggerDefinition` | 0 |
| *greengrass* | `ListGroupCertificateAuthorities` | 0 |
| *greengrass* | `CreateFunctionDefinitionVersion` | 0 |
| *greengrass* | `DeleteDeviceDefinition` | 0 |
| *greengrass* | `GetAssociatedRole` | 0 |
| *greengrass* | `GetLoggerDefinition` | 0 |
| *greengrass* | `UpdateGroup` | 0 |
| *greengrass* | `CreateSubscriptionDefinition` | 0 |
| *greengrass* | `GetDeviceDefinition` | 0 |
| *greengrass* | `DeleteLoggerDefinition` | 0 |
| *greengrass* | `ListConnectorDefinitions` | 0 |
| *greengrass* | `ListCoreDefinitions` | 0 |
| *greengrass* | `DisassociateRoleFromGroup` | 0 |
| *greengrass* | `UpdateDeviceDefinition` | 0 |
| *greengrass* | `GetDeploymentStatus` | 0 |
| *greengrass* | `ListDeviceDefinitions` | 0 |
| *greengrass* | `ListGroupVersions` | 0 |
| *greengrass* | `GetSubscriptionDefinition` | 0 |
| *greengrass* | `ListSubscriptionDefinitions` | 0 |
| *greengrass* | `CreateResourceDefinitionVersion` | 0 |
| *greengrass* | `GetCoreDefinition` | 0 |
| *greengrass* | `DeleteConnectorDefinition` | 0 |
| *greengrass* | `UpdateResourceDefinition` | 0 |
| *greengrass* | `GetFunctionDefinitionVersion` | 0 |
| *greengrass* | `ListResourceDefinitionVersions` | 0 |
| *greengrass* | `GetLoggerDefinitionVersion` | 0 |
| *greengrass* | `CreateCoreDefinitionVersion` | 1 |
| *greengrass* | `ListLoggerDefinitions` | 0 |
| *greengrass* | `GetGroupVersion` | 0 |
| *greengrass* | `GetResourceDefinitionVersion` | 0 |
| *greengrass* | `GetCoreDefinitionVersion` | 0 |
| *greengrass* | `GetGroupCertificateConfiguration` | 0 |
| *groundstation* | `ListDataflowEndpointGroups` | 0 |
| *groundstation* | `GetConfig` | 0 |
| *groundstation* | `DeleteMissionProfile` | 0 |
| *groundstation* | `DeleteConfig` | 0 |
| *groundstation* | `ListSatellites` | 0 |
| *groundstation* | `GetMinuteUsage` | 0 |
| *groundstation* | `TagResource` | 0 |
| *groundstation* | `ListGroundStations` | 0 |
| *groundstation* | `ListContacts` | 0 |
| *groundstation* | `UpdateMissionProfile` | 0 |
| *groundstation* | `CreateMissionProfile` | 0 |
| *groundstation* | `CancelContact` | 0 |
| *groundstation* | `DescribeContact` | 0 |
| *groundstation* | `ListMissionProfiles` | 0 |
| *groundstation* | `DeleteDataflowEndpointGroup` | 0 |
| *groundstation* | `UpdateConfig` | 0 |
| *groundstation* | `ListConfigs` | 0 |
| *groundstation* | `ReserveContact` | 0 |
| *groundstation* | `CreateConfig` | 0 |
| *groundstation* | `CreateDataflowEndpointGroup` | 0 |
| *groundstation* | `GetSatellite` | 0 |
| *groundstation* | `GetDataflowEndpointGroup` | 0 |
| *groundstation* | `UntagResource` | 0 |
| *groundstation* | `ListTagsForResource` | 0 |
| *groundstation* | `GetMissionProfile` | 0 |
| *guardduty* | `GetFindings` | 1 |
| *guardduty* | `ListIPSets` | 2 |
| *guardduty* | `UpdateDetector` | 1 |
| *guardduty* | `GetThreatIntelSet` | 0 |
| *guardduty* | `ListTagsForResource` | 0 |
| *guardduty* | `ListDetectors` | 1 |
| *guardduty* | `ListInvitations` | 1 |
| *guardduty* | `GetInvitationsCount` | 1 |
| *guardduty* | `DisassociateMembers` | 0 |
| *guardduty* | `UpdateFindingsFeedback` | 0 |
| *guardduty* | `ListFilters` | 1 |
| *guardduty* | `UpdateThreatIntelSet` | 0 |
| *guardduty* | `GetMasterAccount` | 2 |
| *guardduty* | `ArchiveFindings` | 1 |
| *guardduty* | `GetIPSet` | 1 |
| *guardduty* | `DisassociateFromMasterAccount` | 0 |
| *guardduty* | `GetMembers` | 0 |
| *guardduty* | `ListFindings` | 1 |
| *guardduty* | `StartMonitoringMembers` | 0 |
| *guardduty* | `CreateFilter` | 1 |
| *guardduty* | `CreateDetector` | 1 |
| *guardduty* | `ListMembers` | 1 |
| *guardduty* | `DeleteMembers` | 1 |
| *guardduty* | `ListThreatIntelSets` | 1 |
| *guardduty* | `CreateIPSet` | 1 |
| *guardduty* | `TagResource` | 0 |
| *guardduty* | `UnarchiveFindings` | 1 |
| *guardduty* | `UpdateFilter` | 0 |
| *guardduty* | `DeclineInvitations` | 0 |
| *guardduty* | `AcceptInvitation` | 1 |
| *guardduty* | `CreateMembers` | 1 |
| *guardduty* | `GetDetector` | 1 |
| *guardduty* | `CreateSampleFindings` | 1 |
| *guardduty* | `UntagResource` | 0 |
| *guardduty* | `GetFilter` | 0 |
| *guardduty* | `DeleteDetector` | 0 |
| *guardduty* | `GetFindingsStatistics` | 1 |
| *guardduty* | `InviteMembers` | 0 |
| *guardduty* | `DeleteInvitations` | 0 |
| *guardduty* | `CreateThreatIntelSet` | 1 |
| *guardduty* | `UpdateIPSet` | 1 |
| *guardduty* | `StopMonitoringMembers` | 0 |
| *guardduty* | `DeleteFilter` | 0 |
| *guardduty* | `DeleteIPSet` | 0 |
| *guardduty* | `DeleteThreatIntelSet` | 0 |
| *health* | `DescribeAffectedEntities` | 0 |
| *health* | `DescribeEventDetails` | 0 |
| *health* | `DescribeEventTypes` | 0 |
| *health* | `DescribeEvents` | 0 |
| *health* | `DescribeEntityAggregates` | 0 |
| *health* | `DescribeEventAggregates` | 0 |
| *iam* | `GetUserPolicy` | 0 |
| *iam* | `CreateServiceLinkedRole` | 1 |
| *iam* | `DeleteInstanceProfile` | 0 |
| *iam* | `GetAccountSummary` | 0 |
| *iam* | `DeleteRolePermissionsBoundary` | 0 |
| *iam* | `ListAttachedGroupPolicies` | 0 |
| *iam* | `UpdateUser` | 0 |
| *iam* | `ListAccountAliases` | 1 |
| *iam* | `GetUser` | 1 |
| *iam* | `ListInstanceProfilesForRole` | 0 |
| *iam* | `UploadSSHPublicKey` | 0 |
| *iam* | `GetServiceLinkedRoleDeletionStatus` | 0 |
| *iam* | `GetOrganizationsAccessReport` | 0 |
| *iam* | `ListGroupPolicies` | 0 |
| *iam* | `CreateAccessKey` | 1 |
| *iam* | `UpdateLoginProfile` | 0 |
| *iam* | `ListGroups` | 3 |
| *iam* | `UpdateRoleDescription` | 0 |
| *iam* | `UpdateSAMLProvider` | 0 |
| *iam* | `ResetServiceSpecificCredential` | 0 |
| *iam* | `UntagRole` | 0 |
| *iam* | `DeleteSSHPublicKey` | 0 |
| *iam* | `UpdateRole` | 0 |
| *iam* | `RemoveUserFromGroup` | 0 |
| *iam* | `ListSAMLProviders` | 0 |
| *iam* | `DeleteRolePolicy` | 0 |
| *iam* | `UploadServerCertificate` | 0 |
| *iam* | `GetContextKeysForPrincipalPolicy` | 0 |
| *iam* | `UpdateAccessKey` | 0 |
| *iam* | `ListRoles` | 15 |
| *iam* | `UntagUser` | 0 |
| *iam* | `DeleteAccessKey` | 1 |
| *iam* | `AddRoleToInstanceProfile` | 1 |
| *iam* | `GenerateOrganizationsAccessReport` | 0 |
| *iam* | `GetContextKeysForCustomPolicy` | 0 |
| *iam* | `GetGroupPolicy` | 0 |
| *iam* | `DeleteServiceLinkedRole` | 0 |
| *iam* | `CreateServiceSpecificCredential` | 0 |
| *iam* | `CreateLoginProfile` | 1 |
| *iam* | `SimulateCustomPolicy` | 0 |
| *iam* | `DeactivateMFADevice` | 0 |
| *iam* | `DetachRolePolicy` | 0 |
| *iam* | `GetPolicy` | 0 |
| *iam* | `CreateSAMLProvider` | 0 |
| *iam* | `ListOpenIDConnectProviders` | 0 |
| *iam* | `DeleteAccountAlias` | 1 |
| *iam* | `ListSigningCertificates` | 0 |
| *iam* | `DeleteRole` | 0 |
| *iam* | `GenerateServiceLastAccessedDetails` | 0 |
| *iam* | `UpdateGroup` | 0 |
| *iam* | `ListPoliciesGrantingServiceAccess` | 0 |
| *iam* | `GetGroup` | 0 |
| *iam* | `GetRolePolicy` | 1 |
| *iam* | `CreateUser` | 1 |
| *iam* | `AddUserToGroup` | 1 |
| *iam* | `RemoveRoleFromInstanceProfile` | 0 |
| *iam* | `ListInstanceProfiles` | 4 |
| *iam* | `UpdateServiceSpecificCredential` | 0 |
| *iam* | `ListAttachedUserPolicies` | 0 |
| *iam* | `CreatePolicyVersion` | 0 |
| *iam* | `SetSecurityTokenServicePreferences` | 0 |
| *iam* | `TagRole` | 0 |
| *iam* | `ListServiceSpecificCredentials` | 0 |
| *iam* | `AttachUserPolicy` | 1 |
| *iam* | `GetServiceLastAccessedDetailsWithEntities` | 0 |
| *iam* | `ListVirtualMFADevices` | 0 |
| *iam* | `UpdateAssumeRolePolicy` | 1 |
| *iam* | `ListServerCertificates` | 1 |
| *iam* | `DeleteGroup` | 0 |
| *iam* | `UpdateOpenIDConnectProviderThumbprint` | 0 |
| *iam* | `CreateGroup` | 1 |
| *iam* | `DeletePolicy` | 1 |
| *iam* | `DeleteUserPolicy` | 0 |
| *iam* | `UpdateServerCertificate` | 0 |
| *iam* | `AddClientIDToOpenIDConnectProvider` | 0 |
| *iam* | `DeleteVirtualMFADevice` | 0 |
| *iam* | `PutRolePermissionsBoundary` | 0 |
| *iam* | `SimulatePrincipalPolicy` | 0 |
| *iam* | `ListAccessKeys` | 2 |
| *iam* | `GetSSHPublicKey` | 0 |
| *iam* | `ListSSHPublicKeys` | 0 |
| *iam* | `DeleteSigningCertificate` | 0 |
| *iam* | `GetPolicyVersion` | 0 |
| *iam* | `PutRolePolicy` | 1 |
| *iam* | `SetDefaultPolicyVersion` | 0 |
| *iam* | `GetAccessKeyLastUsed` | 0 |
| *iam* | `DeleteAccountPasswordPolicy` | 1 |
| *iam* | `ListMFADevices` | 0 |
| *iam* | `UpdateSigningCertificate` | 0 |
| *iam* | `DetachGroupPolicy` | 0 |
| *iam* | `UpdateSSHPublicKey` | 0 |
| *iam* | `GetInstanceProfile` | 0 |
| *iam* | `DeleteUser` | 1 |
| *iam* | `GetOpenIDConnectProvider` | 0 |
| *iam* | `ListUsers` | 2 |
| *iam* | `DeleteLoginProfile` | 0 |
| *iam* | `ListPolicyVersions` | 1 |
| *iam* | `AttachGroupPolicy` | 0 |
| *iam* | `DeleteServiceSpecificCredential` | 0 |
| *iam* | `CreatePolicy` | 1 |
| *iam* | `CreateOpenIDConnectProvider` | 0 |
| *iam* | `DetachUserPolicy` | 0 |
| *iam* | `ListGroupsForUser` | 1 |
| *iam* | `GetCredentialReport` | 0 |
| *iam* | `CreateAccountAlias` | 1 |
| *iam* | `GetAccountAuthorizationDetails` | 0 |
| *iam* | `GetSAMLProvider` | 0 |
| *iam* | `ListRoleTags` | 0 |
| *iam* | `CreateRole` | 1 |
| *iam* | `ListAttachedRolePolicies` | 1 |
| *iam* | `EnableMFADevice` | 0 |
| *iam* | `DeleteSAMLProvider` | 0 |
| *iam* | `ListUserPolicies` | 0 |
| *iam* | `ListRolePolicies` | 0 |
| *iam* | `PutGroupPolicy` | 1 |
| *iam* | `UploadSigningCertificate` | 0 |
| *iam* | `DeleteUserPermissionsBoundary` | 0 |
| *iam* | `GenerateCredentialReport` | 0 |
| *iam* | `ChangePassword` | 0 |
| *iam* | `ListEntitiesForPolicy` | 0 |
| *iam* | `PutUserPermissionsBoundary` | 0 |
| *iam* | `TagUser` | 0 |
| *iam* | `GetLoginProfile` | 1 |
| *iam* | `CreateVirtualMFADevice` | 0 |
| *iam* | `GetServerCertificate` | 0 |
| *iam* | `AttachRolePolicy` | 2 |
| *iam* | `RemoveClientIDFromOpenIDConnectProvider` | 0 |
| *iam* | `UpdateAccountPasswordPolicy` | 1 |
| *iam* | `GetRole` | 3 |
| *iam* | `DeletePolicyVersion` | 0 |
| *iam* | `ListUserTags` | 0 |
| *iam* | `ListPolicies` | 1 |
| *iam* | `PutUserPolicy` | 1 |
| *iam* | `ResyncMFADevice` | 0 |
| *iam* | `DeleteServerCertificate` | 0 |
| *iam* | `GetAccountPasswordPolicy` | 0 |
| *iam* | `CreateInstanceProfile` | 2 |
| *iam* | `GetServiceLastAccessedDetails` | 0 |
| *iam* | `DeleteOpenIDConnectProvider` | 0 |
| *iam* | `DeleteGroupPolicy` | 0 |
| *importexport* | `ListJobs` | 0 |
| *importexport* | `UpdateJob` | 0 |
| *importexport* | `CreateJob` | 0 |
| *importexport* | `GetStatus` | 0 |
| *importexport* | `CancelJob` | 0 |
| *importexport* | `GetShippingLabel` | 0 |
| *inspector* | `DeleteAssessmentTarget` | 0 |
| *inspector* | `UpdateAssessmentTarget` | 0 |
| *inspector* | `RemoveAttributesFromFindings` | 0 |
| *inspector* | `GetTelemetryMetadata` | 0 |
| *inspector* | `DescribeAssessmentRuns` | 0 |
| *inspector* | `StartAssessmentRun` | 0 |
| *inspector* | `DescribeCrossAccountAccessRole` | 1 |
| *inspector* | `DescribeAssessmentTargets` | 1 |
| *inspector* | `ListRulesPackages` | 1 |
| *inspector* | `StopAssessmentRun` | 0 |
| *inspector* | `ListAssessmentTemplates` | 1 |
| *inspector* | `DescribeFindings` | 0 |
| *inspector* | `DeleteAssessmentTemplate` | 0 |
| *inspector* | `DescribeResourceGroups` | 0 |
| *inspector* | `ListFindings` | 0 |
| *inspector* | `ListEventSubscriptions` | 1 |
| *inspector* | `ListAssessmentTargets` | 2 |
| *inspector* | `DescribeExclusions` | 0 |
| *inspector* | `ListExclusions` | 0 |
| *inspector* | `GetExclusionsPreview` | 0 |
| *inspector* | `SetTagsForResource` | 0 |
| *inspector* | `DeleteAssessmentRun` | 0 |
| *inspector* | `CreateAssessmentTarget` | 1 |
| *inspector* | `UnsubscribeFromEvent` | 0 |
| *inspector* | `ListAssessmentRuns` | 0 |
| *inspector* | `CreateExclusionsPreview` | 0 |
| *inspector* | `RegisterCrossAccountAccessRole` | 1 |
| *inspector* | `AddAttributesToFindings` | 0 |
| *inspector* | `CreateResourceGroup` | 1 |
| *inspector* | `DescribeAssessmentTemplates` | 0 |
| *inspector* | `PreviewAgents` | 0 |
| *inspector* | `CreateAssessmentTemplate` | 1 |
| *inspector* | `SubscribeToEvent` | 0 |
| *inspector* | `DescribeRulesPackages` | 1 |
| *inspector* | `ListTagsForResource` | 0 |
| *inspector* | `ListAssessmentRunAgents` | 0 |
| *inspector* | `GetAssessmentReport` | 0 |
| *iot* | `ListThingsInBillingGroup` | 0 |
| *iot* | `ReplaceTopicRule` | 0 |
| *iot* | `ListAuditMitigationActionsTasks` | 0 |
| *iot* | `ListViolationEvents` | 0 |
| *iot* | `UpdateThingGroupsForThing` | 0 |
| *iot* | `DeprecateThingType` | 0 |
| *iot* | `StartOnDemandAuditTask` | 0 |
| *iot* | `ListAuthorizers` | 0 |
| *iot* | `SetV2LoggingOptions` | 0 |
| *iot* | `DeleteAuthorizer` | 0 |
| *iot* | `SetV2LoggingLevel` | 0 |
| *iot* | `DeleteDynamicThingGroup` | 0 |
| *iot* | `RejectCertificateTransfer` | 0 |
| *iot* | `DeleteRoleAlias` | 0 |
| *iot* | `DescribeSecurityProfile` | 0 |
| *iot* | `ListAuditTasks` | 0 |
| *iot* | `SetDefaultAuthorizer` | 0 |
| *iot* | `ListCACertificates` | 0 |
| *iot* | `UpdateJob` | 0 |
| *iot* | `GetTopicRule` | 0 |
| *iot* | `CreateDynamicThingGroup` | 0 |
| *iot* | `DescribeAuthorizer` | 0 |
| *iot* | `DescribeDefaultAuthorizer` | 0 |
| *iot* | `GetJobDocument` | 0 |
| *iot* | `DescribeAuditTask` | 0 |
| *iot* | `DescribeStream` | 0 |
| *iot* | `CancelJob` | 0 |
| *iot* | `ListTargetsForSecurityProfile` | 0 |
| *iot* | `RegisterCACertificate` | 0 |
| *iot* | `DescribeEndpoint` | 1 |
| *iot* | `DescribeThingType` | 0 |
| *iot* | `DeleteJob` | 0 |
| *iot* | `ListThingRegistrationTaskReports` | 0 |
| *iot* | `DetachSecurityProfile` | 0 |
| *iot* | `DescribeJobExecution` | 0 |
| *iot* | `DescribeScheduledAudit` | 0 |
| *iot* | `ListTagsForResource` | 0 |
| *iot* | `DeleteTopicRule` | 1 |
| *iot* | `UntagResource` | 0 |
| *iot* | `UpdateBillingGroup` | 0 |
| *iot* | `DeleteOTAUpdate` | 0 |
| *iot* | `UpdateThing` | 0 |
| *iot* | `EnableTopicRule` | 1 |
| *iot* | `UpdateAccountAuditConfiguration` | 0 |
| *iot* | `GetPolicy` | 0 |
| *iot* | `StartAuditMitigationActionsTask` | 0 |
| *iot* | `TransferCertificate` | 0 |
| *iot* | `DeleteThingGroup` | 0 |
| *iot* | `StartThingRegistrationTask` | 0 |
| *iot* | `SearchIndex` | 0 |
| *iot* | `RemoveThingFromBillingGroup` | 0 |
| *iot* | `DescribeMitigationAction` | 0 |
| *iot* | `DeleteCertificate` | 1 |
| *iot* | `ListStreams` | 0 |
| *iot* | `ListPolicyPrincipals` | 0 |
| *iot* | `GetV2LoggingOptions` | 0 |
| *iot* | `DeleteV2LoggingLevel` | 0 |
| *iot* | `TagResource` | 0 |
| *iot* | `ListOutgoingCertificates` | 0 |
| *iot* | `UpdateDynamicThingGroup` | 0 |
| *iot* | `AttachThingPrincipal` | 2 |
| *iot* | `DeleteMitigationAction` | 0 |
| *iot* | `AcceptCertificateTransfer` | 0 |
| *iot* | `ListAttachedPolicies` | 0 |
| *iot* | `ClearDefaultAuthorizer` | 0 |
| *iot* | `DescribeAuditMitigationActionsTask` | 0 |
| *iot* | `CreateAuthorizer` | 0 |
| *iot* | `TestAuthorization` | 0 |
| *iot* | `AddThingToBillingGroup` | 0 |
| *iot* | `CancelAuditTask` | 0 |
| *iot* | `GetIndexingConfiguration` | 0 |
| *iot* | `DeleteRegistrationCode` | 0 |
| *iot* | `ListTopicRules` | 0 |
| *iot* | `CreatePolicyVersion` | 0 |
| *iot* | `ListActiveViolations` | 0 |
| *iot* | `GetStatistics` | 0 |
| *iot* | `ListScheduledAudits` | 0 |
| *iot* | `DeleteScheduledAudit` | 0 |
| *iot* | `DescribeThingRegistrationTask` | 0 |
| *iot* | `DescribeAuditFinding` | 0 |
| *iot* | `ListV2LoggingLevels` | 0 |
| *iot* | `DescribeIndex` | 0 |
| *iot* | `RemoveThingFromThingGroup` | 0 |
| *iot* | `GetOTAUpdate` | 0 |
| *iot* | `ListCertificates` | 0 |
| *iot* | `ListAuditMitigationActionsExecutions` | 0 |
| *iot* | `DeletePolicy` | 0 |
| *iot* | `DeletePolicyVersion` | 0 |
| *iot* | `CreateRoleAlias` | 0 |
| *iot* | `CreateTopicRule` | 2 |
| *iot* | `DescribeRoleAlias` | 0 |
| *iot* | `ListCertificatesByCA` | 0 |
| *iot* | `CreateThingGroup` | 0 |
| *iot* | `CreateOTAUpdate` | 0 |
| *iot* | `AssociateTargetsWithJob` | 0 |
| *iot* | `DescribeJob` | 0 |
| *iot* | `DescribeCACertificate` | 0 |
| *iot* | `GetPolicyVersion` | 0 |
| *iot* | `ListThingRegistrationTasks` | 0 |
| *iot* | `ListJobs` | 0 |
| *iot* | `ListThingTypes` | 0 |
| *iot* | `CreateKeysAndCertificate` | 1 |
| *iot* | `DeleteSecurityProfile` | 0 |
| *iot* | `ValidateSecurityProfileBehaviors` | 0 |
| *iot* | `UpdateMitigationAction` | 0 |
| *iot* | `DeleteBillingGroup` | 0 |
| *iot* | `CreateScheduledAudit` | 0 |
| *iot* | `ListRoleAliases` | 0 |
| *iot* | `CreateSecurityProfile` | 0 |
| *iot* | `UpdateCertificate` | 0 |
| *iot* | `GetRegistrationCode` | 0 |
| *iot* | `CancelAuditMitigationActionsTask` | 0 |
| *iot* | `ListPrincipalThings` | 0 |
| *iot* | `CancelJobExecution` | 0 |
| *iot* | `ListAuditFindings` | 0 |
| *iot* | `ListBillingGroups` | 0 |
| *iot* | `DeleteJobExecution` | 0 |
| *iot* | `GetEffectivePolicies` | 0 |
| *iot* | `CreateCertificateFromCsr` | 1 |
| *iot* | `SetLoggingOptions` | 0 |
| *iot* | `DeleteStream` | 0 |
| *iot* | `DisableTopicRule` | 1 |
| *iot* | `TestInvokeAuthorizer` | 0 |
| *iot* | `UpdateIndexingConfiguration` | 0 |
| *iot* | `CreatePolicy` | 2 |
| *iot* | `AttachSecurityProfile` | 0 |
| *iot* | `ListJobExecutionsForThing` | 0 |
| *iot* | `RegisterCertificate` | 0 |
| *iot* | `ListOTAUpdates` | 0 |
| *iot* | `DeleteThingType` | 0 |
| *iot* | `DescribeThingGroup` | 0 |
| *iot* | `StopThingRegistrationTask` | 0 |
| *iot* | `AddThingToThingGroup` | 0 |
| *iot* | `ListMitigationActions` | 0 |
| *iot* | `SetDefaultPolicyVersion` | 0 |
| *iot* | `ListPolicyVersions` | 0 |
| *iot* | `DescribeAccountAuditConfiguration` | 0 |
| *iot* | `DescribeBillingGroup` | 0 |
| *iot* | `GetLoggingOptions` | 0 |
| *iot* | `DetachPolicy` | 0 |
| *iot* | `ListThingGroups` | 0 |
| *iot* | `RegisterThing` | 0 |
| *iot* | `UpdateRoleAlias` | 0 |
| *iot* | `DeleteCACertificate` | 0 |
| *iot* | `ListPrincipalPolicies` | 0 |
| *iot* | `AttachPrincipalPolicy` | 1 |
| *iot* | `UpdateSecurityProfile` | 0 |
| *iot* | `CreateThingType` | 1 |
| *iot* | `DeleteAccountAuditConfiguration` | 0 |
| *iot* | `UpdateStream` | 0 |
| *iot* | `CreateBillingGroup` | 0 |
| *iot* | `CreateJob` | 0 |
| *iot* | `DeleteThing` | 0 |
| *iot* | `AttachPolicy` | 1 |
| *iot* | `ListSecurityProfilesForTarget` | 0 |
| *iot* | `ListJobExecutionsForJob` | 0 |
| *iot* | `ListThings` | 1 |
| *iot* | `ListIndices` | 0 |
| *iot* | `CreateMitigationAction` | 0 |
| *iot* | `UpdateCACertificate` | 0 |
| *iot* | `UpdateAuthorizer` | 0 |
| *iot* | `CreateStream` | 0 |
| *iot* | `UpdateEventConfigurations` | 0 |
| *iot* | `CancelCertificateTransfer` | 0 |
| *iot* | `ListThingsInThingGroup` | 0 |
| *iot* | `DescribeCertificate` | 0 |
| *iot* | `DescribeEventConfigurations` | 0 |
| *iot* | `ListSecurityProfiles` | 0 |
| *iot* | `DetachThingPrincipal` | 0 |
| *iot* | `ListTargetsForPolicy` | 0 |
| *iot* | `ListPolicies` | 0 |
| *iot* | `CreateThing` | 2 |
| *iot* | `ListThingGroupsForThing` | 0 |
| *iot* | `UpdateScheduledAudit` | 0 |
| *iot* | `DetachPrincipalPolicy` | 0 |
| *iot* | `ListThingPrincipals` | 0 |
| *iot* | `UpdateThingGroup` | 0 |
| *iot* | `DescribeThing` | 0 |
| *iot-data* | `UpdateThingShadow` | 0 |
| *iot-data* | `GetThingShadow` | 0 |
| *iot-data* | `Publish` | 0 |
| *iot-data* | `DeleteThingShadow` | 0 |
| *iot-jobs-data* | `GetPendingJobExecutions` | 0 |
| *iot-jobs-data* | `StartNextPendingJobExecution` | 0 |
| *iot-jobs-data* | `UpdateJobExecution` | 0 |
| *iot-jobs-data* | `DescribeJobExecution` | 0 |
| *iot1click-devices* | `DescribeDevice` | 0 |
| *iot1click-devices* | `UpdateDeviceState` | 0 |
| *iot1click-devices* | `UnclaimDevice` | 1 |
| *iot1click-devices* | `UntagResource` | 0 |
| *iot1click-devices* | `GetDeviceMethods` | 0 |
| *iot1click-devices* | `ClaimDevicesByClaimCode` | 0 |
| *iot1click-devices* | `InvokeDeviceMethod` | 0 |
| *iot1click-devices* | `InitiateDeviceClaim` | 1 |
| *iot1click-devices* | `FinalizeDeviceClaim` | 1 |
| *iot1click-devices* | `TagResource` | 0 |
| *iot1click-devices* | `ListDevices` | 0 |
| *iot1click-devices* | `ListTagsForResource` | 0 |
| *iot1click-devices* | `ListDeviceEvents` | 0 |
| *iot1click-projects* | `DisassociateDeviceFromPlacement` | 0 |
| *iot1click-projects* | `UpdatePlacement` | 0 |
| *iot1click-projects* | `UntagResource` | 0 |
| *iot1click-projects* | `DeletePlacement` | 0 |
| *iot1click-projects* | `TagResource` | 0 |
| *iot1click-projects* | `ListPlacements` | 0 |
| *iot1click-projects* | `CreateProject` | 1 |
| *iot1click-projects* | `ListProjects` | 1 |
| *iot1click-projects* | `UpdateProject` | 0 |
| *iot1click-projects* | `GetDevicesInPlacement` | 0 |
| *iot1click-projects* | `DescribePlacement` | 0 |
| *iot1click-projects* | `DescribeProject` | 0 |
| *iot1click-projects* | `ListTagsForResource` | 0 |
| *iot1click-projects* | `AssociateDeviceWithPlacement` | 0 |
| *iot1click-projects* | `CreatePlacement` | 1 |
| *iot1click-projects* | `DeleteProject` | 0 |
| *iotanalytics* | `DeleteChannel` | 1 |
| *iotanalytics* | `UpdateChannel` | 0 |
| *iotanalytics* | `CancelPipelineReprocessing` | 0 |
| *iotanalytics* | `BatchPutMessage` | 0 |
| *iotanalytics* | `CreateChannel` | 1 |
| *iotanalytics* | `DeleteDataset` | 0 |
| *iotanalytics* | `DescribeLoggingOptions` | 0 |
| *iotanalytics* | `RunPipelineActivity` | 0 |
| *iotanalytics* | `ListDatasets` | 1 |
| *iotanalytics* | `CreateDatastore` | 1 |
| *iotanalytics* | `GetDatasetContent` | 0 |
| *iotanalytics* | `StartPipelineReprocessing` | 0 |
| *iotanalytics* | `SampleChannelData` | 0 |
| *iotanalytics* | `DeletePipeline` | 1 |
| *iotanalytics* | `DescribeDataset` | 0 |
| *iotanalytics* | `DeleteDatastore` | 1 |
| *iotanalytics* | `ListDatastores` | 1 |
| *iotanalytics* | `CreateDataset` | 1 |
| *iotanalytics* | `TagResource` | 0 |
| *iotanalytics* | `DescribeChannel` | 0 |
| *iotanalytics* | `UpdateDatastore` | 0 |
| *iotanalytics* | `ListDatasetContents` | 0 |
| *iotanalytics* | `ListPipelines` | 1 |
| *iotanalytics* | `DescribePipeline` | 0 |
| *iotanalytics* | `PutLoggingOptions` | 0 |
| *iotanalytics* | `DescribeDatastore` | 0 |
| *iotanalytics* | `CreatePipeline` | 1 |
| *iotanalytics* | `UntagResource` | 0 |
| *iotanalytics* | `ListChannels` | 1 |
| *iotanalytics* | `UpdateDataset` | 0 |
| *iotanalytics* | `UpdatePipeline` | 0 |
| *iotanalytics* | `DeleteDatasetContent` | 0 |
| *iotanalytics* | `ListTagsForResource` | 0 |
| *iotanalytics* | `CreateDatasetContent` | 0 |
| *iotevents* | `DeleteDetectorModel` | 0 |
| *iotevents* | `CreateInput` | 0 |
| *iotevents* | `DescribeDetectorModel` | 0 |
| *iotevents* | `UntagResource` | 0 |
| *iotevents* | `DescribeInput` | 0 |
| *iotevents* | `UpdateDetectorModel` | 0 |
| *iotevents* | `ListInputs` | 0 |
| *iotevents* | `CreateDetectorModel` | 0 |
| *iotevents* | `PutLoggingOptions` | 0 |
| *iotevents* | `UpdateInput` | 0 |
| *iotevents* | `TagResource` | 0 |
| *iotevents* | `DeleteInput` | 0 |
| *iotevents* | `DescribeLoggingOptions` | 0 |
| *iotevents* | `ListTagsForResource` | 0 |
| *iotevents* | `ListDetectorModels` | 0 |
| *iotevents* | `ListDetectorModelVersions` | 0 |
| *iotevents-data* | `ListDetectors` | 0 |
| *iotevents-data* | `DescribeDetector` | 0 |
| *iotevents-data* | `BatchUpdateDetector` | 0 |
| *iotevents-data* | `BatchPutMessage` | 0 |
| *iotthingsgraph* | `DescribeNamespace` | 0 |
| *iotthingsgraph* | `SearchFlowExecutions` | 0 |
| *iotthingsgraph* | `CreateSystemInstance` | 0 |
| *iotthingsgraph* | `UploadEntityDefinitions` | 0 |
| *iotthingsgraph* | `DeleteNamespace` | 0 |
| *iotthingsgraph* | `DeploySystemInstance` | 0 |
| *iotthingsgraph* | `SearchSystemInstances` | 0 |
| *iotthingsgraph* | `CreateSystemTemplate` | 0 |
| *iotthingsgraph* | `DeleteSystemInstance` | 0 |
| *iotthingsgraph* | `SearchSystemTemplates` | 0 |
| *iotthingsgraph* | `DissociateEntityFromThing` | 0 |
| *iotthingsgraph* | `GetFlowTemplate` | 0 |
| *iotthingsgraph* | `TagResource` | 0 |
| *iotthingsgraph* | `SearchEntities` | 0 |
| *iotthingsgraph* | `DeleteFlowTemplate` | 0 |
| *iotthingsgraph* | `GetNamespaceDeletionStatus` | 0 |
| *iotthingsgraph* | `DeprecateFlowTemplate` | 0 |
| *iotthingsgraph* | `ListFlowExecutionMessages` | 0 |
| *iotthingsgraph* | `CreateFlowTemplate` | 0 |
| *iotthingsgraph* | `GetUploadStatus` | 0 |
| *iotthingsgraph* | `SearchThings` | 0 |
| *iotthingsgraph* | `GetSystemTemplate` | 0 |
| *iotthingsgraph* | `UndeploySystemInstance` | 0 |
| *iotthingsgraph* | `DeprecateSystemTemplate` | 0 |
| *iotthingsgraph* | `GetEntities` | 0 |
| *iotthingsgraph* | `UntagResource` | 0 |
| *iotthingsgraph* | `GetSystemInstance` | 0 |
| *iotthingsgraph* | `AssociateEntityToThing` | 0 |
| *iotthingsgraph* | `SearchFlowTemplates` | 0 |
| *iotthingsgraph* | `GetSystemTemplateRevisions` | 0 |
| *iotthingsgraph* | `DeleteSystemTemplate` | 0 |
| *iotthingsgraph* | `ListTagsForResource` | 0 |
| *iotthingsgraph* | `GetFlowTemplateRevisions` | 0 |
| *iotthingsgraph* | `UpdateSystemTemplate` | 0 |
| *iotthingsgraph* | `UpdateFlowTemplate` | 0 |
| *kafka* | `UntagResource` | 0 |
| *kafka* | `UpdateBrokerCount` | 0 |
| *kafka* | `ListClusters` | 0 |
| *kafka* | `DescribeClusterOperation` | 0 |
| *kafka* | `CreateCluster` | 0 |
| *kafka* | `UpdateBrokerStorage` | 0 |
| *kafka* | `DeleteCluster` | 0 |
| *kafka* | `ListConfigurations` | 0 |
| *kafka* | `UpdateClusterConfiguration` | 0 |
| *kafka* | `DescribeConfiguration` | 0 |
| *kafka* | `DescribeConfigurationRevision` | 0 |
| *kafka* | `ListClusterOperations` | 0 |
| *kafka* | `CreateConfiguration` | 0 |
| *kafka* | `ListConfigurationRevisions` | 0 |
| *kafka* | `ListTagsForResource` | 0 |
| *kafka* | `DescribeCluster` | 0 |
| *kafka* | `TagResource` | 0 |
| *kafka* | `ListNodes` | 0 |
| *kafka* | `GetBootstrapBrokers` | 0 |
| *kinesis* | `IncreaseStreamRetentionPeriod` | 0 |
| *kinesis* | `GetShardIterator` | 0 |
| *kinesis* | `AddTagsToStream` | 0 |
| *kinesis* | `DisableEnhancedMonitoring` | 0 |
| *kinesis* | `DecreaseStreamRetentionPeriod` | 0 |
| *kinesis* | `DescribeLimits` | 1 |
| *kinesis* | `ListShards` | 1 |
| *kinesis* | `SubscribeToShard` | 0 |
| *kinesis* | `ListStreams` | 3 |
| *kinesis* | `DeregisterStreamConsumer` | 0 |
| *kinesis* | `ListStreamConsumers` | 1 |
| *kinesis* | `UpdateShardCount` | 0 |
| *kinesis* | `DescribeStreamSummary` | 1 |
| *kinesis* | `StartStreamEncryption` | 0 |
| *kinesis* | `DescribeStreamConsumer` | 0 |
| *kinesis* | `SplitShard` | 0 |
| *kinesis* | `DescribeStream` | 1 |
| *kinesis* | `CreateStream` | 2 |
| *kinesis* | `RegisterStreamConsumer` | 0 |
| *kinesis* | `EnableEnhancedMonitoring` | 0 |
| *kinesis* | `DeleteStream` | 1 |
| *kinesis* | `RemoveTagsFromStream` | 0 |
| *kinesis* | `GetRecords` | 0 |
| *kinesis* | `ListTagsForStream` | 0 |
| *kinesis* | `PutRecords` | 1 |
| *kinesis* | `PutRecord` | 0 |
| *kinesis* | `MergeShards` | 0 |
| *kinesis* | `StopStreamEncryption` | 0 |
| *kinesis-video-archived-media* | `GetMediaForFragmentList` | 0 |
| *kinesis-video-archived-media* | `GetDASHStreamingSessionURL` | 0 |
| *kinesis-video-archived-media* | `ListFragments` | 0 |
| *kinesis-video-archived-media* | `GetHLSStreamingSessionURL` | 0 |
| *kinesis-video-media* | `GetMedia` | 0 |
| *kinesisanalytics* | `UntagResource` | 0 |
| *kinesisanalytics* | `DiscoverInputSchema` | 1 |
| *kinesisanalytics* | `ListTagsForResource` | 0 |
| *kinesisanalytics* | `DeleteApplicationInputProcessingConfiguration` | 0 |
| *kinesisanalytics* | `AddApplicationReferenceDataSource` | 0 |
| *kinesisanalytics* | `ListApplications` | 0 |
| *kinesisanalytics* | `UpdateApplication` | 1 |
| *kinesisanalytics* | `TagResource` | 0 |
| *kinesisanalytics* | `StopApplication` | 1 |
| *kinesisanalytics* | `AddApplicationOutput` | 1 |
| *kinesisanalytics* | `DeleteApplication` | 1 |
| *kinesisanalytics* | `AddApplicationCloudWatchLoggingOption` | 0 |
| *kinesisanalytics* | `DeleteApplicationOutput` | 0 |
| *kinesisanalytics* | `DeleteApplicationCloudWatchLoggingOption` | 0 |
| *kinesisanalytics* | `AddApplicationInput` | 1 |
| *kinesisanalytics* | `DescribeApplication` | 1 |
| *kinesisanalytics* | `AddApplicationInputProcessingConfiguration` | 0 |
| *kinesisanalytics* | `DeleteApplicationReferenceDataSource` | 0 |
| *kinesisanalytics* | `StartApplication` | 1 |
| *kinesisanalytics* | `CreateApplication` | 1 |
| *kinesisanalyticsv2* | `DeleteApplicationSnapshot` | 0 |
| *kinesisanalyticsv2* | `DeleteApplicationInputProcessingConfiguration` | 0 |
| *kinesisanalyticsv2* | `AddApplicationOutput` | 0 |
| *kinesisanalyticsv2* | `DeleteApplication` | 0 |
| *kinesisanalyticsv2* | `CreateApplicationSnapshot` | 0 |
| *kinesisanalyticsv2* | `DeleteApplicationOutput` | 0 |
| *kinesisanalyticsv2* | `StartApplication` | 0 |
| *kinesisanalyticsv2* | `StopApplication` | 0 |
| *kinesisanalyticsv2* | `DescribeApplicationSnapshot` | 0 |
| *kinesisanalyticsv2* | `AddApplicationCloudWatchLoggingOption` | 0 |
| *kinesisanalyticsv2* | `TagResource` | 0 |
| *kinesisanalyticsv2* | `UpdateApplication` | 0 |
| *kinesisanalyticsv2* | `AddApplicationReferenceDataSource` | 1 |
| *kinesisanalyticsv2* | `DescribeApplication` | 0 |
| *kinesisanalyticsv2* | `DiscoverInputSchema` | 0 |
| *kinesisanalyticsv2* | `AddApplicationInputProcessingConfiguration` | 0 |
| *kinesisanalyticsv2* | `CreateApplication` | 0 |
| *kinesisanalyticsv2* | `ListTagsForResource` | 0 |
| *kinesisanalyticsv2* | `ListApplications` | 0 |
| *kinesisanalyticsv2* | `UntagResource` | 0 |
| *kinesisanalyticsv2* | `ListApplicationSnapshots` | 0 |
| *kinesisanalyticsv2* | `DeleteApplicationCloudWatchLoggingOption` | 0 |
| *kinesisanalyticsv2* | `AddApplicationInput` | 0 |
| *kinesisanalyticsv2* | `DeleteApplicationReferenceDataSource` | 0 |
| *kinesisvideo* | `GetDataEndpoint` | 0 |
| *kinesisvideo* | `UpdateDataRetention` | 0 |
| *kinesisvideo* | `DeleteStream` | 0 |
| *kinesisvideo* | `ListStreams` | 0 |
| *kinesisvideo* | `UntagStream` | 0 |
| *kinesisvideo* | `ListTagsForStream` | 0 |
| *kinesisvideo* | `CreateStream` | 0 |
| *kinesisvideo* | `TagStream` | 0 |
| *kinesisvideo* | `UpdateStream` | 0 |
| *kinesisvideo* | `DescribeStream` | 0 |
| *kms* | `Encrypt` | 0 |
| *kms* | `DescribeCustomKeyStores` | 0 |
| *kms* | `Decrypt` | 0 |
| *kms* | `UpdateAlias` | 0 |
| *kms* | `DisconnectCustomKeyStore` | 0 |
| *kms* | `ImportKeyMaterial` | 0 |
| *kms* | `DisableKey` | 0 |
| *kms* | `GenerateDataKeyWithoutPlaintext` | 0 |
| *kms* | `GenerateDataKey` | 0 |
| *kms* | `ListResourceTags` | 1 |
| *kms* | `ListAliases` | 0 |
| *kms* | `CancelKeyDeletion` | 0 |
| *kms* | `GetKeyRotationStatus` | 0 |
| *kms* | `ListKeyPolicies` | 0 |
| *kms* | `DeleteAlias` | 0 |
| *kms* | `RetireGrant` | 0 |
| *kms* | `TagResource` | 0 |
| *kms* | `UpdateCustomKeyStore` | 0 |
| *kms* | `DescribeKey` | 10 |
| *kms* | `ListKeys` | 5 |
| *kms* | `GenerateRandom` | 0 |
| *kms* | `GetKeyPolicy` | 1 |
| *kms* | `CreateGrant` | 0 |
| *kms* | `CreateKey` | 2 |
| *kms* | `ReEncrypt` | 0 |
| *kms* | `GetParametersForImport` | 0 |
| *kms* | `ConnectCustomKeyStore` | 0 |
| *kms* | `DisableKeyRotation` | 0 |
| *kms* | `ListRetirableGrants` | 0 |
| *kms* | `ScheduleKeyDeletion` | 1 |
| *kms* | `CreateAlias` | 1 |
| *kms* | `EnableKeyRotation` | 0 |
| *kms* | `UntagResource` | 0 |
| *kms* | `ListGrants` | 0 |
| *kms* | `CreateCustomKeyStore` | 0 |
| *kms* | `EnableKey` | 0 |
| *kms* | `DeleteImportedKeyMaterial` | 0 |
| *kms* | `UpdateKeyDescription` | 0 |
| *kms* | `RevokeGrant` | 0 |
| *kms* | `DeleteCustomKeyStore` | 0 |
| *kms* | `PutKeyPolicy` | 0 |
| *lakeformation* | `DeregisterResource` | 0 |
| *lakeformation* | `GrantPermissions` | 0 |
| *lakeformation* | `RevokePermissions` | 0 |
| *lakeformation* | `ListResources` | 0 |
| *lakeformation* | `DescribeResource` | 0 |
| *lakeformation* | `BatchRevokePermissions` | 0 |
| *lakeformation* | `GetDataLakeSettings` | 0 |
| *lakeformation* | `PutDataLakeSettings` | 0 |
| *lakeformation* | `BatchGrantPermissions` | 0 |
| *lakeformation* | `RegisterResource` | 0 |
| *lakeformation* | `ListPermissions` | 0 |
| *lakeformation* | `GetEffectivePermissionsForPath` | 0 |
| *lakeformation* | `UpdateResource` | 0 |
| *lambda* | `ListEventSourceMappings` | 0 |
| *lambda* | `DeleteFunction` | 1 |
| *lambda* | `UpdateEventSourceMapping` | 0 |
| *lambda* | `RemoveLayerVersionPermission` | 0 |
| *lambda* | `UpdateAlias` | 0 |
| *lambda* | `GetLayerVersion` | 0 |
| *lambda* | `GetFunctionConfiguration` | 1 |
| *lambda* | `AddLayerVersionPermission` | 0 |
| *lambda* | `DeleteFunctionConcurrency` | 0 |
| *lambda* | `ListAliases` | 1 |
| *lambda* | `PutFunctionConcurrency` | 0 |
| *lambda* | `CreateEventSourceMapping` | 1 |
| *lambda* | `Invoke` | 0 |
| *lambda* | `PublishVersion` | 1 |
| *lambda* | `DeleteLayerVersion` | 1 |
| *lambda* | `GetFunction` | 1 |
| *lambda* | `DeleteAlias` | 0 |
| *lambda* | `AddPermission` | 0 |
| *lambda* | `ListVersionsByFunction` | 1 |
| *lambda* | `RemovePermission` | 0 |
| *lambda* | `InvokeAsync` | 0 |
| *lambda* | `CreateAlias` | 1 |
| *lambda* | `ListTags` | 1 |
| *lambda* | `ListLayers` | 1 |
| *lambda* | `GetLayerVersionByArn` | 0 |
| *lambda* | `TagResource` | 0 |
| *lambda* | `ListLayerVersions` | 0 |
| *lambda* | `DeleteEventSourceMapping` | 0 |
| *lambda* | `GetPolicy` | 0 |
| *lambda* | `ListFunctions` | 6 |
| *lambda* | `UpdateFunctionConfiguration` | 0 |
| *lambda* | `PublishLayerVersion` | 1 |
| *lambda* | `GetEventSourceMapping` | 0 |
| *lambda* | `UntagResource` | 0 |
| *lambda* | `GetAlias` | 1 |
| *lambda* | `GetLayerVersionPolicy` | 0 |
| *lambda* | `GetAccountSettings` | 0 |
| *lambda* | `CreateFunction` | 1 |
| *lambda* | `UpdateFunctionCode` | 0 |
| *lex-models* | `CreateBotVersion` | 0 |
| *lex-models* | `DeleteBotAlias` | 0 |
| *lex-models* | `GetIntents` | 0 |
| *lex-models* | `GetSlotTypes` | 0 |
| *lex-models* | `DeleteSlotTypeVersion` | 0 |
| *lex-models* | `StartImport` | 0 |
| *lex-models* | `CreateSlotTypeVersion` | 0 |
| *lex-models* | `GetIntentVersions` | 0 |
| *lex-models* | `GetBuiltinSlotTypes` | 0 |
| *lex-models* | `CreateIntentVersion` | 0 |
| *lex-models* | `GetUtterancesView` | 0 |
| *lex-models* | `GetBot` | 0 |
| *lex-models* | `GetSlotTypeVersions` | 0 |
| *lex-models* | `DeleteIntentVersion` | 0 |
| *lex-models* | `DeleteUtterances` | 0 |
| *lex-models* | `GetBotAlias` | 0 |
| *lex-models* | `DeleteIntent` | 0 |
| *lex-models* | `GetImport` | 0 |
| *lex-models* | `GetBotVersions` | 0 |
| *lex-models* | `GetExport` | 0 |
| *lex-models* | `DeleteBotChannelAssociation` | 0 |
| *lex-models* | `DeleteBotVersion` | 0 |
| *lex-models* | `GetBotChannelAssociations` | 0 |
| *lex-models* | `PutBot` | 0 |
| *lex-models* | `GetBotChannelAssociation` | 0 |
| *lex-models* | `GetBotAliases` | 0 |
| *lex-models* | `DeleteBot` | 0 |
| *lex-models* | `GetBuiltinIntents` | 0 |
| *lex-models* | `GetBots` | 0 |
| *lex-models* | `DeleteSlotType` | 0 |
| *lex-models* | `GetSlotType` | 0 |
| *lex-models* | `GetIntent` | 0 |
| *lex-models* | `PutSlotType` | 0 |
| *lex-models* | `GetBuiltinIntent` | 0 |
| *lex-models* | `PutIntent` | 0 |
| *lex-models* | `PutBotAlias` | 0 |
| *lex-runtime* | `PostText` | 0 |
| *lex-runtime* | `DeleteSession` | 0 |
| *lex-runtime* | `PostContent` | 0 |
| *lex-runtime* | `GetSession` | 0 |
| *lex-runtime* | `PutSession` | 0 |
| *license-manager* | `DeleteLicenseConfiguration` | 0 |
| *license-manager* | `GetServiceSettings` | 0 |
| *license-manager* | `UpdateLicenseSpecificationsForResource` | 1 |
| *license-manager* | `UntagResource` | 0 |
| *license-manager* | `ListLicenseSpecificationsForResource` | 0 |
| *license-manager* | `ListLicenseConfigurations` | 1 |
| *license-manager* | `UpdateServiceSettings` | 0 |
| *license-manager* | `TagResource` | 0 |
| *license-manager* | `ListResourceInventory` | 0 |
| *license-manager* | `ListUsageForLicenseConfiguration` | 1 |
| *license-manager* | `UpdateLicenseConfiguration` | 0 |
| *license-manager* | `ListAssociationsForLicenseConfiguration` | 1 |
| *license-manager* | `GetLicenseConfiguration` | 1 |
| *license-manager* | `ListTagsForResource` | 0 |
| *license-manager* | `CreateLicenseConfiguration` | 1 |
| *lightsail* | `GetKeyPair` | 0 |
| *lightsail* | `DetachDisk` | 0 |
| *lightsail* | `GetCloudFormationStackRecords` | 1 |
| *lightsail* | `CreateDiskSnapshot` | 0 |
| *lightsail* | `AttachStaticIp` | 1 |
| *lightsail* | `GetDiskSnapshots` | 0 |
| *lightsail* | `GetKeyPairs` | 0 |
| *lightsail* | `ImportKeyPair` | 0 |
| *lightsail* | `CreateRelationalDatabase` | 0 |
| *lightsail* | `GetRelationalDatabaseParameters` | 0 |
| *lightsail* | `AttachInstancesToLoadBalancer` | 0 |
| *lightsail* | `DetachInstancesFromLoadBalancer` | 0 |
| *lightsail* | `GetInstance` | 0 |
| *lightsail* | `UpdateDomainEntry` | 0 |
| *lightsail* | `DisableAddOn` | 0 |
| *lightsail* | `AllocateStaticIp` | 1 |
| *lightsail* | `GetRelationalDatabase` | 0 |
| *lightsail* | `GetInstanceSnapshots` | 0 |
| *lightsail* | `StopRelationalDatabase` | 0 |
| *lightsail* | `GetDisk` | 0 |
| *lightsail* | `GetRelationalDatabaseBlueprints` | 0 |
| *lightsail* | `GetDisks` | 1 |
| *lightsail* | `CreateDiskFromSnapshot` | 0 |
| *lightsail* | `GetDomain` | 0 |
| *lightsail* | `CreateLoadBalancerTlsCertificate` | 0 |
| *lightsail* | `GetLoadBalancers` | 0 |
| *lightsail* | `PutInstancePublicPorts` | 0 |
| *lightsail* | `DeleteDomain` | 1 |
| *lightsail* | `CreateDomainEntry` | 0 |
| *lightsail* | `StartRelationalDatabase` | 0 |
| *lightsail* | `CreateInstancesFromSnapshot` | 0 |
| *lightsail* | `GetExportSnapshotRecords` | 1 |
| *lightsail* | `CopySnapshot` | 0 |
| *lightsail* | `UpdateLoadBalancerAttribute` | 0 |
| *lightsail* | `TagResource` | 0 |
| *lightsail* | `DeleteRelationalDatabase` | 0 |
| *lightsail* | `GetInstanceState` | 0 |
| *lightsail* | `CreateInstanceSnapshot` | 0 |
| *lightsail* | `RebootInstance` | 1 |
| *lightsail* | `CreateDisk` | 0 |
| *lightsail* | `StopInstance` | 1 |
| *lightsail* | `DeleteRelationalDatabaseSnapshot` | 0 |
| *lightsail* | `PeerVpc` | 0 |
| *lightsail* | `CreateRelationalDatabaseSnapshot` | 0 |
| *lightsail* | `DeleteInstanceSnapshot` | 0 |
| *lightsail* | `GetLoadBalancer` | 0 |
| *lightsail* | `GetRegions` | 1 |
| *lightsail* | `GetInstancePortStates` | 0 |
| *lightsail* | `UpdateRelationalDatabase` | 0 |
| *lightsail* | `GetLoadBalancerMetricData` | 0 |
| *lightsail* | `GetStaticIps` | 1 |
| *lightsail* | `CreateCloudFormationStack` | 0 |
| *lightsail* | `CloseInstancePublicPorts` | 0 |
| *lightsail* | `GetActiveNames` | 0 |
| *lightsail* | `DeleteInstance` | 1 |
| *lightsail* | `GetStaticIp` | 0 |
| *lightsail* | `CreateKeyPair` | 1 |
| *lightsail* | `OpenInstancePublicPorts` | 0 |
| *lightsail* | `GetDiskSnapshot` | 0 |
| *lightsail* | `GetAutoSnapshots` | 0 |
| *lightsail* | `DeleteLoadBalancer` | 0 |
| *lightsail* | `EnableAddOn` | 0 |
| *lightsail* | `GetRelationalDatabaseLogEvents` | 0 |
| *lightsail* | `GetRelationalDatabases` | 0 |
| *lightsail* | `StartInstance` | 1 |
| *lightsail* | `GetRelationalDatabaseEvents` | 0 |
| *lightsail* | `CreateDomain` | 1 |
| *lightsail* | `ExportSnapshot` | 0 |
| *lightsail* | `GetRelationalDatabaseMetricData` | 0 |
| *lightsail* | `AttachDisk` | 0 |
| *lightsail* | `GetLoadBalancerTlsCertificates` | 0 |
| *lightsail* | `AttachLoadBalancerTlsCertificate` | 0 |
| *lightsail* | `DeleteDisk` | 0 |
| *lightsail* | `GetRelationalDatabaseLogStreams` | 0 |
| *lightsail* | `GetInstanceAccessDetails` | 0 |
| *lightsail* | `UntagResource` | 0 |
| *lightsail* | `DeleteDiskSnapshot` | 0 |
| *lightsail* | `IsVpcPeered` | 0 |
| *lightsail* | `CreateLoadBalancer` | 0 |
| *lightsail* | `GetInstanceMetricData` | 0 |
| *lightsail* | `ReleaseStaticIp` | 1 |
| *lightsail* | `UpdateRelationalDatabaseParameters` | 0 |
| *lightsail* | `DeleteAutoSnapshot` | 0 |
| *lightsail* | `GetOperationsForResource` | 0 |
| *lightsail* | `GetInstanceSnapshot` | 0 |
| *lightsail* | `DeleteLoadBalancerTlsCertificate` | 0 |
| *lightsail* | `GetOperation` | 0 |
| *lightsail* | `GetBlueprints` | 1 |
| *lightsail* | `DownloadDefaultKeyPair` | 0 |
| *lightsail* | `GetRelationalDatabaseSnapshots` | 0 |
| *lightsail* | `DeleteDomainEntry` | 0 |
| *lightsail* | `GetBundles` | 1 |
| *lightsail* | `CreateRelationalDatabaseFromSnapshot` | 0 |
| *lightsail* | `GetOperations` | 1 |
| *lightsail* | `GetInstances` | 1 |
| *lightsail* | `GetRelationalDatabaseBundles` | 0 |
| *lightsail* | `GetRelationalDatabaseSnapshot` | 0 |
| *lightsail* | `GetDomains` | 0 |
| *lightsail* | `DeleteKeyPair` | 0 |
| *lightsail* | `RebootRelationalDatabase` | 0 |
| *lightsail* | `DetachStaticIp` | 1 |
| *lightsail* | `GetRelationalDatabaseMasterUserPassword` | 0 |
| *lightsail* | `UnpeerVpc` | 0 |
| *lightsail* | `DeleteKnownHostKeys` | 0 |
| *lightsail* | `CreateInstances` | 1 |
| *logs* | `PutDestination` | 0 |
| *logs* | `PutResourcePolicy` | 0 |
| *logs* | `DeleteDestination` | 0 |
| *logs* | `ListTagsLogGroup` | 0 |
| *logs* | `DescribeSubscriptionFilters` | 0 |
| *logs* | `DescribeLogStreams` | 0 |
| *logs* | `CreateExportTask` | 0 |
| *logs* | `TestMetricFilter` | 0 |
| *logs* | `DeleteLogGroup` | 0 |
| *logs* | `DescribeMetricFilters` | 0 |
| *logs* | `FilterLogEvents` | 0 |
| *logs* | `CreateLogGroup` | 2 |
| *logs* | `DeleteSubscriptionFilter` | 0 |
| *logs* | `PutMetricFilter` | 1 |
| *logs* | `DeleteResourcePolicy` | 0 |
| *logs* | `PutLogEvents` | 0 |
| *logs* | `DescribeLogGroups` | 3 |
| *logs* | `GetQueryResults` | 0 |
| *logs* | `StartQuery` | 0 |
| *logs* | `DescribeDestinations` | 0 |
| *logs* | `AssociateKmsKey` | 0 |
| *logs* | `GetLogRecord` | 0 |
| *logs* | `DescribeQueries` | 0 |
| *logs* | `DescribeExportTasks` | 0 |
| *logs* | `TagLogGroup` | 0 |
| *logs* | `PutSubscriptionFilter` | 1 |
| *logs* | `GetLogGroupFields` | 0 |
| *logs* | `DeleteMetricFilter` | 0 |
| *logs* | `PutDestinationPolicy` | 0 |
| *logs* | `CreateLogStream` | 2 |
| *logs* | `CancelExportTask` | 0 |
| *logs* | `UntagLogGroup` | 0 |
| *logs* | `StopQuery` | 0 |
| *logs* | `DisassociateKmsKey` | 0 |
| *logs* | `DeleteRetentionPolicy` | 0 |
| *logs* | `GetLogEvents` | 0 |
| *logs* | `DescribeResourcePolicies` | 0 |
| *logs* | `PutRetentionPolicy` | 0 |
| *logs* | `DeleteLogStream` | 0 |
| *machinelearning* | `DeleteTags` | 0 |
| *machinelearning* | `UpdateBatchPrediction` | 0 |
| *machinelearning* | `GetEvaluation` | 0 |
| *machinelearning* | `DescribeBatchPredictions` | 0 |
| *machinelearning* | `CreateEvaluation` | 0 |
| *machinelearning* | `DescribeMLModels` | 0 |
| *machinelearning* | `DeleteMLModel` | 0 |
| *machinelearning* | `CreateDataSourceFromS3` | 0 |
| *machinelearning* | `UpdateDataSource` | 0 |
| *machinelearning* | `DescribeEvaluations` | 0 |
| *machinelearning* | `CreateDataSourceFromRedshift` | 0 |
| *machinelearning* | `DescribeDataSources` | 0 |
| *machinelearning* | `GetDataSource` | 0 |
| *machinelearning* | `CreateRealtimeEndpoint` | 0 |
| *machinelearning* | `UpdateMLModel` | 0 |
| *machinelearning* | `DeleteRealtimeEndpoint` | 0 |
| *machinelearning* | `DescribeTags` | 0 |
| *machinelearning* | `DeleteDataSource` | 0 |
| *machinelearning* | `CreateDataSourceFromRDS` | 0 |
| *machinelearning* | `UpdateEvaluation` | 0 |
| *machinelearning* | `AddTags` | 0 |
| *machinelearning* | `Predict` | 0 |
| *machinelearning* | `GetBatchPrediction` | 0 |
| *machinelearning* | `DeleteEvaluation` | 0 |
| *machinelearning* | `DeleteBatchPrediction` | 0 |
| *machinelearning* | `CreateMLModel` | 0 |
| *machinelearning* | `CreateBatchPrediction` | 0 |
| *machinelearning* | `GetMLModel` | 0 |
| *macie* | `ListS3Resources` | 0 |
| *macie* | `DisassociateMemberAccount` | 0 |
| *macie* | `AssociateMemberAccount` | 0 |
| *macie* | `DisassociateS3Resources` | 0 |
| *macie* | `UpdateS3Resources` | 0 |
| *macie* | `ListMemberAccounts` | 0 |
| *macie* | `AssociateS3Resources` | 0 |
| *managedblockchain* | `ListProposalVotes` | 0 |
| *managedblockchain* | `ListNetworks` | 0 |
| *managedblockchain* | `ListMembers` | 0 |
| *managedblockchain* | `DeleteNode` | 0 |
| *managedblockchain* | `GetProposal` | 0 |
| *managedblockchain* | `CreateProposal` | 0 |
| *managedblockchain* | `GetNetwork` | 0 |
| *managedblockchain* | `CreateNode` | 0 |
| *managedblockchain* | `RejectInvitation` | 0 |
| *managedblockchain* | `ListProposals` | 0 |
| *managedblockchain* | `CreateMember` | 0 |
| *managedblockchain* | `CreateNetwork` | 0 |
| *managedblockchain* | `VoteOnProposal` | 0 |
| *managedblockchain* | `ListInvitations` | 0 |
| *managedblockchain* | `GetNode` | 0 |
| *managedblockchain* | `GetMember` | 0 |
| *managedblockchain* | `ListNodes` | 0 |
| *managedblockchain* | `DeleteMember` | 0 |
| *marketplace-entitlement* | `GetEntitlements` | 0 |
| *marketplacecommerceanalytics* | `GenerateDataSet` | 0 |
| *marketplacecommerceanalytics* | `StartSupportDataExport` | 0 |
| *mediaconnect* | `ListTagsForResource` | 0 |
| *mediaconnect* | `StartFlow` | 0 |
| *mediaconnect* | `UntagResource` | 0 |
| *mediaconnect* | `UpdateFlowOutput` | 0 |
| *mediaconnect* | `UpdateFlowEntitlement` | 0 |
| *mediaconnect* | `ListFlows` | 0 |
| *mediaconnect* | `TagResource` | 0 |
| *mediaconnect* | `CreateFlow` | 0 |
| *mediaconnect* | `RevokeFlowEntitlement` | 0 |
| *mediaconnect* | `GrantFlowEntitlements` | 0 |
| *mediaconnect* | `RemoveFlowOutput` | 0 |
| *mediaconnect* | `DeleteFlow` | 0 |
| *mediaconnect* | `StopFlow` | 0 |
| *mediaconnect* | `UpdateFlowSource` | 0 |
| *mediaconnect* | `AddFlowOutputs` | 0 |
| *mediaconnect* | `ListEntitlements` | 0 |
| *mediaconnect* | `DescribeFlow` | 0 |
| *mediaconvert* | `CreateQueue` | 0 |
| *mediaconvert* | `UpdatePreset` | 0 |
| *mediaconvert* | `ListPresets` | 0 |
| *mediaconvert* | `DeletePreset` | 0 |
| *mediaconvert* | `ListJobTemplates` | 0 |
| *mediaconvert* | `ListQueues` | 0 |
| *mediaconvert* | `GetJobTemplate` | 0 |
| *mediaconvert* | `DescribeEndpoints` | 0 |
| *mediaconvert* | `TagResource` | 0 |
| *mediaconvert* | `GetQueue` | 0 |
| *mediaconvert* | `DeleteJobTemplate` | 0 |
| *mediaconvert* | `ListJobs` | 0 |
| *mediaconvert* | `CreateJob` | 0 |
| *mediaconvert* | `GetPreset` | 0 |
| *mediaconvert* | `UpdateQueue` | 0 |
| *mediaconvert* | `AssociateCertificate` | 0 |
| *mediaconvert* | `DisassociateCertificate` | 0 |
| *mediaconvert* | `CancelJob` | 0 |
| *mediaconvert* | `CreatePreset` | 0 |
| *mediaconvert* | `UntagResource` | 0 |
| *mediaconvert* | `GetJob` | 0 |
| *mediaconvert* | `DeleteQueue` | 0 |
| *mediaconvert* | `CreateJobTemplate` | 0 |
| *mediaconvert* | `UpdateJobTemplate` | 0 |
| *mediaconvert* | `ListTagsForResource` | 0 |
| *medialive* | `DeleteTags` | 0 |
| *medialive* | `PurchaseOffering` | 0 |
| *medialive* | `UpdateChannel` | 0 |
| *medialive* | `ListOfferings` | 0 |
| *medialive* | `CreateChannel` | 1 |
| *medialive* | `DeleteInputSecurityGroup` | 0 |
| *medialive* | `StopChannel` | 0 |
| *medialive* | `DescribeChannel` | 0 |
| *medialive* | `ListInputs` | 0 |
| *medialive* | `DeleteSchedule` | 0 |
| *medialive* | `ListInputSecurityGroups` | 1 |
| *medialive* | `DeleteChannel` | 0 |
| *medialive* | `ListReservations` | 0 |
| *medialive* | `DescribeInput` | 0 |
| *medialive* | `UpdateChannelClass` | 0 |
| *medialive* | `DeleteInput` | 0 |
| *medialive* | `UpdateInputSecurityGroup` | 0 |
| *medialive* | `CreateInput` | 0 |
| *medialive* | `DeleteReservation` | 0 |
| *medialive* | `DescribeOffering` | 0 |
| *medialive* | `UpdateReservation` | 0 |
| *medialive* | `CreateInputSecurityGroup` | 1 |
| *medialive* | `DescribeInputSecurityGroup` | 0 |
| *medialive* | `DescribeReservation` | 0 |
| *medialive* | `CreateTags` | 0 |
| *medialive* | `BatchUpdateSchedule` | 0 |
| *medialive* | `ListChannels` | 1 |
| *medialive* | `UpdateInput` | 0 |
| *medialive* | `DescribeSchedule` | 0 |
| *medialive* | `StartChannel` | 0 |
| *medialive* | `ListTagsForResource` | 0 |
| *mediapackage* | `RotateChannelCredentials` | 0 |
| *mediapackage* | `DeleteOriginEndpoint` | 0 |
| *mediapackage* | `ListTagsForResource` | 0 |
| *mediapackage* | `CreateOriginEndpoint` | 0 |
| *mediapackage* | `UntagResource` | 0 |
| *mediapackage* | `DescribeHarvestJob` | 0 |
| *mediapackage* | `ListHarvestJobs` | 0 |
| *mediapackage* | `DeleteChannel` | 0 |
| *mediapackage* | `ListChannels` | 0 |
| *mediapackage* | `UpdateOriginEndpoint` | 0 |
| *mediapackage* | `CreateChannel` | 0 |
| *mediapackage* | `UpdateChannel` | 0 |
| *mediapackage* | `CreateHarvestJob` | 0 |
| *mediapackage* | `TagResource` | 0 |
| *mediapackage* | `ListOriginEndpoints` | 0 |
| *mediapackage* | `DescribeOriginEndpoint` | 0 |
| *mediapackage* | `RotateIngestEndpointCredentials` | 0 |
| *mediapackage* | `DescribeChannel` | 0 |
| *mediapackage-vod* | `DescribeAsset` | 0 |
| *mediapackage-vod* | `ListAssets` | 0 |
| *mediapackage-vod* | `DeletePackagingConfiguration` | 0 |
| *mediapackage-vod* | `DescribePackagingConfiguration` | 0 |
| *mediapackage-vod* | `ListPackagingGroups` | 0 |
| *mediapackage-vod* | `DeletePackagingGroup` | 0 |
| *mediapackage-vod* | `CreateAsset` | 0 |
| *mediapackage-vod* | `ListPackagingConfigurations` | 0 |
| *mediapackage-vod* | `DeleteAsset` | 0 |
| *mediapackage-vod* | `CreatePackagingGroup` | 0 |
| *mediapackage-vod* | `DescribePackagingGroup` | 0 |
| *mediapackage-vod* | `CreatePackagingConfiguration` | 0 |
| *mediastore* | `PutCorsPolicy` | 0 |
| *mediastore* | `GetLifecyclePolicy` | 0 |
| *mediastore* | `UntagResource` | 0 |
| *mediastore* | `StartAccessLogging` | 0 |
| *mediastore* | `PutContainerPolicy` | 1 |
| *mediastore* | `DeleteCorsPolicy` | 0 |
| *mediastore* | `DeleteContainerPolicy` | 0 |
| *mediastore* | `PutLifecyclePolicy` | 0 |
| *mediastore* | `GetCorsPolicy` | 1 |
| *mediastore* | `StopAccessLogging` | 0 |
| *mediastore* | `ListContainers` | 1 |
| *mediastore* | `DescribeContainer` | 1 |
| *mediastore* | `CreateContainer` | 1 |
| *mediastore* | `TagResource` | 0 |
| *mediastore* | `DeleteLifecyclePolicy` | 0 |
| *mediastore* | `ListTagsForResource` | 0 |
| *mediastore* | `DeleteContainer` | 1 |
| *mediastore* | `GetContainerPolicy` | 1 |
| *mediastore-data* | `GetObject` | 0 |
| *mediastore-data* | `DescribeObject` | 0 |
| *mediastore-data* | `PutObject` | 0 |
| *mediastore-data* | `DeleteObject` | 0 |
| *mediastore-data* | `ListItems` | 0 |
| *mediatailor* | `GetPlaybackConfiguration` | 0 |
| *mediatailor* | `UntagResource` | 0 |
| *mediatailor* | `DeletePlaybackConfiguration` | 0 |
| *mediatailor* | `ListPlaybackConfigurations` | 0 |
| *mediatailor* | `PutPlaybackConfiguration` | 0 |
| *mediatailor* | `TagResource` | 0 |
| *mediatailor* | `ListTagsForResource` | 0 |
| *meteringmarketplace* | `RegisterUsage` | 0 |
| *meteringmarketplace* | `BatchMeterUsage` | 0 |
| *meteringmarketplace* | `MeterUsage` | 0 |
| *meteringmarketplace* | `ResolveCustomer` | 0 |
| *mgh* | `CreateProgressUpdateStream` | 0 |
| *mgh* | `NotifyApplicationState` | 0 |
| *mgh* | `AssociateDiscoveredResource` | 0 |
| *mgh* | `PutResourceAttributes` | 0 |
| *mgh* | `ListProgressUpdateStreams` | 0 |
| *mgh* | `DisassociateDiscoveredResource` | 0 |
| *mgh* | `DeleteProgressUpdateStream` | 0 |
| *mgh* | `ListCreatedArtifacts` | 0 |
| *mgh* | `NotifyMigrationTaskState` | 0 |
| *mgh* | `ImportMigrationTask` | 0 |
| *mgh* | `DescribeMigrationTask` | 0 |
| *mgh* | `ListMigrationTasks` | 0 |
| *mgh* | `DisassociateCreatedArtifact` | 0 |
| *mgh* | `AssociateCreatedArtifact` | 0 |
| *mgh* | `ListDiscoveredResources` | 0 |
| *mgh* | `DescribeApplicationState` | 0 |
| *mobile* | `ExportProject` | 0 |
| *mobile* | `DescribeBundle` | 0 |
| *mobile* | `ListBundles` | 0 |
| *mobile* | `DescribeProject` | 0 |
| *mobile* | `CreateProject` | 0 |
| *mobile* | `ListProjects` | 0 |
| *mobile* | `UpdateProject` | 0 |
| *mobile* | `DeleteProject` | 0 |
| *mobile* | `ExportBundle` | 0 |
| *mq* | `DeleteTags` | 0 |
| *mq* | `DescribeBroker` | 0 |
| *mq* | `ListUsers` | 0 |
| *mq* | `CreateConfiguration` | 1 |
| *mq* | `UpdateUser` | 0 |
| *mq* | `DescribeBrokerEngineTypes` | 0 |
| *mq* | `ListConfigurations` | 1 |
| *mq* | `DescribeUser` | 0 |
| *mq* | `DescribeBrokerInstanceOptions` | 0 |
| *mq* | `DescribeConfiguration` | 1 |
| *mq* | `ListBrokers` | 1 |
| *mq* | `ListTags` | 0 |
| *mq* | `DescribeConfigurationRevision` | 1 |
| *mq* | `ListConfigurationRevisions` | 0 |
| *mq* | `UpdateBroker` | 1 |
| *mq* | `DeleteBroker` | 0 |
| *mq* | `DeleteUser` | 0 |
| *mq* | `CreateTags` | 0 |
| *mq* | `UpdateConfiguration` | 0 |
| *mq* | `CreateUser` | 0 |
| *mq* | `CreateBroker` | 1 |
| *mq* | `RebootBroker` | 0 |
| *mturk* | `SendBonus` | 0 |
| *mturk* | `AssociateQualificationWithWorker` | 0 |
| *mturk* | `UpdateHITTypeOfHIT` | 0 |
| *mturk* | `UpdateNotificationSettings` | 0 |
| *mturk* | `ListWorkerBlocks` | 0 |
| *mturk* | `ListReviewableHITs` | 0 |
| *mturk* | `ListAssignmentsForHIT` | 0 |
| *mturk* | `DisassociateQualificationFromWorker` | 0 |
| *mturk* | `RejectQualificationRequest` | 0 |
| *mturk* | `CreateHITType` | 0 |
| *mturk* | `GetFileUploadURL` | 0 |
| *mturk* | `ListReviewPolicyResultsForHIT` | 0 |
| *mturk* | `UpdateExpirationForHIT` | 0 |
| *mturk* | `UpdateQualificationType` | 0 |
| *mturk* | `NotifyWorkers` | 0 |
| *mturk* | `ListQualificationTypes` | 0 |
| *mturk* | `CreateHIT` | 0 |
| *mturk* | `ListHITsForQualificationType` | 0 |
| *mturk* | `AcceptQualificationRequest` | 0 |
| *mturk* | `ListQualificationRequests` | 0 |
| *mturk* | `UpdateHITReviewStatus` | 0 |
| *mturk* | `ListHITs` | 0 |
| *mturk* | `ListWorkersWithQualificationType` | 0 |
| *mturk* | `GetAccountBalance` | 0 |
| *mturk* | `DeleteQualificationType` | 0 |
| *mturk* | `SendTestEventNotification` | 0 |
| *mturk* | `CreateHITWithHITType` | 0 |
| *mturk* | `CreateWorkerBlock` | 0 |
| *mturk* | `ApproveAssignment` | 0 |
| *mturk* | `ListBonusPayments` | 0 |
| *mturk* | `DeleteHIT` | 0 |
| *mturk* | `GetQualificationScore` | 0 |
| *mturk* | `DeleteWorkerBlock` | 0 |
| *mturk* | `GetQualificationType` | 0 |
| *mturk* | `RejectAssignment` | 0 |
| *mturk* | `CreateQualificationType` | 0 |
| *mturk* | `GetAssignment` | 0 |
| *mturk* | `GetHIT` | 0 |
| *mturk* | `CreateAdditionalAssignmentsForHIT` | 0 |
| *neptune* | `DeleteDBSubnetGroup` | 1 |
| *neptune* | `DescribeValidDBInstanceModifications` | 0 |
| *neptune* | `DescribeDBClusterParameters` | 0 |
| *neptune* | `DescribeDBSubnetGroups` | 2 |
| *neptune* | `RestoreDBClusterToPointInTime` | 0 |
| *neptune* | `RemoveRoleFromDBCluster` | 0 |
| *neptune* | `DeleteDBInstance` | 0 |
| *neptune* | `CreateDBClusterParameterGroup` | 1 |
| *neptune* | `CreateEventSubscription` | 0 |
| *neptune* | `CopyDBParameterGroup` | 0 |
| *neptune* | `DeleteDBClusterSnapshot` | 0 |
| *neptune* | `RestoreDBClusterFromSnapshot` | 0 |
| *neptune* | `DescribePendingMaintenanceActions` | 0 |
| *neptune* | `RebootDBInstance` | 0 |
| *neptune* | `RemoveSourceIdentifierFromSubscription` | 0 |
| *neptune* | `CreateDBParameterGroup` | 1 |
| *neptune* | `DeleteDBParameterGroup` | 1 |
| *neptune* | `CreateDBSubnetGroup` | 1 |
| *neptune* | `DescribeEventCategories` | 0 |
| *neptune* | `AddRoleToDBCluster` | 0 |
| *neptune* | `ModifyDBCluster` | 0 |
| *neptune* | `DescribeDBParameters` | 0 |
| *neptune* | `DescribeDBParameterGroups` | 1 |
| *neptune* | `AddTagsToResource` | 0 |
| *neptune* | `ModifyDBClusterParameterGroup` | 0 |
| *neptune* | `DescribeDBEngineVersions` | 1 |
| *neptune* | `FailoverDBCluster` | 0 |
| *neptune* | `DescribeDBClusterSnapshots` | 0 |
| *neptune* | `ApplyPendingMaintenanceAction` | 0 |
| *neptune* | `DescribeDBInstances` | 1 |
| *neptune* | `RemoveTagsFromResource` | 0 |
| *neptune* | `ResetDBParameterGroup` | 0 |
| *neptune* | `DescribeOrderableDBInstanceOptions` | 0 |
| *neptune* | `CreateDBInstance` | 1 |
| *neptune* | `DescribeDBClusters` | 0 |
| *neptune* | `DeleteEventSubscription` | 0 |
| *neptune* | `ModifyEventSubscription` | 0 |
| *neptune* | `ModifyDBSubnetGroup` | 0 |
| *neptune* | `DescribeEvents` | 0 |
| *neptune* | `DescribeEventSubscriptions` | 0 |
| *neptune* | `AddSourceIdentifierToSubscription` | 0 |
| *neptune* | `ModifyDBParameterGroup` | 0 |
| *neptune* | `DescribeEngineDefaultClusterParameters` | 0 |
| *neptune* | `PromoteReadReplicaDBCluster` | 0 |
| *neptune* | `CopyDBClusterParameterGroup` | 0 |
| *neptune* | `CreateDBClusterSnapshot` | 0 |
| *neptune* | `DeleteDBClusterParameterGroup` | 0 |
| *neptune* | `DescribeDBClusterSnapshotAttributes` | 0 |
| *neptune* | `CreateDBCluster` | 1 |
| *neptune* | `DescribeDBClusterParameterGroups` | 1 |
| *neptune* | `DescribeEngineDefaultParameters` | 0 |
| *neptune* | `CopyDBClusterSnapshot` | 0 |
| *neptune* | `ModifyDBInstance` | 0 |
| *neptune* | `DeleteDBCluster` | 0 |
| *neptune* | `ResetDBClusterParameterGroup` | 0 |
| *neptune* | `ModifyDBClusterSnapshotAttribute` | 0 |
| *neptune* | `ListTagsForResource` | 0 |
| *opsworks* | `DeregisterVolume` | 0 |
| *opsworks* | `DeregisterInstance` | 0 |
| *opsworks* | `DescribeTimeBasedAutoScaling` | 0 |
| *opsworks* | `RebootInstance` | 0 |
| *opsworks* | `StartStack` | 0 |
| *opsworks* | `UnassignInstance` | 0 |
| *opsworks* | `DisassociateElasticIp` | 0 |
| *opsworks* | `CreateUserProfile` | 1 |
| *opsworks* | `AttachElasticLoadBalancer` | 1 |
| *opsworks* | `RegisterEcsCluster` | 0 |
| *opsworks* | `DescribeLayers` | 2 |
| *opsworks* | `UpdateElasticIp` | 0 |
| *opsworks* | `DeleteInstance` | 1 |
| *opsworks* | `DetachElasticLoadBalancer` | 1 |
| *opsworks* | `CreateDeployment` | 0 |
| *opsworks* | `UnassignVolume` | 0 |
| *opsworks* | `DeleteUserProfile` | 0 |
| *opsworks* | `CreateStack` | 1 |
| *opsworks* | `UpdateRdsDbInstance` | 0 |
| *opsworks* | `GetHostnameSuggestion` | 0 |
| *opsworks* | `DescribeInstances` | 1 |
| *opsworks* | `DescribeCommands` | 0 |
| *opsworks* | `DescribeMyUserProfile` | 0 |
| *opsworks* | `DeregisterElasticIp` | 0 |
| *opsworks* | `DescribeAgentVersions` | 0 |
| *opsworks* | `StopStack` | 0 |
| *opsworks* | `DescribeApps` | 0 |
| *opsworks* | `DescribeElasticIps` | 0 |
| *opsworks* | `UpdateVolume` | 0 |
| *opsworks* | `StartInstance` | 0 |
| *opsworks* | `UpdateMyUserProfile` | 0 |
| *opsworks* | `DeleteApp` | 1 |
| *opsworks* | `UpdateInstance` | 0 |
| *opsworks* | `RegisterElasticIp` | 0 |
| *opsworks* | `TagResource` | 0 |
| *opsworks* | `DeregisterRdsDbInstance` | 0 |
| *opsworks* | `DescribeRaidArrays` | 0 |
| *opsworks* | `RegisterVolume` | 1 |
| *opsworks* | `SetPermission` | 0 |
| *opsworks* | `DescribeDeployments` | 0 |
| *opsworks* | `ListTags` | 0 |
| *opsworks* | `CloneStack` | 0 |
| *opsworks* | `DescribeServiceErrors` | 0 |
| *opsworks* | `AssociateElasticIp` | 0 |
| *opsworks* | `CreateInstance` | 1 |
| *opsworks* | `DescribeElasticLoadBalancers` | 1 |
| *opsworks* | `DescribeOperatingSystems` | 0 |
| *opsworks* | `DeregisterEcsCluster` | 0 |
| *opsworks* | `DescribeEcsClusters` | 0 |
| *opsworks* | `RegisterRdsDbInstance` | 1 |
| *opsworks* | `SetTimeBasedAutoScaling` | 0 |
| *opsworks* | `DescribeLoadBasedAutoScaling` | 0 |
| *opsworks* | `AssignInstance` | 0 |
| *opsworks* | `SetLoadBasedAutoScaling` | 0 |
| *opsworks* | `DescribeStackSummary` | 0 |
| *opsworks* | `StopInstance` | 0 |
| *opsworks* | `DescribeStacks` | 0 |
| *opsworks* | `CreateLayer` | 1 |
| *opsworks* | `UpdateStack` | 0 |
| *opsworks* | `GrantAccess` | 0 |
| *opsworks* | `CreateApp` | 1 |
| *opsworks* | `UntagResource` | 0 |
| *opsworks* | `DescribePermissions` | 0 |
| *opsworks* | `DeleteLayer` | 1 |
| *opsworks* | `DescribeVolumes` | 1 |
| *opsworks* | `DescribeStackProvisioningParameters` | 0 |
| *opsworks* | `RegisterInstance` | 0 |
| *opsworks* | `DeleteStack` | 1 |
| *opsworks* | `AssignVolume` | 0 |
| *opsworks* | `UpdateApp` | 0 |
| *opsworks* | `DescribeRdsDbInstances` | 0 |
| *opsworks* | `UpdateUserProfile` | 0 |
| *opsworks* | `DescribeUserProfiles` | 0 |
| *opsworks* | `UpdateLayer` | 1 |
| *opsworkscm* | `DescribeServers` | 1 |
| *opsworkscm* | `DescribeNodeAssociationStatus` | 0 |
| *opsworkscm* | `DeleteServer` | 1 |
| *opsworkscm* | `DescribeBackups` | 1 |
| *opsworkscm* | `RestoreServer` | 0 |
| *opsworkscm* | `UpdateServerEngineAttributes` | 0 |
| *opsworkscm* | `ExportServerEngineAttribute` | 0 |
| *opsworkscm* | `DescribeAccountAttributes` | 0 |
| *opsworkscm* | `DescribeEvents` | 0 |
| *opsworkscm* | `DeleteBackup` | 0 |
| *opsworkscm* | `CreateServer` | 1 |
| *opsworkscm* | `CreateBackup` | 0 |
| *opsworkscm* | `StartMaintenance` | 0 |
| *opsworkscm* | `UpdateServer` | 0 |
| *opsworkscm* | `AssociateNode` | 0 |
| *opsworkscm* | `DisassociateNode` | 0 |
| *organizations* | `DeclineHandshake` | 0 |
| *organizations* | `DisablePolicyType` | 0 |
| *organizations* | `DescribeCreateAccountStatus` | 0 |
| *organizations* | `ListOrganizationalUnitsForParent` | 1 |
| *organizations* | `DescribeOrganizationalUnit` | 0 |
| *organizations* | `AcceptHandshake` | 0 |
| *organizations* | `DeleteOrganizationalUnit` | 1 |
| *organizations* | `DeletePolicy` | 1 |
| *organizations* | `EnableAWSServiceAccess` | 0 |
| *organizations* | `DetachPolicy` | 0 |
| *organizations* | `ListHandshakesForAccount` | 0 |
| *organizations* | `RemoveAccountFromOrganization` | 0 |
| *organizations* | `EnableAllFeatures` | 0 |
| *organizations* | `DeleteOrganization` | 0 |
| *organizations* | `ListPoliciesForTarget` | 0 |
| *organizations* | `DescribePolicy` | 1 |
| *organizations* | `ListCreateAccountStatus` | 1 |
| *organizations* | `CreateGovCloudAccount` | 0 |
| *organizations* | `CreateAccount` | 1 |
| *organizations* | `CancelHandshake` | 0 |
| *organizations* | `ListAccounts` | 1 |
| *organizations* | `AttachPolicy` | 0 |
| *organizations* | `MoveAccount` | 1 |
| *organizations* | `DescribeOrganization` | 1 |
| *organizations* | `ListChildren` | 0 |
| *organizations* | `CreateOrganization` | 0 |
| *organizations* | `UpdatePolicy` | 0 |
| *organizations* | `DisableAWSServiceAccess` | 0 |
| *organizations* | `LeaveOrganization` | 0 |
| *organizations* | `TagResource` | 0 |
| *organizations* | `CreateOrganizationalUnit` | 1 |
| *organizations* | `EnablePolicyType` | 0 |
| *organizations* | `UntagResource` | 0 |
| *organizations* | `DescribeAccount` | 1 |
| *organizations* | `InviteAccountToOrganization` | 0 |
| *organizations* | `ListPolicies` | 1 |
| *organizations* | `ListAccountsForParent` | 0 |
| *organizations* | `DescribeHandshake` | 0 |
| *organizations* | `ListParents` | 1 |
| *organizations* | `UpdateOrganizationalUnit` | 0 |
| *organizations* | `ListHandshakesForOrganization` | 0 |
| *organizations* | `CreatePolicy` | 1 |
| *organizations* | `ListTargetsForPolicy` | 0 |
| *organizations* | `ListRoots` | 1 |
| *organizations* | `ListTagsForResource` | 0 |
| *organizations* | `ListAWSServiceAccessForOrganization` | 0 |
| *personalize* | `ListDatasetGroups` | 0 |
| *personalize* | `DescribeDatasetGroup` | 0 |
| *personalize* | `CreateSchema` | 0 |
| *personalize* | `DeleteSolution` | 0 |
| *personalize* | `DeleteDataset` | 0 |
| *personalize* | `CreateSolutionVersion` | 0 |
| *personalize* | `CreateCampaign` | 0 |
| *personalize* | `ListSolutionVersions` | 0 |
| *personalize* | `CreateSolution` | 0 |
| *personalize* | `ListDatasets` | 0 |
| *personalize* | `DescribeDatasetImportJob` | 0 |
| *personalize* | `DescribeFeatureTransformation` | 0 |
| *personalize* | `DescribeDataset` | 0 |
| *personalize* | `DescribeAlgorithm` | 0 |
| *personalize* | `DescribeRecipe` | 0 |
| *personalize* | `DescribeSchema` | 0 |
| *personalize* | `CreateDataset` | 0 |
| *personalize* | `UpdateCampaign` | 0 |
| *personalize* | `ListRecipes` | 0 |
| *personalize* | `DescribeSolution` | 0 |
| *personalize* | `CreateDatasetGroup` | 0 |
| *personalize* | `CreateEventTracker` | 0 |
| *personalize* | `DeleteSchema` | 0 |
| *personalize* | `DescribeSolutionVersion` | 0 |
| *personalize* | `ListDatasetImportJobs` | 0 |
| *personalize* | `DescribeEventTracker` | 0 |
| *personalize* | `CreateDatasetImportJob` | 0 |
| *personalize* | `DeleteCampaign` | 0 |
| *personalize* | `ListEventTrackers` | 0 |
| *personalize* | `ListSchemas` | 0 |
| *personalize* | `ListCampaigns` | 0 |
| *personalize* | `DescribeCampaign` | 0 |
| *personalize* | `ListSolutions` | 0 |
| *personalize* | `DeleteDatasetGroup` | 0 |
| *personalize* | `GetSolutionMetrics` | 0 |
| *personalize* | `DeleteEventTracker` | 0 |
| *personalize-events* | `PutEvents` | 0 |
| *personalize-runtime* | `GetRecommendations` | 0 |
| *personalize-runtime* | `GetPersonalizedRanking` | 0 |
| *pi* | `DescribeDimensionKeys` | 0 |
| *pi* | `GetResourceMetrics` | 0 |
| *pinpoint* | `UpdateJourney` | 0 |
| *pinpoint* | `CreateCampaign` | 0 |
| *pinpoint* | `GetEndpoint` | 0 |
| *pinpoint* | `DeleteSmsChannel` | 0 |
| *pinpoint* | `UpdateCampaign` | 0 |
| *pinpoint* | `DeleteVoiceChannel` | 0 |
| *pinpoint* | `GetSegmentExportJobs` | 0 |
| *pinpoint* | `GetApnsVoipSandboxChannel` | 0 |
| *pinpoint* | `ListTemplates` | 0 |
| *pinpoint* | `GetApnsSandboxChannel` | 0 |
| *pinpoint* | `UpdateVoiceChannel` | 0 |
| *pinpoint* | `GetCampaigns` | 0 |
| *pinpoint* | `DeleteApnsVoipChannel` | 0 |
| *pinpoint* | `GetCampaignActivities` | 0 |
| *pinpoint* | `DeleteUserEndpoints` | 0 |
| *pinpoint* | `UpdateApnsVoipSandboxChannel` | 0 |
| *pinpoint* | `SendMessages` | 0 |
| *pinpoint* | `GetJourneyExecutionMetrics` | 0 |
| *pinpoint* | `DeleteEventStream` | 0 |
| *pinpoint* | `CreateSmsTemplate` | 0 |
| *pinpoint* | `UpdateGcmChannel` | 1 |
| *pinpoint* | `UpdateSegment` | 0 |
| *pinpoint* | `GetSegmentVersions` | 0 |
| *pinpoint* | `UpdateEndpointsBatch` | 0 |
| *pinpoint* | `ListTagsForResource` | 0 |
| *pinpoint* | `GetExportJob` | 0 |
| *pinpoint* | `GetImportJobs` | 0 |
| *pinpoint* | `DeleteSmsTemplate` | 0 |
| *pinpoint* | `UpdateAdmChannel` | 1 |
| *pinpoint* | `GetApp` | 0 |
| *pinpoint* | `UpdateSmsChannel` | 1 |
| *pinpoint* | `CreateImportJob` | 0 |
| *pinpoint* | `GetCampaignVersion` | 0 |
| *pinpoint* | `DeleteApp` | 0 |
| *pinpoint* | `CreateJourney` | 0 |
| *pinpoint* | `TagResource` | 0 |
| *pinpoint* | `GetPushTemplate` | 0 |
| *pinpoint* | `UpdateApnsSandboxChannel` | 0 |
| *pinpoint* | `CreatePushTemplate` | 0 |
| *pinpoint* | `GetSmsChannel` | 0 |
| *pinpoint* | `PutEventStream` | 0 |
| *pinpoint* | `DeleteEmailChannel` | 0 |
| *pinpoint* | `GetApnsChannel` | 0 |
| *pinpoint* | `UpdateEmailTemplate` | 0 |
| *pinpoint* | `DeleteAdmChannel` | 0 |
| *pinpoint* | `CreateEmailTemplate` | 0 |
| *pinpoint* | `GetSmsTemplate` | 0 |
| *pinpoint* | `CreateApp` | 1 |
| *pinpoint* | `UntagResource` | 0 |
| *pinpoint* | `CreateExportJob` | 0 |
| *pinpoint* | `UpdateEmailChannel` | 0 |
| *pinpoint* | `PhoneNumberValidate` | 0 |
| *pinpoint* | `UpdateBaiduChannel` | 1 |
| *pinpoint* | `UpdateEndpoint` | 0 |
| *pinpoint* | `UpdateJourneyState` | 0 |
| *pinpoint* | `UpdateSmsTemplate` | 0 |
| *pinpoint* | `UpdatePushTemplate` | 0 |
| *pinpoint* | `GetExportJobs` | 0 |
| *pinpoint* | `PutEvents` | 0 |
| *pinpoint* | `UpdateApnsChannel` | 1 |
| *pinpoint* | `GetCampaign` | 0 |
| *pinpoint* | `UpdateApplicationSettings` | 0 |
| *pinpoint* | `GetSegments` | 0 |
| *pinpoint* | `GetApps` | 1 |
| *pinpoint* | `UpdateApnsVoipChannel` | 0 |
| *pinpoint* | `GetJourney` | 0 |
| *pinpoint* | `GetUserEndpoints` | 0 |
| *pinpoint* | `DeleteEmailTemplate` | 0 |
| *pinpoint* | `SendUsersMessages` | 0 |
| *pinpoint* | `DeleteCampaign` | 0 |
| *pinpoint* | `GetApplicationDateRangeKpi` | 0 |
| *pinpoint* | `GetJourneyDateRangeKpi` | 0 |
| *pinpoint* | `GetJourneyExecutionActivityMetrics` | 0 |
| *pinpoint* | `GetApplicationSettings` | 0 |
| *pinpoint* | `GetBaiduChannel` | 0 |
| *pinpoint* | `DeleteJourney` | 0 |
| *pinpoint* | `DeletePushTemplate` | 0 |
| *pinpoint* | `GetApnsVoipChannel` | 0 |
| *pinpoint* | `CreateSegment` | 0 |
| *pinpoint* | `GetSegmentVersion` | 0 |
| *pinpoint* | `DeleteApnsSandboxChannel` | 0 |
| *pinpoint* | `DeleteApnsVoipSandboxChannel` | 0 |
| *pinpoint* | `DeleteGcmChannel` | 0 |
| *pinpoint* | `GetChannels` | 0 |
| *pinpoint* | `GetEmailTemplate` | 0 |
| *pinpoint* | `RemoveAttributes` | 0 |
| *pinpoint* | `GetImportJob` | 0 |
| *pinpoint* | `GetGcmChannel` | 0 |
| *pinpoint* | `GetEmailChannel` | 0 |
| *pinpoint* | `GetVoiceChannel` | 0 |
| *pinpoint* | `ListJourneys` | 0 |
| *pinpoint* | `DeleteSegment` | 0 |
| *pinpoint* | `GetCampaignVersions` | 0 |
| *pinpoint* | `GetAdmChannel` | 0 |
| *pinpoint* | `GetSegmentImportJobs` | 0 |
| *pinpoint* | `GetCampaignDateRangeKpi` | 0 |
| *pinpoint* | `DeleteBaiduChannel` | 0 |
| *pinpoint* | `GetEventStream` | 0 |
| *pinpoint* | `GetSegment` | 0 |
| *pinpoint* | `DeleteEndpoint` | 0 |
| *pinpoint* | `DeleteApnsChannel` | 0 |
| *pinpoint-email* | `ListEmailIdentities` | 0 |
| *pinpoint-email* | `UpdateConfigurationSetEventDestination` | 0 |
| *pinpoint-email* | `PutAccountDedicatedIpWarmupAttributes` | 0 |
| *pinpoint-email* | `PutConfigurationSetDeliveryOptions` | 0 |
| *pinpoint-email* | `PutDedicatedIpInPool` | 0 |
| *pinpoint-email* | `GetDomainStatisticsReport` | 0 |
| *pinpoint-email* | `PutEmailIdentityFeedbackAttributes` | 0 |
| *pinpoint-email* | `GetConfigurationSet` | 0 |
| *pinpoint-email* | `GetEmailIdentity` | 0 |
| *pinpoint-email* | `PutConfigurationSetSendingOptions` | 0 |
| *pinpoint-email* | `PutEmailIdentityDkimAttributes` | 0 |
| *pinpoint-email* | `CreateConfigurationSetEventDestination` | 0 |
| *pinpoint-email* | `DeleteDedicatedIpPool` | 0 |
| *pinpoint-email* | `DeleteConfigurationSet` | 0 |
| *pinpoint-email* | `TagResource` | 0 |
| *pinpoint-email* | `GetConfigurationSetEventDestinations` | 0 |
| *pinpoint-email* | `GetDedicatedIp` | 0 |
| *pinpoint-email* | `CreateDedicatedIpPool` | 0 |
| *pinpoint-email* | `DeleteEmailIdentity` | 0 |
| *pinpoint-email* | `CreateDeliverabilityTestReport` | 0 |
| *pinpoint-email* | `GetDomainDeliverabilityCampaign` | 0 |
| *pinpoint-email* | `GetDeliverabilityDashboardOptions` | 0 |
| *pinpoint-email* | `ListDedicatedIpPools` | 0 |
| *pinpoint-email* | `PutEmailIdentityMailFromAttributes` | 0 |
| *pinpoint-email* | `PutDedicatedIpWarmupAttributes` | 0 |
| *pinpoint-email* | `ListDomainDeliverabilityCampaigns` | 0 |
| *pinpoint-email* | `CreateConfigurationSet` | 0 |
| *pinpoint-email* | `PutConfigurationSetTrackingOptions` | 0 |
| *pinpoint-email* | `GetAccount` | 0 |
| *pinpoint-email* | `ListConfigurationSets` | 0 |
| *pinpoint-email* | `PutConfigurationSetReputationOptions` | 0 |
| *pinpoint-email* | `GetDeliverabilityTestReport` | 0 |
| *pinpoint-email* | `GetDedicatedIps` | 0 |
| *pinpoint-email* | `CreateEmailIdentity` | 0 |
| *pinpoint-email* | `UntagResource` | 0 |
| *pinpoint-email* | `PutAccountSendingAttributes` | 0 |
| *pinpoint-email* | `SendEmail` | 0 |
| *pinpoint-email* | `PutDeliverabilityDashboardOption` | 0 |
| *pinpoint-email* | `DeleteConfigurationSetEventDestination` | 0 |
| *pinpoint-email* | `GetBlacklistReports` | 0 |
| *pinpoint-email* | `ListTagsForResource` | 0 |
| *pinpoint-email* | `ListDeliverabilityTestReports` | 0 |
| *pinpoint-sms-voice* | `CreateConfigurationSetEventDestination` | 0 |
| *pinpoint-sms-voice* | `DeleteConfigurationSet` | 0 |
| *pinpoint-sms-voice* | `DeleteConfigurationSetEventDestination` | 0 |
| *pinpoint-sms-voice* | `UpdateConfigurationSetEventDestination` | 0 |
| *pinpoint-sms-voice* | `GetConfigurationSetEventDestinations` | 0 |
| *pinpoint-sms-voice* | `CreateConfigurationSet` | 0 |
| *pinpoint-sms-voice* | `SendVoiceMessage` | 0 |
| *polly* | `DescribeVoices` | 0 |
| *polly* | `ListSpeechSynthesisTasks` | 0 |
| *polly* | `GetLexicon` | 0 |
| *polly* | `StartSpeechSynthesisTask` | 0 |
| *polly* | `GetSpeechSynthesisTask` | 0 |
| *polly* | `PutLexicon` | 0 |
| *polly* | `ListLexicons` | 0 |
| *polly* | `SynthesizeSpeech` | 0 |
| *polly* | `DeleteLexicon` | 0 |
| *pricing* | `GetProducts` | 0 |
| *pricing* | `DescribeServices` | 0 |
| *pricing* | `GetAttributeValues` | 0 |
| *qldb* | `GetRevision` | 0 |
| *qldb* | `DeleteLedger` | 0 |
| *qldb* | `CreateLedger` | 0 |
| *qldb* | `UntagResource` | 0 |
| *qldb* | `DescribeLedger` | 0 |
| *qldb* | `TagResource` | 0 |
| *qldb* | `UpdateLedger` | 0 |
| *qldb* | `ListLedgers` | 0 |
| *qldb* | `ListJournalS3ExportsForLedger` | 0 |
| *qldb* | `ListJournalS3Exports` | 0 |
| *qldb* | `GetDigest` | 0 |
| *qldb* | `ListTagsForResource` | 0 |
| *qldb* | `DescribeJournalS3Export` | 0 |
| *qldb* | `ExportJournalToS3` | 0 |
| *qldb* | `GetBlock` | 0 |
| *qldb-session* | `SendCommand` | 0 |
| *quicksight* | `CreateGroupMembership` | 0 |
| *quicksight* | `UpdateUser` | 0 |
| *quicksight* | `DeleteUser` | 0 |
| *quicksight* | `DeleteUserByPrincipalId` | 0 |
| *quicksight* | `ListGroupMemberships` | 0 |
| *quicksight* | `GetDashboardEmbedUrl` | 0 |
| *quicksight* | `DeleteGroup` | 0 |
| *quicksight* | `DescribeUser` | 0 |
| *quicksight* | `ListUserGroups` | 0 |
| *quicksight* | `ListUsers` | 0 |
| *quicksight* | `UpdateGroup` | 0 |
| *quicksight* | `ListGroups` | 0 |
| *quicksight* | `CreateGroup` | 0 |
| *quicksight* | `DeleteGroupMembership` | 0 |
| *quicksight* | `RegisterUser` | 0 |
| *quicksight* | `DescribeGroup` | 0 |
| *ram* | `ListResources` | 0 |
| *ram* | `EnableSharingWithAwsOrganization` | 0 |
| *ram* | `AssociateResourceShare` | 0 |
| *ram* | `UntagResource` | 0 |
| *ram* | `DeleteResourceShare` | 1 |
| *ram* | `RejectResourceShareInvitation` | 0 |
| *ram* | `TagResource` | 0 |
| *ram* | `GetResourcePolicies` | 0 |
| *ram* | `UpdateResourceShare` | 0 |
| *ram* | `GetResourceShareAssociations` | 0 |
| *ram* | `CreateResourceShare` | 1 |
| *ram* | `GetResourceShareInvitations` | 1 |
| *ram* | `AcceptResourceShareInvitation` | 0 |
| *ram* | `DisassociateResourceShare` | 0 |
| *ram* | `ListPrincipals` | 0 |
| *ram* | `GetResourceShares` | 1 |
| *ram* | `ListPendingInvitationResources` | 0 |
| *rds* | `DescribeCertificates` | 0 |
| *rds* | `StopActivityStream` | 0 |
| *rds* | `RestoreDBClusterToPointInTime` | 0 |
| *rds* | `DescribeEngineDefaultParameters` | 0 |
| *rds* | `DeleteDBClusterSnapshot` | 0 |
| *rds* | `CreateDBParameterGroup` | 1 |
| *rds* | `AuthorizeDBSecurityGroupIngress` | 0 |
| *rds* | `DescribeDBSubnetGroups` | 2 |
| *rds* | `DescribeDBEngineVersions` | 0 |
| *rds* | `CopyDBSnapshot` | 0 |
| *rds* | `DeleteCustomAvailabilityZone` | 0 |
| *rds* | `StopDBCluster` | 1 |
| *rds* | `DeleteEventSubscription` | 0 |
| *rds* | `CreateDBInstanceReadReplica` | 0 |
| *rds* | `ImportInstallationMedia` | 0 |
| *rds* | `CreateDBClusterSnapshot` | 0 |
| *rds* | `DescribeReservedDBInstancesOfferings` | 0 |
| *rds* | `RemoveFromGlobalCluster` | 0 |
| *rds* | `DescribeInstallationMedia` | 0 |
| *rds* | `DeleteDBCluster` | 0 |
| *rds* | `ResetDBClusterParameterGroup` | 0 |
| *rds* | `ListTagsForResource` | 0 |
| *rds* | `ModifyDBSnapshot` | 0 |
| *rds* | `DescribeDBInstanceAutomatedBackups` | 0 |
| *rds* | `DeleteDBSubnetGroup` | 0 |
| *rds* | `RestoreDBInstanceFromDBSnapshot` | 0 |
| *rds* | `DescribeOptionGroups` | 1 |
| *rds* | `RestoreDBClusterFromS3` | 0 |
| *rds* | `CreateCustomAvailabilityZone` | 0 |
| *rds* | `CreateDBClusterParameterGroup` | 1 |
| *rds* | `DeleteDBParameterGroup` | 0 |
| *rds* | `DescribeEventCategories` | 0 |
| *rds* | `DescribeDBClusterBacktracks` | 0 |
| *rds* | `AddTagsToResource` | 0 |
| *rds* | `ModifyOptionGroup` | 0 |
| *rds* | `DescribeDBLogFiles` | 1 |
| *rds* | `BacktrackDBCluster` | 0 |
| *rds* | `RemoveTagsFromResource` | 0 |
| *rds* | `AddRoleToDBInstance` | 0 |
| *rds* | `DescribeReservedDBInstances` | 1 |
| *rds* | `CopyOptionGroup` | 0 |
| *rds* | `CreateDBInstance` | 1 |
| *rds* | `ModifyEventSubscription` | 0 |
| *rds* | `ModifyDBSubnetGroup` | 0 |
| *rds* | `DeleteDBSecurityGroup` | 0 |
| *rds* | `RestoreDBInstanceToPointInTime` | 0 |
| *rds* | `AddSourceIdentifierToSubscription` | 0 |
| *rds* | `ModifyGlobalCluster` | 0 |
| *rds* | `CopyDBClusterParameterGroup` | 0 |
| *rds* | `StartDBInstance` | 0 |
| *rds* | `DescribeDBClusterSnapshotAttributes` | 0 |
| *rds* | `DeleteDBInstanceAutomatedBackup` | 0 |
| *rds* | `RestoreDBClusterFromSnapshot` | 0 |
| *rds* | `ModifyDBSnapshotAttribute` | 0 |
| *rds* | `PromoteReadReplicaDBCluster` | 0 |
| *rds* | `CreateDBClusterEndpoint` | 0 |
| *rds* | `StartActivityStream` | 0 |
| *rds* | `DescribeDBClusterParameters` | 0 |
| *rds* | `DeleteDBSnapshot` | 0 |
| *rds* | `DescribeEvents` | 1 |
| *rds* | `DescribeEventSubscriptions` | 1 |
| *rds* | `DeleteOptionGroup` | 0 |
| *rds* | `DeleteInstallationMedia` | 0 |
| *rds* | `PromoteReadReplica` | 0 |
| *rds* | `DescribeGlobalClusters` | 1 |
| *rds* | `AddRoleToDBCluster` | 0 |
| *rds* | `ModifyDBClusterEndpoint` | 0 |
| *rds* | `DescribeDBParameters` | 0 |
| *rds* | `DescribeDBParameterGroups` | 2 |
| *rds* | `ModifyDBClusterParameterGroup` | 0 |
| *rds* | `FailoverDBCluster` | 0 |
| *rds* | `ApplyPendingMaintenanceAction` | 0 |
| *rds* | `DescribeDBInstances` | 4 |
| *rds* | `PurchaseReservedDBInstancesOffering` | 0 |
| *rds* | `DescribeOrderableDBInstanceOptions` | 0 |
| *rds* | `RestoreDBInstanceFromS3` | 0 |
| *rds* | `DescribeCustomAvailabilityZones` | 0 |
| *rds* | `CreateOptionGroup` | 1 |
| *rds* | `DeleteDBInstance` | 1 |
| *rds* | `DescribeOptionGroupOptions` | 0 |
| *rds* | `ResetDBParameterGroup` | 0 |
| *rds* | `DescribeEngineDefaultClusterParameters` | 0 |
| *rds* | `CreateDBSnapshot` | 0 |
| *rds* | `CreateDBCluster` | 1 |
| *rds* | `DescribeAccountAttributes` | 0 |
| *rds* | `DescribeDBSnapshots` | 2 |
| *rds* | `ModifyDBClusterSnapshotAttribute` | 0 |
| *rds* | `CreateDBSecurityGroup` | 0 |
| *rds* | `ModifyCurrentDBClusterCapacity` | 0 |
| *rds* | `DescribeValidDBInstanceModifications` | 0 |
| *rds* | `StopDBInstance` | 0 |
| *rds* | `RemoveRoleFromDBCluster` | 0 |
| *rds* | `RevokeDBSecurityGroupIngress` | 0 |
| *rds* | `CreateGlobalCluster` | 0 |
| *rds* | `CreateEventSubscription` | 1 |
| *rds* | `CopyDBParameterGroup` | 0 |
| *rds* | `DeleteGlobalCluster` | 0 |
| *rds* | `RebootDBInstance` | 0 |
| *rds* | `CreateDBSubnetGroup` | 1 |
| *rds* | `ModifyDBCluster` | 0 |
| *rds* | `RemoveRoleFromDBInstance` | 0 |
| *rds* | `DescribeDBClusterEndpoints` | 0 |
| *rds* | `DescribeDBSnapshotAttributes` | 0 |
| *rds* | `DeleteDBClusterEndpoint` | 0 |
| *rds* | `DescribeSourceRegions` | 0 |
| *rds* | `DownloadDBLogFilePortion` | 0 |
| *rds* | `DescribeDBClusters` | 2 |
| *rds* | `DescribeDBClusterSnapshots` | 1 |
| *rds* | `ModifyDBParameterGroup` | 0 |
| *rds* | `StartDBCluster` | 1 |
| *rds* | `DescribeDBSecurityGroups` | 1 |
| *rds* | `DeleteDBClusterParameterGroup` | 0 |
| *rds* | `DescribePendingMaintenanceActions` | 1 |
| *rds* | `DescribeDBClusterParameterGroups` | 1 |
| *rds* | `CopyDBClusterSnapshot` | 0 |
| *rds* | `ModifyDBInstance` | 0 |
| *rds* | `RemoveSourceIdentifierFromSubscription` | 0 |
| *rds-data* | `BeginTransaction` | 0 |
| *rds-data* | `ExecuteSql` | 0 |
| *rds-data* | `BatchExecuteStatement` | 0 |
| *rds-data* | `RollbackTransaction` | 0 |
| *rds-data* | `CommitTransaction` | 0 |
| *rds-data* | `ExecuteStatement` | 0 |
| *redshift* | `DeleteTags` | 0 |
| *redshift* | `DescribeDefaultClusterParameters` | 0 |
| *redshift* | `DescribeClusterParameterGroups` | 1 |
| *redshift* | `GetReservedNodeExchangeOfferings` | 0 |
| *redshift* | `RestoreFromClusterSnapshot` | 0 |
| *redshift* | `CreateCluster` | 1 |
| *redshift* | `RevokeSnapshotAccess` | 0 |
| *redshift* | `CopyClusterSnapshot` | 0 |
| *redshift* | `CreateClusterSecurityGroup` | 0 |
| *redshift* | `DescribeEvents` | 0 |
| *redshift* | `DescribeEventSubscriptions` | 0 |
| *redshift* | `CreateEventSubscription` | 0 |
| *redshift* | `DescribeHsmClientCertificates` | 0 |
| *redshift* | `RotateEncryptionKey` | 0 |
| *redshift* | `ModifySnapshotSchedule` | 0 |
| *redshift* | `DeleteCluster` | 0 |
| *redshift* | `DescribeResize` | 0 |
| *redshift* | `CreateSnapshotCopyGrant` | 0 |
| *redshift* | `DescribeEventCategories` | 0 |
| *redshift* | `ModifySnapshotCopyRetentionPeriod` | 0 |
| *redshift* | `GetClusterCredentials` | 0 |
| *redshift* | `DeleteSnapshotCopyGrant` | 0 |
| *redshift* | `DescribeReservedNodes` | 0 |
| *redshift* | `AuthorizeSnapshotAccess` | 0 |
| *redshift* | `EnableLogging` | 0 |
| *redshift* | `ModifyClusterSubnetGroup` | 0 |
| *redshift* | `DisableSnapshotCopy` | 0 |
| *redshift* | `DeleteHsmClientCertificate` | 0 |
| *redshift* | `ModifyClusterSnapshotSchedule` | 0 |
| *redshift* | `DescribeClusterDbRevisions` | 0 |
| *redshift* | `CreateHsmConfiguration` | 0 |
| *redshift* | `DeleteClusterParameterGroup` | 0 |
| *redshift* | `DescribeClusters` | 0 |
| *redshift* | `ModifyClusterIamRoles` | 0 |
| *redshift* | `ResetClusterParameterGroup` | 0 |
| *redshift* | `DescribeTableRestoreStatus` | 0 |
| *redshift* | `DeleteClusterSecurityGroup` | 0 |
| *redshift* | `ModifyClusterParameterGroup` | 0 |
| *redshift* | `DescribeClusterSnapshots` | 0 |
| *redshift* | `BatchDeleteClusterSnapshots` | 0 |
| *redshift* | `DisableLogging` | 0 |
| *redshift* | `RebootCluster` | 0 |
| *redshift* | `DescribeClusterVersions` | 0 |
| *redshift* | `CreateSnapshotSchedule` | 0 |
| *redshift* | `DescribeClusterSubnetGroups` | 1 |
| *redshift* | `AuthorizeClusterSecurityGroupIngress` | 0 |
| *redshift* | `DescribeHsmConfigurations` | 0 |
| *redshift* | `DeleteEventSubscription` | 0 |
| *redshift* | `DescribeTags` | 0 |
| *redshift* | `DeleteHsmConfiguration` | 0 |
| *redshift* | `CreateClusterSubnetGroup` | 1 |
| *redshift* | `ResizeCluster` | 0 |
| *redshift* | `ModifyClusterDbRevision` | 0 |
| *redshift* | `DescribeClusterParameters` | 0 |
| *redshift* | `ModifyClusterSnapshot` | 0 |
| *redshift* | `CreateHsmClientCertificate` | 0 |
| *redshift* | `DescribeStorage` | 0 |
| *redshift* | `DescribeSnapshotCopyGrants` | 0 |
| *redshift* | `AcceptReservedNodeExchange` | 0 |
| *redshift* | `DescribeOrderableClusterOptions` | 0 |
| *redshift* | `ModifyCluster` | 0 |
| *redshift* | `DeleteSnapshotSchedule` | 0 |
| *redshift* | `EnableSnapshotCopy` | 0 |
| *redshift* | `DescribeSnapshotSchedules` | 0 |
| *redshift* | `CreateClusterSnapshot` | 0 |
| *redshift* | `DeleteClusterSnapshot` | 0 |
| *redshift* | `CreateTags` | 0 |
| *redshift* | `DescribeClusterTracks` | 0 |
| *redshift* | `DescribeReservedNodeOfferings` | 0 |
| *redshift* | `DescribeLoggingStatus` | 0 |
| *redshift* | `BatchModifyClusterSnapshots` | 0 |
| *redshift* | `ModifyEventSubscription` | 0 |
| *redshift* | `RestoreTableFromClusterSnapshot` | 0 |
| *redshift* | `DescribeClusterSecurityGroups` | 0 |
| *redshift* | `DescribeAccountAttributes` | 0 |
| *redshift* | `RevokeClusterSecurityGroupIngress` | 0 |
| *redshift* | `CreateClusterParameterGroup` | 1 |
| *redshift* | `CancelResize` | 0 |
| *redshift* | `DeleteClusterSubnetGroup` | 0 |
| *redshift* | `PurchaseReservedNodeOffering` | 0 |
| *redshift* | `ModifyClusterMaintenance` | 0 |
| *redshift* | `DescribeNodeConfigurationOptions` | 0 |
| *rekognition* | `DeleteStreamProcessor` | 0 |
| *rekognition* | `IndexFaces` | 0 |
| *rekognition* | `GetFaceSearch` | 0 |
| *rekognition* | `GetCelebrityInfo` | 0 |
| *rekognition* | `DetectText` | 0 |
| *rekognition* | `RecognizeCelebrities` | 0 |
| *rekognition* | `DeleteFaces` | 0 |
| *rekognition* | `StartPersonTracking` | 0 |
| *rekognition* | `SearchFacesByImage` | 0 |
| *rekognition* | `GetLabelDetection` | 0 |
| *rekognition* | `GetPersonTracking` | 0 |
| *rekognition* | `CreateStreamProcessor` | 0 |
| *rekognition* | `GetCelebrityRecognition` | 0 |
| *rekognition* | `StartLabelDetection` | 0 |
| *rekognition* | `DetectLabels` | 0 |
| *rekognition* | `DetectFaces` | 0 |
| *rekognition* | `DeleteCollection` | 0 |
| *rekognition* | `ListFaces` | 0 |
| *rekognition* | `SearchFaces` | 0 |
| *rekognition* | `GetContentModeration` | 0 |
| *rekognition* | `StopStreamProcessor` | 0 |
| *rekognition* | `GetFaceDetection` | 0 |
| *rekognition* | `ListCollections` | 0 |
| *rekognition* | `StartStreamProcessor` | 0 |
| *rekognition* | `CreateCollection` | 0 |
| *rekognition* | `CompareFaces` | 0 |
| *rekognition* | `DescribeCollection` | 0 |
| *rekognition* | `StartCelebrityRecognition` | 0 |
| *rekognition* | `DetectModerationLabels` | 0 |
| *rekognition* | `ListStreamProcessors` | 0 |
| *rekognition* | `StartFaceSearch` | 0 |
| *rekognition* | `StartContentModeration` | 0 |
| *rekognition* | `DescribeStreamProcessor` | 0 |
| *rekognition* | `StartFaceDetection` | 0 |
| *resource-groups* | `GetTags` | 0 |
| *resource-groups* | `ListGroups` | 0 |
| *resource-groups* | `UpdateGroupQuery` | 0 |
| *resource-groups* | `ListGroupResources` | 0 |
| *resource-groups* | `UpdateGroup` | 0 |
| *resource-groups* | `DeleteGroup` | 0 |
| *resource-groups* | `Tag` | 0 |
| *resource-groups* | `CreateGroup` | 1 |
| *resource-groups* | `GetGroupQuery` | 0 |
| *resource-groups* | `SearchResources` | 0 |
| *resource-groups* | `GetGroup` | 0 |
| *resource-groups* | `Untag` | 0 |
| *resourcegroupstaggingapi* | `GetTagKeys` | 0 |
| *resourcegroupstaggingapi* | `TagResources` | 0 |
| *resourcegroupstaggingapi* | `UntagResources` | 0 |
| *resourcegroupstaggingapi* | `GetTagValues` | 0 |
| *resourcegroupstaggingapi* | `GetResources` | 0 |
| *robomaker* | `ListRobots` | 1 |
| *robomaker* | `DeleteFleet` | 1 |
| *robomaker* | `SyncDeploymentJob` | 0 |
| *robomaker* | `DeregisterRobot` | 0 |
| *robomaker* | `RegisterRobot` | 0 |
| *robomaker* | `ListSimulationJobs` | 1 |
| *robomaker* | `ListFleets` | 1 |
| *robomaker* | `DescribeFleet` | 1 |
| *robomaker* | `CancelSimulationJob` | 0 |
| *robomaker* | `CreateRobotApplication` | 1 |
| *robomaker* | `CreateDeploymentJob` | 0 |
| *robomaker* | `ListRobotApplications` | 1 |
| *robomaker* | `CreateSimulationApplication` | 1 |
| *robomaker* | `CancelDeploymentJob` | 0 |
| *robomaker* | `DeleteRobot` | 0 |
| *robomaker* | `DescribeDeploymentJob` | 0 |
| *robomaker* | `DescribeSimulationApplication` | 1 |
| *robomaker* | `TagResource` | 0 |
| *robomaker* | `CreateRobot` | 1 |
| *robomaker* | `DeleteRobotApplication` | 2 |
| *robomaker* | `CreateFleet` | 1 |
| *robomaker* | `CreateSimulationJob` | 0 |
| *robomaker* | `BatchDescribeSimulationJob` | 0 |
| *robomaker* | `CreateSimulationApplicationVersion` | 1 |
| *robomaker* | `ListDeploymentJobs` | 0 |
| *robomaker* | `UpdateRobotApplication` | 0 |
| *robomaker* | `UntagResource` | 0 |
| *robomaker* | `ListSimulationApplications` | 1 |
| *robomaker* | `DescribeRobotApplication` | 2 |
| *robomaker* | `UpdateSimulationApplication` | 0 |
| *robomaker* | `DescribeRobot` | 2 |
| *robomaker* | `CreateRobotApplicationVersion` | 1 |
| *robomaker* | `DeleteSimulationApplication` | 2 |
| *robomaker* | `DescribeSimulationJob` | 0 |
| *robomaker* | `ListTagsForResource` | 0 |
| *robomaker* | `RestartSimulationJob` | 0 |
| *route53* | `CreateQueryLoggingConfig` | 0 |
| *route53* | `ListTrafficPolicyInstancesByHostedZone` | 0 |
| *route53* | `GetCheckerIpRanges` | 0 |
| *route53* | `ListTrafficPolicyInstancesByPolicy` | 0 |
| *route53* | `ListTrafficPolicies` | 0 |
| *route53* | `UpdateHostedZoneComment` | 0 |
| *route53* | `ChangeTagsForResource` | 0 |
| *route53* | `GetHealthCheck` | 0 |
| *route53* | `ListHostedZonesByName` | 0 |
| *route53* | `GetHostedZone` | 0 |
| *route53* | `DeleteReusableDelegationSet` | 0 |
| *route53* | `DeleteTrafficPolicyInstance` | 0 |
| *route53* | `GetReusableDelegationSetLimit` | 0 |
| *route53* | `GetHostedZoneCount` | 1 |
| *route53* | `GetTrafficPolicyInstanceCount` | 0 |
| *route53* | `UpdateTrafficPolicyComment` | 0 |
| *route53* | `ChangeResourceRecordSets` | 1 |
| *route53* | `DeleteTrafficPolicy` | 0 |
| *route53* | `CreateHostedZone` | 2 |
| *route53* | `CreateVPCAssociationAuthorization` | 0 |
| *route53* | `DeleteHostedZone` | 1 |
| *route53* | `GetHealthCheckStatus` | 0 |
| *route53* | `TestDNSAnswer` | 0 |
| *route53* | `ListReusableDelegationSets` | 0 |
| *route53* | `GetAccountLimit` | 0 |
| *route53* | `CreateTrafficPolicyInstance` | 0 |
| *route53* | `GetGeoLocation` | 0 |
| *route53* | `GetReusableDelegationSet` | 0 |
| *route53* | `ListHostedZones` | 1 |
| *route53* | `GetTrafficPolicy` | 0 |
| *route53* | `ListQueryLoggingConfigs` | 0 |
| *route53* | `UpdateHealthCheck` | 0 |
| *route53* | `GetHealthCheckCount` | 0 |
| *route53* | `DisassociateVPCFromHostedZone` | 0 |
| *route53* | `DeleteVPCAssociationAuthorization` | 0 |
| *route53* | `GetTrafficPolicyInstance` | 0 |
| *route53* | `DeleteHealthCheck` | 1 |
| *route53* | `CreateHealthCheck` | 1 |
| *route53* | `CreateTrafficPolicyVersion` | 0 |
| *route53* | `CreateTrafficPolicy` | 0 |
| *route53* | `ListGeoLocations` | 1 |
| *route53* | `ListTagsForResources` | 0 |
| *route53* | `GetQueryLoggingConfig` | 0 |
| *route53* | `GetHostedZoneLimit` | 0 |
| *route53* | `AssociateVPCWithHostedZone` | 0 |
| *route53* | `ListTrafficPolicyInstances` | 0 |
| *route53* | `ListVPCAssociationAuthorizations` | 0 |
| *route53* | `GetHealthCheckLastFailureReason` | 0 |
| *route53* | `GetChange` | 0 |
| *route53* | `UpdateTrafficPolicyInstance` | 0 |
| *route53* | `CreateReusableDelegationSet` | 0 |
| *route53* | `DeleteQueryLoggingConfig` | 0 |
| *route53* | `ListResourceRecordSets` | 0 |
| *route53* | `ListTagsForResource` | 0 |
| *route53* | `ListHealthChecks` | 1 |
| *route53* | `ListTrafficPolicyVersions` | 0 |
| *route53domains* | `CheckDomainTransferability` | 0 |
| *route53domains* | `ResendContactReachabilityEmail` | 0 |
| *route53domains* | `ListDomains` | 0 |
| *route53domains* | `DisableDomainTransferLock` | 0 |
| *route53domains* | `RetrieveDomainAuthCode` | 0 |
| *route53domains* | `ViewBilling` | 0 |
| *route53domains* | `UpdateDomainNameservers` | 0 |
| *route53domains* | `UpdateTagsForDomain` | 0 |
| *route53domains* | `CheckDomainAvailability` | 0 |
| *route53domains* | `GetDomainSuggestions` | 0 |
| *route53domains* | `GetOperationDetail` | 0 |
| *route53domains* | `RenewDomain` | 0 |
| *route53domains* | `TransferDomain` | 0 |
| *route53domains* | `DisableDomainAutoRenew` | 0 |
| *route53domains* | `DeleteTagsForDomain` | 0 |
| *route53domains* | `RegisterDomain` | 0 |
| *route53domains* | `GetDomainDetail` | 0 |
| *route53domains* | `EnableDomainAutoRenew` | 0 |
| *route53domains* | `UpdateDomainContactPrivacy` | 0 |
| *route53domains* | `ListTagsForDomain` | 0 |
| *route53domains* | `GetContactReachabilityStatus` | 0 |
| *route53domains* | `UpdateDomainContact` | 0 |
| *route53domains* | `ListOperations` | 0 |
| *route53domains* | `EnableDomainTransferLock` | 0 |
| *route53resolver* | `ListResolverEndpoints` | 1 |
| *route53resolver* | `PutResolverRulePolicy` | 0 |
| *route53resolver* | `GetResolverRuleAssociation` | 0 |
| *route53resolver* | `GetResolverEndpoint` | 0 |
| *route53resolver* | `ListResolverRules` | 2 |
| *route53resolver* | `DisassociateResolverRule` | 0 |
| *route53resolver* | `GetResolverRule` | 0 |
| *route53resolver* | `UpdateResolverEndpoint` | 0 |
| *route53resolver* | `AssociateResolverEndpointIpAddress` | 0 |
| *route53resolver* | `TagResource` | 0 |
| *route53resolver* | `CreateResolverEndpoint` | 1 |
| *route53resolver* | `UpdateResolverRule` | 1 |
| *route53resolver* | `ListResolverRuleAssociations` | 1 |
| *route53resolver* | `DisassociateResolverEndpointIpAddress` | 0 |
| *route53resolver* | `GetResolverRulePolicy` | 0 |
| *route53resolver* | `ListResolverEndpointIpAddresses` | 0 |
| *route53resolver* | `AssociateResolverRule` | 1 |
| *route53resolver* | `DeleteResolverEndpoint` | 1 |
| *route53resolver* | `UntagResource` | 0 |
| *route53resolver* | `CreateResolverRule` | 1 |
| *route53resolver* | `DeleteResolverRule` | 1 |
| *route53resolver* | `ListTagsForResource` | 1 |
| *s3* | `PutBucketWebsite` | 0 |
| *s3* | `UploadPart` | 0 |
| *s3* | `GetBucketCors` | 0 |
| *s3* | `DeleteBucketWebsite` | 0 |
| *s3* | `GetBucketNotificationConfiguration` | 1 |
| *s3* | `GetObjectTagging` | 0 |
| *s3* | `GetObjectRetention` | 0 |
| *s3* | `GetBucketAnalyticsConfiguration` | 1 |
| *s3* | `DeleteObjectTagging` | 0 |
| *s3* | `PutObjectRetention` | 0 |
| *s3* | `CreateBucket` | 3 |
| *s3* | `PutObject` | 0 |
| *s3* | `DeletePublicAccessBlock` | 0 |
| *s3* | `PutBucketTagging` | 1 |
| *s3* | `GetBucketInventoryConfiguration` | 0 |
| *s3* | `GetBucketNotification` | 0 |
| *s3* | `ListBucketAnalyticsConfigurations` | 0 |
| *s3* | `GetObjectAcl` | 0 |
| *s3* | `PutBucketMetricsConfiguration` | 1 |
| *s3* | `ListBucketInventoryConfigurations` | 0 |
| *s3* | `PutBucketRequestPayment` | 0 |
| *s3* | `PutBucketLogging` | 1 |
| *s3* | `PutBucketAccelerateConfiguration` | 0 |
| *s3* | `DeleteBucketCors` | 0 |
| *s3* | `PutBucketVersioning` | 1 |
| *s3* | `CopyObject` | 0 |
| *s3* | `DeleteBucketMetricsConfiguration` | 0 |
| *s3* | `HeadBucket` | 2 |
| *s3* | `DeleteBucketPolicy` | 0 |
| *s3* | `DeleteObjects` | 0 |
| *s3* | `DeleteBucketReplication` | 0 |
| *s3* | `PutBucketNotificationConfiguration` | 0 |
| *s3* | `GetObjectTorrent` | 0 |
| *s3* | `PutBucketPolicy` | 1 |
| *s3* | `GetBucketWebsite` | 1 |
| *s3* | `CreateMultipartUpload` | 0 |
| *s3* | `DeleteBucket` | 1 |
| *s3* | `GetObjectLockConfiguration` | 1 |
| *s3* | `ListMultipartUploads` | 0 |
| *s3* | `PutPublicAccessBlock` | 0 |
| *s3* | `GetBucketLocation` | 0 |
| *s3* | `PutBucketCors` | 0 |
| *s3* | `UploadPartCopy` | 0 |
| *s3* | `DeleteBucketAnalyticsConfiguration` | 0 |
| *s3* | `DeleteObject` | 0 |
| *s3* | `RestoreObject` | 0 |
| *s3* | `GetBucketEncryption` | 1 |
| *s3* | `ListObjects` | 1 |
| *s3* | `GetBucketMetricsConfiguration` | 1 |
| *s3* | `PutBucketNotification` | 0 |
| *s3* | `PutBucketLifecycle` | 0 |
| *s3* | `GetBucketAcl` | 1 |
| *s3* | `GetBucketLogging` | 1 |
| *s3* | `PutBucketAcl` | 1 |
| *s3* | `PutBucketReplication` | 0 |
| *s3* | `GetBucketReplication` | 1 |
| *s3* | `GetBucketAccelerateConfiguration` | 1 |
| *s3* | `DeleteBucketTagging` | 0 |
| *s3* | `GetBucketLifecycle` | 0 |
| *s3* | `CompleteMultipartUpload` | 0 |
| *s3* | `GetBucketPolicyStatus` | 0 |
| *s3* | `PutBucketEncryption` | 0 |
| *s3* | `PutObjectLegalHold` | 0 |
| *s3* | `GetBucketRequestPayment` | 1 |
| *s3* | `DeleteBucketLifecycle` | 0 |
| *s3* | `GetObjectLegalHold` | 0 |
| *s3* | `ListObjectVersions` | 0 |
| *s3* | `SelectObjectContent` | 0 |
| *s3* | `PutBucketLifecycleConfiguration` | 0 |
| *s3* | `ListParts` | 0 |
| *s3* | `DeleteBucketEncryption` | 0 |
| *s3* | `PutBucketAnalyticsConfiguration` | 0 |
| *s3* | `PutObjectAcl` | 0 |
| *s3* | `PutBucketInventoryConfiguration` | 0 |
| *s3* | `ListBucketMetricsConfigurations` | 0 |
| *s3* | `DeleteBucketInventoryConfiguration` | 0 |
| *s3* | `GetObject` | 0 |
| *s3* | `HeadObject` | 0 |
| *s3* | `GetBucketTagging` | 1 |
| *s3* | `PutObjectLockConfiguration` | 0 |
| *s3* | `GetBucketPolicy` | 1 |
| *s3* | `GetBucketVersioning` | 1 |
| *s3* | `GetBucketLifecycleConfiguration` | 1 |
| *s3* | `ListBuckets` | 7 |
| *s3* | `AbortMultipartUpload` | 0 |
| *s3* | `ListObjectsV2` | 1 |
| *s3* | `GetPublicAccessBlock` | 0 |
| *s3* | `PutObjectTagging` | 0 |
| *s3control* | `ListJobs` | 0 |
| *s3control* | `UpdateJobStatus` | 0 |
| *s3control* | `CreateJob` | 0 |
| *s3control* | `PutPublicAccessBlock` | 1 |
| *s3control* | `DeletePublicAccessBlock` | 0 |
| *s3control* | `GetPublicAccessBlock` | 0 |
| *s3control* | `UpdateJobPriority` | 0 |
| *s3control* | `DescribeJob` | 0 |
| *sagemaker* | `DeleteTags` | 0 |
| *sagemaker* | `ListEndpointConfigs` | 1 |
| *sagemaker* | `DeleteNotebookInstanceLifecycleConfig` | 1 |
| *sagemaker* | `UpdateWorkteam` | 0 |
| *sagemaker* | `DescribeLabelingJob` | 0 |
| *sagemaker* | `ListModelPackages` | 0 |
| *sagemaker* | `CreateLabelingJob` | 0 |
| *sagemaker* | `UpdateNotebookInstance` | 0 |
| *sagemaker* | `DeleteEndpointConfig` | 1 |
| *sagemaker* | `ListLabelingJobs` | 0 |
| *sagemaker* | `ListHyperParameterTuningJobs` | 0 |
| *sagemaker* | `CreateTransformJob` | 0 |
| *sagemaker* | `CreateCompilationJob` | 0 |
| *sagemaker* | `CreateEndpointConfig` | 1 |
| *sagemaker* | `ListTrainingJobs` | 0 |
| *sagemaker* | `StartNotebookInstance` | 0 |
| *sagemaker* | `DeleteNotebookInstance` | 1 |
| *sagemaker* | `CreateWorkteam` | 0 |
| *sagemaker* | `ListTransformJobs` | 0 |
| *sagemaker* | `CreateAlgorithm` | 0 |
| *sagemaker* | `CreateNotebookInstanceLifecycleConfig` | 1 |
| *sagemaker* | `ListTrainingJobsForHyperParameterTuningJob` | 0 |
| *sagemaker* | `DescribeCompilationJob` | 0 |
| *sagemaker* | `UpdateNotebookInstanceLifecycleConfig` | 0 |
| *sagemaker* | `DescribeAlgorithm` | 0 |
| *sagemaker* | `ListSubscribedWorkteams` | 0 |
| *sagemaker* | `ListWorkteams` | 0 |
| *sagemaker* | `CreatePresignedNotebookInstanceUrl` | 0 |
| *sagemaker* | `ListNotebookInstances` | 1 |
| *sagemaker* | `ListTags` | 0 |
| *sagemaker* | `GetSearchSuggestions` | 0 |
| *sagemaker* | `ListAlgorithms` | 0 |
| *sagemaker* | `CreateEndpoint` | 1 |
| *sagemaker* | `CreateCodeRepository` | 0 |
| *sagemaker* | `DescribeEndpoint` | 0 |
| *sagemaker* | `Search` | 0 |
| *sagemaker* | `ListNotebookInstanceLifecycleConfigs` | 1 |
| *sagemaker* | `DescribeTrainingJob` | 0 |
| *sagemaker* | `StopNotebookInstance` | 1 |
| *sagemaker* | `DescribeHyperParameterTuningJob` | 0 |
| *sagemaker* | `DescribeTransformJob` | 0 |
| *sagemaker* | `ListModels` | 1 |
| *sagemaker* | `ListLabelingJobsForWorkteam` | 0 |
| *sagemaker* | `ListCompilationJobs` | 0 |
| *sagemaker* | `DescribeModelPackage` | 0 |
| *sagemaker* | `CreateNotebookInstance` | 1 |
| *sagemaker* | `DeleteAlgorithm` | 0 |
| *sagemaker* | `DeleteEndpoint` | 1 |
| *sagemaker* | `DescribeCodeRepository` | 0 |
| *sagemaker* | `DescribeEndpointConfig` | 0 |
| *sagemaker* | `RenderUiTemplate` | 0 |
| *sagemaker* | `DescribeSubscribedWorkteam` | 0 |
| *sagemaker* | `CreateModel` | 1 |
| *sagemaker* | `DeleteCodeRepository` | 0 |
| *sagemaker* | `DescribeWorkteam` | 0 |
| *sagemaker* | `CreateHyperParameterTuningJob` | 0 |
| *sagemaker* | `AddTags` | 0 |
| *sagemaker* | `DescribeNotebookInstanceLifecycleConfig` | 0 |
| *sagemaker* | `CreateModelPackage` | 0 |
| *sagemaker* | `ListEndpoints` | 1 |
| *sagemaker* | `DeleteModelPackage` | 0 |
| *sagemaker* | `DeleteWorkteam` | 0 |
| *sagemaker* | `DescribeModel` | 0 |
| *sagemaker* | `DescribeNotebookInstance` | 0 |
| *sagemaker* | `StopHyperParameterTuningJob` | 0 |
| *sagemaker* | `UpdateCodeRepository` | 0 |
| *sagemaker* | `UpdateEndpoint` | 0 |
| *sagemaker* | `DeleteModel` | 1 |
| *sagemaker* | `CreateTrainingJob` | 0 |
| *sagemaker* | `UpdateEndpointWeightsAndCapacities` | 0 |
| *sagemaker* | `StopLabelingJob` | 0 |
| *sagemaker* | `StopCompilationJob` | 0 |
| *sagemaker* | `StopTrainingJob` | 0 |
| *sagemaker* | `StopTransformJob` | 0 |
| *sagemaker* | `ListCodeRepositories` | 0 |
| *sagemaker-runtime* | `InvokeEndpoint` | 0 |
| *sdb* | `DeleteAttributes` | 0 |
| *sdb* | `BatchPutAttributes` | 0 |
| *sdb* | `PutAttributes` | 0 |
| *sdb* | `ListDomains` | 0 |
| *sdb* | `DomainMetadata` | 0 |
| *sdb* | `BatchDeleteAttributes` | 0 |
| *sdb* | `GetAttributes` | 0 |
| *sdb* | `DeleteDomain` | 0 |
| *sdb* | `CreateDomain` | 0 |
| *sdb* | `Select` | 0 |
| *secretsmanager* | `PutSecretValue` | 0 |
| *secretsmanager* | `ListSecretVersionIds` | 0 |
| *secretsmanager* | `RestoreSecret` | 0 |
| *secretsmanager* | `CreateSecret` | 1 |
| *secretsmanager* | `DeleteSecret` | 1 |
| *secretsmanager* | `UntagResource` | 0 |
| *secretsmanager* | `GetRandomPassword` | 0 |
| *secretsmanager* | `PutResourcePolicy` | 0 |
| *secretsmanager* | `DeleteResourcePolicy` | 0 |
| *secretsmanager* | `ListSecrets` | 1 |
| *secretsmanager* | `CancelRotateSecret` | 0 |
| *secretsmanager* | `GetResourcePolicy` | 0 |
| *secretsmanager* | `DescribeSecret` | 1 |
| *secretsmanager* | `UpdateSecret` | 0 |
| *secretsmanager* | `TagResource` | 0 |
| *secretsmanager* | `UpdateSecretVersionStage` | 0 |
| *secretsmanager* | `RotateSecret` | 1 |
| *secretsmanager* | `GetSecretValue` | 1 |
| *securityhub* | `GetFindings` | 1 |
| *securityhub* | `GetInsightResults` | 0 |
| *securityhub* | `DeleteInsight` | 0 |
| *securityhub* | `CreateActionTarget` | 0 |
| *securityhub* | `ListInvitations` | 1 |
| *securityhub* | `GetInvitationsCount` | 0 |
| *securityhub* | `DisassociateMembers` | 0 |
| *securityhub* | `DisableImportFindingsForProduct` | 0 |
| *securityhub* | `UpdateInsight` | 0 |
| *securityhub* | `GetMasterAccount` | 1 |
| *securityhub* | `DescribeActionTargets` | 0 |
| *securityhub* | `DescribeProducts` | 0 |
| *securityhub* | `BatchImportFindings` | 0 |
| *securityhub* | `GetMembers` | 0 |
| *securityhub* | `ListEnabledProductsForImport` | 1 |
| *securityhub* | `GetEnabledStandards` | 0 |
| *securityhub* | `UpdateActionTarget` | 0 |
| *securityhub* | `ListMembers` | 1 |
| *securityhub* | `DeleteActionTarget` | 0 |
| *securityhub* | `DeleteMembers` | 0 |
| *securityhub* | `GetInsights` | 1 |
| *securityhub* | `TagResource` | 0 |
| *securityhub* | `EnableSecurityHub` | 1 |
| *securityhub* | `DisassociateFromMasterAccount` | 0 |
| *securityhub* | `BatchEnableStandards` | 1 |
| *securityhub* | `AcceptInvitation` | 0 |
| *securityhub* | `CreateMembers` | 0 |
| *securityhub* | `UpdateFindings` | 0 |
| *securityhub* | `BatchDisableStandards` | 0 |
| *securityhub* | `EnableImportFindingsForProduct` | 1 |
| *securityhub* | `DeclineInvitations` | 0 |
| *securityhub* | `CreateInsight` | 0 |
| *securityhub* | `UntagResource` | 0 |
| *securityhub* | `InviteMembers` | 0 |
| *securityhub* | `DescribeHub` | 0 |
| *securityhub* | `DeleteInvitations` | 0 |
| *securityhub* | `DisableSecurityHub` | 1 |
| *securityhub* | `ListTagsForResource` | 0 |
| *serverlessrepo* | `UpdateApplication` | 0 |
| *serverlessrepo* | `ListApplications` | 0 |
| *serverlessrepo* | `PutApplicationPolicy` | 0 |
| *serverlessrepo* | `GetApplication` | 0 |
| *serverlessrepo* | `CreateCloudFormationChangeSet` | 1 |
| *serverlessrepo* | `ListApplicationDependencies` | 0 |
| *serverlessrepo* | `GetCloudFormationTemplate` | 0 |
| *serverlessrepo* | `GetApplicationPolicy` | 0 |
| *serverlessrepo* | `CreateCloudFormationTemplate` | 0 |
| *serverlessrepo* | `DeleteApplication` | 0 |
| *serverlessrepo* | `CreateApplicationVersion` | 0 |
| *serverlessrepo* | `ListApplicationVersions` | 0 |
| *serverlessrepo* | `CreateApplication` | 0 |
| *service-quotas* | `GetRequestedServiceQuotaChange` | 0 |
| *service-quotas* | `DeleteServiceQuotaIncreaseRequestFromTemplate` | 0 |
| *service-quotas* | `DisassociateServiceQuotaTemplate` | 0 |
| *service-quotas* | `ListServices` | 0 |
| *service-quotas* | `ListRequestedServiceQuotaChangeHistoryByQuota` | 0 |
| *service-quotas* | `AssociateServiceQuotaTemplate` | 0 |
| *service-quotas* | `RequestServiceQuotaIncrease` | 0 |
| *service-quotas* | `PutServiceQuotaIncreaseRequestIntoTemplate` | 0 |
| *service-quotas* | `GetAssociationForServiceQuotaTemplate` | 0 |
| *service-quotas* | `ListRequestedServiceQuotaChangeHistory` | 0 |
| *service-quotas* | `ListServiceQuotaIncreaseRequestsInTemplate` | 0 |
| *service-quotas* | `ListAWSDefaultServiceQuotas` | 0 |
| *service-quotas* | `GetAWSDefaultServiceQuota` | 0 |
| *service-quotas* | `GetServiceQuota` | 0 |
| *service-quotas* | `GetServiceQuotaIncreaseRequestFromTemplate` | 0 |
| *service-quotas* | `ListServiceQuotas` | 0 |
| *servicecatalog* | `ExecuteProvisionedProductPlan` | 0 |
| *servicecatalog* | `CreateConstraint` | 1 |
| *servicecatalog* | `DescribeProductAsAdmin` | 0 |
| *servicecatalog* | `AssociateProductWithPortfolio` | 1 |
| *servicecatalog* | `ListPrincipalsForPortfolio` | 0 |
| *servicecatalog* | `ListPortfoliosForProduct` | 0 |
| *servicecatalog* | `DeleteTagOption` | 0 |
| *servicecatalog* | `AssociatePrincipalWithPortfolio` | 1 |
| *servicecatalog* | `ListRecordHistory` | 0 |
| *servicecatalog* | `UpdatePortfolio` | 0 |
| *servicecatalog* | `UpdateProduct` | 0 |
| *servicecatalog* | `SearchProductsAsAdmin` | 0 |
| *servicecatalog* | `ListProvisionedProductPlans` | 0 |
| *servicecatalog* | `CreateProvisioningArtifact` | 0 |
| *servicecatalog* | `DescribePortfolioShareStatus` | 0 |
| *servicecatalog* | `ListPortfolios` | 1 |
| *servicecatalog* | `SearchProvisionedProducts` | 0 |
| *servicecatalog* | `DescribeProduct` | 0 |
| *servicecatalog* | `CreatePortfolioShare` | 1 |
| *servicecatalog* | `ListTagOptions` | 1 |
| *servicecatalog* | `DisassociateServiceActionFromProvisioningArtifact` | 0 |
| *servicecatalog* | `BatchAssociateServiceActionWithProvisioningArtifact` | 0 |
| *servicecatalog* | `ListLaunchPaths` | 0 |
| *servicecatalog* | `DisassociatePrincipalFromPortfolio` | 0 |
| *servicecatalog* | `ListServiceActionsForProvisioningArtifact` | 0 |
| *servicecatalog* | `CreateProduct` | 1 |
| *servicecatalog* | `ListAcceptedPortfolioShares` | 0 |
| *servicecatalog* | `ListBudgetsForResource` | 0 |
| *servicecatalog* | `DescribeServiceAction` | 0 |
| *servicecatalog* | `UpdateConstraint` | 0 |
| *servicecatalog* | `AssociateServiceActionWithProvisioningArtifact` | 0 |
| *servicecatalog* | `DeleteProvisionedProductPlan` | 0 |
| *servicecatalog* | `CreateServiceAction` | 0 |
| *servicecatalog* | `ListProvisioningArtifacts` | 0 |
| *servicecatalog* | `ListProvisioningArtifactsForServiceAction` | 0 |
| *servicecatalog* | `RejectPortfolioShare` | 0 |
| *servicecatalog* | `DisassociateProductFromPortfolio` | 0 |
| *servicecatalog* | `AcceptPortfolioShare` | 1 |
| *servicecatalog* | `DescribeProvisionedProduct` | 0 |
| *servicecatalog* | `UpdateProvisionedProductProperties` | 0 |
| *servicecatalog* | `ListServiceActions` | 0 |
| *servicecatalog* | `DeleteServiceAction` | 0 |
| *servicecatalog* | `UpdateTagOption` | 0 |
| *servicecatalog* | `DisableAWSOrganizationsAccess` | 0 |
| *servicecatalog* | `DeleteProvisioningArtifact` | 0 |
| *servicecatalog* | `UpdateProvisioningArtifact` | 0 |
| *servicecatalog* | `DescribeConstraint` | 0 |
| *servicecatalog* | `ScanProvisionedProducts` | 0 |
| *servicecatalog* | `BatchDisassociateServiceActionFromProvisioningArtifact` | 0 |
| *servicecatalog* | `DescribeProvisionedProductPlan` | 0 |
| *servicecatalog* | `DeletePortfolio` | 0 |
| *servicecatalog* | `DescribeRecord` | 0 |
| *servicecatalog* | `AssociateTagOptionWithResource` | 1 |
| *servicecatalog* | `DescribeCopyProductStatus` | 0 |
| *servicecatalog* | `DeleteProduct` | 0 |
| *servicecatalog* | `EnableAWSOrganizationsAccess` | 0 |
| *servicecatalog* | `AssociateBudgetWithResource` | 0 |
| *servicecatalog* | `DescribeProductView` | 0 |
| *servicecatalog* | `TerminateProvisionedProduct` | 0 |
| *servicecatalog* | `DescribePortfolio` | 0 |
| *servicecatalog* | `ListOrganizationPortfolioAccess` | 0 |
| *servicecatalog* | `DeletePortfolioShare` | 0 |
| *servicecatalog* | `GetAWSOrganizationsAccessStatus` | 0 |
| *servicecatalog* | `UpdateServiceAction` | 0 |
| *servicecatalog* | `CreateProvisionedProductPlan` | 0 |
| *servicecatalog* | `DescribeServiceActionExecutionParameters` | 0 |
| *servicecatalog* | `DisassociateBudgetFromResource` | 0 |
| *servicecatalog* | `ListStackInstancesForProvisionedProduct` | 0 |
| *servicecatalog* | `CreateTagOption` | 1 |
| *servicecatalog* | `UpdateProvisionedProduct` | 0 |
| *servicecatalog* | `ListResourcesForTagOption` | 0 |
| *servicecatalog* | `DescribeProvisioningArtifact` | 0 |
| *servicecatalog* | `ListPortfolioAccess` | 0 |
| *servicecatalog* | `ListConstraintsForPortfolio` | 0 |
| *servicecatalog* | `DisassociateTagOptionFromResource` | 0 |
| *servicecatalog* | `ExecuteProvisionedProductServiceAction` | 0 |
| *servicecatalog* | `SearchProducts` | 0 |
| *servicecatalog* | `CopyProduct` | 0 |
| *servicecatalog* | `DeleteConstraint` | 1 |
| *servicecatalog* | `ProvisionProduct` | 1 |
| *servicecatalog* | `CreatePortfolio` | 1 |
| *servicecatalog* | `DescribeProvisioningParameters` | 0 |
| *servicecatalog* | `DescribeTagOption` | 1 |
| *servicediscovery* | `CreateService` | 0 |
| *servicediscovery* | `DeregisterInstance` | 0 |
| *servicediscovery* | `ListServices` | 0 |
| *servicediscovery* | `GetInstance` | 0 |
| *servicediscovery* | `CreatePublicDnsNamespace` | 1 |
| *servicediscovery* | `ListNamespaces` | 2 |
| *servicediscovery* | `GetOperation` | 1 |
| *servicediscovery* | `GetNamespace` | 0 |
| *servicediscovery* | `GetService` | 0 |
| *servicediscovery* | `CreatePrivateDnsNamespace` | 1 |
| *servicediscovery* | `UpdateInstanceCustomHealthStatus` | 0 |
| *servicediscovery* | `GetInstancesHealthStatus` | 0 |
| *servicediscovery* | `ListInstances` | 0 |
| *servicediscovery* | `DeleteService` | 0 |
| *servicediscovery* | `UpdateService` | 0 |
| *servicediscovery* | `RegisterInstance` | 0 |
| *servicediscovery* | `CreateHttpNamespace` | 1 |
| *servicediscovery* | `ListOperations` | 0 |
| *servicediscovery* | `DeleteNamespace` | 1 |
| *servicediscovery* | `DiscoverInstances` | 0 |
| *ses* | `UpdateConfigurationSetTrackingOptions` | 0 |
| *ses* | `CreateReceiptRule` | 1 |
| *ses* | `GetSendStatistics` | 1 |
| *ses* | `ReorderReceiptRuleSet` | 0 |
| *ses* | `GetIdentityPolicies` | 0 |
| *ses* | `GetAccountSendingEnabled` | 0 |
| *ses* | `UpdateConfigurationSetSendingEnabled` | 0 |
| *ses* | `VerifyEmailAddress` | 0 |
| *ses* | `SetIdentityMailFromDomain` | 0 |
| *ses* | `DeleteConfigurationSet` | 1 |
| *ses* | `ListReceiptFilters` | 0 |
| *ses* | `DescribeReceiptRuleSet` | 0 |
| *ses* | `DeleteReceiptFilter` | 0 |
| *ses* | `UpdateConfigurationSetEventDestination` | 0 |
| *ses* | `SetIdentityFeedbackForwardingEnabled` | 0 |
| *ses* | `UpdateReceiptRule` | 0 |
| *ses* | `SendBounce` | 0 |
| *ses* | `CreateConfigurationSetEventDestination` | 1 |
| *ses* | `GetIdentityDkimAttributes` | 0 |
| *ses* | `GetIdentityMailFromDomainAttributes` | 0 |
| *ses* | `DeleteReceiptRule` | 0 |
| *ses* | `DescribeConfigurationSet` | 1 |
| *ses* | `ListVerifiedEmailAddresses` | 0 |
| *ses* | `DeleteTemplate` | 0 |
| *ses* | `CloneReceiptRuleSet` | 0 |
| *ses* | `SendCustomVerificationEmail` | 0 |
| *ses* | `TestRenderTemplate` | 0 |
| *ses* | `VerifyDomainIdentity` | 0 |
| *ses* | `UpdateCustomVerificationEmailTemplate` | 0 |
| *ses* | `VerifyEmailIdentity` | 0 |
| *ses* | `DescribeActiveReceiptRuleSet` | 0 |
| *ses* | `DeleteIdentityPolicy` | 0 |
| *ses* | `SetActiveReceiptRuleSet` | 0 |
| *ses* | `PutConfigurationSetDeliveryOptions` | 0 |
| *ses* | `DeleteReceiptRuleSet` | 0 |
| *ses* | `SetIdentityDkimEnabled` | 0 |
| *ses* | `GetTemplate` | 0 |
| *ses* | `ListTemplates` | 0 |
| *ses* | `DescribeReceiptRule` | 0 |
| *ses* | `SendBulkTemplatedEmail` | 0 |
| *ses* | `UpdateConfigurationSetReputationMetricsEnabled` | 0 |
| *ses* | `GetSendQuota` | 1 |
| *ses* | `GetCustomVerificationEmailTemplate` | 0 |
| *ses* | `UpdateAccountSendingEnabled` | 0 |
| *ses* | `SetReceiptRulePosition` | 0 |
| *ses* | `CreateConfigurationSet` | 1 |
| *ses* | `DeleteVerifiedEmailAddress` | 0 |
| *ses* | `ListConfigurationSets` | 1 |
| *ses* | `CreateReceiptFilter` | 1 |
| *ses* | `SendTemplatedEmail` | 0 |
| *ses* | `CreateConfigurationSetTrackingOptions` | 0 |
| *ses* | `ListReceiptRuleSets` | 0 |
| *ses* | `CreateTemplate` | 0 |
| *ses* | `GetIdentityNotificationAttributes` | 0 |
| *ses* | `SetIdentityHeadersInNotificationsEnabled` | 0 |
| *ses* | `SendEmail` | 0 |
| *ses* | `DeleteIdentity` | 0 |
| *ses* | `VerifyDomainDkim` | 0 |
| *ses* | `CreateCustomVerificationEmailTemplate` | 0 |
| *ses* | `ListIdentityPolicies` | 0 |
| *ses* | `DeleteConfigurationSetEventDestination` | 0 |
| *ses* | `DeleteConfigurationSetTrackingOptions` | 0 |
| *ses* | `ListIdentities` | 0 |
| *ses* | `GetIdentityVerificationAttributes` | 0 |
| *ses* | `UpdateTemplate` | 0 |
| *ses* | `ListCustomVerificationEmailTemplates` | 0 |
| *ses* | `DeleteCustomVerificationEmailTemplate` | 0 |
| *ses* | `SetIdentityNotificationTopic` | 0 |
| *ses* | `PutIdentityPolicy` | 0 |
| *ses* | `SendRawEmail` | 0 |
| *ses* | `CreateReceiptRuleSet` | 1 |
| *shield* | `DisassociateDRTRole` | 0 |
| *shield* | `DeleteProtection` | 0 |
| *shield* | `AssociateDRTLogBucket` | 0 |
| *shield* | `GetSubscriptionState` | 0 |
| *shield* | `DescribeEmergencyContactSettings` | 0 |
| *shield* | `UpdateEmergencyContactSettings` | 0 |
| *shield* | `DescribeSubscription` | 0 |
| *shield* | `CreateSubscription` | 0 |
| *shield* | `DescribeProtection` | 0 |
| *shield* | `UpdateSubscription` | 0 |
| *shield* | `DescribeDRTAccess` | 0 |
| *shield* | `DisassociateDRTLogBucket` | 0 |
| *shield* | `CreateProtection` | 0 |
| *shield* | `DeleteSubscription` | 0 |
| *shield* | `AssociateDRTRole` | 0 |
| *shield* | `ListProtections` | 0 |
| *shield* | `ListAttacks` | 0 |
| *shield* | `DescribeAttack` | 0 |
| *signer* | `ListSigningJobs` | 0 |
| *signer* | `DescribeSigningJob` | 0 |
| *signer* | `GetSigningProfile` | 0 |
| *signer* | `ListSigningProfiles` | 0 |
| *signer* | `CancelSigningProfile` | 0 |
| *signer* | `ListSigningPlatforms` | 0 |
| *signer* | `StartSigningJob` | 0 |
| *signer* | `GetSigningPlatform` | 0 |
| *signer* | `PutSigningProfile` | 0 |
| *sms* | `StopAppReplication` | 0 |
| *sms* | `PutAppReplicationConfiguration` | 0 |
| *sms* | `GetApp` | 0 |
| *sms* | `GenerateTemplate` | 0 |
| *sms* | `PutAppLaunchConfiguration` | 0 |
| *sms* | `DeleteReplicationJob` | 0 |
| *sms* | `UpdateApp` | 0 |
| *sms* | `DeleteAppLaunchConfiguration` | 0 |
| *sms* | `DisassociateConnector` | 0 |
| *sms* | `DeleteAppReplicationConfiguration` | 0 |
| *sms* | `DeleteApp` | 0 |
| *sms* | `DeleteServerCatalog` | 0 |
| *sms* | `UpdateReplicationJob` | 0 |
| *sms* | `StartAppReplication` | 0 |
| *sms* | `GetReplicationRuns` | 0 |
| *sms* | `GetReplicationJobs` | 0 |
| *sms* | `StartOnDemandReplicationRun` | 0 |
| *sms* | `LaunchApp` | 0 |
| *sms* | `ImportServerCatalog` | 0 |
| *sms* | `GetConnectors` | 0 |
| *sms* | `GenerateChangeSet` | 0 |
| *sms* | `CreateApp` | 0 |
| *sms* | `GetServers` | 0 |
| *sms* | `TerminateApp` | 0 |
| *sms* | `CreateReplicationJob` | 0 |
| *sms* | `ListApps` | 0 |
| *sms* | `GetAppReplicationConfiguration` | 0 |
| *sms* | `GetAppLaunchConfiguration` | 0 |
| *sms-voice* | `CreateConfigurationSetEventDestination` | 0 |
| *sms-voice* | `DeleteConfigurationSet` | 0 |
| *sms-voice* | `DeleteConfigurationSetEventDestination` | 0 |
| *sms-voice* | `UpdateConfigurationSetEventDestination` | 0 |
| *sms-voice* | `GetConfigurationSetEventDestinations` | 0 |
| *sms-voice* | `CreateConfigurationSet` | 0 |
| *sms-voice* | `ListConfigurationSets` | 0 |
| *sms-voice* | `SendVoiceMessage` | 0 |
| *snowball* | `ListJobs` | 0 |
| *snowball* | `ListClusterJobs` | 0 |
| *snowball* | `GetJobUnlockCode` | 0 |
| *snowball* | `CreateJob` | 0 |
| *snowball* | `ListClusters` | 0 |
| *snowball* | `CreateCluster` | 0 |
| *snowball* | `UpdateJob` | 0 |
| *snowball* | `GetSoftwareUpdates` | 0 |
| *snowball* | `GetSnowballUsage` | 0 |
| *snowball* | `GetJobManifest` | 0 |
| *snowball* | `CreateAddress` | 0 |
| *snowball* | `DescribeAddress` | 0 |
| *snowball* | `DescribeAddresses` | 0 |
| *snowball* | `ListCompatibleImages` | 0 |
| *snowball* | `CancelJob` | 0 |
| *snowball* | `CancelCluster` | 0 |
| *snowball* | `DescribeJob` | 0 |
| *snowball* | `DescribeCluster` | 0 |
| *snowball* | `UpdateCluster` | 0 |
| *sns* | `ListPlatformApplications` | 2 |
| *sns* | `SetPlatformApplicationAttributes` | 0 |
| *sns* | `ListSubscriptionsByTopic` | 0 |
| *sns* | `ListSubscriptions` | 2 |
| *sns* | `ListPhoneNumbersOptedOut` | 0 |
| *sns* | `ConfirmSubscription` | 0 |
| *sns* | `SetTopicAttributes` | 1 |
| *sns* | `SetEndpointAttributes` | 0 |
| *sns* | `CheckIfPhoneNumberIsOptedOut` | 0 |
| *sns* | `AddPermission` | 0 |
| *sns* | `Unsubscribe` | 0 |
| *sns* | `GetSubscriptionAttributes` | 0 |
| *sns* | `TagResource` | 0 |
| *sns* | `CreatePlatformEndpoint` | 0 |
| *sns* | `RemovePermission` | 0 |
| *sns* | `CreateTopic` | 2 |
| *sns* | `SetSubscriptionAttributes` | 0 |
| *sns* | `CreatePlatformApplication` | 0 |
| *sns* | `GetPlatformApplicationAttributes` | 0 |
| *sns* | `GetSMSAttributes` | 0 |
| *sns* | `Publish` | 0 |
| *sns* | `SetSMSAttributes` | 0 |
| *sns* | `ListEndpointsByPlatformApplication` | 0 |
| *sns* | `GetEndpointAttributes` | 0 |
| *sns* | `DeleteEndpoint` | 0 |
| *sns* | `UntagResource` | 0 |
| *sns* | `DeletePlatformApplication` | 0 |
| *sns* | `GetTopicAttributes` | 1 |
| *sns* | `Subscribe` | 1 |
| *sns* | `DeleteTopic` | 1 |
| *sns* | `ListTagsForResource` | 0 |
| *sns* | `OptInPhoneNumber` | 0 |
| *sns* | `ListTopics` | 12 |
| *sqs* | `CreateQueue` | 1 |
| *sqs* | `GetQueueAttributes` | 1 |
| *sqs* | `SetQueueAttributes` | 1 |
| *sqs* | `GetQueueUrl` | 0 |
| *sqs* | `DeleteMessageBatch` | 0 |
| *sqs* | `SendMessageBatch` | 0 |
| *sqs* | `UntagQueue` | 0 |
| *sqs* | `ListDeadLetterSourceQueues` | 0 |
| *sqs* | `TagQueue` | 0 |
| *sqs* | `ChangeMessageVisibility` | 0 |
| *sqs* | `AddPermission` | 0 |
| *sqs* | `ChangeMessageVisibilityBatch` | 0 |
| *sqs* | `SendMessage` | 0 |
| *sqs* | `DeleteQueue` | 2 |
| *sqs* | `PurgeQueue` | 0 |
| *sqs* | `ListQueueTags` | 0 |
| *sqs* | `DeleteMessage` | 0 |
| *sqs* | `ReceiveMessage` | 0 |
| *sqs* | `ListQueues` | 2 |
| *sqs* | `RemovePermission` | 0 |
| *ssm* | `DescribeInventoryDeletions` | 0 |
| *ssm* | `GetPatchBaselineForPatchGroup` | 0 |
| *ssm* | `DescribeAssociation` | 0 |
| *ssm* | `DescribeAssociationExecutionTargets` | 0 |
| *ssm* | `DescribeInstancePatchStatesForPatchGroup` | 0 |
| *ssm* | `GetDefaultPatchBaseline` | 0 |
| *ssm* | `GetDocument` | 0 |
| *ssm* | `CreateDocument` | 1 |
| *ssm* | `DeleteAssociation` | 1 |
| *ssm* | `CreatePatchBaseline` | 1 |
| *ssm* | `ListResourceComplianceSummaries` | 0 |
| *ssm* | `DeregisterTargetFromMaintenanceWindow` | 0 |
| *ssm* | `TerminateSession` | 0 |
| *ssm* | `ListComplianceSummaries` | 0 |
| *ssm* | `GetParameter` | 0 |
| *ssm* | `StartAssociationsOnce` | 0 |
| *ssm* | `DescribeMaintenanceWindowTasks` | 0 |
| *ssm* | `GetConnectionStatus` | 0 |
| *ssm* | `CreateActivation` | 0 |
| *ssm* | `UpdateDocument` | 0 |
| *ssm* | `DescribeDocumentPermission` | 1 |
| *ssm* | `StartAutomationExecution` | 0 |
| *ssm* | `DescribeInstanceAssociationsStatus` | 0 |
| *ssm* | `UpdateMaintenanceWindowTarget` | 0 |
| *ssm* | `DeleteDocument` | 1 |
| *ssm* | `DescribePatchProperties` | 0 |
| *ssm* | `DescribeMaintenanceWindowExecutions` | 0 |
| *ssm* | `DescribeMaintenanceWindowSchedule` | 0 |
| *ssm* | `ListInventoryEntries` | 0 |
| *ssm* | `DeregisterPatchBaselineForPatchGroup` | 0 |
| *ssm* | `ListTagsForResource` | 0 |
| *ssm* | `ResetServiceSetting` | 0 |
| *ssm* | `GetDeployablePatchSnapshotForInstance` | 0 |
| *ssm* | `SendAutomationSignal` | 0 |
| *ssm* | `RegisterDefaultPatchBaseline` | 0 |
| *ssm* | `GetParametersByPath` | 1 |
| *ssm* | `UpdateAssociationStatus` | 0 |
| *ssm* | `AddTagsToResource` | 0 |
| *ssm* | `GetInventory` | 0 |
| *ssm* | `RemoveTagsFromResource` | 0 |
| *ssm* | `ListCommandInvocations` | 0 |
| *ssm* | `DescribeMaintenanceWindowsForTarget` | 0 |
| *ssm* | `GetInventorySchema` | 1 |
| *ssm* | `LabelParameterVersion` | 0 |
| *ssm* | `DescribeOpsItems` | 0 |
| *ssm* | `DescribeParameters` | 1 |
| *ssm* | `GetMaintenanceWindowExecutionTaskInvocation` | 0 |
| *ssm* | `DeleteInventory` | 0 |
| *ssm* | `DescribeAutomationStepExecutions` | 0 |
| *ssm* | `DescribeEffectiveInstanceAssociations` | 0 |
| *ssm* | `CancelMaintenanceWindowExecution` | 0 |
| *ssm* | `DescribeMaintenanceWindowExecutionTaskInvocations` | 0 |
| *ssm* | `UpdateDocumentDefaultVersion` | 0 |
| *ssm* | `CreateOpsItem` | 0 |
| *ssm* | `ListComplianceItems` | 0 |
| *ssm* | `ListAssociationVersions` | 0 |
| *ssm* | `ListDocuments` | 2 |
| *ssm* | `DeleteActivation` | 0 |
| *ssm* | `DescribeAvailablePatches` | 0 |
| *ssm* | `DescribeInstancePatches` | 0 |
| *ssm* | `DescribeInstancePatchStates` | 0 |
| *ssm* | `UpdateServiceSetting` | 0 |
| *ssm* | `ModifyDocumentPermission` | 0 |
| *ssm* | `PutComplianceItems` | 0 |
| *ssm* | `UpdateManagedInstanceRole` | 0 |
| *ssm* | `DescribeAssociationExecutions` | 0 |
| *ssm* | `StopAutomationExecution` | 0 |
| *ssm* | `StartSession` | 0 |
| *ssm* | `SendCommand` | 0 |
| *ssm* | `GetPatchBaseline` | 0 |
| *ssm* | `UpdatePatchBaseline` | 0 |
| *ssm* | `DescribePatchGroupState` | 0 |
| *ssm* | `UpdateAssociation` | 0 |
| *ssm* | `DescribeSessions` | 0 |
| *ssm* | `CreateAssociationBatch` | 0 |
| *ssm* | `DescribeMaintenanceWindowExecutionTasks` | 0 |
| *ssm* | `GetMaintenanceWindow` | 0 |
| *ssm* | `DeleteParameters` | 1 |
| *ssm* | `DescribeMaintenanceWindowTargets` | 0 |
| *ssm* | `DescribeInstanceInformation` | 0 |
| *ssm* | `ListAssociations` | 1 |
| *ssm* | `DeleteMaintenanceWindow` | 1 |
| *ssm* | `DescribeDocument` | 0 |
| *ssm* | `DeletePatchBaseline` | 0 |
| *ssm* | `RegisterTaskWithMaintenanceWindow` | 1 |
| *ssm* | `GetParameters` | 0 |
| *ssm* | `DeregisterManagedInstance` | 0 |
| *ssm* | `ResumeSession` | 0 |
| *ssm* | `CreateMaintenanceWindow` | 1 |
| *ssm* | `ListResourceDataSync` | 1 |
| *ssm* | `UpdateMaintenanceWindow` | 0 |
| *ssm* | `DescribeEffectivePatchesForPatchBaseline` | 0 |
| *ssm* | `GetOpsSummary` | 0 |
| *ssm* | `CreateResourceDataSync` | 1 |
| *ssm* | `PutParameter` | 1 |
| *ssm* | `GetAutomationExecution` | 0 |
| *ssm* | `PutInventory` | 0 |
| *ssm* | `DescribeAutomationExecutions` | 0 |
| *ssm* | `GetServiceSetting` | 0 |
| *ssm* | `DescribePatchBaselines` | 1 |
| *ssm* | `DeleteResourceDataSync` | 0 |
| *ssm* | `GetMaintenanceWindowExecutionTask` | 0 |
| *ssm* | `CancelCommand` | 0 |
| *ssm* | `GetCommandInvocation` | 0 |
| *ssm* | `GetOpsItem` | 0 |
| *ssm* | `ListDocumentVersions` | 0 |
| *ssm* | `GetParameterHistory` | 0 |
| *ssm* | `DeregisterTaskFromMaintenanceWindow` | 0 |
| *ssm* | `RegisterPatchBaselineForPatchGroup` | 0 |
| *ssm* | `UpdateOpsItem` | 0 |
| *ssm* | `DescribeMaintenanceWindows` | 1 |
| *ssm* | `DescribePatchGroups` | 0 |
| *ssm* | `CreateAssociation` | 1 |
| *ssm* | `UpdateMaintenanceWindowTask` | 0 |
| *ssm* | `ListCommands` | 0 |
| *ssm* | `GetMaintenanceWindowExecution` | 0 |
| *ssm* | `GetMaintenanceWindowTask` | 0 |
| *ssm* | `DeleteParameter` | 0 |
| *ssm* | `RegisterTargetWithMaintenanceWindow` | 1 |
| *ssm* | `DescribeActivations` | 0 |
| *stepfunctions* | `DescribeExecution` | 0 |
| *stepfunctions* | `GetActivityTask` | 0 |
| *stepfunctions* | `ListTagsForResource` | 0 |
| *stepfunctions* | `CreateStateMachine` | 1 |
| *stepfunctions* | `DeleteStateMachine` | 1 |
| *stepfunctions* | `StopExecution` | 0 |
| *stepfunctions* | `SendTaskHeartbeat` | 0 |
| *stepfunctions* | `ListActivities` | 1 |
| *stepfunctions* | `ListStateMachines` | 2 |
| *stepfunctions* | `TagResource` | 0 |
| *stepfunctions* | `CreateActivity` | 1 |
| *stepfunctions* | `SendTaskFailure` | 0 |
| *stepfunctions* | `DescribeStateMachineForExecution` | 0 |
| *stepfunctions* | `DescribeStateMachine` | 1 |
| *stepfunctions* | `DescribeActivity` | 0 |
| *stepfunctions* | `StartExecution` | 0 |
| *stepfunctions* | `ListExecutions` | 0 |
| *stepfunctions* | `UntagResource` | 0 |
| *stepfunctions* | `DeleteActivity` | 1 |
| *stepfunctions* | `GetExecutionHistory` | 0 |
| *stepfunctions* | `SendTaskSuccess` | 0 |
| *stepfunctions* | `UpdateStateMachine` | 0 |
| *storagegateway* | `UpdateSMBFileShare` | 0 |
| *storagegateway* | `DescribeSnapshotSchedule` | 0 |
| *storagegateway* | `UpdateNFSFileShare` | 0 |
| *storagegateway* | `DescribeTapeArchives` | 0 |
| *storagegateway* | `DeleteFileShare` | 1 |
| *storagegateway* | `RefreshCache` | 1 |
| *storagegateway* | `DescribeMaintenanceStartTime` | 1 |
| *storagegateway* | `DescribeGatewayInformation` | 1 |
| *storagegateway* | `UpdateGatewayInformation` | 0 |
| *storagegateway* | `DisableGateway` | 0 |
| *storagegateway* | `CreateSMBFileShare` | 1 |
| *storagegateway* | `DescribeSMBSettings` | 1 |
| *storagegateway* | `ListFileShares` | 1 |
| *storagegateway* | `UpdateMaintenanceStartTime` | 0 |
| *storagegateway* | `CreateTapes` | 1 |
| *storagegateway* | `ListVolumeRecoveryPoints` | 0 |
| *storagegateway* | `CreateCachediSCSIVolume` | 1 |
| *storagegateway* | `CreateTapeWithBarcode` | 0 |
| *storagegateway* | `DescribeChapCredentials` | 0 |
| *storagegateway* | `ListLocalDisks` | 1 |
| *storagegateway* | `UpdateGatewaySoftwareNow` | 0 |
| *storagegateway* | `UpdateChapCredentials` | 0 |
| *storagegateway* | `ResetCache` | 0 |
| *storagegateway* | `AddTagsToResource` | 0 |
| *storagegateway* | `RetrieveTapeArchive` | 0 |
| *storagegateway* | `JoinDomain` | 0 |
| *storagegateway* | `NotifyWhenUploaded` | 0 |
| *storagegateway* | `SetLocalConsolePassword` | 0 |
| *storagegateway* | `AssignTapePool` | 0 |
| *storagegateway* | `ListVolumeInitiators` | 1 |
| *storagegateway* | `RemoveTagsFromResource` | 0 |
| *storagegateway* | `CreateSnapshotFromVolumeRecoveryPoint` | 0 |
| *storagegateway* | `UpdateSMBSecurityStrategy` | 0 |
| *storagegateway* | `DescribeSMBFileShares` | 0 |
| *storagegateway* | `ListGateways` | 1 |
| *storagegateway* | `DeleteChapCredentials` | 0 |
| *storagegateway* | `DeleteBandwidthRateLimit` | 1 |
| *storagegateway* | `DescribeTapeRecoveryPoints` | 0 |
| *storagegateway* | `ActivateGateway` | 1 |
| *storagegateway* | `DeleteTapeArchive` | 0 |
| *storagegateway* | `DescribeVTLDevices` | 1 |
| *storagegateway* | `AddCache` | 1 |
| *storagegateway* | `DeleteVolume` | 1 |
| *storagegateway* | `ListVolumes` | 1 |
| *storagegateway* | `DescribeCachediSCSIVolumes` | 0 |
| *storagegateway* | `RetrieveTapeRecoveryPoint` | 0 |
| *storagegateway* | `CancelRetrieval` | 0 |
| *storagegateway* | `DescribeBandwidthRateLimit` | 1 |
| *storagegateway* | `DescribeStorediSCSIVolumes` | 0 |
| *storagegateway* | `AttachVolume` | 0 |
| *storagegateway* | `DetachVolume` | 0 |
| *storagegateway* | `UpdateBandwidthRateLimit` | 1 |
| *storagegateway* | `StartGateway` | 0 |
| *storagegateway* | `DescribeCache` | 0 |
| *storagegateway* | `AddUploadBuffer` | 1 |
| *storagegateway* | `DeleteSnapshotSchedule` | 0 |
| *storagegateway* | `UpdateVTLDeviceType` | 1 |
| *storagegateway* | `DeleteTape` | 1 |
| *storagegateway* | `UpdateSnapshotSchedule` | 0 |
| *storagegateway* | `ShutdownGateway` | 0 |
| *storagegateway* | `DescribeTapes` | 0 |
| *storagegateway* | `CreateNFSFileShare` | 1 |
| *storagegateway* | `CreateStorediSCSIVolume` | 0 |
| *storagegateway* | `CancelArchival` | 0 |
| *storagegateway* | `SetSMBGuestPassword` | 1 |
| *storagegateway* | `ListTapes` | 1 |
| *storagegateway* | `DeleteGateway` | 1 |
| *storagegateway* | `DescribeNFSFileShares` | 1 |
| *storagegateway* | `DescribeWorkingStorage` | 0 |
| *storagegateway* | `DescribeUploadBuffer` | 1 |
| *storagegateway* | `ListTagsForResource` | 0 |
| *storagegateway* | `AddWorkingStorage` | 0 |
| *storagegateway* | `CreateSnapshot` | 0 |
| *sts* | `GetFederationToken` | 0 |
| *sts* | `AssumeRole` | 0 |
| *sts* | `DecodeAuthorizationMessage` | 0 |
| *sts* | `AssumeRoleWithWebIdentity` | 0 |
| *sts* | `GetSessionToken` | 0 |
| *sts* | `GetCallerIdentity` | 1 |
| *sts* | `AssumeRoleWithSAML` | 0 |
| *sts* | `GetAccessKeyInfo` | 0 |
| *support* | `CreateCase` | 1 |
| *support* | `AddAttachmentsToSet` | 0 |
| *support* | `AddCommunicationToCase` | 1 |
| *support* | `DescribeTrustedAdvisorCheckRefreshStatuses` | 0 |
| *support* | `DescribeTrustedAdvisorCheckResult` | 0 |
| *support* | `ResolveCase` | 1 |
| *support* | `DescribeSeverityLevels` | 1 |
| *support* | `DescribeCases` | 1 |
| *support* | `RefreshTrustedAdvisorCheck` | 0 |
| *support* | `DescribeAttachment` | 0 |
| *support* | `DescribeTrustedAdvisorChecks` | 0 |
| *support* | `DescribeTrustedAdvisorCheckSummaries` | 0 |
| *support* | `DescribeCommunications` | 1 |
| *support* | `DescribeServices` | 1 |
| *swf* | `SignalWorkflowExecution` | 0 |
| *swf* | `DescribeWorkflowExecution` | 0 |
| *swf* | `GetWorkflowExecutionHistory` | 0 |
| *swf* | `CountOpenWorkflowExecutions` | 0 |
| *swf* | `RespondActivityTaskFailed` | 0 |
| *swf* | `PollForActivityTask` | 0 |
| *swf* | `ListDomains` | 0 |
| *swf* | `StartWorkflowExecution` | 0 |
| *swf* | `RespondDecisionTaskCompleted` | 0 |
| *swf* | `RegisterActivityType` | 0 |
| *swf* | `RequestCancelWorkflowExecution` | 0 |
| *swf* | `ListActivityTypes` | 0 |
| *swf* | `PollForDecisionTask` | 0 |
| *swf* | `ListOpenWorkflowExecutions` | 0 |
| *swf* | `CountClosedWorkflowExecutions` | 0 |
| *swf* | `TagResource` | 0 |
| *swf* | `RegisterWorkflowType` | 0 |
| *swf* | `DescribeDomain` | 0 |
| *swf* | `ListClosedWorkflowExecutions` | 0 |
| *swf* | `TerminateWorkflowExecution` | 0 |
| *swf* | `DeprecateWorkflowType` | 0 |
| *swf* | `UndeprecateActivityType` | 0 |
| *swf* | `DescribeActivityType` | 0 |
| *swf* | `CountPendingDecisionTasks` | 0 |
| *swf* | `ListWorkflowTypes` | 0 |
| *swf* | `DeprecateDomain` | 0 |
| *swf* | `RespondActivityTaskCanceled` | 0 |
| *swf* | `DescribeWorkflowType` | 0 |
| *swf* | `RegisterDomain` | 1 |
| *swf* | `RecordActivityTaskHeartbeat` | 0 |
| *swf* | `UndeprecateDomain` | 0 |
| *swf* | `UntagResource` | 0 |
| *swf* | `UndeprecateWorkflowType` | 0 |
| *swf* | `CountPendingActivityTasks` | 0 |
| *swf* | `DeprecateActivityType` | 0 |
| *swf* | `ListTagsForResource` | 0 |
| *swf* | `RespondActivityTaskCompleted` | 0 |
| *textract* | `StartDocumentTextDetection` | 0 |
| *textract* | `DetectDocumentText` | 0 |
| *textract* | `GetDocumentAnalysis` | 0 |
| *textract* | `GetDocumentTextDetection` | 0 |
| *textract* | `AnalyzeDocument` | 0 |
| *textract* | `StartDocumentAnalysis` | 0 |
| *transcribe* | `ListTranscriptionJobs` | 0 |
| *transcribe* | `DeleteTranscriptionJob` | 0 |
| *transcribe* | `UpdateVocabulary` | 0 |
| *transcribe* | `CreateVocabulary` | 0 |
| *transcribe* | `ListVocabularies` | 0 |
| *transcribe* | `StartTranscriptionJob` | 0 |
| *transcribe* | `GetVocabulary` | 0 |
| *transcribe* | `DeleteVocabulary` | 0 |
| *transcribe* | `GetTranscriptionJob` | 0 |
| *transfer* | `UpdateUser` | 0 |
| *transfer* | `DeleteUser` | 1 |
| *transfer* | `StopServer` | 1 |
| *transfer* | `DeleteServer` | 1 |
| *transfer* | `UntagResource` | 0 |
| *transfer* | `ListUsers` | 1 |
| *transfer* | `ListServers` | 1 |
| *transfer* | `TestIdentityProvider` | 0 |
| *transfer* | `DescribeUser` | 0 |
| *transfer* | `CreateServer` | 1 |
| *transfer* | `ImportSshPublicKey` | 0 |
| *transfer* | `StartServer` | 1 |
| *transfer* | `CreateUser` | 1 |
| *transfer* | `DescribeServer` | 1 |
| *transfer* | `ListTagsForResource` | 0 |
| *transfer* | `DeleteSshPublicKey` | 0 |
| *transfer* | `UpdateServer` | 0 |
| *transfer* | `TagResource` | 0 |
| *translate* | `TranslateText` | 0 |
| *translate* | `GetTerminology` | 0 |
| *translate* | `ListTerminologies` | 0 |
| *translate* | `ImportTerminology` | 0 |
| *translate* | `DeleteTerminology` | 0 |
| *waf* | `CreateSizeConstraintSet` | 1 |
| *waf* | `UpdateRegexMatchSet` | 0 |
| *waf* | `GetPermissionPolicy` | 0 |
| *waf* | `ListIPSets` | 1 |
| *waf* | `DeleteRateBasedRule` | 0 |
| *waf* | `GetRegexPatternSet` | 0 |
| *waf* | `PutLoggingConfiguration` | 0 |
| *waf* | `GetRegexMatchSet` | 0 |
| *waf* | `GetChangeTokenStatus` | 0 |
| *waf* | `UpdateRegexPatternSet` | 0 |
| *waf* | `DeleteSqlInjectionMatchSet` | 1 |
| *waf* | `ListByteMatchSets` | 1 |
| *waf* | `CreateRule` | 1 |
| *waf* | `UpdateGeoMatchSet` | 0 |
| *waf* | `GetRateBasedRuleManagedKeys` | 0 |
| *waf* | `ListRuleGroups` | 1 |
| *waf* | `CreateSqlInjectionMatchSet` | 1 |
| *waf* | `UpdateSqlInjectionMatchSet` | 1 |
| *waf* | `ListRateBasedRules` | 1 |
| *waf* | `ListRegexMatchSets` | 1 |
| *waf* | `PutPermissionPolicy` | 0 |
| *waf* | `DeleteByteMatchSet` | 1 |
| *waf* | `DeleteRegexPatternSet` | 0 |
| *waf* | `CreateRegexPatternSet` | 0 |
| *waf* | `GetIPSet` | 0 |
| *waf* | `ListSqlInjectionMatchSets` | 1 |
| *waf* | `DeleteRule` | 1 |
| *waf* | `ListSubscribedRuleGroups` | 1 |
| *waf* | `DeleteXssMatchSet` | 1 |
| *waf* | `UpdateIPSet` | 1 |
| *waf* | `ListWebACLs` | 0 |
| *waf* | `UpdateRuleGroup` | 0 |
| *waf* | `TagResource` | 0 |
| *waf* | `GetByteMatchSet` | 0 |
| *waf* | `CreateXssMatchSet` | 1 |
| *waf* | `UpdateRateBasedRule` | 0 |
| *waf* | `GetLoggingConfiguration` | 0 |
| *waf* | `CreateByteMatchSet` | 1 |
| *waf* | `ListXssMatchSets` | 1 |
| *waf* | `UpdateRule` | 1 |
| *waf* | `GetSizeConstraintSet` | 0 |
| *waf* | `ListLoggingConfigurations` | 0 |
| *waf* | `GetGeoMatchSet` | 0 |
| *waf* | `CreateIPSet` | 1 |
| *waf* | `DeleteRuleGroup` | 0 |
| *waf* | `GetXssMatchSet` | 0 |
| *waf* | `ListGeoMatchSets` | 1 |
| *waf* | `DeleteLoggingConfiguration` | 0 |
| *waf* | `GetRateBasedRule` | 0 |
| *waf* | `DeleteGeoMatchSet` | 0 |
| *waf* | `CreateRateBasedRule` | 0 |
| *waf* | `GetRuleGroup` | 0 |
| *waf* | `ListRules` | 1 |
| *waf* | `GetSampledRequests` | 0 |
| *waf* | `CreateWebACL` | 1 |
| *waf* | `DeleteWebACL` | 0 |
| *waf* | `CreateRuleGroup` | 0 |
| *waf* | `GetRule` | 0 |
| *waf* | `DeleteSizeConstraintSet` | 1 |
| *waf* | `ListTagsForResource` | 0 |
| *waf* | `UntagResource` | 0 |
| *waf* | `UpdateSizeConstraintSet` | 1 |
| *waf* | `ListActivatedRulesInRuleGroup` | 0 |
| *waf* | `UpdateWebACL` | 1 |
| *waf* | `GetChangeToken` | 1 |
| *waf* | `DeleteRegexMatchSet` | 0 |
| *waf* | `GetWebACL` | 0 |
| *waf* | `DeletePermissionPolicy` | 0 |
| *waf* | `UpdateByteMatchSet` | 1 |
| *waf* | `UpdateXssMatchSet` | 1 |
| *waf* | `CreateRegexMatchSet` | 0 |
| *waf* | `GetSqlInjectionMatchSet` | 0 |
| *waf* | `ListRegexPatternSets` | 0 |
| *waf* | `ListSizeConstraintSets` | 1 |
| *waf* | `DeleteIPSet` | 1 |
| *waf* | `CreateGeoMatchSet` | 0 |
| *waf-regional* | `CreateSizeConstraintSet` | 1 |
| *waf-regional* | `UpdateRegexMatchSet` | 0 |
| *waf-regional* | `GetPermissionPolicy` | 0 |
| *waf-regional* | `UpdateXssMatchSet` | 1 |
| *waf-regional* | `ListIPSets` | 1 |
| *waf-regional* | `DeleteRateBasedRule` | 0 |
| *waf-regional* | `GetRegexPatternSet` | 0 |
| *waf-regional* | `PutLoggingConfiguration` | 0 |
| *waf-regional* | `GetRegexMatchSet` | 0 |
| *waf-regional* | `GetChangeTokenStatus` | 0 |
| *waf-regional* | `UpdateRegexPatternSet` | 0 |
| *waf-regional* | `DeleteSqlInjectionMatchSet` | 1 |
| *waf-regional* | `ListByteMatchSets` | 1 |
| *waf-regional* | `CreateRule` | 1 |
| *waf-regional* | `UpdateGeoMatchSet` | 0 |
| *waf-regional* | `GetRateBasedRuleManagedKeys` | 0 |
| *waf-regional* | `UpdateWebACL` | 1 |
| *waf-regional* | `ListRuleGroups` | 1 |
| *waf-regional* | `CreateSqlInjectionMatchSet` | 1 |
| *waf-regional* | `UpdateSqlInjectionMatchSet` | 1 |
| *waf-regional* | `ListRateBasedRules` | 1 |
| *waf-regional* | `ListRegexMatchSets` | 1 |
| *waf-regional* | `PutPermissionPolicy` | 0 |
| *waf-regional* | `DeleteByteMatchSet` | 1 |
| *waf-regional* | `DeleteRegexPatternSet` | 0 |
| *waf-regional* | `CreateRegexPatternSet` | 0 |
| *waf-regional* | `GetIPSet` | 0 |
| *waf-regional* | `ListSqlInjectionMatchSets` | 1 |
| *waf-regional* | `DeleteRule` | 1 |
| *waf-regional* | `ListSubscribedRuleGroups` | 1 |
| *waf-regional* | `DeleteXssMatchSet` | 1 |
| *waf-regional* | `UpdateIPSet` | 1 |
| *waf-regional* | `ListTagsForResource` | 0 |
| *waf-regional* | `UpdateRuleGroup` | 0 |
| *waf-regional* | `TagResource` | 0 |
| *waf-regional* | `GetByteMatchSet` | 0 |
| *waf-regional* | `CreateXssMatchSet` | 1 |
| *waf-regional* | `UpdateRateBasedRule` | 0 |
| *waf-regional* | `GetLoggingConfiguration` | 0 |
| *waf-regional* | `CreateByteMatchSet` | 1 |
| *waf-regional* | `ListXssMatchSets` | 1 |
| *waf-regional* | `AssociateWebACL` | 1 |
| *waf-regional* | `UpdateRule` | 1 |
| *waf-regional* | `GetSizeConstraintSet` | 0 |
| *waf-regional* | `ListLoggingConfigurations` | 0 |
| *waf-regional* | `GetGeoMatchSet` | 0 |
| *waf-regional* | `CreateIPSet` | 1 |
| *waf-regional* | `DeleteRuleGroup` | 0 |
| *waf-regional* | `GetXssMatchSet` | 0 |
| *waf-regional* | `ListGeoMatchSets` | 1 |
| *waf-regional* | `DeleteLoggingConfiguration` | 0 |
| *waf-regional* | `GetRateBasedRule` | 0 |
| *waf-regional* | `DeleteGeoMatchSet` | 0 |
| *waf-regional* | `CreateRateBasedRule` | 0 |
| *waf-regional* | `GetRuleGroup` | 0 |
| *waf-regional* | `ListRules` | 1 |
| *waf-regional* | `GetSampledRequests` | 0 |
| *waf-regional* | `CreateWebACL` | 1 |
| *waf-regional* | `DeleteWebACL` | 0 |
| *waf-regional* | `CreateRuleGroup` | 0 |
| *waf-regional* | `GetRule` | 0 |
| *waf-regional* | `DeleteSizeConstraintSet` | 1 |
| *waf-regional* | `GetWebACLForResource` | 1 |
| *waf-regional* | `UntagResource` | 0 |
| *waf-regional* | `UpdateSizeConstraintSet` | 1 |
| *waf-regional* | `ListActivatedRulesInRuleGroup` | 0 |
| *waf-regional* | `ListResourcesForWebACL` | 0 |
| *waf-regional* | `GetChangeToken` | 1 |
| *waf-regional* | `DeleteRegexMatchSet` | 0 |
| *waf-regional* | `GetWebACL` | 0 |
| *waf-regional* | `DeletePermissionPolicy` | 0 |
| *waf-regional* | `UpdateByteMatchSet` | 1 |
| *waf-regional* | `DisassociateWebACL` | 0 |
| *waf-regional* | `CreateRegexMatchSet` | 0 |
| *waf-regional* | `ListWebACLs` | 0 |
| *waf-regional* | `GetSqlInjectionMatchSet` | 0 |
| *waf-regional* | `ListRegexPatternSets` | 0 |
| *waf-regional* | `ListSizeConstraintSets` | 1 |
| *waf-regional* | `DeleteIPSet` | 1 |
| *waf-regional* | `CreateGeoMatchSet` | 0 |
| *workdocs* | `DeactivateUser` | 0 |
| *workdocs* | `DescribeRootFolders` | 0 |
| *workdocs* | `AddResourcePermissions` | 0 |
| *workdocs* | `DeleteFolder` | 0 |
| *workdocs* | `DescribeUsers` | 0 |
| *workdocs* | `UpdateDocumentVersion` | 0 |
| *workdocs* | `DeleteCustomMetadata` | 0 |
| *workdocs* | `CreateLabels` | 0 |
| *workdocs* | `GetResources` | 0 |
| *workdocs* | `InitiateDocumentVersionUpload` | 0 |
| *workdocs* | `CreateCustomMetadata` | 0 |
| *workdocs* | `GetDocument` | 0 |
| *workdocs* | `GetDocumentPath` | 0 |
| *workdocs* | `RemoveAllResourcePermissions` | 0 |
| *workdocs* | `UpdateUser` | 0 |
| *workdocs* | `GetCurrentUser` | 0 |
| *workdocs* | `UpdateFolder` | 0 |
| *workdocs* | `DeleteComment` | 0 |
| *workdocs* | `CreateUser` | 0 |
| *workdocs* | `DeleteFolderContents` | 0 |
| *workdocs* | `DeleteNotificationSubscription` | 0 |
| *workdocs* | `AbortDocumentVersionUpload` | 0 |
| *workdocs* | `DescribeNotificationSubscriptions` | 0 |
| *workdocs* | `CreateComment` | 0 |
| *workdocs* | `GetDocumentVersion` | 0 |
| *workdocs* | `GetFolderPath` | 0 |
| *workdocs* | `DescribeFolderContents` | 0 |
| *workdocs* | `CreateFolder` | 0 |
| *workdocs* | `DescribeGroups` | 0 |
| *workdocs* | `DescribeResourcePermissions` | 0 |
| *workdocs* | `CreateNotificationSubscription` | 0 |
| *workdocs* | `UpdateDocument` | 0 |
| *workdocs* | `DescribeComments` | 0 |
| *workdocs* | `DeleteUser` | 0 |
| *workdocs* | `DeleteDocument` | 0 |
| *workdocs* | `DescribeActivities` | 0 |
| *workdocs* | `ActivateUser` | 0 |
| *workdocs* | `GetFolder` | 0 |
| *workdocs* | `RemoveResourcePermission` | 0 |
| *workdocs* | `DeleteLabels` | 0 |
| *workdocs* | `DescribeDocumentVersions` | 0 |
| *worklink* | `UpdateDevicePolicyConfiguration` | 0 |
| *worklink* | `DisassociateWebsiteAuthorizationProvider` | 0 |
| *worklink* | `DeleteFleet` | 1 |
| *worklink* | `DescribeAuditStreamConfiguration` | 1 |
| *worklink* | `UpdateDomainMetadata` | 0 |
| *worklink* | `DescribeDevice` | 0 |
| *worklink* | `ListFleets` | 1 |
| *worklink* | `DescribeDevicePolicyConfiguration` | 1 |
| *worklink* | `ListDomains` | 0 |
| *worklink* | `DisassociateWebsiteCertificateAuthority` | 0 |
| *worklink* | `UpdateFleetMetadata` | 0 |
| *worklink* | `UpdateCompanyNetworkConfiguration` | 0 |
| *worklink* | `DescribeDomain` | 0 |
| *worklink* | `ListWebsiteCertificateAuthorities` | 1 |
| *worklink* | `CreateFleet` | 1 |
| *worklink* | `DisassociateDomain` | 0 |
| *worklink* | `RevokeDomainAccess` | 0 |
| *worklink* | `AssociateWebsiteAuthorizationProvider` | 0 |
| *worklink* | `DescribeFleetMetadata` | 1 |
| *worklink* | `ListWebsiteAuthorizationProviders` | 0 |
| *worklink* | `RestoreDomainAccess` | 0 |
| *worklink* | `AssociateDomain` | 0 |
| *worklink* | `ListDevices` | 0 |
| *worklink* | `UpdateIdentityProviderConfiguration` | 0 |
| *worklink* | `DescribeWebsiteCertificateAuthority` | 0 |
| *worklink* | `UpdateAuditStreamConfiguration` | 0 |
| *worklink* | `DescribeIdentityProviderConfiguration` | 1 |
| *worklink* | `SignOutUser` | 0 |
| *worklink* | `DescribeCompanyNetworkConfiguration` | 1 |
| *worklink* | `AssociateWebsiteCertificateAuthority` | 0 |
| *workmail* | `GetMailboxDetails` | 0 |
| *workmail* | `ListGroupMembers` | 0 |
| *workmail* | `UpdatePrimaryEmailAddress` | 0 |
| *workmail* | `PutMailboxPermissions` | 0 |
| *workmail* | `ListUsers` | 0 |
| *workmail* | `DisassociateMemberFromGroup` | 0 |
| *workmail* | `DescribeResource` | 0 |
| *workmail* | `CreateGroup` | 0 |
| *workmail* | `ListAliases` | 0 |
| *workmail* | `DescribeGroup` | 0 |
| *workmail* | `ResetPassword` | 0 |
| *workmail* | `DeleteMailboxPermissions` | 0 |
| *workmail* | `DisassociateDelegateFromResource` | 0 |
| *workmail* | `ListOrganizations` | 0 |
| *workmail* | `RegisterToWorkMail` | 0 |
| *workmail* | `DeleteAlias` | 0 |
| *workmail* | `DeleteGroup` | 0 |
| *workmail* | `DescribeUser` | 0 |
| *workmail* | `ListMailboxPermissions` | 0 |
| *workmail* | `AssociateDelegateToResource` | 0 |
| *workmail* | `ListResourceDelegates` | 0 |
| *workmail* | `ListGroups` | 0 |
| *workmail* | `DeregisterFromWorkMail` | 0 |
| *workmail* | `UpdateMailboxQuota` | 0 |
| *workmail* | `DescribeOrganization` | 0 |
| *workmail* | `AssociateMemberToGroup` | 0 |
| *workmail* | `DeleteUser` | 0 |
| *workmail* | `CreateAlias` | 0 |
| *workmail* | `ListResources` | 0 |
| *workmail* | `DeleteResource` | 0 |
| *workmail* | `CreateResource` | 0 |
| *workmail* | `CreateUser` | 0 |
| *workmail* | `UpdateResource` | 0 |
| *workmailmessageflow* | `GetRawMessageContent` | 0 |
| *workspaces* | `DeleteTags` | 0 |
| *workspaces* | `DescribeWorkspaceDirectories` | 0 |
| *workspaces* | `TerminateWorkspaces` | 1 |
| *workspaces* | `DescribeWorkspaceBundles` | 1 |
| *workspaces* | `DescribeWorkspaceImages` | 0 |
| *workspaces* | `DescribeClientProperties` | 0 |
| *workspaces* | `DescribeIpGroups` | 0 |
| *workspaces* | `RebootWorkspaces` | 0 |
| *workspaces* | `CreateWorkspaces` | 1 |
| *workspaces* | `ModifyWorkspaceState` | 0 |
| *workspaces* | `UpdateRulesOfIpGroup` | 0 |
| *workspaces* | `ModifyWorkspaceProperties` | 0 |
| *workspaces* | `DescribeAccountModifications` | 0 |
| *workspaces* | `RestoreWorkspace` | 0 |
| *workspaces* | `ImportWorkspaceImage` | 0 |
| *workspaces* | `DeleteIpGroup` | 0 |
| *workspaces* | `DescribeWorkspaces` | 1 |
| *workspaces* | `DisassociateIpGroups` | 0 |
| *workspaces* | `RevokeIpRules` | 0 |
| *workspaces* | `StopWorkspaces` | 0 |
| *workspaces* | `CopyWorkspaceImage` | 0 |
| *workspaces* | `StartWorkspaces` | 0 |
| *workspaces* | `RebuildWorkspaces` | 0 |
| *workspaces* | `DescribeWorkspaceSnapshots` | 0 |
| *workspaces* | `DescribeTags` | 0 |
| *workspaces* | `CreateIpGroup` | 0 |
| *workspaces* | `ListAvailableManagementCidrRanges` | 0 |
| *workspaces* | `CreateTags` | 0 |
| *workspaces* | `DescribeAccount` | 0 |
| *workspaces* | `DeleteWorkspaceImage` | 0 |
| *workspaces* | `DescribeWorkspacesConnectionStatus` | 0 |
| *workspaces* | `AssociateIpGroups` | 0 |
| *workspaces* | `ModifyAccount` | 0 |
| *workspaces* | `AuthorizeIpRules` | 0 |
| *workspaces* | `ModifyClientProperties` | 0 |
| *xray* | `GetServiceGraph` | 1 |
| *xray* | `PutTelemetryRecords` | 0 |
| *xray* | `GetSamplingRules` | 1 |
| *xray* | `CreateGroup` | 0 |
| *xray* | `GetTraceSummaries` | 1 |
| *xray* | `GetSamplingStatisticSummaries` | 1 |
| *xray* | `UpdateSamplingRule` | 0 |
| *xray* | `DeleteGroup` | 0 |
| *xray* | `GetSamplingTargets` | 0 |
| *xray* | `PutTraceSegments` | 0 |
| *xray* | `GetTraceGraph` | 0 |
| *xray* | `UpdateGroup` | 0 |
| *xray* | `GetTimeSeriesServiceStatistics` | 0 |
| *xray* | `GetEncryptionConfig` | 1 |
| *xray* | `CreateSamplingRule` | 1 |
| *xray* | `BatchGetTraces` | 0 |
| *xray* | `PutEncryptionConfig` | 0 |
| *xray* | `GetGroup` | 0 |
| *xray* | `DeleteSamplingRule` | 1 |
| *xray* | `GetGroups` | 0 |


**Total Services: 192**

**Total Operations: 6244**

**Total Unique Occurances: 1208 (19%)**
